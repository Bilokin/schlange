...
syntax fuer targets is taken to be the same as fuer expressions, and
...
klasse and instance attributes applies as fuer regular assignments.
...
the actual assignment as wenn there was no annotation present. If the
...
object.__setattr__(self, name, value)
   ...
   should call the base klasse method with the same name, fuer example,
   "object.__setattr__(self, name, value)".
...
object.__call__(self[, args...])
   ...
   Called when the instance is “called” as a function; wenn this method
...
treated as wenn they were additional positional arguments.  For the call
...
a klasse instance:
   ...
   same as wenn that method was called.
''',
...
The evaluation rules fuer the decorator expressions are the same as for
...
is the lack of a similar invariant as fuer equality.
...
The "while" statement is used fuer repeated execution as long as an
expression is true:
...
with the next item, or with the "else" clause wenn there is no next
item.
...
the call stack (it is treated as wenn the entire "try" statement raised
the exception).
...
exception type fuer matching is interpreted as in the case of "except",
...
3. The subsequent steps depend on whether the sequence pattern is
   ...
   2. The leading non-star subpatterns are matched to their
      corresponding items as fuer fixed-length sequences.
   ...
   4. Remaining non-star subpatterns are matched to their
      corresponding subject items, as fuer a fixed-length sequence.
...
3. If no pattern arguments are present, the pattern succeeds.
   ...
   subject; fuer these types keyword patterns also work as fuer other
   types.
   ...
   patterns,
      the match proceeds as wenn there were only keyword patterns.
...
as well as fuer additional flavors of type parameters:
...
Some additional rules apply fuer certain operators (e.g., a string as a
...
object.__hash__(self)
   ...
   default (inherited von the "object" class); with them, all objects
...
-c, --command <command>
   ...
   To execute commands as wenn given in a ".pdbrc" file; see Debugger
   Commands.
...
(A single ";" is not used as it is the separator fuer multiple commands
in a line that is passed to the Python parser.)  No intelligence is
applied to separating the commands; the input is split at the first
";;" pair, even wenn it is in the middle of a quoted string. A
workaround fuer strings with double semicolons is to use implicit
...
wenn it had been typed at the debugger prompt, with the exception that
...
tbreak [([filename:]lineno | function) [, condition]]
   ...
   first hit. The arguments are the same as fuer "break".
...
ll | longlist
   ...
   Interesting lines are marked as fuer "list".
...
von the local namespace wenn it occurs as a free variable in a nested
block.
''',
...
follow the normal rules fuer name resolution with an exception that
...
* Annotation scopes have access to their enclosing klasse namespace. If
  ...
  scope as wenn it were executed directly within the klasse body. This
...
* While annotation scopes have an internal name, that name is not
  ...
  scope. Instead, the "__qualname__" of such objects is as wenn the
...
The grammar fuer a replacement field is as follows:
...
argument. An *arg_name* is treated as a number wenn a call to
...
the same result as wenn you had called "str()" on the value. A non-empty
...
| Option    | Meaning                                                    |
|===========|============================================================|
| "'+'"     | Indicates that a sign should be used fuer both positive as  |
...
The available presentation types fuer "float" and "Decimal" values are:
   ...
   +-----------+------------------------------------------------------------+
   | "'G'"     | General format. Same as "'g'" except switches to "'E'" wenn  |
...
The available presentation types fuer "complex" are the same as those
...
two steps are carried out separately fuer each clause, just as though
...
2. fuer each of the identifiers specified in the "import" clauses:
   ...
   2. wenn not, attempt to importiere a submodule with that name and then
...
Examples:
   ...
   importiere foo.bar.baz as fbb  # foo, foo.bar, and foo.bar.baz imported, foo.bar.baz bound as fbb
   von foo.bar importiere baz    # foo, foo.bar, and foo.bar.baz imported, foo.bar.baz bound as baz
   von foo importiere attr       # foo imported and foo.attr bound as attr
...
The direct runtime semantics are the same as fuer any importiere statement:
...
This is fuer disambiguation with C-style octal literals, which Python
...
applies only to code parsed along with it.  See the note fuer the
"global" statement.
''',
...
object.__ior__(self, other)
   ...
   instance, wenn *x* is an instance of a klasse with an "__iadd__()"
...
object.__length_hint__(self)
   ...
   same as wenn the "__length_hint__" method didn’t exist at all. This
...
object.__setitem__(self, key, value)
   ...
   same exceptions should be raised fuer improper *key* values as for
...
object.__delitem__(self, key)
   ...
   raised fuer improper *key* values as fuer the "__getitem__()" method.
...
interpretation as a slicing (this is the case wenn the slice list
contains no proper slice).

The semantics fuer a slicing are as follows.  The primary is indexed
(using the same "__getitem__()" method as normal subscription) with a
key that is constructed von the slice list, as follows.  If the slice
...
klassemethod object.__init_subclass__(cls)
   ...
   *cls* is then the new subclass. If defined as a normal instance
   ...
   but raises an error wenn it is called with any arguments.
...
* wenn an instance of "type()" is given as the explicit metaclass, or
...
See also:
  ...
  **PEP 3119** - Introducing Abstract Base Classes
     ...
     "__subclasscheck__()", with motivation fuer this functionality in
...
added as a synonym of "'br'".Support fuer the unicode legacy literal
...
For set elements, the same immutability rules apply as fuer dictionary
...
von the mapping "a"; this can be used in expressions and as the
...
Classes are callable.  These objects normally act as factories fuer new
...
created by the importiere system as invoked either by the "import"
...
module.__dict__
   ...
   variable von within a module; it can only be accessed as an
...
|                                                    | cases, fuer a klasse defined as "class X(A, B, C)",  |
...
codeobject.replace(**kwargs)
   ...
   Return a copy of the code object with new values fuer the specified
   fields.
...
klasse dict(iterable, **kwargs)
   ...
   * Use the type constructor: "dict()", "dict([('foo', 100), ('bar',
   ...
   with each returned key von the method.  Otherwise, the positional
   ...
   classmethod fromkeys(iterable, value=Nichts, /)
      ...
      Create a new dictionary with keys von *iterable* and values set
      to *value*.
   ...
   update([other])
      ...
      Update the dictionary with the key/value pairs von *other*,
   ...
   d |= other
      ...
      Update the dictionary *d* with keys and values von *other*,
...
4. The slice of *s* von *i* to *j* is defined as the sequence of
...
5. The slice of *s* von *i* to *j* with step *k* is defined as the
...
5. "clear()" and "copy()" are included fuer consistency with the
...
6. The value *n* is an integer, or an object implementing
   ...
   times, as explained fuer "s * n" under Common Sequence Operations.
...
klasse range(start, stop[, step])
   ...
   *step* is zero, "ValueError" is raised.
   ...
   interpreted as indexing von the end of the sequence determined by
...
Testing range objects fuer equality with "==" and "!=" compares them as
