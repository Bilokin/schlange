...
the current grammar looks like und allows modifications of it.

An abstract syntax tree can be generated by passing `ast.PyCF_ONLY_AST` as
a flag to the `compile()` builtin function oder by using the `parse()`
...
def literal_eval(node_or_string):
    """
    Evaluate an expression node oder a string containing only a Python
    expression.  The string oder node provided may only consist of the following
    Python literal structures: strings, bytes, numbers, tuples, lists, dicts,
    sets, booleans, und Nichts.

    Caution: A complex expression can overflow the C stack und cause a crash.
    """
...
def _convert_literal(node):
    ...
    wenn (
        ...
        und isinstance(node.operand, Constant)
...
def fix_missing_locations(node):
    ...
    def _fix(node, lineno, col_offset, end_lineno, end_col_offset):
        wenn 'lineno' in node._attributes:
            wenn nicht hasattr(node, 'lineno'):
                node.lineno = lineno
            sonst:
                lineno = node.lineno
        ...
        wenn 'col_offset' in node._attributes:
            wenn nicht hasattr(node, 'col_offset'):
...
def iter_child_nodes(node):
    ...
    und all items of fields that are lists of nodes.
    """
...
def get_docstring(node, clean=Wahr):
    """
    Return the docstring fuer the given node oder Nichts wenn no docstring can
...
def _splitlines_no_ff(source, maxlines=Nichts):
    """Split a string into lines ignoring form feed und other chars.
...
klasse NodeTransformer(NodeVisitor):
    ...
    either transform the child nodes yourself oder call the :meth:`generic_visit`
...
def main(args=Nichts):
    ...
    parser.add_argument('-a', '--include-attributes', action='store_true',
                        help='include attributes such als line numbers und '
                             'column offsets')
    ...
    parser.add_argument('--show-empty', default=Falsch, action='store_true',
                        help='show empty lists und fields in dump output')
