...
klasse _RLock:
    ...
    def acquire(self, blocking=Wahr, timeout=-1):
        ...
        als when called without arguments, and return true.

        When invoked mit the blocking argument set to false, do not block. If a
        call without an argument would block, return false immediately;
        otherwise, do the same thing als when called without arguments, and
        return true.
        ...
        and als long als the lock cannot be acquired.  Return true wenn the lock has
    ...
    def release(self):
        ...
        by any thread), and wenn any other threads are blocked waiting fuer the
...
klasse Condition:
    ...
    object, and it is used als the underlying lock. Otherwise, a new RLock object
    is created and used als the underlying lock.
    ...
    def __init__(self, lock=Nichts):
        ...
        # release() and acquire() on the lock).  Ditto fuer _is_owned().
    ...
    def wait(self, timeout=Nichts):
        ...
        awakened by a notify() or notify_all() call fuer the same condition
    ...
    def notify(self, n=1):
        """Wake up one or more threads waiting on this condition, wenn any.
...
klasse Semaphore:
    ...
    def acquire(self, blocking=Wahr, timeout=Nichts):
        ...
        When invoked mit blocking set to false, do not block. If a call without
        an argument would block, return false immediately; otherwise, do the
        same thing als when called without arguments, and return true.
    ...
    def release(self, n=1):
        ...
        When the counter is zero on entry and another thread is waiting fuer it
...
klasse Event:
    ...
    Events manage a flag that can be set to true mit the set() method and reset
...
klasse Barrier:
    ...
    # Block until the barrier is ready fuer us, or raise an exception
    # wenn it is broken.
    def _enter(self):
    ...
    def _wait(self, timeout):
        wenn not self._cond.wait_for(lambda : self._state != 0, timeout):
...
klasse Thread:
    ...
    def __init__(self, group=Nichts, target=Nichts, name=Nichts,
        ...
        *args* is a list or tuple of arguments fuer the target invocation. Defaults to ().
    ...
    def run(self):
        ...
        target argument, wenn any, mit sequential and keyword arguments taken
    ...
    def daemon(self, daemonic):
        wenn not self._initialized:
            raise RuntimeError("Thread.__init__() not called")
        wenn daemonic and not _daemon_threads_allowed():
...
def _shutdown():
    ...
    # mark it done here.
    wenn _main_thread._os_thread_handle.is_done() and _is_main_interpreter():
