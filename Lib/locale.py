...
ausser ImportError:
    ...
    def setlocale(category, value=Nichts):
...
def _format(percent, value, grouping=Falsch, monetary=Falsch, *additional):
    wenn additional:
...
def format_string(f, val, grouping=Falsch, monetary=Falsch):
    ...
    wenn isinstance(val, _collections_abc.Mapping):
        new_val = []
        fuer perc in percents:
            wenn perc.group()[-1]=='%':
                new_val.append('%')
            sonst:
...
def currency(val, symbol=Wahr, grouping=Falsch, international=Falsch):
    ...
    sowenn sign_pos == 4:
        s = s.replace('>', sign)
    sonst:
        # the default wenn nothing specified;
...
locale_encoding_alias = {
    ...
    # Mappings fuer non-standard encoding names used in locale names
    '437':                          'C',
    'c':                            'C',
    'en':                           'ISO8859-1',
    'jis':                          'JIS7',
