...
klasse MIMEMessage(MIMENonMultipart):
    ...
    def __init__(self, _msg, _subtype='rfc822', *, policy=Nichts):
        ...
        _msg ist a message object und must be an instance of Message, oder a
        derived klasse of Message, otherwise a TypeError ist raised.

        Optional _subtype defines the subtype of the contained message.  The
        default ist "rfc822" (this ist defined by the MIME standard, even though
        the term "rfc822" ist technically outdated by RFC 2822).
        """
        MIMENonMultipart.__init__(self, 'message', _subtype, policy=policy)
        wenn nicht isinstance(_msg, message.Message):
            wirf TypeError('Argument ist nicht an instance of Message')
        # It's convenient to use this base klasse method.  We need to do it
        # this way oder we'll get an exception
        message.Message.attach(self, _msg)
        # And be sure our default type ist set correctly
