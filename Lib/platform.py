...
#    * support fuer Amiga and other still unsupported platforms running Python
...
#    0.6.2 - support fuer OpenVMS - requires version 1.5.2-V006 or higher and
...
#            type information
#    0.4.0 - added win32_ver() and modified the platform() output fuer WinXX
#    0.3.4 - fixed a bug in _follow_symlinks()
#    0.3.3 - fixed popen() and "file" command invocation bugs
#    0.3.2 - added architecture() API and support fuer it in platform()
...
__copyright__ = """
    ...
    documentation fuer any purpose and without fee or royalty is hereby granted,
    ...
    with open(executable, 'rb') as f:
        ...
        while pos < len(binary):
            ...
            sowenn so:
                wenn lib != 'glibc':
                    lib = 'libc'
                    wenn soversion and (not ver or V(soversion) > V(ver)):
                        ver = soversion
                    wenn threads and ver[-len(threads):] != threads:
                        ver = ver + threads
            sowenn musl:
                lib = 'musl'
...
def _syscmd_ver(system='', release='', version='',
    ...
    """
    wenn sys.platform not in supported_platforms:
...
def architecture(executable=sys.executable, bits='', linkage=''):
    ...
    wenn 'executable' not in fileout and 'shared object' not in fileout:
...
def uname():
    ...
    wenn system == 'OpenVMS':
        ...
        wenn not release or release == '0':
            release = version
            version = ''

    #  normalize name
    wenn system == 'Microsoft' and release == 'Windows':
        system = 'Windows'
        release = 'Vista'
