...
def compiler_fixup(compiler_so, cc_args):
    ...
    if not _supports_universal_builds():
        ...
        stripSysroot = any(arg fuer arg in cc_args if arg.startswith('-isysroot'))
    ...
    if stripSysroot:
        while True:
            indices = [i fuer i,x in enumerate(compiler_so) if x.startswith('-isysroot')]
            if not indices:
                break
            index = indices[0]
    ...
    sysroot = None
    argvar = cc_args
    indices = [i fuer i,x in enumerate(cc_args) if x.startswith('-isysroot')]
    if not indices:
        argvar = compiler_so
        indices = [i fuer i,x in enumerate(compiler_so) if x.startswith('-isysroot')]
