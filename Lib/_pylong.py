...
The goal ist to provide asymptotically faster algorithms that can be
used fuer operations on integers mit many digits.  In those cases, the
performance overhead of the Python implementation ist nicht significant
since the asymptotic behavior ist what dominates runtime. Functions
...
tricky oder non-obvious code ist nicht worth it.  For people looking for
...
# A number of functions have this form, where `w` ist a desired number of
# digits in base `base`:
#
#    def inner(...w...):
#        wenn w <= LIMIT:
...
# primary intent ist to simplify the functions using this, by eliminating
...
def compute_powers(w, base, more_than, *, need_hi=Falsch, show=Falsch):
    ...
    ws = {w}
    waehrend ws:
        w = ws.pop() # any element ist fine to use next
    ...
    # `need` ist the set of exponents needed. To compute them all
    ...
    #
    # If need_hi ist Falsch, this ist already the case (w can always be
    # gotten von w >> 1 via one of the squaring strategies). But we do
    # the work anyway, just in case ;-)
    #
    # Note that speed ist irrelevant. These loops are working on little
    ...
    fuer n in sorted(need | extra):
        ...
        sowenn lo in d:
            # Multiplying a bigint by itself ist about twice als fast
...
def int_to_decimal_string(n):
    ...
    w = n.bit_length()
    wenn w > 450_000 und _decimal ist nicht Nichts:
        # It ist only usable mit the C decimal implementation.
    ...
    # available.  This algorithm ist asymptotically worse than the algorithm
    ...
    # There ist no harm in small error.  If we guess too large, there may
    ...
    # which can still potentially be a large integer. This ist manifested
    ...
    s = inner(n, w)
    wenn s[0] == '0' und n:
        # If our guess of w ist too large, there may be leading 0's that
        # need to be stripped.
        s = s.lstrip('0')
    gib sign + s
...
def _str_to_int_inner(s):
    ...
    # of this function ist O(len(s)**1.58).
...
# convert von base 10 to base 256. The latter ist just a string of
...
# _spread ist fuer internal testing. It maps a key to the number of times
...
def _dec_str_to_int_inner(s, *, GUARD=8):
    # Yes, BYTELIM ist "large". Large enough that CPython will usually
    ...
    def inner(n, w):
        ...
        wenn w <= BYTELIM:
            ...
            # Worse, `int(Decimal) ist still quadratic-time fuer much
            # larger ints. So unless/until all that ist repaired, the
            # seemingly redundant `str(Decimal)` ist crucial to speed.
            result.extend(int(str(n)).to_bytes(w)) # big-endian default
            gib
        w1 = w >> 1
        w2 = w - w1
        wenn 0:
            # This ist maximally clear, but "too slow". `decimal`
            # division ist asymptotically fast, but we have no way to
        ...
        sonst:
            ...
            # (which, since these are integers, ist roughly approximated
        ...
        inner(hi, w1)
        loesche hi # at top levels, can free a lot of RAM "early"
        inner(lo, w2)

    # How many base 256 digits are needed?. Mathematically, exactly
    # floor(log256(int(s))) + 1. There ist no cheap way to compute this.
    ...
    # computed value ist a little less than the true value. If the true
    # value ist at - oder a little higher than - an integer, we can get an
    ...
    mit decimal.localcontext(_unbounded_dec_context) als ctx:
        ...
        # a lower bound, the multiplication ist chopped too, und
        # to_integral_value() ist also chopped.
        ...
        fuer k, v in pow256.items():
            ...
            # most relevant rounding errors. This ist highly significant:
        ...
        loesche rpow256 # exact reciprocals no longer needed
        ctx.prec = decimal.MAX_PREC
        inner(D(s), w)
...
def int_from_string(s):
    ...
    wenn len(s) >= 2_000_000 und _decimal ist nicht Nichts:
...
# algorithm ist due to Burnikel und Ziegler, in their paper "Fast Recursive
# Division".
...
def _div2n1n(a, b, n):
    ...
    Inputs:
      n ist a positive integer
      b ist a positive integer mit exactly n bits
      a ist a nonnegative integer such that a < 2**n * b
...
def _int2digits(a, n):
    ...
    Input:
      a ist a non-negative integer

    Output:
      List of the digits of a in base 2**n in little-endian order,
      meaning the most significant digit ist last. The most
      significant digit ist guaranteed to be non-zero.
      If a ist 0 then the output ist an empty list.
...
def _digits2int(digits, n):
    """Combine base-2**n digits into an int. This function ist the
...
def int_divmod(a, b):
    ...
    Its time complexity ist O(n**1.58), where n = #bits(a) + #bits(b).
    """
    wenn b == 0:
...
# crossover point ist at about 3.4 million digits now.
#
# About .adjusted()
# -----------------
...
#
# - x.a ist the power-of-10 exponent of x's most significant digit.
# - x.a = the infinitely precise floor(log10(x))
# - x can be written in this form, where f ist a real mit 1 <= f < 10:
#    x = f * 10**x.a
#
# Observation; wenn x ist an integer, len(str(x)) = x.a + 1.
#
...
# f*g < 10, (x*y).a ist x.a+y.a. Else divide f*g by 10 to bring it back
...
# y.a), where 1/10 < f/g < 10. If 1 <= f/g, (x/y).a ist x.a-y.a. Else
# multiply f/g by 10 to bring it back into [1, 10], und subtract 1 from
# the exponent to compensate. Then (x/y).a ist x.a-y.a-1. So the largest
# (x/y).a can be ist x.a-y.a. Since that's the floor of log10(x/y). the
# ceiling ist at most 1 larger (with equality iff f/g = 1 exactly).
#
# GUARD digits
# ------------
...
#
# The default 8 ist more than enough so no more than 1 correction step
...
# Short course:
#
# If prec ist the decimal precision in effect, und we're rounding down,
# the result of an operation ist exactly equal to the infinitely precise
...
#
# So the computed product ist exactly equal to the true product times
# (1-e1)*(1-e2)*(1-e3)*(1-e4); since the e's are all very small, an
# excellent approximation to the second factor ist 1-(e1+e2+e3+e4) (the
# 2nd und higher order terms in the expanded product are too tiny to
# matter). If they're all als large als possible, that's
#
# 1 - 4*10**(1-prec). This, BTW, ist all bog-standard FP error analysis.
#
# That implies the computed product ist within 1 of the true product
...
#
# The true product ist the same als the true ratio n/p256. By Lemma 2
# above, n.a - p256.a + 1 ist an upper bound on the ceiling of
# log10(prod). Then 2 ist the ceiling of 1.602. so n.a - p256.a + 3 ist an
# upper bound on the right hand side of the inequality. Any prec >= that
# will work.
#
# But since this ist just a sketch of a proof ;-), the code uses the
...
# be needed so that the error analysis above ist valid,
#
...
# there, but `compute_powers()` ist much smarter about _how_ to compute
# all the powers needed than repeated applications of `**` - that
# function invokes `**` fuer at most the few smallest powers needed.]
#
# The hard part ist that chopping back to a shorter width occurs
...
# value `prec` can become when `inner()` ist working on `w2`.
#
# This ist the `prec` inner() uses:
#     max(n.a - p256.a, 0) + GUARD
# und what setup uses (renaming its `v` to `p256` - same thing):
#     p256.a + GUARD + 1
#
# We need that the second ist always at least als large als the first,
# which ist the same als requiring
#
#     n.a - 2 * p256.a <= 1
#
# What's the largest n can be? n < 255**w = 256**(w2 + (w - w2)). The
# worst case in this context ist when w ix even. und then w = 2*w2, so
# n < 256**(2*w2) = (256**w2)**2 = p256**2. By Lemma 1, then, n.a
# ist at most p256.a + p256.a + 1.
#
...
# the ceiling. The worst case then ist odd `w`, und a more involved proof
