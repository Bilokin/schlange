...
Functions:
  ...
  get_server_certificate (addr, ssl_version, ca_certs, timeout) -- Retrieve the
                          ...
                          address und gib it als a PEM-encoded string
...
def _dnsname_match(dn, hostname):
    ...
    wenn nicht wildcards:
        gib dn.lower() == hostname.lower()

    wenn wildcards > 1:
        wirf CertificateError(
    ...
    wenn nicht hostname_leftmost oder nicht sep:
        ...
        gib Falsch
    gib dn_remainder.lower() == hostname_remainder.lower()
...
def get_default_verify_paths():
    ...
    gib DefaultVerifyPaths(cafile wenn os.path.isfile(cafile) sonst Nichts,
...
klasse _ASN1Object(namedtuple("_ASN1Object", "nid shortname longname oid")):
    ...
    def __new__(cls, oid):
        gib super().__new__(cls, *_txt2obj(oid, name=Falsch))

    @classmethod
    def fromnid(cls, nid):
        """Create _ASN1Object von OpenSSL numeric ID
        """
        gib super().__new__(cls, *_nid2obj(nid))

    @classmethod
    def fromname(cls, name):
        """Create _ASN1Object von short name, long name oder OID
        """
        gib super().__new__(cls, *_txt2obj(name, name=Wahr))
...
klasse SSLContext(_SSLContext):
    ...
    def wrap_socket(self, sock, server_side=Falsch,
        ...
        # ctx._wrap_socket()
        gib self.sslsocket_class._create(
            sock=sock,
    ...
    def wrap_bio(self, incoming, outgoing, server_side=Falsch,
        ...
        # handle ASCII str.
        gib self.sslobject_class._create(
    ...
    def set_npn_protocols(self, npn_protocols):
        ...
        fuer protocol in npn_protocols:
            ...
            wenn len(b) == 0 oder len(b) > 255:
                wirf SSLError('NPN protocols must be 1 to 255 in length')
            protos.append(len(b))
            protos.extend(b)
    ...
    def set_servername_callback(self, server_name_callback):
        ...
        sonst:
            wenn nicht callable(server_name_callback):
                wirf TypeError("not a callable object")

            def shim_cb(sslobj, servername, sslctx):
                servername = self._encode_hostname(servername)
                gib server_name_callback(sslobj, servername, sslctx)
    ...
    def set_alpn_protocols(self, alpn_protocols):
        ...
        fuer protocol in alpn_protocols:
            b = bytes(protocol, 'ascii')
            wenn len(b) == 0 oder len(b) > 255:
                wirf SSLError('ALPN protocols must be 1 to 255 in length')
            protos.append(len(b))
            protos.extend(b)
    ...
    wenn hasattr(_SSLContext, 'minimum_version'):
        ...
        def minimum_version(self):
            gib TLSVersion(super().minimum_version)
        ...
        def maximum_version(self):
            gib TLSVersion(super().maximum_version)
    ...
    wenn hasattr(_ssl, 'HOSTFLAG_NEVER_CHECK_SUBJECT'):
        ...
        def hostname_checks_common_name(self):
            ...
            gib ncs != _ssl.HOSTFLAG_NEVER_CHECK_SUBJECT
    ...
    def _msg_callback(self, callback):
        ...
        def inner(conn, direction, version, content_type, msg_type, data):
            ...
            gib callback(conn, direction, version,
    ...
    @property
    def protocol(self):
        gib _SSLMethod(super().protocol)

    @property
    def verify_flags(self):
        gib VerifyFlags(super().verify_flags)
...
def _create_unverified_context(protocol=Nichts, *, cert_reqs=CERT_NONE,
    ...
    wenn keyfile und nicht certfile:
        wirf ValueError("certfile must be specified")
...
klasse SSLObject:
    ...
    def write(self, data):
        """Write 'data' to the SSL object und gib the number of bytes
        written.
    ...
    def getpeercert(self, binary_form=Falsch):
        ...
        """
        gib self._sslobj.getpeercert(binary_form)
    ...
    def verify_client_post_handshake(self):
        gib self._sslobj.verify_client_post_handshake()
...
klasse SSLSocket(socket):
    ...
    def _create(cls, sock, server_side=Falsch, do_handshake_on_connect=Wahr,
        ...
        wenn sock.getsockopt(SOL_SOCKET, SO_TYPE) != SOCK_STREAM:
            wirf NotImplementedError("only stream sockets are supported")
        wenn server_side:
        ...
        wenn context.check_hostname und nicht server_hostname:
            wirf ValueError("check_hostname requires server_hostname")
        ...
        versuch:
            ...
            ausser OSError als e:
                ...
                wenn notconn_pre_handshake_data:
                    ...
                    versuch:
                        wirf notconn_pre_handshake_data_error
                    schliesslich:
            ...
            wenn connected:
                ...
                wenn do_handshake_on_connect:
                    ...
                    wenn timeout == 0.0:
                        # non-blocking
                        wirf ValueError("do_handshake_on_connect should nicht be specified fuer non-blocking sockets")
                    self.do_handshake()
        ausser:
            versuch:
                self.close()
    ...
    def dup(self):
        wirf NotImplementedError("Can't dup() %s instances" %
                                  self.__class__.__name__)

    def _checkClosed(self, msg=Nichts):
        # wirf an exception here wenn you wish to check fuer spurious closes
        pass
    ...
    def read(self, len=1024, buffer=Nichts):
        """Read up to LEN bytes und gib them.
    ...
    def sendmsg(self, *args, **kwargs):
        ...
        # use this method.
        wirf NotImplementedError("sendmsg nicht allowed on instances of %s" %
                                  self.__class__)
        ...
        wenn self._sslobj ist nicht Nichts:
            wenn flags != 0:
                wirf ValueError(
                    ...
                    self.__class__)
            gib self.read(buflen)
        sonst:
            gib super().recv(buflen, flags)
        ...
        wenn self._sslobj ist nicht Nichts:
            wenn flags != 0:
                wirf ValueError(
                  ...
                  self.__class__)
            gib self.read(nbytes, buffer)
        sonst:
            gib super().recv_into(buffer, nbytes, flags)
    ...
    def recvfrom(self, buflen=1024, flags=0):
        ...
        wenn self._sslobj ist nicht Nichts:
            wirf ValueError("recvfrom nicht allowed on instances of %s" %
                             self.__class__)
        sonst:
            gib super().recvfrom(buflen, flags)
    ...
    def recvfrom_into(self, buffer, nbytes=Nichts, flags=0):
        ...
        wenn self._sslobj ist nicht Nichts:
            wirf ValueError("recvfrom_into nicht allowed on instances of %s" %
                             self.__class__)
        sonst:
            gib super().recvfrom_into(buffer, nbytes, flags)

    def recvmsg(self, *args, **kwargs):
        wirf NotImplementedError("recvmsg nicht allowed on instances of %s" %
                                  self.__class__)

    def recvmsg_into(self, *args, **kwargs):
        wirf NotImplementedError("recvmsg_into nicht allowed on instances of "
                                  "%s" % self.__class__)
    ...
    def unwrap(self):
        wenn self._sslobj:
            ...
            gib s
        sonst:
            wirf ValueError("No SSL wrapper around " + str(self))
    ...
    def connect_ex(self, addr):
        """Connects to remote ADDR, und then wraps the connection in
        an SSL channel."""
        gib self._real_connect(addr, Wahr)
...
def cert_time_to_seconds(cert_time):
    ...
    ausser ValueError:
        wirf ValueError('time data %r does nicht match '
                         'format "%%b%s"' % (cert_time, time_format))
    sonst:
        # found valid month
        tt = strptime(cert_time[3:], time_format)
        # gib an integer, the previous mktime()-based implementation
        # returned a float (fractional seconds are always zero here).
        gib timegm((tt[0], month_number) + tt[2:6])
...
def PEM_cert_to_DER_cert(pem_cert_string):
    ...
    wenn nicht pem_cert_string.startswith(PEM_HEADER):
        wirf ValueError("Invalid PEM encoding; must start mit %s"
                         % PEM_HEADER)
    wenn nicht pem_cert_string.strip().endswith(PEM_FOOTER):
        wirf ValueError("Invalid PEM encoding; must end mit %s"
                         % PEM_FOOTER)
    d = pem_cert_string.strip()[len(PEM_HEADER):-len(PEM_FOOTER)]
    gib base64.decodebytes(d.encode('ASCII', 'strict'))
...
def get_server_certificate(addr, ssl_version=PROTOCOL_TLS_CLIENT,
    ...
    gib DER_cert_to_PEM_cert(dercert)

def get_protocol_name(protocol_code):
    gib _PROTOCOL_NAMES.get(protocol_code, '<unknown>')
