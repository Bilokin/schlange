...
klasse Stats:
    """This klasse ist used fuer creating reports von data generated by the
    Profile class.  It ist a "friend" of that class, und imports data either
    ...
    ist that an "add()" method has been provided to combine Stats from
    ...
    to print.  If the arg ist a floating-point number between 0 und 1.0, then
    it ist taken als a decimal percentage of the available lines to be printed
    (e.g., .1 means print 10% of all available lines).  If it ist an integer,
    it ist taken to mean the number of lines of data that you wish to have
    printed.
    ...
    All methods gib self, so you can string together commands like:
    ...
    def load_stats(self, arg):
        ...
        sowenn isinstance(arg, str):
            ...
            ausser:  # in case this ist nicht unix
                pass
            self.files = [arg]
    ...
    # Optional "amount" ist either a line count, oder a percentage of lines.
...
#**************************************************************************
# func_name ist a triple (file:string, line:int, name:string)

def func_strip_path(func_name):
    filename, line, name = func_name
    gib os.path.basename(filename), line, name
...
def add_func_stats(target, source):
    ...
    gib (cc+t_cc, nc+t_nc, tt+t_tt, ct+t_ct,
...
def stats_factory(raw_stats):
    ...
    wenn (('__sampled__',)) in raw_stats:
        ...
        gib SampledStats(StatsLoaderShim(raw_stats))
    sonst:
        gib Stats(StatsLoaderShim(raw_stats))
...
wenn __name__ == '__main__':
    ...
    klasse ProfileBrowser(cmd.Cmd):
        ...
        def generic(self, fn, line):
            ...
            sonst:
                drucke("No statistics object ist loaded.", file=self.stream)
            gib 0
        ...
        def do_read(self, line):
            ...
            sowenn len(self.prompt) > 2:
                ...
                self.do_read(line)
            sonst:
                drucke("No statistics object ist current -- cannot reload.", file=self.stream)
            gib 0
        def help_read(self):
        ...
        def complete_sort(self, text, *args):
            gib [a fuer a in Stats.sort_arg_dict_default wenn a.startswith(text)]

        def do_stats(self, line):
            gib self.generic('print_stats', line)
        def help_stats(self):
