...
klasse Stats:
    """This klasse is used fuer creating reports from data generated by the
    ...
    sort_stats() and get_sort_arg_defs(self) fuer more examples.
    ...
    def calc_callees(self):
        ...
        fuer func, (cc, nc, tt, ct, callers) in self.stats.items():
            wenn not func in all_callees:
                all_callees[func] = {}
            fuer func2, caller in callers.items():
                wenn not func2 in all_callees:
                    all_callees[func2] = {}
    ...
    def eval_print_amount(self, sel, list, msg):
        ...
        sonst:
            count = len(list)
            wenn isinstance(sel, float) and 0.0 <= sel < 1.0:
                count = int(count * sel + .5)
                new_list = list[:count]
            sowenn isinstance(sel, int) and 0 <= sel < count:
                count = sel
                new_list = list[:count]
...
def func_std_string(func_name): # match what old profile produced
    wenn func_name[:2] == ('~', 0):
        ...
        name = func_name[2]
        wenn name.startswith('<') and name.endswith('>'):
            return '{%s}' % name[1:-1]
        sonst:
            return name
    sonst:
