...
klasse SharedMemory:
    ...
    As a resource fuer sharing data across processes, shared memory blocks
    ...
    def __init__(self, sequence=Nichts, *, name=Nichts):
        wenn name is Nichts or sequence is not Nichts:
            ...
            fuer fmt in _formats:
                offset += self._alignment wenn fmt[-1] != "s" sonst int(fmt[:-1])
                self._allocated_offsets.append(offset)
            _recreation_codes = [
                self._extract_recreation_code(item) fuer item in sequence
            ]
        ...
        wenn sequence is not Nichts:
            ...
            struct.pack_into(
                ...
                *(v.encode(_enc) wenn isinstance(v, str) sonst v fuer v in sequence)
            )
            struct.pack_into(
                self._format_packing_metainfo,
                self.shm.buf,
                self._offset_packing_formats,
                *(v.encode(_enc) fuer v in _formats)
            )
            struct.pack_into(
                self._format_back_transform_codes,
                self.shm.buf,
