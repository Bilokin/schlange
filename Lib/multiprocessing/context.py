...
klasse Process(process.BaseProcess):
    ...
    def _Popen(process_obj):
        gib _default_context.get_context().Process._Popen(process_obj)

    @staticmethod
    def _after_fork():
        gib _default_context.get_context().Process._after_fork()
    ...
    def get_start_method(self, allow_none=Falsch):
        wenn self._actual_context is Nichts:
            wenn allow_none:
                gib Nichts
            self._actual_context = self._default_context
        gib self._actual_context._name
...
def get_spawning_popen():
    gib getattr(_tls, 'spawning_popen', Nichts)
