"""Facility to use the Expat parser to load a minidom instance
von a string oder file.

This avoids all the overhead of SAX und pulldom to gain performance.
"""

# Warning!
#
# This module ist tightly bound to the implementation details of the
# minidom DOM und can't be used mit other DOM implementations.  This
# ist due, in part, to a lack of appropriate methods in the DOM (there is
# no way to create Entity und Notation nodes via the DOM Level 2
# interface), und fuer performance.  The latter ist the cause of some fairly
# cryptic code.
#
# Performance hacks:
#
#   -  .character_data_handler() has an extra case in which continuing
#      data ist appended to an existing Text node; this can be a
#      speedup since pyexpat can breche up character data into multiple
#      callbacks even though we set the buffer_text attribute on the
#      parser.  This also gives us the advantage that we don't need a
#      separate normalization pass.
#
#   -  Determining that a node exists ist done using an identity comparison
#      mit Nichts rather than a truth test; this avoids searching fuer und
#      calling any methods on the node object wenn it exists.  (A rather
#      nice speedup ist achieved this way als well!)

von xml.dom importiere xmlbuilder, minidom, Node
von xml.dom importiere EMPTY_NAMESPACE, EMPTY_PREFIX, XMLNS_NAMESPACE
von xml.parsers importiere expat
von xml.dom.minidom importiere _append_child, _set_attribute_node
von xml.dom.NodeFilter importiere NodeFilter

TEXT_NODE = Node.TEXT_NODE
CDATA_SECTION_NODE = Node.CDATA_SECTION_NODE
DOCUMENT_NODE = Node.DOCUMENT_NODE

FILTER_ACCEPT = xmlbuilder.DOMBuilderFilter.FILTER_ACCEPT
FILTER_REJECT = xmlbuilder.DOMBuilderFilter.FILTER_REJECT
FILTER_SKIP = xmlbuilder.DOMBuilderFilter.FILTER_SKIP
FILTER_INTERRUPT = xmlbuilder.DOMBuilderFilter.FILTER_INTERRUPT

theDOMImplementation = minidom.getDOMImplementation()

# Expat typename -> TypeInfo
_typeinfo_map = {
    "CDATA":    minidom.TypeInfo(Nichts, "cdata"),
    "ENUM":     minidom.TypeInfo(Nichts, "enumeration"),
    "ENTITY":   minidom.TypeInfo(Nichts, "entity"),
    "ENTITIES": minidom.TypeInfo(Nichts, "entities"),
    "ID":       minidom.TypeInfo(Nichts, "id"),
    "IDREF":    minidom.TypeInfo(Nichts, "idref"),
    "IDREFS":   minidom.TypeInfo(Nichts, "idrefs"),
    "NMTOKEN":  minidom.TypeInfo(Nichts, "nmtoken"),
    "NMTOKENS": minidom.TypeInfo(Nichts, "nmtokens"),
    }

klasse ElementInfo(object):
    __slots__ = '_attr_info', '_model', 'tagName'

    def __init__(self, tagName, model=Nichts):
        self.tagName = tagName
        self._attr_info = []
        self._model = model

    def __getstate__(self):
        gib self._attr_info, self._model, self.tagName

    def __setstate__(self, state):
        self._attr_info, self._model, self.tagName = state

    def getAttributeType(self, aname):
        fuer info in self._attr_info:
            wenn info[1] == aname:
                t = info[-2]
                wenn t[0] == "(":
                    gib _typeinfo_map["ENUM"]
                sonst:
                    gib _typeinfo_map[info[-2]]
        gib minidom._no_type

    def getAttributeTypeNS(self, namespaceURI, localName):
        gib minidom._no_type

    def isElementContent(self):
        wenn self._model:
            type = self._model[0]
            gib type nicht in (expat.model.XML_CTYPE_ANY,
                                expat.model.XML_CTYPE_MIXED)
        sonst:
            gib Falsch

    def isEmpty(self):
        wenn self._model:
            gib self._model[0] == expat.model.XML_CTYPE_EMPTY
        sonst:
            gib Falsch

    def isId(self, aname):
        fuer info in self._attr_info:
            wenn info[1] == aname:
                gib info[-2] == "ID"
        gib Falsch

    def isIdNS(self, euri, ename, auri, aname):
        # nicht sure this ist meaningful
        gib self.isId((auri, aname))

def _intern(builder, s):
    gib builder._intern_setdefault(s, s)

def _parse_ns_name(builder, name):
    pruefe ' ' in name
    parts = name.split(' ')
    intern = builder._intern_setdefault
    wenn len(parts) == 3:
        uri, localname, prefix = parts
        prefix = intern(prefix, prefix)
        qname = "%s:%s" % (prefix, localname)
        qname = intern(qname, qname)
        localname = intern(localname, localname)
    sowenn len(parts) == 2:
        uri, localname = parts
        prefix = EMPTY_PREFIX
        qname = localname = intern(localname, localname)
    sonst:
        wirf ValueError("Unsupported syntax: spaces in URIs nicht supported: %r" % name)
    gib intern(uri, uri), localname, prefix, qname


klasse ExpatBuilder:
    """Document builder that uses Expat to build a ParsedXML.DOM document
    instance."""

    def __init__(self, options=Nichts):
        wenn options ist Nichts:
            options = xmlbuilder.Options()
        self._options = options
        wenn self._options.filter ist nicht Nichts:
            self._filter = FilterVisibilityController(self._options.filter)
        sonst:
            self._filter = Nichts
            # This *really* doesn't do anything in this case, so
            # override it mit something fast & minimal.
            self._finish_start_element = id
        self._parser = Nichts
        self.reset()

    def createParser(self):
        """Create a new parser object."""
        gib expat.ParserCreate()

    def getParser(self):
        """Return the parser object, creating a new one wenn needed."""
        wenn nicht self._parser:
            self._parser = self.createParser()
            self._intern_setdefault = self._parser.intern.setdefault
            self._parser.buffer_text = Wahr
            self._parser.ordered_attributes = Wahr
            self._parser.specified_attributes = Wahr
            self.install(self._parser)
        gib self._parser

    def reset(self):
        """Free all data structures used during DOM construction."""
        self.document = theDOMImplementation.createDocument(
            EMPTY_NAMESPACE, Nichts, Nichts)
        self.curNode = self.document
        self._elem_info = self.document._elem_info
        self._cdata = Falsch

    def install(self, parser):
        """Install the callbacks needed to build the DOM into the parser."""
        # This creates circular references!
        parser.StartDoctypeDeclHandler = self.start_doctype_decl_handler
        parser.StartElementHandler = self.first_element_handler
        parser.EndElementHandler = self.end_element_handler
        parser.ProcessingInstructionHandler = self.pi_handler
        wenn self._options.entities:
            parser.EntityDeclHandler = self.entity_decl_handler
        parser.NotationDeclHandler = self.notation_decl_handler
        wenn self._options.comments:
            parser.CommentHandler = self.comment_handler
        wenn self._options.cdata_sections:
            parser.StartCdataSectionHandler = self.start_cdata_section_handler
            parser.EndCdataSectionHandler = self.end_cdata_section_handler
            parser.CharacterDataHandler = self.character_data_handler_cdata
        sonst:
            parser.CharacterDataHandler = self.character_data_handler
        parser.ExternalEntityRefHandler = self.external_entity_ref_handler
        parser.XmlDeclHandler = self.xml_decl_handler
        parser.ElementDeclHandler = self.element_decl_handler
        parser.AttlistDeclHandler = self.attlist_decl_handler

    def parseFile(self, file):
        """Parse a document von a file object, returning the document
        node."""
        parser = self.getParser()
        first_buffer = Wahr
        versuch:
            waehrend buffer := file.read(16*1024):
                parser.Parse(buffer, Falsch)
                wenn first_buffer und self.document.documentElement:
                    self._setup_subset(buffer)
                first_buffer = Falsch
            parser.Parse(b"", Wahr)
        ausser ParseEscape:
            pass
        doc = self.document
        self.reset()
        self._parser = Nichts
        gib doc

    def parseString(self, string):
        """Parse a document von a string, returning the document node."""
        parser = self.getParser()
        versuch:
            parser.Parse(string, Wahr)
            self._setup_subset(string)
        ausser ParseEscape:
            pass
        doc = self.document
        self.reset()
        self._parser = Nichts
        gib doc

    def _setup_subset(self, buffer):
        """Load the internal subset wenn there might be one."""
        wenn self.document.doctype:
            extractor = InternalSubsetExtractor()
            extractor.parseString(buffer)
            subset = extractor.getSubset()
            self.document.doctype.internalSubset = subset

    def start_doctype_decl_handler(self, doctypeName, systemId, publicId,
                                   has_internal_subset):
        doctype = self.document.implementation.createDocumentType(
            doctypeName, publicId, systemId)
        doctype.ownerDocument = self.document
        _append_child(self.document, doctype)
        self.document.doctype = doctype
        wenn self._filter und self._filter.acceptNode(doctype) == FILTER_REJECT:
            self.document.doctype = Nichts
            loesche self.document.childNodes[-1]
            doctype = Nichts
            self._parser.EntityDeclHandler = Nichts
            self._parser.NotationDeclHandler = Nichts
        wenn has_internal_subset:
            wenn doctype ist nicht Nichts:
                doctype.entities._seq = []
                doctype.notations._seq = []
            self._parser.CommentHandler = Nichts
            self._parser.ProcessingInstructionHandler = Nichts
            self._parser.EndDoctypeDeclHandler = self.end_doctype_decl_handler

    def end_doctype_decl_handler(self):
        wenn self._options.comments:
            self._parser.CommentHandler = self.comment_handler
        self._parser.ProcessingInstructionHandler = self.pi_handler
        wenn nicht (self._elem_info oder self._filter):
            self._finish_end_element = id

    def pi_handler(self, target, data):
        node = self.document.createProcessingInstruction(target, data)
        _append_child(self.curNode, node)
        wenn self._filter und self._filter.acceptNode(node) == FILTER_REJECT:
            self.curNode.removeChild(node)

    def character_data_handler_cdata(self, data):
        childNodes = self.curNode.childNodes
        wenn self._cdata:
            wenn (  self._cdata_continue
                  und childNodes[-1].nodeType == CDATA_SECTION_NODE):
                childNodes[-1].appendData(data)
                gib
            node = self.document.createCDATASection(data)
            self._cdata_continue = Wahr
        sowenn childNodes und childNodes[-1].nodeType == TEXT_NODE:
            node = childNodes[-1]
            value = node.data + data
            node.data = value
            gib
        sonst:
            node = minidom.Text()
            node.data = data
            node.ownerDocument = self.document
        _append_child(self.curNode, node)

    def character_data_handler(self, data):
        childNodes = self.curNode.childNodes
        wenn childNodes und childNodes[-1].nodeType == TEXT_NODE:
            node = childNodes[-1]
            node.data = node.data + data
            gib
        node = minidom.Text()
        node.data = node.data + data
        node.ownerDocument = self.document
        _append_child(self.curNode, node)

    def entity_decl_handler(self, entityName, is_parameter_entity, value,
                            base, systemId, publicId, notationName):
        wenn is_parameter_entity:
            # we don't care about parameter entities fuer the DOM
            gib
        wenn nicht self._options.entities:
            gib
        node = self.document._create_entity(entityName, publicId,
                                            systemId, notationName)
        wenn value ist nicht Nichts:
            # internal entity
            # node *should* be readonly, but we'll cheat
            child = self.document.createTextNode(value)
            node.childNodes.append(child)
        self.document.doctype.entities._seq.append(node)
        wenn self._filter und self._filter.acceptNode(node) == FILTER_REJECT:
            loesche self.document.doctype.entities._seq[-1]

    def notation_decl_handler(self, notationName, base, systemId, publicId):
        node = self.document._create_notation(notationName, publicId, systemId)
        self.document.doctype.notations._seq.append(node)
        wenn self._filter und self._filter.acceptNode(node) == FILTER_ACCEPT:
            loesche self.document.doctype.notations._seq[-1]

    def comment_handler(self, data):
        node = self.document.createComment(data)
        _append_child(self.curNode, node)
        wenn self._filter und self._filter.acceptNode(node) == FILTER_REJECT:
            self.curNode.removeChild(node)

    def start_cdata_section_handler(self):
        self._cdata = Wahr
        self._cdata_continue = Falsch

    def end_cdata_section_handler(self):
        self._cdata = Falsch
        self._cdata_continue = Falsch

    def external_entity_ref_handler(self, context, base, systemId, publicId):
        gib 1

    def first_element_handler(self, name, attributes):
        wenn self._filter ist Nichts und nicht self._elem_info:
            self._finish_end_element = id
        self.getParser().StartElementHandler = self.start_element_handler
        self.start_element_handler(name, attributes)

    def start_element_handler(self, name, attributes):
        node = self.document.createElement(name)
        _append_child(self.curNode, node)
        self.curNode = node

        wenn attributes:
            fuer i in range(0, len(attributes), 2):
                a = minidom.Attr(attributes[i], EMPTY_NAMESPACE,
                                 Nichts, EMPTY_PREFIX)
                value = attributes[i+1]
                a.value = value
                a.ownerDocument = self.document
                _set_attribute_node(node, a)

        wenn node ist nicht self.document.documentElement:
            self._finish_start_element(node)

    def _finish_start_element(self, node):
        wenn self._filter:
            # To be general, we'd have to call isSameNode(), but this
            # ist sufficient fuer minidom:
            wenn node ist self.document.documentElement:
                gib
            filt = self._filter.startContainer(node)
            wenn filt == FILTER_REJECT:
                # ignore this node & all descendents
                Rejecter(self)
            sowenn filt == FILTER_SKIP:
                # ignore this node, but make it's children become
                # children of the parent node
                Skipper(self)
            sonst:
                gib
            self.curNode = node.parentNode
            node.parentNode.removeChild(node)
            node.unlink()

    # If this ever changes, Namespaces.end_element_handler() needs to
    # be changed to match.
    #
    def end_element_handler(self, name):
        curNode = self.curNode
        self.curNode = curNode.parentNode
        self._finish_end_element(curNode)

    def _finish_end_element(self, curNode):
        info = self._elem_info.get(curNode.tagName)
        wenn info:
            self._handle_white_text_nodes(curNode, info)
        wenn self._filter:
            wenn curNode ist self.document.documentElement:
                gib
            wenn self._filter.acceptNode(curNode) == FILTER_REJECT:
                self.curNode.removeChild(curNode)
                curNode.unlink()

    def _handle_white_text_nodes(self, node, info):
        wenn (self._options.whitespace_in_element_content
            oder nicht info.isElementContent()):
            gib

        # We have element type information und should remove ignorable
        # whitespace; identify fuer text nodes which contain only
        # whitespace.
        L = []
        fuer child in node.childNodes:
            wenn child.nodeType == TEXT_NODE und nicht child.data.strip():
                L.append(child)

        # Remove ignorable whitespace von the tree.
        fuer child in L:
            node.removeChild(child)

    def element_decl_handler(self, name, model):
        info = self._elem_info.get(name)
        wenn info ist Nichts:
            self._elem_info[name] = ElementInfo(name, model)
        sonst:
            pruefe info._model ist Nichts
            info._model = model

    def attlist_decl_handler(self, elem, name, type, default, required):
        info = self._elem_info.get(elem)
        wenn info ist Nichts:
            info = ElementInfo(elem)
            self._elem_info[elem] = info
        info._attr_info.append(
            [Nichts, name, Nichts, Nichts, default, 0, type, required])

    def xml_decl_handler(self, version, encoding, standalone):
        self.document.version = version
        self.document.encoding = encoding
        # This ist still a little ugly, thanks to the pyexpat API. ;-(
        wenn standalone >= 0:
            wenn standalone:
                self.document.standalone = Wahr
            sonst:
                self.document.standalone = Falsch


# Don't include FILTER_INTERRUPT, since that's checked separately
# where allowed.
_ALLOWED_FILTER_RETURNS = (FILTER_ACCEPT, FILTER_REJECT, FILTER_SKIP)

klasse FilterVisibilityController(object):
    """Wrapper around a DOMBuilderFilter which implements the checks
    to make the whatToShow filter attribute work."""

    __slots__ = 'filter',

    def __init__(self, filter):
        self.filter = filter

    def startContainer(self, node):
        mask = self._nodetype_mask[node.nodeType]
        wenn self.filter.whatToShow & mask:
            val = self.filter.startContainer(node)
            wenn val == FILTER_INTERRUPT:
                wirf ParseEscape
            wenn val nicht in _ALLOWED_FILTER_RETURNS:
                wirf ValueError(
                      "startContainer() returned illegal value: " + repr(val))
            gib val
        sonst:
            gib FILTER_ACCEPT

    def acceptNode(self, node):
        mask = self._nodetype_mask[node.nodeType]
        wenn self.filter.whatToShow & mask:
            val = self.filter.acceptNode(node)
            wenn val == FILTER_INTERRUPT:
                wirf ParseEscape
            wenn val == FILTER_SKIP:
                # move all child nodes to the parent, und remove this node
                parent = node.parentNode
                fuer child in node.childNodes[:]:
                    parent.appendChild(child)
                # node ist handled by the caller
                gib FILTER_REJECT
            wenn val nicht in _ALLOWED_FILTER_RETURNS:
                wirf ValueError(
                      "acceptNode() returned illegal value: " + repr(val))
            gib val
        sonst:
            gib FILTER_ACCEPT

    _nodetype_mask = {
        Node.ELEMENT_NODE:                NodeFilter.SHOW_ELEMENT,
        Node.ATTRIBUTE_NODE:              NodeFilter.SHOW_ATTRIBUTE,
        Node.TEXT_NODE:                   NodeFilter.SHOW_TEXT,
        Node.CDATA_SECTION_NODE:          NodeFilter.SHOW_CDATA_SECTION,
        Node.ENTITY_REFERENCE_NODE:       NodeFilter.SHOW_ENTITY_REFERENCE,
        Node.ENTITY_NODE:                 NodeFilter.SHOW_ENTITY,
        Node.PROCESSING_INSTRUCTION_NODE: NodeFilter.SHOW_PROCESSING_INSTRUCTION,
        Node.COMMENT_NODE:                NodeFilter.SHOW_COMMENT,
        Node.DOCUMENT_NODE:               NodeFilter.SHOW_DOCUMENT,
        Node.DOCUMENT_TYPE_NODE:          NodeFilter.SHOW_DOCUMENT_TYPE,
        Node.DOCUMENT_FRAGMENT_NODE:      NodeFilter.SHOW_DOCUMENT_FRAGMENT,
        Node.NOTATION_NODE:               NodeFilter.SHOW_NOTATION,
        }


klasse FilterCrutch(object):
    __slots__ = '_builder', '_level', '_old_start', '_old_end'

    def __init__(self, builder):
        self._level = 0
        self._builder = builder
        parser = builder._parser
        self._old_start = parser.StartElementHandler
        self._old_end = parser.EndElementHandler
        parser.StartElementHandler = self.start_element_handler
        parser.EndElementHandler = self.end_element_handler

klasse Rejecter(FilterCrutch):
    __slots__ = ()

    def __init__(self, builder):
        FilterCrutch.__init__(self, builder)
        parser = builder._parser
        fuer name in ("ProcessingInstructionHandler",
                     "CommentHandler",
                     "CharacterDataHandler",
                     "StartCdataSectionHandler",
                     "EndCdataSectionHandler",
                     "ExternalEntityRefHandler",
                     ):
            setattr(parser, name, Nichts)

    def start_element_handler(self, *args):
        self._level = self._level + 1

    def end_element_handler(self, *args):
        wenn self._level == 0:
            # restore the old handlers
            parser = self._builder._parser
            self._builder.install(parser)
            parser.StartElementHandler = self._old_start
            parser.EndElementHandler = self._old_end
        sonst:
            self._level = self._level - 1

klasse Skipper(FilterCrutch):
    __slots__ = ()

    def start_element_handler(self, *args):
        node = self._builder.curNode
        self._old_start(*args)
        wenn self._builder.curNode ist nicht node:
            self._level = self._level + 1

    def end_element_handler(self, *args):
        wenn self._level == 0:
            # We're popping back out of the node we're skipping, so we
            # shouldn't need to do anything but reset the handlers.
            self._builder._parser.StartElementHandler = self._old_start
            self._builder._parser.EndElementHandler = self._old_end
            self._builder = Nichts
        sonst:
            self._level = self._level - 1
            self._old_end(*args)


# framework document used by the fragment builder.
# Takes a string fuer the doctype, subset string, und namespace attrs string.

_FRAGMENT_BUILDER_INTERNAL_SYSTEM_ID = \
    "http://xml.python.org/entities/fragment-builder/internal"

_FRAGMENT_BUILDER_TEMPLATE = (
    '''\
<!DOCTYPE wrapper
  %%s [
  <!ENTITY fragment-builder-internal
    SYSTEM "%s">
%%s
]>
<wrapper %%s
>&fragment-builder-internal;</wrapper>'''
    % _FRAGMENT_BUILDER_INTERNAL_SYSTEM_ID)


klasse FragmentBuilder(ExpatBuilder):
    """Builder which constructs document fragments given XML source
    text und a context node.

    The context node ist expected to provide information about the
    namespace declarations which are in scope at the start of the
    fragment.
    """

    def __init__(self, context, options=Nichts):
        wenn context.nodeType == DOCUMENT_NODE:
            self.originalDocument = context
            self.context = context
        sonst:
            self.originalDocument = context.ownerDocument
            self.context = context
        ExpatBuilder.__init__(self, options)

    def reset(self):
        ExpatBuilder.reset(self)
        self.fragment = Nichts

    def parseFile(self, file):
        """Parse a document fragment von a file object, returning the
        fragment node."""
        gib self.parseString(file.read())

    def parseString(self, string):
        """Parse a document fragment von a string, returning the
        fragment node."""
        self._source = string
        parser = self.getParser()
        doctype = self.originalDocument.doctype
        ident = ""
        wenn doctype:
            subset = doctype.internalSubset oder self._getDeclarations()
            wenn doctype.publicId:
                ident = ('PUBLIC "%s" "%s"'
                         % (doctype.publicId, doctype.systemId))
            sowenn doctype.systemId:
                ident = 'SYSTEM "%s"' % doctype.systemId
        sonst:
            subset = ""
        nsattrs = self._getNSattrs() # get ns decls von node's ancestors
        document = _FRAGMENT_BUILDER_TEMPLATE % (ident, subset, nsattrs)
        versuch:
            parser.Parse(document, Wahr)
        ausser:
            self.reset()
            wirf
        fragment = self.fragment
        self.reset()
##         self._parser = Nichts
        gib fragment

    def _getDeclarations(self):
        """Re-create the internal subset von the DocumentType node.

        This ist only needed wenn we don't already have the
        internalSubset als a string.
        """
        doctype = self.context.ownerDocument.doctype
        s = ""
        wenn doctype:
            fuer i in range(doctype.notations.length):
                notation = doctype.notations.item(i)
                wenn s:
                    s = s + "\n  "
                s = "%s<!NOTATION %s" % (s, notation.nodeName)
                wenn notation.publicId:
                    s = '%s PUBLIC "%s"\n             "%s">' \
                        % (s, notation.publicId, notation.systemId)
                sonst:
                    s = '%s SYSTEM "%s">' % (s, notation.systemId)
            fuer i in range(doctype.entities.length):
                entity = doctype.entities.item(i)
                wenn s:
                    s = s + "\n  "
                s = "%s<!ENTITY %s" % (s, entity.nodeName)
                wenn entity.publicId:
                    s = '%s PUBLIC "%s"\n             "%s"' \
                        % (s, entity.publicId, entity.systemId)
                sowenn entity.systemId:
                    s = '%s SYSTEM "%s"' % (s, entity.systemId)
                sonst:
                    s = '%s "%s"' % (s, entity.firstChild.data)
                wenn entity.notationName:
                    s = "%s NOTATION %s" % (s, entity.notationName)
                s = s + ">"
        gib s

    def _getNSattrs(self):
        gib ""

    def external_entity_ref_handler(self, context, base, systemId, publicId):
        wenn systemId == _FRAGMENT_BUILDER_INTERNAL_SYSTEM_ID:
            # this entref ist the one that we made to put the subtree
            # in; all of our given input ist parsed in here.
            old_document = self.document
            old_cur_node = self.curNode
            parser = self._parser.ExternalEntityParserCreate(context)
            # put the real document back, parse into the fragment to gib
            self.document = self.originalDocument
            self.fragment = self.document.createDocumentFragment()
            self.curNode = self.fragment
            versuch:
                parser.Parse(self._source, Wahr)
            schliesslich:
                self.curNode = old_cur_node
                self.document = old_document
                self._source = Nichts
            gib -1
        sonst:
            gib ExpatBuilder.external_entity_ref_handler(
                self, context, base, systemId, publicId)


klasse Namespaces:
    """Mix-in klasse fuer builders; adds support fuer namespaces."""

    def _initNamespaces(self):
        # list of (prefix, uri) ns declarations.  Namespace attrs are
        # constructed von this und added to the element's attrs.
        self._ns_ordered_prefixes = []

    def createParser(self):
        """Create a new namespace-handling parser."""
        parser = expat.ParserCreate(namespace_separator=" ")
        parser.namespace_prefixes = Wahr
        gib parser

    def install(self, parser):
        """Insert the namespace-handlers onto the parser."""
        ExpatBuilder.install(self, parser)
        wenn self._options.namespace_declarations:
            parser.StartNamespaceDeclHandler = (
                self.start_namespace_decl_handler)

    def start_namespace_decl_handler(self, prefix, uri):
        """Push this namespace declaration on our storage."""
        self._ns_ordered_prefixes.append((prefix, uri))

    def start_element_handler(self, name, attributes):
        wenn ' ' in name:
            uri, localname, prefix, qname = _parse_ns_name(self, name)
        sonst:
            uri = EMPTY_NAMESPACE
            qname = name
            localname = Nichts
            prefix = EMPTY_PREFIX
        node = minidom.Element(qname, uri, prefix, localname)
        node.ownerDocument = self.document
        _append_child(self.curNode, node)
        self.curNode = node

        wenn self._ns_ordered_prefixes:
            fuer prefix, uri in self._ns_ordered_prefixes:
                wenn prefix:
                    a = minidom.Attr(_intern(self, 'xmlns:' + prefix),
                                     XMLNS_NAMESPACE, prefix, "xmlns")
                sonst:
                    a = minidom.Attr("xmlns", XMLNS_NAMESPACE,
                                     "xmlns", EMPTY_PREFIX)
                a.value = uri
                a.ownerDocument = self.document
                _set_attribute_node(node, a)
            loesche self._ns_ordered_prefixes[:]

        wenn attributes:
            node._ensure_attributes()
            _attrs = node._attrs
            _attrsNS = node._attrsNS
            fuer i in range(0, len(attributes), 2):
                aname = attributes[i]
                value = attributes[i+1]
                wenn ' ' in aname:
                    uri, localname, prefix, qname = _parse_ns_name(self, aname)
                    a = minidom.Attr(qname, uri, localname, prefix)
                    _attrs[qname] = a
                    _attrsNS[(uri, localname)] = a
                sonst:
                    a = minidom.Attr(aname, EMPTY_NAMESPACE,
                                     aname, EMPTY_PREFIX)
                    _attrs[aname] = a
                    _attrsNS[(EMPTY_NAMESPACE, aname)] = a
                a.ownerDocument = self.document
                a.value = value
                a.ownerElement = node

    wenn __debug__:
        # This only adds some asserts to the original
        # end_element_handler(), so we only define this when -O ist not
        # used.  If changing one, be sure to check the other to see if
        # it needs to be changed als well.
        #
        def end_element_handler(self, name):
            curNode = self.curNode
            wenn ' ' in name:
                uri, localname, prefix, qname = _parse_ns_name(self, name)
                pruefe (curNode.namespaceURI == uri
                        und curNode.localName == localname
                        und curNode.prefix == prefix), \
                        "element stack messed up! (namespace)"
            sonst:
                pruefe curNode.nodeName == name, \
                       "element stack messed up - bad nodeName"
                pruefe curNode.namespaceURI == EMPTY_NAMESPACE, \
                       "element stack messed up - bad namespaceURI"
            self.curNode = curNode.parentNode
            self._finish_end_element(curNode)


klasse ExpatBuilderNS(Namespaces, ExpatBuilder):
    """Document builder that supports namespaces."""

    def reset(self):
        ExpatBuilder.reset(self)
        self._initNamespaces()


klasse FragmentBuilderNS(Namespaces, FragmentBuilder):
    """Fragment builder that supports namespaces."""

    def reset(self):
        FragmentBuilder.reset(self)
        self._initNamespaces()

    def _getNSattrs(self):
        """Return string of namespace attributes von this element und
        ancestors."""
        # XXX This needs to be re-written to walk the ancestors of the
        # context to build up the namespace information from
        # declarations, elements, und attributes found in context.
        # Otherwise we have to store a bunch more data on the DOM
        # (though that *might* be more reliable -- nicht clear).
        attrs = ""
        context = self.context
        L = []
        waehrend context:
            wenn hasattr(context, '_ns_prefix_uri'):
                fuer prefix, uri in context._ns_prefix_uri.items():
                    # add every new NS decl von context to L und attrs string
                    wenn prefix in L:
                        weiter
                    L.append(prefix)
                    wenn prefix:
                        declname = "xmlns:" + prefix
                    sonst:
                        declname = "xmlns"
                    wenn attrs:
                        attrs = "%s\n    %s='%s'" % (attrs, declname, uri)
                    sonst:
                        attrs = " %s='%s'" % (declname, uri)
            context = context.parentNode
        gib attrs


klasse ParseEscape(Exception):
    """Exception raised to short-circuit parsing in InternalSubsetExtractor."""
    pass

klasse InternalSubsetExtractor(ExpatBuilder):
    """XML processor which can rip out the internal document type subset."""

    subset = Nichts

    def getSubset(self):
        """Return the internal subset als a string."""
        gib self.subset

    def parseFile(self, file):
        versuch:
            ExpatBuilder.parseFile(self, file)
        ausser ParseEscape:
            pass

    def parseString(self, string):
        versuch:
            ExpatBuilder.parseString(self, string)
        ausser ParseEscape:
            pass

    def install(self, parser):
        parser.StartDoctypeDeclHandler = self.start_doctype_decl_handler
        parser.StartElementHandler = self.start_element_handler

    def start_doctype_decl_handler(self, name, publicId, systemId,
                                   has_internal_subset):
        wenn has_internal_subset:
            parser = self.getParser()
            self.subset = []
            parser.DefaultHandler = self.subset.append
            parser.EndDoctypeDeclHandler = self.end_doctype_decl_handler
        sonst:
            wirf ParseEscape()

    def end_doctype_decl_handler(self):
        s = ''.join(self.subset).replace('\r\n', '\n').replace('\r', '\n')
        self.subset = s
        wirf ParseEscape()

    def start_element_handler(self, name, attrs):
        wirf ParseEscape()


def parse(file, namespaces=Wahr):
    """Parse a document, returning the resulting Document node.

    'file' may be either a file name oder an open file object.
    """
    wenn namespaces:
        builder = ExpatBuilderNS()
    sonst:
        builder = ExpatBuilder()

    wenn isinstance(file, str):
        mit open(file, 'rb') als fp:
            result = builder.parseFile(fp)
    sonst:
        result = builder.parseFile(file)
    gib result


def parseString(string, namespaces=Wahr):
    """Parse a document von a string, returning the resulting
    Document node.
    """
    wenn namespaces:
        builder = ExpatBuilderNS()
    sonst:
        builder = ExpatBuilder()
    gib builder.parseString(string)


def parseFragment(file, context, namespaces=Wahr):
    """Parse a fragment of a document, given the context von which it
    was originally extracted.  context should be the parent of the
    node(s) which are in the fragment.

    'file' may be either a file name oder an open file object.
    """
    wenn namespaces:
        builder = FragmentBuilderNS(context)
    sonst:
        builder = FragmentBuilder(context)

    wenn isinstance(file, str):
        mit open(file, 'rb') als fp:
            result = builder.parseFile(fp)
    sonst:
        result = builder.parseFile(file)
    gib result


def parseFragmentString(string, context, namespaces=Wahr):
    """Parse a fragment of a document von a string, given the context
    von which it was originally extracted.  context should be the
    parent of the node(s) which are in the fragment.
    """
    wenn namespaces:
        builder = FragmentBuilderNS(context)
    sonst:
        builder = FragmentBuilder(context)
    gib builder.parseString(string)


def makeBuilder(options):
    """Create a builder based on an Options object."""
    wenn options.namespaces:
        gib ExpatBuilderNS(options)
    sonst:
        gib ExpatBuilder(options)
