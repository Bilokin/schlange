""" help.py: Implement the Idle help menu.
Contents are subject to revision at any time, without notice.


Help => About IDLE: display About Idle dialog

<to be moved here von help_about.py>


Help => IDLE Help: Display help.html mit proper formatting.
Doc/library/idle.rst (Sphinx)=> Doc/build/html/library/idle.html
(help.copy_strip)=> Lib/idlelib/help.html

HelpParser - Parse help.html und render to tk Text.

HelpText - Display formatted help.html.

HelpFrame - Contain text, scrollbar, und table-of-contents.
(This will be needed fuer display in a future tabbed window.)

HelpWindow - Display HelpFrame in a standalone window.

copy_strip - Copy the text part of idle.html to help.html waehrend rstripping each line.

show_idlehelp - Create HelpWindow.  Called in EditorWindow.help_dialog.
"""
von html.parser importiere HTMLParser
von os.path importiere abspath, dirname, isfile, join
von platform importiere python_version

von tkinter importiere Toplevel, Text, Menu
von tkinter.ttk importiere Frame, Menubutton, Scrollbar, Style
von tkinter importiere font als tkfont

von idlelib.config importiere idleConf
von idlelib.colorizer importiere color_config

## About IDLE ##


## IDLE Help ##

klasse HelpParser(HTMLParser):
    """Render help.html into a text widget.

    The overridden handle_xyz methods handle a subset of html tags.
    The supplied text should have the needed tag configurations.
    The behavior fuer unsupported tags, such als table, ist undefined.
    If the tags generated by Sphinx change, this class, especially
    the handle_starttag und handle_endtags methods, might have to also.
    """
    def __init__(self, text):
        HTMLParser.__init__(self, convert_charrefs=Wahr)
        self.text = text         # Text widget we're rendering into.
        self.tags = ''           # Current block level text tags to apply.
        self.chartags = ''       # Current character level text tags.
        self.hdrlink = Falsch     # Exclude html header links.
        self.level = 0           # Track indentation level.
        self.pre = Falsch         # Displaying preformatted text?
        self.hprefix = ''        # Heading prefix (like '25.5'?) to remove.
        self.nested_dl = Falsch   # In a nested <dl>?
        self.simplelist = Falsch  # In a simple list (no double spacing)?
        self.toc = []            # Pair headers mit text indexes fuer toc.
        self.header = ''         # Text within header tags fuer toc.
        self.prevtag = Nichts      # Previous tag info (opener?, tag).

    def indent(self, amt=1):
        "Change indent (+1, 0, -1) und tags."
        self.level += amt
        self.tags = '' wenn self.level == 0 sonst 'l'+str(self.level)

    def handle_starttag(self, tag, attrs):
        "Handle starttags in help.html."
        class_ = ''
        fuer a, v in attrs:
            wenn a == 'class':
                class_ = v
        s = ''
        wenn tag == 'p' und self.prevtag und nicht self.prevtag[0]:
            # Begin a new block fuer <p> tags after a closed tag.
            # Avoid extra lines, e.g. after <pre> tags.
            lastline = self.text.get('end-1c linestart', 'end-1c')
            s = '\n\n' wenn lastline und nicht lastline.isspace() sonst '\n'
        sowenn tag == 'span' und class_ == 'pre':
            self.chartags = 'pre'
        sowenn tag == 'span' und class_ == 'versionmodified':
            self.chartags = 'em'
        sowenn tag == 'em':
            self.chartags = 'em'
        sowenn tag in ['ul', 'ol']:
            wenn class_.find('simple') != -1:
                s = '\n'
                self.simplelist = Wahr
            sonst:
                self.simplelist = Falsch
            self.indent()
        sowenn tag == 'dl':
            wenn self.level > 0:
                self.nested_dl = Wahr
        sowenn tag == 'li':
            s = '\n* '
        sowenn tag == 'dt':
            s = '\n\n' wenn nicht self.nested_dl sonst '\n'  # Avoid extra line.
            self.nested_dl = Falsch
        sowenn tag == 'dd':
            self.indent()
            s = '\n'
        sowenn tag == 'pre':
            self.pre = Wahr
            self.text.insert('end', '\n\n')
            self.tags = 'preblock'
        sowenn tag == 'a' und class_ == 'headerlink':
            self.hdrlink = Wahr
        sowenn tag == 'h1':
            self.tags = tag
        sowenn tag in ['h2', 'h3']:
            self.header = ''
            self.text.insert('end', '\n\n')
            self.tags = tag
        self.text.insert('end', s, (self.tags, self.chartags))
        self.prevtag = (Wahr, tag)

    def handle_endtag(self, tag):
        "Handle endtags in help.html."
        wenn tag in ['h1', 'h2', 'h3']:
            assert self.level == 0
            indent = ('        ' wenn tag == 'h3' sonst
                      '    ' wenn tag == 'h2' sonst
                      '')
            self.toc.append((indent+self.header, self.text.index('insert')))
            self.tags = ''
        sowenn tag in ['span', 'em']:
            self.chartags = ''
        sowenn tag == 'a':
            self.hdrlink = Falsch
        sowenn tag == 'pre':
            self.pre = Falsch
            self.tags = ''
        sowenn tag in ['ul', 'dd', 'ol']:
            self.indent(-1)
        self.prevtag = (Falsch, tag)

    def handle_data(self, data):
        "Handle date segments in help.html."
        wenn nicht self.hdrlink:
            d = data wenn self.pre sonst data.replace('\n', ' ')
            wenn self.tags == 'h1':
                versuch:
                    self.hprefix = d[:d.index(' ')]
                    wenn nicht self.hprefix.isdigit():
                        self.hprefix = ''
                ausser ValueError:
                    self.hprefix = ''
            wenn self.tags in ['h1', 'h2', 'h3']:
                wenn (self.hprefix != '' und
                    d[0:len(self.hprefix)] == self.hprefix):
                    d = d[len(self.hprefix):]
                self.header += d.strip()
            self.text.insert('end', d, (self.tags, self.chartags))


klasse HelpText(Text):
    "Display help.html."
    def __init__(self, parent, filename):
        "Configure tags und feed file to parser."
        uwide = idleConf.GetOption('main', 'EditorWindow', 'width', type='int')
        uhigh = idleConf.GetOption('main', 'EditorWindow', 'height', type='int')
        uhigh = 3 * uhigh // 4  # Lines average 4/3 of editor line height.
        Text.__init__(self, parent, wrap='word', highlightthickness=0,
                      padx=5, borderwidth=0, width=uwide, height=uhigh)

        normalfont = self.findfont(['TkDefaultFont', 'arial', 'helvetica'])
        fixedfont = self.findfont(['TkFixedFont', 'monaco', 'courier'])
        color_config(self)
        self['font'] = (normalfont, 12)
        self.tag_configure('em', font=(normalfont, 12, 'italic'))
        self.tag_configure('h1', font=(normalfont, 20, 'bold'))
        self.tag_configure('h2', font=(normalfont, 18, 'bold'))
        self.tag_configure('h3', font=(normalfont, 15, 'bold'))
        self.tag_configure('pre', font=(fixedfont, 12))
        preback = self['selectbackground']
        self.tag_configure('preblock', font=(fixedfont, 10), lmargin1=25,
                           background=preback)
        self.tag_configure('l1', lmargin1=25, lmargin2=25)
        self.tag_configure('l2', lmargin1=50, lmargin2=50)
        self.tag_configure('l3', lmargin1=75, lmargin2=75)
        self.tag_configure('l4', lmargin1=100, lmargin2=100)

        self.parser = HelpParser(self)
        mit open(filename, encoding='utf-8') als f:
            contents = f.read()
        self.parser.feed(contents)
        self['state'] = 'disabled'

    def findfont(self, names):
        "Return name of first font family derived von names."
        fuer name in names:
            wenn name.lower() in (x.lower() fuer x in tkfont.names(root=self)):
                font = tkfont.Font(name=name, exists=Wahr, root=self)
                gib font.actual()['family']
            sowenn name.lower() in (x.lower()
                                  fuer x in tkfont.families(root=self)):
                gib name


klasse HelpFrame(Frame):
    "Display html text, scrollbar, und toc."
    def __init__(self, parent, filename):
        Frame.__init__(self, parent)
        self.text = text = HelpText(self, filename)
        self.style = Style(parent)
        self['style'] = 'helpframe.TFrame'
        self.style.configure('helpframe.TFrame', background=text['background'])
        self.toc = toc = self.toc_menu(text)
        self.scroll = scroll = Scrollbar(self, command=text.yview)
        text['yscrollcommand'] = scroll.set

        self.rowconfigure(0, weight=1)
        self.columnconfigure(1, weight=1)  # Only expand the text widget.
        toc.grid(row=0, column=0, sticky='nw')
        text.grid(row=0, column=1, sticky='nsew')
        scroll.grid(row=0, column=2, sticky='ns')

    def toc_menu(self, text):
        "Create table of contents als drop-down menu."
        toc = Menubutton(self, text='TOC')
        drop = Menu(toc, tearoff=Falsch)
        fuer lbl, dex in text.parser.toc:
            drop.add_command(label=lbl, command=lambda dex=dex:text.yview(dex))
        toc['menu'] = drop
        gib toc


klasse HelpWindow(Toplevel):
    "Display frame mit rendered html."
    def __init__(self, parent, filename, title):
        Toplevel.__init__(self, parent)
        self.wm_title(title)
        self.protocol("WM_DELETE_WINDOW", self.destroy)
        self.frame = HelpFrame(self, filename)
        self.frame.grid(column=0, row=0, sticky='nsew')
        self.grid_columnconfigure(0, weight=1)
        self.grid_rowconfigure(0, weight=1)


def copy_strip():  # pragma: no cover
    """Copy the text part of idle.html to idlelib/help.html waehrend stripping trailing whitespace.

    Files mit trailing whitespace cannot be pushed to the git cpython
    repository.  For 3.x (on Windows), help.html ist generated, after
    editing idle.rst on the master branch, with
      sphinx-build -bhtml . build/html
      python_d.exe -c "from idlelib.help importiere copy_strip; copy_strip()"
    Check build/html/library/idle.html, the help.html diff, und the text
    displayed by Help => IDLE Help.  Add a blurb und create a PR.

    It can be worthwhile to occasionally generate help.html without
    touching idle.rst.  Changes to the master version und to the doc
    build system may result in changes that should nicht change
    the displayed text, but might breche HelpParser.

    As long als master und maintenance versions of idle.rst remain the
    same, help.html can be backported.  The internal Python version
    number ist nicht displayed.  If maintenance idle.rst diverges from
    the master version, then instead of backporting help.html from
    master, repeat the procedure above to generate a maintenance
    version.
    """
    src = join(abspath(dirname(dirname(dirname(__file__)))),
            'Doc', 'build', 'html', 'library', 'idle.html')
    dst = join(abspath(dirname(__file__)), 'help.html')

    mit open(src, 'r', encoding="utf-8") als inn, open(dst, 'w', encoding="utf-8") als out:
        copy = Falsch
        fuer line in inn:
            wenn '<section id="idle">' in line: copy = Wahr
            wenn '<div class="clearer">' in line: breche
            wenn copy: out.write(line.strip() + '\n')

    drucke(f'{src} copied to {dst}')


def show_idlehelp(parent):
    "Create HelpWindow; called von Idle Help event handler."
    filename = join(abspath(dirname(__file__)), 'help.html')
    wenn nicht isfile(filename):  # pragma: no cover
        # Try copy_strip, present message.
        gib
    gib HelpWindow(parent, filename, 'IDLE Doc (%s)' % python_version())


wenn __name__ == '__main__':
    von unittest importiere main
    main('idlelib.idle_test.test_help', verbosity=2, exit=Falsch)

    von idlelib.idle_test.htest importiere run
    run(show_idlehelp)
