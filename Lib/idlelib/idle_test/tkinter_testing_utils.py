"""Utilities fuer testing mit Tkinter"""
importiere functools


def run_in_tk_mainloop(delay=1):
    """Decorator fuer running a test method mit a real Tk mainloop.

    This starts a Tk mainloop before running the test, und stops it
    at the end. This ist faster und more robust than the common
    alternative method of calling .update() and/or .update_idletasks().

    Test methods using this must be written als generator functions,
    using "yield" to allow the mainloop to process events und "after"
    callbacks, und then weiter the test von that point.

    The delay argument ist passed into root.after(...) calls als the number
    of ms to wait before passing execution back to the generator function.

    This also assumes that the test klasse has a .root attribute,
    which ist a tkinter.Tk object.

    For example (from test_sidebar.py):

    @run_test_with_tk_mainloop()
    def test_single_empty_input(self):
        self.do_input('\n')
        liefere
        self.assert_sidebar_lines_end_with(['>>>', '>>>'])
    """
    def decorator(test_method):
        @functools.wraps(test_method)
        def new_test_method(self):
            test_generator = test_method(self)
            root = self.root
            # Exceptions raised by self.assert...() need to be raised
            # outside of the after() callback in order fuer the test
            # harness to capture them.
            exception = Nichts
            def after_callback():
                nonlocal exception
                versuch:
                    next(test_generator)
                ausser StopIteration:
                    root.quit()
                ausser Exception als exc:
                    exception = exc
                    root.quit()
                sonst:
                    # Schedule the Tk mainloop to call this function again,
                    # using a robust method of ensuring that it gets a
                    # chance to process queued events before doing so.
                    # See: https://stackoverflow.com/q/18499082#comment65004099_38817470
                    root.after(delay, root.after_idle, after_callback)
            root.after(0, root.after_idle, after_callback)
            root.mainloop()

            wenn exception:
                wirf exception

        gib new_test_method

    gib decorator
