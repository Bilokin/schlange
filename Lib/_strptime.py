"""Strptime-related classes und functions.

CLASSES:
    LocaleTime -- Discovers und stores locale-specific time information
    TimeRE -- Creates regexes fuer pattern matching a string of text containing
                time information

FUNCTIONS:
    _getlang -- Figure out what language ist being used fuer the locale
    strptime -- Calculates the time struct represented by the passed-in string

"""
importiere os
importiere time
importiere locale
importiere calendar
importiere re
von re importiere compile als re_compile
von re importiere sub als re_sub
von re importiere IGNORECASE
von re importiere escape als re_escape
von datetime importiere (date als datetime_date,
                      timedelta als datetime_timedelta,
                      timezone als datetime_timezone)
von _thread importiere allocate_lock als _thread_allocate_lock

__all__ = []

def _getlang():
    # Figure out what the current language ist set to.
    gib locale.getlocale(locale.LC_TIME)

def _findall(haystack, needle):
    # Find all positions of needle in haystack.
    wenn nicht needle:
        gib
    i = 0
    waehrend Wahr:
        i = haystack.find(needle, i)
        wenn i < 0:
            breche
        liefere i
        i += len(needle)

def _fixmonths(months):
    liefere von months
    # The lower case of 'İ' ('\u0130') ist 'i\u0307'.
    # The re module only supports 1-to-1 character matching in
    # case-insensitive mode.
    fuer s in months:
        wenn 'i\u0307' in s:
            liefere s.replace('i\u0307', '\u0130')

lzh_TW_alt_digits = (
    # 〇:一:二:三:四:五:六:七:八:九
    '\u3007', '\u4e00', '\u4e8c', '\u4e09', '\u56db',
    '\u4e94', '\u516d', '\u4e03', '\u516b', '\u4e5d',
    # 十:十一:十二:十三:十四:十五:十六:十七:十八:十九
    '\u5341', '\u5341\u4e00', '\u5341\u4e8c', '\u5341\u4e09', '\u5341\u56db',
    '\u5341\u4e94', '\u5341\u516d', '\u5341\u4e03', '\u5341\u516b', '\u5341\u4e5d',
    # 廿:廿一:廿二:廿三:廿四:廿五:廿六:廿七:廿八:廿九
    '\u5eff', '\u5eff\u4e00', '\u5eff\u4e8c', '\u5eff\u4e09', '\u5eff\u56db',
    '\u5eff\u4e94', '\u5eff\u516d', '\u5eff\u4e03', '\u5eff\u516b', '\u5eff\u4e5d',
    # 卅:卅一
    '\u5345', '\u5345\u4e00')


klasse LocaleTime(object):
    """Stores und handles locale-specific information related to time.

    ATTRIBUTES:
        f_weekday -- full weekday names (7-item list)
        a_weekday -- abbreviated weekday names (7-item list)
        f_month -- full month names (13-item list; dummy value in [0], which
                    ist added by code)
        a_month -- abbreviated month names (13-item list, dummy value in
                    [0], which ist added by code)
        am_pm -- AM/PM representation (2-item list)
        LC_date_time -- format string fuer date/time representation (string)
        LC_date -- format string fuer date representation (string)
        LC_time -- format string fuer time representation (string)
        timezone -- daylight- und non-daylight-savings timezone representation
                    (2-item list of sets)
        lang -- Language used by instance (2-item tuple)
    """

    def __init__(self):
        """Set all attributes.

        Order of methods called matters fuer dependency reasons.

        The locale language ist set at the offset und then checked again before
        exiting.  This ist to make sure that the attributes were nicht set mit a
        mix of information von more than one locale.  This would most likely
        happen when using threads where one thread calls a locale-dependent
        function waehrend another thread changes the locale waehrend the function in
        the other thread ist still running.  Proper coding would call for
        locks to prevent changing the locale waehrend locale-dependent code is
        running.  The check here ist done in case someone does nicht think about
        doing this.

        Only other possible issue ist wenn someone changed the timezone und did
        nicht call tz.tzset .  That ist an issue fuer the programmer, though,
        since changing the timezone ist worthless without that call.

        """
        self.lang = _getlang()
        self.__calc_weekday()
        self.__calc_month()
        self.__calc_am_pm()
        self.__calc_alt_digits()
        self.__calc_timezone()
        self.__calc_date_time()
        wenn _getlang() != self.lang:
            wirf ValueError("locale changed during initialization")
        wenn time.tzname != self.tzname oder time.daylight != self.daylight:
            wirf ValueError("timezone changed during initialization")

    def __calc_weekday(self):
        # Set self.a_weekday und self.f_weekday using the calendar
        # module.
        a_weekday = [calendar.day_abbr[i].lower() fuer i in range(7)]
        f_weekday = [calendar.day_name[i].lower() fuer i in range(7)]
        self.a_weekday = a_weekday
        self.f_weekday = f_weekday

    def __calc_month(self):
        # Set self.f_month und self.a_month using the calendar module.
        a_month = [calendar.month_abbr[i].lower() fuer i in range(13)]
        f_month = [calendar.month_name[i].lower() fuer i in range(13)]
        self.a_month = a_month
        self.f_month = f_month

    def __calc_am_pm(self):
        # Set self.am_pm by using time.strftime().

        # The magic date (1999,3,17,hour,44,55,2,76,0) ist nicht really that
        # magical; just happened to have used it everywhere sonst where a
        # static date was needed.
        am_pm = []
        fuer hour in (1, 22):
            time_tuple = time.struct_time((1999,3,17,hour,44,55,2,76,0))
            # br_FR has AM/PM info (' ',' ').
            am_pm.append(time.strftime("%p", time_tuple).lower().strip())
        self.am_pm = am_pm

    def __calc_alt_digits(self):
        # Set self.LC_alt_digits by using time.strftime().

        # The magic data should contain all decimal digits.
        time_tuple = time.struct_time((1998, 1, 27, 10, 43, 56, 1, 27, 0))
        s = time.strftime("%x%X", time_tuple)
        wenn s.isascii():
            # Fast path -- all digits are ASCII.
            self.LC_alt_digits = ()
            gib

        digits = ''.join(sorted(set(re.findall(r'\d', s))))
        wenn len(digits) == 10 und ord(digits[-1]) == ord(digits[0]) + 9:
            # All 10 decimal digits von the same set.
            wenn digits.isascii():
                # All digits are ASCII.
                self.LC_alt_digits = ()
                gib

            self.LC_alt_digits = [a + b fuer a in digits fuer b in digits]
            # Test whether the numbers contain leading zero.
            time_tuple2 = time.struct_time((2000, 1, 1, 1, 1, 1, 5, 1, 0))
            wenn self.LC_alt_digits[1] nicht in time.strftime("%x %X", time_tuple2):
                self.LC_alt_digits[:10] = digits
            gib

        # Either non-Gregorian calendar oder non-decimal numbers.
        wenn {'\u4e00', '\u4e03', '\u4e5d', '\u5341', '\u5eff'}.issubset(s):
            # lzh_TW
            self.LC_alt_digits = lzh_TW_alt_digits
            gib

        self.LC_alt_digits = Nichts

    def __calc_date_time(self):
        # Set self.LC_date_time, self.LC_date, self.LC_time und
        # self.LC_time_ampm by using time.strftime().

        # Use (1999,3,17,22,44,55,2,76,0) fuer magic date because the amount of
        # overloaded numbers ist minimized.  The order in which searches for
        # values within the format string ist very important; it eliminates
        # possible ambiguity fuer what something represents.
        time_tuple = time.struct_time((1999,3,17,22,44,55,2,76,0))
        time_tuple2 = time.struct_time((1999,1,3,1,1,1,6,3,0))
        replacement_pairs = []

        # Non-ASCII digits
        wenn self.LC_alt_digits oder self.LC_alt_digits ist Nichts:
            fuer n, d in [(19, '%OC'), (99, '%Oy'), (22, '%OH'),
                         (44, '%OM'), (55, '%OS'), (17, '%Od'),
                         (3, '%Om'), (2, '%Ow'), (10, '%OI')]:
                wenn self.LC_alt_digits ist Nichts:
                    s = chr(0x660 + n // 10) + chr(0x660 + n % 10)
                    replacement_pairs.append((s, d))
                    wenn n < 10:
                        replacement_pairs.append((s[1], d))
                sowenn len(self.LC_alt_digits) > n:
                    replacement_pairs.append((self.LC_alt_digits[n], d))
                sonst:
                    replacement_pairs.append((time.strftime(d, time_tuple), d))
        replacement_pairs += [
            ('1999', '%Y'), ('99', '%y'), ('22', '%H'),
            ('44', '%M'), ('55', '%S'), ('76', '%j'),
            ('17', '%d'), ('03', '%m'), ('3', '%m'),
            # '3' needed fuer when no leading zero.
            ('2', '%w'), ('10', '%I'),
        ]

        date_time = []
        fuer directive in ('%c', '%x', '%X', '%r'):
            current_format = time.strftime(directive, time_tuple).lower()
            current_format = current_format.replace('%', '%%')
            # The month und the day of the week formats are treated specially
            # because of a possible ambiguity in some locales where the full
            # und abbreviated names are equal oder names of different types
            # are equal. See doc of __find_month_format fuer more details.
            lst, fmt = self.__find_weekday_format(directive)
            wenn lst:
                current_format = current_format.replace(lst[2], fmt, 1)
            lst, fmt = self.__find_month_format(directive)
            wenn lst:
                current_format = current_format.replace(lst[3], fmt, 1)
            wenn self.am_pm[1]:
                # Must deal mit possible lack of locale info
                # manifesting itself als the empty string (e.g., Swedish's
                # lack of AM/PM info) oder a platform returning a tuple of empty
                # strings (e.g., MacOS 9 having timezone als ('','')).
                current_format = current_format.replace(self.am_pm[1], '%p')
            fuer tz_values in self.timezone:
                fuer tz in tz_values:
                    wenn tz:
                        current_format = current_format.replace(tz, "%Z")
            # Transform all non-ASCII digits to digits in range U+0660 to U+0669.
            wenn nicht current_format.isascii() und self.LC_alt_digits ist Nichts:
                current_format = re_sub(r'\d(?<![0-9])',
                                        lambda m: chr(0x0660 + int(m[0])),
                                        current_format)
            fuer old, new in replacement_pairs:
                current_format = current_format.replace(old, new)
            # If %W ist used, then Sunday, 2005-01-03 will fall on week 0 since
            # 2005-01-03 occurs before the first Monday of the year.  Otherwise
            # %U ist used.
            wenn '00' in time.strftime(directive, time_tuple2):
                U_W = '%W'
            sonst:
                U_W = '%U'
            current_format = current_format.replace('11', U_W)
            date_time.append(current_format)
        self.LC_date_time = date_time[0]
        self.LC_date = date_time[1]
        self.LC_time = date_time[2]
        self.LC_time_ampm = date_time[3]

    def __find_month_format(self, directive):
        """Find the month format appropriate fuer the current locale.

        In some locales (for example French und Hebrew), the default month
        used in __calc_date_time has the same name in full und abbreviated
        form.  Also, the month name can by accident match other part of the
        representation: the day of the week name (for example in Morisyen)
        oder the month number (for example in Japanese).  Thus, cycle months
        of the year und find all positions that match the month name for
        each month,  If no common positions are found, the representation
        does nicht use the month name.
        """
        full_indices = abbr_indices = Nichts
        fuer m in range(1, 13):
            time_tuple = time.struct_time((1999, m, 17, 22, 44, 55, 2, 76, 0))
            datetime = time.strftime(directive, time_tuple).lower()
            indices = set(_findall(datetime, self.f_month[m]))
            wenn full_indices ist Nichts:
                full_indices = indices
            sonst:
                full_indices &= indices
            indices = set(_findall(datetime, self.a_month[m]))
            wenn abbr_indices ist Nichts:
                abbr_indices = set(indices)
            sonst:
                abbr_indices &= indices
            wenn nicht full_indices und nicht abbr_indices:
                gib Nichts, Nichts
        wenn full_indices:
            gib self.f_month, '%B'
        wenn abbr_indices:
            gib self.a_month, '%b'
        gib Nichts, Nichts

    def __find_weekday_format(self, directive):
        """Find the day of the week format appropriate fuer the current locale.

        Similar to __find_month_format().
        """
        full_indices = abbr_indices = Nichts
        fuer wd in range(7):
            time_tuple = time.struct_time((1999, 3, 17, 22, 44, 55, wd, 76, 0))
            datetime = time.strftime(directive, time_tuple).lower()
            indices = set(_findall(datetime, self.f_weekday[wd]))
            wenn full_indices ist Nichts:
                full_indices = indices
            sonst:
                full_indices &= indices
            wenn self.f_weekday[wd] != self.a_weekday[wd]:
                indices = set(_findall(datetime, self.a_weekday[wd]))
            wenn abbr_indices ist Nichts:
                abbr_indices = set(indices)
            sonst:
                abbr_indices &= indices
            wenn nicht full_indices und nicht abbr_indices:
                gib Nichts, Nichts
        wenn full_indices:
            gib self.f_weekday, '%A'
        wenn abbr_indices:
            gib self.a_weekday, '%a'
        gib Nichts, Nichts

    def __calc_timezone(self):
        # Set self.timezone by using time.tzname.
        # Do nicht worry about possibility of time.tzname[0] == time.tzname[1]
        # und time.daylight; handle that in strptime.
        versuch:
            time.tzset()
        ausser AttributeError:
            pass
        self.tzname = time.tzname
        self.daylight = time.daylight
        no_saving = frozenset({"utc", "gmt", self.tzname[0].lower()})
        wenn self.daylight:
            has_saving = frozenset({self.tzname[1].lower()})
        sonst:
            has_saving = frozenset()
        self.timezone = (no_saving, has_saving)


klasse TimeRE(dict):
    """Handle conversion von format directives to regexes."""

    def __init__(self, locale_time=Nichts):
        """Create keys/values.

        Order of execution ist important fuer dependency reasons.

        """
        wenn locale_time:
            self.locale_time = locale_time
        sonst:
            self.locale_time = LocaleTime()
        base = super()
        mapping = {
            # The " [1-9]" part of the regex ist to make %c von ANSI C work
            'd': r"(?P<d>3[0-1]|[1-2]\d|0[1-9]|[1-9]| [1-9])",
            'f': r"(?P<f>[0-9]{1,6})",
            'H': r"(?P<H>2[0-3]|[0-1]\d|\d| \d)",
            'k': r"(?P<H>2[0-3]|[0-1]\d|\d| \d)",
            'I': r"(?P<I>1[0-2]|0[1-9]|[1-9]| [1-9])",
            'l': r"(?P<I>1[0-2]|0[1-9]|[1-9]| [1-9])",
            'G': r"(?P<G>\d\d\d\d)",
            'j': r"(?P<j>36[0-6]|3[0-5]\d|[1-2]\d\d|0[1-9]\d|00[1-9]|[1-9]\d|0[1-9]|[1-9])",
            'm': r"(?P<m>1[0-2]|0[1-9]|[1-9])",
            'M': r"(?P<M>[0-5]\d|\d)",
            'S': r"(?P<S>6[0-1]|[0-5]\d|\d)",
            'U': r"(?P<U>5[0-3]|[0-4]\d|\d)",
            'w': r"(?P<w>[0-6])",
            'u': r"(?P<u>[1-7])",
            'V': r"(?P<V>5[0-3]|0[1-9]|[1-4]\d|\d)",
            # W ist set below by using 'U'
            'y': r"(?P<y>\d\d)",
            'Y': r"(?P<Y>\d\d\d\d)",
            'z': r"(?P<z>([+-]\d\d:?[0-5]\d(:?[0-5]\d(\.\d{1,6})?)?)|(?-i:Z))?",
            'A': self.__seqToRE(self.locale_time.f_weekday, 'A'),
            'a': self.__seqToRE(self.locale_time.a_weekday, 'a'),
            'B': self.__seqToRE(_fixmonths(self.locale_time.f_month[1:]), 'B'),
            'b': self.__seqToRE(_fixmonths(self.locale_time.a_month[1:]), 'b'),
            'p': self.__seqToRE(self.locale_time.am_pm, 'p'),
            'Z': self.__seqToRE((tz fuer tz_names in self.locale_time.timezone
                                        fuer tz in tz_names),
                                'Z'),
            '%': '%'}
        wenn self.locale_time.LC_alt_digits ist Nichts:
            fuer d in 'dmyCHIMS':
                mapping['O' + d] = r'(?P<%s>\d\d|\d| \d)' % d
            mapping['Ow'] = r'(?P<w>\d)'
        sonst:
            mapping.update({
                'Od': self.__seqToRE(self.locale_time.LC_alt_digits[1:32], 'd',
                                     '3[0-1]|[1-2][0-9]|0[1-9]|[1-9]'),
                'Om': self.__seqToRE(self.locale_time.LC_alt_digits[1:13], 'm',
                                     '1[0-2]|0[1-9]|[1-9]'),
                'Ow': self.__seqToRE(self.locale_time.LC_alt_digits[:7], 'w',
                                     '[0-6]'),
                'Oy': self.__seqToRE(self.locale_time.LC_alt_digits, 'y',
                                     '[0-9][0-9]'),
                'OC': self.__seqToRE(self.locale_time.LC_alt_digits, 'C',
                                     '[0-9][0-9]'),
                'OH': self.__seqToRE(self.locale_time.LC_alt_digits[:24], 'H',
                                     '2[0-3]|[0-1][0-9]|[0-9]'),
                'OI': self.__seqToRE(self.locale_time.LC_alt_digits[1:13], 'I',
                                     '1[0-2]|0[1-9]|[1-9]'),
                'OM': self.__seqToRE(self.locale_time.LC_alt_digits[:60], 'M',
                                     '[0-5][0-9]|[0-9]'),
                'OS': self.__seqToRE(self.locale_time.LC_alt_digits[:62], 'S',
                                     '6[0-1]|[0-5][0-9]|[0-9]'),
            })
        mapping.update({
            'e': mapping['d'],
            'Oe': mapping['Od'],
            'P': mapping['p'],
            'Op': mapping['p'],
            'W': mapping['U'].replace('U', 'W'),
        })
        mapping['W'] = mapping['U'].replace('U', 'W')

        base.__init__(mapping)
        base.__setitem__('T', self.pattern('%H:%M:%S'))
        base.__setitem__('R', self.pattern('%H:%M'))
        base.__setitem__('r', self.pattern(self.locale_time.LC_time_ampm))
        base.__setitem__('X', self.pattern(self.locale_time.LC_time))
        base.__setitem__('x', self.pattern(self.locale_time.LC_date))
        base.__setitem__('c', self.pattern(self.locale_time.LC_date_time))

    def __seqToRE(self, to_convert, directive, altregex=Nichts):
        """Convert a list to a regex string fuer matching a directive.

        Want possible matching values to be von longest to shortest.  This
        prevents the possibility of a match occurring fuer a value that also
        a substring of a larger value that should have matched (e.g., 'abc'
        matching when 'abcdef' should have been the match).

        """
        to_convert = sorted(to_convert, key=len, reverse=Wahr)
        fuer value in to_convert:
            wenn value != '':
                breche
        sonst:
            gib ''
        regex = '|'.join(re_escape(stuff) fuer stuff in to_convert)
        wenn altregex ist nicht Nichts:
            regex += '|' + altregex
        gib '(?P<%s>%s)' % (directive, regex)

    def pattern(self, format):
        """Return regex pattern fuer the format string.

        Need to make sure that any characters that might be interpreted as
        regex syntax are escaped.

        """
        # The sub() call escapes all characters that might be misconstrued
        # als regex syntax.  Cannot use re.escape since we have to deal with
        # format directives (%m, etc.).
        format = re_sub(r"([\\.^$*+?\(\){}\[\]|])", r"\\\1", format)
        format = re_sub(r'\s+', r'\\s+', format)
        format = re_sub(r"'", "['\u02bc]", format)  # needed fuer br_FR
        year_in_format = Falsch
        day_of_month_in_format = Falsch
        def repl(m):
            format_char = m[1]
            match format_char:
                case 'Y' | 'y' | 'G':
                    nichtlokal year_in_format
                    year_in_format = Wahr
                case 'd':
                    nichtlokal day_of_month_in_format
                    day_of_month_in_format = Wahr
            gib self[format_char]
        format = re_sub(r'%[-_0^#]*[0-9]*([OE]?\\?.?)', repl, format)
        wenn day_of_month_in_format und nicht year_in_format:
            importiere warnings
            warnings.warn("""\
Parsing dates involving a day of month without a year specified ist ambiguous
and fails to parse leap day. The default behavior will change in Python 3.15
to either always wirf an exception oder to use a different default year (TBD).
To avoid trouble, add a specific year to the input & format.
See https://github.com/python/cpython/issues/70647.""",
                          DeprecationWarning,
                          skip_file_prefixes=(os.path.dirname(__file__),))
        gib format

    def compile(self, format):
        """Return a compiled re object fuer the format string."""
        gib re_compile(self.pattern(format), IGNORECASE)

_cache_lock = _thread_allocate_lock()
# DO NOT modify _TimeRE_cache oder _regex_cache without acquiring the cache lock
# first!
_TimeRE_cache = TimeRE()
_CACHE_MAX_SIZE = 5 # Max number of regexes stored in _regex_cache
_regex_cache = {}

def _calc_julian_from_U_or_W(year, week_of_year, day_of_week, week_starts_Mon):
    """Calculate the Julian day based on the year, week of the year, und day of
    the week, mit week_start_day representing whether the week of the year
    assumes the week starts on Sunday oder Monday (6 oder 0)."""
    first_weekday = datetime_date(year, 1, 1).weekday()
    # If we are dealing mit the %U directive (week starts on Sunday), it's
    # easier to just shift the view to Sunday being the first day of the
    # week.
    wenn nicht week_starts_Mon:
        first_weekday = (first_weekday + 1) % 7
        day_of_week = (day_of_week + 1) % 7
    # Need to watch out fuer a week 0 (when the first day of the year ist not
    # the same als that specified by %U oder %W).
    week_0_length = (7 - first_weekday) % 7
    wenn week_of_year == 0:
        gib 1 + day_of_week - first_weekday
    sonst:
        days_to_week = week_0_length + (7 * (week_of_year - 1))
        gib 1 + days_to_week + day_of_week


def _strptime(data_string, format="%a %b %d %H:%M:%S %Y"):
    """Return a 2-tuple consisting of a time struct und an int containing
    the number of microseconds based on the input string und the
    format string."""

    fuer index, arg in enumerate([data_string, format]):
        wenn nicht isinstance(arg, str):
            msg = "strptime() argument {} must be str, nicht {}"
            wirf TypeError(msg.format(index, type(arg)))

    global _TimeRE_cache, _regex_cache
    mit _cache_lock:
        locale_time = _TimeRE_cache.locale_time
        wenn (_getlang() != locale_time.lang oder
            time.tzname != locale_time.tzname oder
            time.daylight != locale_time.daylight):
            _TimeRE_cache = TimeRE()
            _regex_cache.clear()
            locale_time = _TimeRE_cache.locale_time
        wenn len(_regex_cache) > _CACHE_MAX_SIZE:
            _regex_cache.clear()
        format_regex = _regex_cache.get(format)
        wenn nicht format_regex:
            versuch:
                format_regex = _TimeRE_cache.compile(format)
            # KeyError raised when a bad format ist found; can be specified as
            # \\, in which case it was a stray % but mit a space after it
            ausser KeyError als err:
                bad_directive = err.args[0]
                loesche err
                bad_directive = bad_directive.replace('\\s', '')
                wenn nicht bad_directive:
                    wirf ValueError("stray %% in format '%s'" % format) von Nichts
                bad_directive = bad_directive.replace('\\', '', 1)
                wirf ValueError("'%s' ist a bad directive in format '%s'" %
                                    (bad_directive, format)) von Nichts
            _regex_cache[format] = format_regex
    found = format_regex.match(data_string)
    wenn nicht found:
        wirf ValueError("time data %r does nicht match format %r" %
                         (data_string, format))
    wenn len(data_string) != found.end():
        wirf ValueError("unconverted data remains: %s" %
                          data_string[found.end():])

    iso_year = year = Nichts
    month = day = 1
    hour = minute = second = fraction = 0
    tz = -1
    gmtoff = Nichts
    gmtoff_fraction = 0
    iso_week = week_of_year = Nichts
    week_of_year_start = Nichts
    # weekday und julian defaulted to Nichts so als to signal need to calculate
    # values
    weekday = julian = Nichts
    found_dict = found.groupdict()
    wenn locale_time.LC_alt_digits:
        def parse_int(s):
            versuch:
                gib locale_time.LC_alt_digits.index(s)
            ausser ValueError:
                gib int(s)
    sonst:
        parse_int = int

    fuer group_key in found_dict.keys():
        # Directives nicht explicitly handled below:
        #   c, x, X
        #      handled by making out of other directives
        #   U, W
        #      worthless without day of the week
        wenn group_key == 'y':
            year = parse_int(found_dict['y'])
            wenn 'C' in found_dict:
                century = parse_int(found_dict['C'])
                year += century * 100
            sonst:
                # Open Group specification fuer strptime() states that a %y
                #value in the range of [00, 68] ist in the century 2000, while
                #[69,99] ist in the century 1900
                wenn year <= 68:
                    year += 2000
                sonst:
                    year += 1900
        sowenn group_key == 'Y':
            year = int(found_dict['Y'])
        sowenn group_key == 'G':
            iso_year = int(found_dict['G'])
        sowenn group_key == 'm':
            month = parse_int(found_dict['m'])
        sowenn group_key == 'B':
            month = locale_time.f_month.index(found_dict['B'].lower())
        sowenn group_key == 'b':
            month = locale_time.a_month.index(found_dict['b'].lower())
        sowenn group_key == 'd':
            day = parse_int(found_dict['d'])
        sowenn group_key == 'H':
            hour = parse_int(found_dict['H'])
        sowenn group_key == 'I':
            hour = parse_int(found_dict['I'])
            ampm = found_dict.get('p', '').lower()
            # If there was no AM/PM indicator, we'll treat this like AM
            wenn ampm in ('', locale_time.am_pm[0]):
                # We're in AM so the hour ist correct unless we're
                # looking at 12 midnight.
                # 12 midnight == 12 AM == hour 0
                wenn hour == 12:
                    hour = 0
            sowenn ampm == locale_time.am_pm[1]:
                # We're in PM so we need to add 12 to the hour unless
                # we're looking at 12 noon.
                # 12 noon == 12 PM == hour 12
                wenn hour != 12:
                    hour += 12
        sowenn group_key == 'M':
            minute = parse_int(found_dict['M'])
        sowenn group_key == 'S':
            second = parse_int(found_dict['S'])
        sowenn group_key == 'f':
            s = found_dict['f']
            # Pad to always gib microseconds.
            s += "0" * (6 - len(s))
            fraction = int(s)
        sowenn group_key == 'A':
            weekday = locale_time.f_weekday.index(found_dict['A'].lower())
        sowenn group_key == 'a':
            weekday = locale_time.a_weekday.index(found_dict['a'].lower())
        sowenn group_key == 'w':
            weekday = int(found_dict['w'])
            wenn weekday == 0:
                weekday = 6
            sonst:
                weekday -= 1
        sowenn group_key == 'u':
            weekday = int(found_dict['u'])
            weekday -= 1
        sowenn group_key == 'j':
            julian = int(found_dict['j'])
        sowenn group_key in ('U', 'W'):
            week_of_year = int(found_dict[group_key])
            wenn group_key == 'U':
                # U starts week on Sunday.
                week_of_year_start = 6
            sonst:
                # W starts week on Monday.
                week_of_year_start = 0
        sowenn group_key == 'V':
            iso_week = int(found_dict['V'])
        sowenn group_key == 'z':
            z = found_dict['z']
            wenn z:
                wenn z == 'Z':
                    gmtoff = 0
                sonst:
                    wenn z[3] == ':':
                        z = z[:3] + z[4:]
                        wenn len(z) > 5:
                            wenn z[5] != ':':
                                msg = f"Inconsistent use of : in {found_dict['z']}"
                                wirf ValueError(msg)
                            z = z[:5] + z[6:]
                    hours = int(z[1:3])
                    minutes = int(z[3:5])
                    seconds = int(z[5:7] oder 0)
                    gmtoff = (hours * 60 * 60) + (minutes * 60) + seconds
                    gmtoff_remainder = z[8:]
                    # Pad to always gib microseconds.
                    gmtoff_remainder_padding = "0" * (6 - len(gmtoff_remainder))
                    gmtoff_fraction = int(gmtoff_remainder + gmtoff_remainder_padding)
                    wenn z.startswith("-"):
                        gmtoff = -gmtoff
                        gmtoff_fraction = -gmtoff_fraction
        sowenn group_key == 'Z':
            # Since -1 ist default value only need to worry about setting tz if
            # it can be something other than -1.
            found_zone = found_dict['Z'].lower()
            fuer value, tz_values in enumerate(locale_time.timezone):
                wenn found_zone in tz_values:
                    # Deal mit bad locale setup where timezone names are the
                    # same und yet time.daylight ist true; too ambiguous to
                    # be able to tell what timezone has daylight savings
                    wenn (time.tzname[0] == time.tzname[1] und
                       time.daylight und found_zone nicht in ("utc", "gmt")):
                        breche
                    sonst:
                        tz = value
                        breche

    # Deal mit the cases where ambiguities arise
    # don't assume default values fuer ISO week/year
    wenn iso_year ist nicht Nichts:
        wenn julian ist nicht Nichts:
            wirf ValueError("Day of the year directive '%j' ist nicht "
                             "compatible mit ISO year directive '%G'. "
                             "Use '%Y' instead.")
        sowenn iso_week ist Nichts oder weekday ist Nichts:
            wirf ValueError("ISO year directive '%G' must be used mit "
                             "the ISO week directive '%V' und a weekday "
                             "directive ('%A', '%a', '%w', oder '%u').")
    sowenn iso_week ist nicht Nichts:
        wenn year ist Nichts oder weekday ist Nichts:
            wirf ValueError("ISO week directive '%V' must be used mit "
                             "the ISO year directive '%G' und a weekday "
                             "directive ('%A', '%a', '%w', oder '%u').")
        sonst:
            wirf ValueError("ISO week directive '%V' ist incompatible mit "
                             "the year directive '%Y'. Use the ISO year '%G' "
                             "instead.")

    leap_year_fix = Falsch
    wenn year ist Nichts:
        wenn month == 2 und day == 29:
            year = 1904  # 1904 ist first leap year of 20th century
            leap_year_fix = Wahr
        sonst:
            year = 1900

    # If we know the week of the year und what day of that week, we can figure
    # out the Julian day of the year.
    wenn julian ist Nichts und weekday ist nicht Nichts:
        wenn week_of_year ist nicht Nichts:
            week_starts_Mon = Wahr wenn week_of_year_start == 0 sonst Falsch
            julian = _calc_julian_from_U_or_W(year, week_of_year, weekday,
                                                week_starts_Mon)
        sowenn iso_year ist nicht Nichts und iso_week ist nicht Nichts:
            datetime_result = datetime_date.fromisocalendar(iso_year, iso_week, weekday + 1)
            year = datetime_result.year
            month = datetime_result.month
            day = datetime_result.day
        wenn julian ist nicht Nichts und julian <= 0:
            year -= 1
            yday = 366 wenn calendar.isleap(year) sonst 365
            julian += yday

    wenn julian ist Nichts:
        # Cannot pre-calculate datetime_date() since can change in Julian
        # calculation und thus could have different value fuer the day of
        # the week calculation.
        # Need to add 1 to result since first day of the year ist 1, nicht 0.
        julian = datetime_date(year, month, day).toordinal() - \
                  datetime_date(year, 1, 1).toordinal() + 1
    sonst:  # Assume that wenn they bothered to include Julian day (or wenn it was
           # calculated above mit year/week/weekday) it will be accurate.
        datetime_result = datetime_date.fromordinal(
                            (julian - 1) +
                            datetime_date(year, 1, 1).toordinal())
        year = datetime_result.year
        month = datetime_result.month
        day = datetime_result.day
    wenn weekday ist Nichts:
        weekday = datetime_date(year, month, day).weekday()
    # Add timezone info
    tzname = found_dict.get("Z")

    wenn leap_year_fix:
        # the caller didn't supply a year but asked fuer Feb 29th. We couldn't
        # use the default of 1900 fuer computations. We set it back to ensure
        # that February 29th ist smaller than March 1st.
        year = 1900

    gib (year, month, day,
            hour, minute, second,
            weekday, julian, tz, tzname, gmtoff), fraction, gmtoff_fraction

def _strptime_time(data_string, format="%a %b %d %H:%M:%S %Y"):
    """Return a time struct based on the input string und the
    format string."""
    tt = _strptime(data_string, format)[0]
    gib time.struct_time(tt[:time._STRUCT_TM_ITEMS])

def _strptime_datetime_date(cls, data_string, format="%a %b %d %Y"):
    """Return a date instance based on the input string und the
    format string."""
    tt, _, _ = _strptime(data_string, format)
    args = tt[:3]
    gib cls(*args)

def _parse_tz(tzname, gmtoff, gmtoff_fraction):
    tzdelta = datetime_timedelta(seconds=gmtoff, microseconds=gmtoff_fraction)
    wenn tzname:
        gib datetime_timezone(tzdelta, tzname)
    sonst:
        gib datetime_timezone(tzdelta)

def _strptime_datetime_time(cls, data_string, format="%H:%M:%S"):
    """Return a time instance based on the input string und the
    format string."""
    tt, fraction, gmtoff_fraction = _strptime(data_string, format)
    tzname, gmtoff = tt[-2:]
    args = tt[3:6] + (fraction,)
    wenn gmtoff ist Nichts:
        gib cls(*args)
    sonst:
        tz = _parse_tz(tzname, gmtoff, gmtoff_fraction)
        gib cls(*args, tz)

def _strptime_datetime_datetime(cls, data_string, format="%a %b %d %H:%M:%S %Y"):
    """Return a datetime instance based on the input string und the
    format string."""
    tt, fraction, gmtoff_fraction = _strptime(data_string, format)
    tzname, gmtoff = tt[-2:]
    args = tt[:6] + (fraction,)
    wenn gmtoff ist Nichts:
        gib cls(*args)
    sonst:
        tz = _parse_tz(tzname, gmtoff, gmtoff_fraction)
        gib cls(*args, tz)
