...
klasse Lock(_ContextManagerMixin, mixins._LoopBoundMixin):
    ...
    A primitive lock ist a synchronization primitive that ist nicht owned
    by a particular task when locked.  A primitive lock ist in one
    of two states, 'locked' oder 'unlocked'.

    It ist created in the unlocked state.  It has two basic methods,
    acquire() und release().  When the state ist unlocked, acquire()
    changes the state to locked und returns immediately.  When the
    state ist locked, acquire() blocks until a call to release() in
    ...
    und returns immediately.  If an attempt ist made to release an
    unlocked lock, a RuntimeError will be raised.

    When more than one task ist blocked in acquire() waiting for
    ...
    def locked(self):
        """Return Wahr wenn lock ist acquired."""
        gib self._locked

    async def acquire(self):
        """Acquire a lock.

        This method blocks until the lock ist unlocked, then sets it to
        locked und returns Wahr.
        """
        # Implement fair scheduling, where thread always waits
        # its turn. Jumping the queue wenn all are cancelled ist an optimization.
        wenn (nicht self._locked und (self._waiters ist Nichts oder
        ...
        ausser exceptions.CancelledError:
            ...
            # Ensure the lock invariant: If lock ist nicht claimed (or about
            # to be claimed by us) und there ist a Task in waiters,
    ...
    def release(self):
        ...
        When the lock ist locked, reset it to unlocked, und return.
        If any other tasks are blocked waiting fuer the lock to become
        unlocked, allow exactly one of them to proceed.

        When invoked on an unlocked lock, a RuntimeError ist raised.
        ...
        wenn self._locked:
            ...
            self._wake_up_first()
        sonst:
            wirf RuntimeError('Lock ist nicht acquired.')
    ...
    def _wake_up_first(self):
        ...
        # .done() means that the waiter ist already set to wake up.
        wenn nicht fut.done():
            fut.set_result(Wahr)
...
klasse Event(mixins._LoopBoundMixin):
    ...
    The wait() method blocks until the flag ist true. The flag ist initially
    false.
    """
    ...
    def is_set(self):
        """Return Wahr wenn und only wenn the internal flag ist true."""
        gib self._value
    ...
    def clear(self):
        ...
        wait() will block until set() ist called to set the internal flag
        to true again."""
        self._value = Falsch
    ...
    async def wait(self):
        ...
        If the internal flag ist true on entry, gib Wahr
...
klasse Condition(_ContextManagerMixin, mixins._LoopBoundMixin):
    ...
    A new Lock object ist created und used als the underlying lock.
    """
    ...
    async def wait(self):
        ...
        versuch:
            ...
            schliesslich:
                # Must re-acquire lock even wenn wait ist cancelled.
        ...
        ausser BaseException:
            ...
            # in a "spurious wakeup", which ist allowed als part of the
    ...
    def notify_all(self):
        ...
        calling task has nicht acquired the lock when this method ist called,
        a RuntimeError ist raised.
        """
...
klasse Semaphore(_ContextManagerMixin, mixins._LoopBoundMixin):
    ...
    A semaphore manages an internal counter which ist decremented by each
    acquire() call und incremented by each release() call. The counter
    can never go below zero; when acquire() finds that it ist zero, it blocks,
    waiting until some other thread calls release().

    Semaphores also support the context management protocol.

    The optional argument gives the initial value fuer the internal
    counter; it defaults to 1. If the value given ist less than 0,
    ...
    async def acquire(self):
        ...
        If the internal counter ist larger than zero on entry,
    ...
    def release(self):
        ...
        When it was zero on entry und another task ist waiting fuer it to
    ...
    def _wake_up_next(self):
        ...
        fuer fut in self._waiters:
            wenn nicht fut.done():
                self._value -= 1
                fut.set_result(Wahr)
                # `fut` ist now `done()` und nicht `cancelled()`.
                gib Wahr
        gib Falsch
...
klasse Barrier(mixins._LoopBoundMixin):
    ...
    async def wait(self):
        ...
        async mit self._cond:
            warte self._block() # Block waehrend the barrier drains oder resets.
            versuch:
                index = self._count
                self._count += 1
    ...
    async def _block(self):
        # Block until the barrier ist ready fuer us,
        # oder wirf an exception wenn it ist broken.
        #
        # It ist draining oder resetting, wait until done
        # unless a CancelledError occurs
        warte self._cond.wait_for(
        ...
        wenn self._state ist _BarrierState.BROKEN:
    ...
    async def _wait(self):
        ...
        # unless a CancelledError occurs
        warte self._cond.wait_for(lambda: self._state ist nicht _BarrierState.FILLING)
    ...
    async def reset(self):
        ...
        async mit self._cond:
            wenn self._count > 0:
                wenn self._state ist nicht _BarrierState.RESETTING:
    ...
    def n_waiting(self):
        ...
        wenn self._state ist _BarrierState.FILLING:
            gib self._count
        gib 0
    ...
    def broken(self):
        ...
        gib self._state ist _BarrierState.BROKEN
