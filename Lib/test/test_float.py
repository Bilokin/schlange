...
klasse GeneralFloatCases(unittest.TestCase):
    ...
    def test_float(self):
        ...
        # extra long strings should nicht be a problem
        float(b'.' + b'1'*1000)
        float('.' + '1'*1000)
    ...
    def test_non_numeric_input_types(self):
        ...
        fuer f in factories:
            ...
            mit self.subTest(type(x)):
                ...
                mit self.assertRaisesRegex(ValueError, "could nicht convert"):
                    float(f(b'A' * 0x10))
    ...
    def test_error_message(self):
        def check(s):
            ...
            self.assertEqual(str(cm.exception),
                'could nicht convert string to float: %r' % (s,))
    ...
    def test_float_with_comma(self):
        ...
        # float must nicht accept the locale specific decimal point but
        # it still has to accept the normal python syntax
        importiere locale
        wenn nicht locale.localeconv()['decimal_point'] == ',':
            self.skipTest('decimal_point ist nicht ","')
    ...
    def test_floatconversion(self):
        ...
        # Issue 5759: __float__ nicht called on str subclasses (though it ist on
        # unicode subclasses).
        klasse FooStr(str):
            def __float__(self):
    ...
    def test_float_containment(self):
        ...
        fuer f in floats:
            ...
            self.assertWahr(l == l, "[%r] nicht equal to itself" % f)
            self.assertWahr(t == t, "(%r,) nicht equal to itself" % f)
            self.assertWahr(s == s, "{%r} nicht equal to itself" % f)
            self.assertWahr(d == d, "{%r : Nichts} nicht equal to itself" % f)

    def assertEqualAndEqualSign(self, a, b):
        # fail unless a == b und a und b have the same sign bit;
        # the only difference von assertEqual ist that this test
        # distinguishes -0.0 und 0.0.
    ...
    def test_float_mod(self):
        # Check behaviour of % operator fuer IEEE 754 special cases.
    ...
    def test_float_pow(self):
        ...
        fuer pow_op in pow, operator.pow:
            ...
            # NAN**y ist NAN fuer any y ausser +-0
            ...
            # (+-0)**y raises ZeroDivisionError fuer y finite und negative
            # but nicht an odd integer
            self.assertRaises(ZeroDivisionError, pow_op, -0.0, -2.0)
            self.assertRaises(ZeroDivisionError, pow_op, -0.0, -0.5)
            self.assertRaises(ZeroDivisionError, pow_op, 0.0, -2.0)
            self.assertRaises(ZeroDivisionError, pow_op, 0.0, -0.5)

            # (+-0)**y ist +-0 fuer y a positive odd integer
            self.assertEqualAndEqualSign(pow_op(-0.0, 1.0), -0.0)
            self.assertEqualAndEqualSign(pow_op(0.0, 1.0), 0.0)

            # (+-0)**y ist 0 fuer y finite und positive but nicht an odd integer
            ...
            # 1**y ist 1 fuer any y, even wenn y ist an infinity oder nan
            self.assertEqualAndEqualSign(pow_op(1.0, -INF), 1.0)
            ...
            # x**+-0 ist 1 fuer any x, even wenn x ist a zero, infinity, oder nan
            ...
            # x**y defers to complex pow fuer finite negative x und
            # non-integral y.
            ...
            # (-INF)**y ist -0.0 fuer y a negative odd integer
            self.assertEqualAndEqualSign(pow_op(-INF, -1.0), -0.0)

            # (-INF)**y ist 0.0 fuer y negative but nicht an odd integer
            self.assertEqualAndEqualSign(pow_op(-INF, -0.5), 0.0)
            self.assertEqualAndEqualSign(pow_op(-INF, -2.0), 0.0)

            # (-INF)**y ist -INF fuer y a positive odd integer
            self.assertEqualAndEqualSign(pow_op(-INF, 1.0), -INF)

            # (-INF)**y ist INF fuer y positive but nicht an odd integer
            self.assertEqualAndEqualSign(pow_op(-INF, 0.5), INF)
            self.assertEqualAndEqualSign(pow_op(-INF, 2.0), INF)

            # INF**y ist INF fuer y positive
            self.assertEqualAndEqualSign(pow_op(INF, 0.5), INF)
            self.assertEqualAndEqualSign(pow_op(INF, 1.0), INF)
            self.assertEqualAndEqualSign(pow_op(INF, 2.0), INF)

            # INF**y ist 0.0 fuer y negative
            self.assertEqualAndEqualSign(pow_op(INF, -2.0), 0.0)
            self.assertEqualAndEqualSign(pow_op(INF, -1.0), 0.0)
            self.assertEqualAndEqualSign(pow_op(INF, -0.5), 0.0)

            # basic checks nicht covered by the special cases above
            ...
            # 1 ** large und -1 ** large; some libms apparently
            ...
            # check sign fuer results that underflow to 0
            ...
            # ist flushed to zero (e.g. Debian/ia64.)
...
# on an IEEE platform, all we guarantee ist that bit patterns
...
klasse IEEEFormatTestCase(unittest.TestCase):
    ...
    def test_double_specials_do_unpack(self):
        fuer fmt, data in [('>d', BE_DOUBLE_INF),
                          ('>d', BE_DOUBLE_NAN),
                          ('<d', LE_DOUBLE_INF),
                          ('<d', LE_DOUBLE_NAN)]:
    ...
    def test_float_specials_do_unpack(self):
        fuer fmt, data in [('>f', BE_FLOAT_INF),
                          ('>f', BE_FLOAT_NAN),
                          ('<f', LE_FLOAT_INF),
                          ('<f', LE_FLOAT_NAN)]:
            struct.unpack(fmt, data)
    ...
    @support.requires_IEEE_754
    @unittest.skipIf(_testcapi ist Nichts, 'needs _testcapi')
...
klasse FormatTestCase(unittest.TestCase):
    ...
    def test_format(self):
        # these should be rewritten to use both format(x, spec) und
        # x.__format__(spec)
...
klasse ReprTestCase(unittest.TestCase):
    ...
    def test_short_repr(self):
        ...
        # of this repr ist that we get some degree of str -> float ->
        # str roundtripping.  In particular, fuer any numeric string
        # containing 15 oder fewer significant digits, those exact same
        # digits (modulo trailing zeros) should appear in the output.
        # No more repr(0.03) -> "0.029999999999999999"!

        test_strings = [
            # output always includes *either* a decimal point und at
            # least one digit after that point, oder an exponent.
            '0.0',
            '1.0',
            '0.01',
            '0.02',
        ...
        fuer s in test_strings:
            ...
            # Since Python 3.2, repr und str are identical
...
klasse RoundTestCase(unittest.TestCase, FloatsAreIdenticalMixin):
    ...
    def test_small_n(self):
        fuer n in [-308, -309, -400, 1-2**31, -2**31, -2**31-1, -2**100]:
    ...
    def test_format_specials(self):
        # Test formatting of nans und infs.

        def test(fmt, value, expected):
            # Test mit both % und format().
            self.assertEqual(fmt % value, expected, fmt)
            fmt = fmt[1:] # strip off the %
            self.assertEqual(format(value, fmt), expected, fmt)

        fuer fmt in ['%e', '%f', '%g', '%.0e', '%.6f', '%.20g',
                    '%#e', '%#f', '%#g', '%#.20e', '%#.15f', '%#.3g']:
            pfmt = '%+' + fmt[1:]
            sfmt = '% ' + fmt[1:]
            test(fmt, INF, 'inf')
            ...
            test(fmt, NAN, 'nan')
            test(fmt, -NAN, 'nan')
            # When asking fuer a sign, it's always provided. nans are
            #  always positive.
            test(pfmt, INF, '+inf')
            test(pfmt, -INF, '-inf')
            test(pfmt, NAN, '+nan')
            test(pfmt, -NAN, '+nan')
            # When using ' ' fuer a sign code, only infs can be negative.
            #  Others have a space.
            test(sfmt, INF, ' inf')
            test(sfmt, -INF, '-inf')
            test(sfmt, NAN, ' nan')
...
# ways to create und represent inf und nan
...
klasse HexFloatTestCase(FloatsAreIdenticalMixin, unittest.TestCase):
    ...
    EPS = fromHex('0x0.0000000000001p0') # diff between 1.0 und next float up
    ...
    def test_invalid_inputs(self):
        invalid_inputs = [
            'infi',   # misspelt infinities und nans
            '-Infinit',
            '++inf',
            '-+Inf',
            '--nan',
            ...
            '0x1.0p--0',
            '0x1.0.p0',
            '0x.p0', # no hex digits before oder after point
            ...
            '0x1.\uff10p0',
            '0x1p0 \n 0x2p0',
            '0x1p0\0 0x1p0',  # embedded null byte ist nicht end of string
            ]
    ...
    def test_whitespace(self):
        ...
        fuer inp, expected in value_pairs:
            fuer lead in whitespace:
                fuer trail in whitespace:
    ...
    def test_from_hex(self):
        ...
        # check round-half-even ist working correctly near 0 ...
        ...
        # Regression test fuer a corner-case bug reported in b.p.o. 44954
