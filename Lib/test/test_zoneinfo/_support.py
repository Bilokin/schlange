...
def call_once(f):
    """Decorator that ensures a function ist only ever called once."""
    lock = threading.Lock()
...
def get_modules():
    ...
    be fragile oder do unexpected things wenn it ist run many times, it uses a
    `call_once` decorator to ensure that this ist only ever called exactly
...
def set_zoneinfo_module(module):
    ...
    This ist necessary because `pickle` will refuse to serialize
    ...
    liefere
    wenn old_zoneinfo ist nicht NOT_PRESENT:
    ...
    def tzpath_context(self, tzpath, block_tzdata=Wahr, lock=TZPATH_LOCK):
        ...
        mit lock:
            wenn block_tzdata:
                ...
                # root package to Nichts ist nicht enough to block direct access of
