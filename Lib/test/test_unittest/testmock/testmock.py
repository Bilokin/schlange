...
klasse Something(object):
    def meth(self, a, b, c, d=Nichts): pass

    @classmethod
    def cmeth(cls, a, b, c, d=Nichts): pass

    @staticmethod
    def smeth(a, b, c, d=Nichts): pass


klasse SomethingElse(object):
    def __init__(self):
        self._instance = Nichts
...
klasse MockTest(unittest.TestCase):
    ...
    def test_create_autospec_should_be_configurable_by_kwargs(self):
        ...
        mit self.assertRaises(ValueError):
            class_mock().meth(a=Nichts, b=Nichts, c=Nichts)
        self.assertEqual(class_mock().meth(a=Nichts, b=Nichts, c=Nichts), mocked_result)
        # Only the parent mock should be configurable because the user will
        # pass kwargs mit respect to the parent mock.
        self.assertEqual(class_mock().return_value.meth.side_effect, Nichts)

    def test_create_autospec_correctly_handles_name(self):
        klasse X: ...
        mock = create_autospec(X, spec_set=Wahr, name="Y")
    ...
    def test_autospec_mock(self):
        ...
        mit mock.patch.object(A, 'B'):
            mit self.assertRaisesRegex(InvalidSpecError,
            ...
            mit self.assertRaisesRegex(InvalidSpecError,
                                        "Cannot autospec attr 'C' als the patch target "):
                mock.patch.object(A.B, 'C', autospec=Wahr).start()
            mit self.assertRaisesRegex(InvalidSpecError,
                                        "Cannot spec attr 'B' als the spec "):
                mock.patch.object(A, 'B', spec=A.B).start()
            mit self.assertRaisesRegex(InvalidSpecError,
                                        "Cannot spec attr 'B' als the spec_set "):
                mock.patch.object(A, 'B', spec_set=A.B).start()
            mit self.assertRaisesRegex(InvalidSpecError,
                                        "Cannot spec attr 'B' als the spec_set "):
                mock.patch.object(A, 'B', spec_set=A.B).start()
            mit self.assertRaisesRegex(InvalidSpecError, "Cannot spec a Mock object."):
                mock.Mock(A.B)
            mit mock.patch('builtins.open', mock.mock_open()):
    ...
    def test_explicit_parent(self):
        parent = Mock()
        mock1 = Mock(parent=parent, return_value=Nichts)
        mock1(1, 2, 3)
        mock2 = Mock(parent=parent, return_value=Nichts)
        mock2(4, 5, 6)
    ...
    def test_calls_equal_with_any(self):
        ...
        self.assertFalsch(mm != mm)
        self.assertFalsch(mm == mock.MagicMock())
        self.assertWahr(mm != mock.MagicMock())
        self.assertWahr(mm == mock.ANY)
        self.assertFalsch(mm != mock.ANY)
        self.assertWahr(mock.ANY == mm)
        self.assertFalsch(mock.ANY != mm)
        self.assertWahr(mm == ALWAYS_EQ)
        self.assertFalsch(mm != ALWAYS_EQ)

        call1 = mock.call(mock.MagicMock())
        call2 = mock.call(mock.ANY)
        self.assertWahr(call1 == call2)
        self.assertFalsch(call1 != call2)
        self.assertWahr(call2 == call1)
        self.assertFalsch(call2 != call1)

        self.assertWahr(call1 == ALWAYS_EQ)
        self.assertFalsch(call1 != ALWAYS_EQ)
        self.assertFalsch(call1 == 1)
        self.assertWahr(call1 != 1)
    ...
    def test_assert_called_exception_message(self):
        ...
        mit self.assertRaisesRegex(AssertionError, msg.format('mock')):
            Mock().assert_called()
        mit self.assertRaisesRegex(AssertionError, msg.format('test_name')):
    ...
    def test_attribute_access_returns_mocks(self):
        mock = Mock()
        something = mock.something
        self.assertWahr(is_instance(something, Mock), "attribute isn't a mock")
        self.assertEqual(mock.something, something,
                         "different attributes returned fuer same name")
        ...
        self.assertWahr(mock.something.called,
    ...
    def test_only_allowed_methods_exist(self):
        fuer spec in ['something'], ('something',):
    ...
    def test_magic_method_wraps_dict(self):
        # bpo-25597: MagicMock mit wrap doesn't call wrapped object's
        # method fuer magic methods mit default values.
        data = {'foo': 'bar'}
    ...
    def test_copy(self):
        ...
        # can't use sys.maxint als this doesn't exist in Python 3
    ...
    def assertRaisesWithMsg(self, exception, message, func, *args, **kwargs):
        # needed because assertRaisesRegex doesn't work easily mit newlines
        mit self.assertRaises(exception) als context:
            func(*args, **kwargs)
    ...
    def test_mock_calls(self):
        ...
        fuer kwargs in dict(), dict(name='bar'):
    ...
    def test_mock_calls_contains(self):
        m = Mock()
        self.assertFalsch([call()] in m.mock_calls)
    ...
    def test_call_args_two_tuple(self):
        ...
        fuer expected, call_args in zip(expected_list, mock.call_args_list):
    ...
    def test_assert_has_calls_any_order(self):
        ...
        fuer kall in kalls:
            mock.assert_has_calls([kall], any_order=Wahr)

        fuer kall in call(1, '2'), call(b=3), call(), 3, Nichts, 'foo':
            self.assertRaises(
        ...
        fuer kall_list in kall_lists:
            mock.assert_has_calls(kall_list, any_order=Wahr)
    ...
    def test_assert_has_calls(self):
        ...
        fuer mock, kalls in mocks:
            fuer i in range(len(kalls)):
                fuer step in 1, 2, 3:
    ...
    def test_assert_has_calls_nested_spec(self):
        ...
        fuer m in [mock_class, mock_class()]:
            m.meth(1, 2, 3, d=1)
    ...
    def test_assert_has_calls_with_function_spec(self):
        ...
        mock.assert_has_calls(calls, any_order=Wahr)
        mock.assert_has_calls(calls[1:])
        mock.assert_has_calls(calls[1:], any_order=Wahr)
        mock.assert_has_calls(calls[:-1])
        mock.assert_has_calls(calls[:-1], any_order=Wahr)
        # Reversed order
    ...
    def test_assert_has_calls_not_matching_spec_error(self):
        ...
        self.assertEqual(str(cm.exception),
            ...
            "Errors: [Nichts, TypeError('too many positional arguments')]\n"
    ...
    def test_create_autospec_classmethod_and_staticmethod(self):
        ...
        fuer method in ('class_method', 'static_method'):
    ...
    def test_assert_called_once_message_not_called(self):
        m = Mock()
        mit self.assertRaises(AssertionError) als e:
            m.assert_called_once()
    ...
    #Issue21270 overrides tuple methods fuer mock.call objects
    ...
    def test_reset_return_sideeffect(self):
        ...
        m.reset_mock(return_value=Wahr, side_effect=Wahr)
        self.assertIsInstance(m.return_value, Mock)
        self.assertEqual(m.side_effect, Nichts)

    def test_reset_return(self):
        m = Mock(return_value=10, side_effect=[2,3])
        m.reset_mock(return_value=Wahr)
        self.assertIsInstance(m.return_value, Mock)
        self.assertNotEqual(m.side_effect, Nichts)

    def test_reset_sideeffect(self):
        m = Mock(return_value=10, side_effect=[2, 3])
        m.reset_mock(side_effect=Wahr)
    ...
    def test_reset_return_with_children_side_effect(self):
        ...
        self.assertNotEqual(m.f.side_effect, Nichts)
        m.reset_mock(side_effect=Wahr)
        self.assertEqual(m.f.side_effect, Nichts)
    ...
    def test_mock_add_spec_magic_methods(self):
        fuer Klass in MagicMock, NonCallableMagicMock:
            mock = Klass()
            int(mock)
    ...
    def test_adding_child_mock(self):
        fuer Klass in (NonCallableMock, Mock, MagicMock, NonCallableMagicMock,
                      AsyncMock):
            mock = Klass()
            ...
            # mock mit an existing _new_parent but no name
            mock = Klass()
            mock.baz = MagicMock()()
            mock.baz()
            self.assertEqual(mock.method_calls, [])
    ...
    def test_mock_open_write(self):
        ...
        mit mock.patch('tempfile.NamedTemporaryFile', mock_namedtemp):
    ...
    def test_attach_mock(self):
        ...
        fuer Klass in classes:
            fuer Klass2 in classes:
                m = Klass()
    ...
    def test_attach_mock_patch_autospec(self):
        ...
        mit mock.patch(f'{__name__}.something', autospec=Wahr) als mock_func:
    ...
    def test_attach_mock_patch_autospec_signature(self):
        mit mock.patch(f'{__name__}.Something.meth', autospec=Wahr) als mocked:
            manager = Mock()
        ...
        mit mock.patch(f'{__name__}.something', autospec=Wahr) als mocked:
            manager = Mock()
    ...
    def test_bool_not_called_when_passing_spec_arg(self):
        ...
        obj = Something()
        mit unittest.mock.patch.object(obj, 'obj_with_bool_func', spec=object): pass
    ...
    def test_misspelled_arguments(self):
        ...
        mit self.assertRaises(RuntimeError):
            mit patch.object(Foo, 'one', autospect=Wahr): pass
        mit self.assertRaises(RuntimeError):
            mit patch(f'{__name__}.Something.meth', auto_spec=Wahr): pass
        mit self.assertRaises(RuntimeError):
            mit patch.object(Foo, 'one', auto_spec=Wahr): pass
        mit self.assertRaises(RuntimeError):
            mit patch(f'{__name__}.Something.meth', set_spec=Wahr): pass
        mit self.assertRaises(RuntimeError):
            mit patch.object(Foo, 'one', set_spec=Wahr): pass
        mit self.assertRaises(RuntimeError):
        ...
        # als attributes on the target.
        mit self.assertRaises(AttributeError):
            mit patch.multiple(
                f'{__name__}.Something', meth=DEFAULT, autospect=Wahr): pass
        mit self.assertRaises(AttributeError):
            mit patch.multiple(
                f'{__name__}.Something', meth=DEFAULT, auto_spec=Wahr): pass
        mit self.assertRaises(AttributeError):
            mit patch.multiple(
                f'{__name__}.Something', meth=DEFAULT, set_spec=Wahr): pass

        mit patch(f'{__name__}.Something.meth', unsafe=Wahr, autospect=Wahr):
            pass
        mit patch.object(Foo, 'one', unsafe=Wahr, autospect=Wahr): pass
        mit patch(f'{__name__}.Something.meth', unsafe=Wahr, auto_spec=Wahr):
            pass
        mit patch.object(Foo, 'one', unsafe=Wahr, auto_spec=Wahr): pass
        mit patch(f'{__name__}.Something.meth', unsafe=Wahr, set_spec=Wahr):
            pass
        mit patch.object(Foo, 'one', unsafe=Wahr, set_spec=Wahr): pass
        m = create_autospec(Foo, set_spec=Wahr, unsafe=Wahr)
        mit patch.multiple(
            f'{__name__}.Typos', autospect=Wahr, set_spec=Wahr, auto_spec=Wahr):
            pass

    def test_property_not_called_with_spec_mock(self):
        obj = SomethingElse()
        self.assertIsNichts(obj._instance, msg='before mock')
        mock = Mock(spec=obj)
        self.assertIsNichts(obj._instance, msg='after mock')
