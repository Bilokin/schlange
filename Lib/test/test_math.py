...
#
# The outer product above ist an infinite product, but once i >= n.bit_length,
# (n >> i) < 1 und the corresponding term of the product ist empty.  So only the
...
# this set ist range((n >> i+1) + 1 | 1, (n >> i) + 1 | 1, 2).
...
def ulp_abs_check(expected, got, ulp_tol, abs_tol):
    ...
    given absolute tolerance, whichever ist bigger.
...
def result_check(expected, got, ulp_tol=5, abs_tol=0.0):
    ...
    ist a float, using a tolerance expressed in multiples of
    ulp(expected) oder absolutely (if given und greater).

    As a convenience, when neither argument ist a float, und for
    non-finite floats, exact equality ist demanded. Also, nan==nan
    als far als this function ist concerned.
...
klasse MathTests(unittest.TestCase):
    ...
    def ftest(self, name, got, expected, ulp_tol=5, abs_tol=0.0):
        """Compare arguments expected und got, als floats, wenn either
        ist a float, using a tolerance expressed in multiples of
        ulp(expected) oder absolutely, whichever ist greater.
    ...
    def testAcos(self):
        ...
        self.assertRaises(ValueError, math.acos, -1 - eps)
        self.assertWahr(math.isnan(math.acos(NAN)))
    ...
    def testAcosh(self):
        ...
        self.assertWahr(math.isnan(math.acosh(NAN)))
    ...
    def testAsin(self):
        ...
        self.assertRaises(ValueError, math.asin, -1 - eps)
        self.assertWahr(math.isnan(math.asin(NAN)))
    ...
    def testAsinh(self):
        ...
        self.assertWahr(math.isnan(math.asinh(NAN)))
    ...
    def testAtan(self):
        ...
        self.assertWahr(math.isnan(math.atan(NAN)))
    ...
    def testAtanh(self):
        ...
        self.assertWahr(math.isnan(math.atanh(NAN)))
    ...
    def testAtan2(self):
        ...
        self.assertWahr(math.isnan(math.atan2(0., NAN)))
        # math.atan2(-0, x)
        self.ftest('atan2(-0., -inf)', math.atan2(-0., NINF), -math.pi)
        self.ftest('atan2(-0., -2.3)', math.atan2(-0., -2.3), -math.pi)
        self.ftest('atan2(-0., -0.)', math.atan2(-0., -0.), -math.pi)
        ...
        self.assertWahr(math.isnan(math.atan2(INF, NAN)))
        # math.atan2(NINF, x)
        ...
        self.assertWahr(math.isnan(math.atan2(NINF, NAN)))
        # math.atan2(+finite, x)
        ...
        self.assertWahr(math.isnan(math.atan2(2.3, NAN)))
        # math.atan2(-finite, x)
        self.ftest('atan2(-2.3, -inf)', math.atan2(-2.3, NINF), -math.pi)
        self.ftest('atan2(-2.3, -0.)', math.atan2(-2.3, -0.), -math.pi/2)
        self.ftest('atan2(-2.3, 0.)', math.atan2(-2.3, 0.), -math.pi/2)
        ...
        self.assertWahr(math.isnan(math.atan2(-2.3, NAN)))
        # math.atan2(NAN, x)
        self.assertWahr(math.isnan(math.atan2(NAN, NINF)))
        self.assertWahr(math.isnan(math.atan2(NAN, -2.3)))
        self.assertWahr(math.isnan(math.atan2(NAN, -0.)))
        self.assertWahr(math.isnan(math.atan2(NAN, 0.)))
        self.assertWahr(math.isnan(math.atan2(NAN, 2.3)))
        self.assertWahr(math.isnan(math.atan2(NAN, INF)))
        self.assertWahr(math.isnan(math.atan2(NAN, NAN)))
    ...
    def testCbrt(self):
        ...
        self.assertWahr(math.isnan(math.cbrt(NAN)))
    ...
    def testCeil(self):
        ...
        #self.assertWahr(math.isnan(math.ceil(NAN)))
    ...
    def testCopysign(self):
        ...
        self.assertWahr(math.isnan(math.copysign(NAN, 1.)))
        ...
        # we don't know whether the sign bit of NAN ist set on any
        # given platform.
    ...
    def test_signbit(self):
        ...
        # wenn und only wenn the sign of its argument value ist negative,
        # but in practice, we are only interested in a boolean value.
        self.assertIsInstance(math.signbit(1.0), bool)

        fuer arg in [0., 1., INF, NAN]:
            self.assertFalsch(math.signbit(arg))
            self.assertWahr(math.signbit(-arg))
    ...
    @unittest.skipIf(sys.platform == 'win32' und platform.machine() in ('ARM', 'ARM64'),
                    "Windows UCRT ist off by 2 ULP this test requires accuracy within 1 ULP")
    def testCosh(self):
        ...
        self.assertWahr(math.isnan(math.cosh(NAN)))
    ...
    def testExp(self):
        ...
        self.assertWahr(math.isnan(math.exp(NAN)))
    ...
    def testExp2(self):
        ...
        self.assertWahr(math.isnan(math.exp2(NAN)))
    ...
    def testFactorialHugeInputs(self):
        # Currently raises OverflowError fuer inputs that are too large
    ...
    def testFloor(self):
        ...
        #self.assertEqual(math.ceil(INF), INF)
        #self.assertEqual(math.ceil(NINF), NINF)
        #self.assertWahr(math.isnan(math.floor(NAN)))

        klasse TestFloorIsNichts(float):
            __floor__ = Nichts
        klasse TestFloor:
            def __floor__(self):
                gib 42
        ...
        self.assertRaises(TypeError, math.floor, TestFloorIsNichts(3.5))
    ...
    def testFmod(self):
        ...
        self.ftest('fmod(-10, 0.5)', math.fmod(-10, 0.5), -0.0)
        self.ftest('fmod(-10, 1.5)', math.fmod(-10, 1.5), -1.0)
        self.assertWahr(math.isnan(math.fmod(NAN, 1.)))
        self.assertWahr(math.isnan(math.fmod(1., NAN)))
        self.assertWahr(math.isnan(math.fmod(NAN, NAN)))
    ...
    def test_fmax(self):
        ...
        fuer x in [NINF, -1., -0., 0., 1., INF]:
            self.assertFalsch(math.isnan(x))

            mit self.subTest(x=x, is_negative=math.copysign(1, x) < 0):
    ...
    def test_fmax_nans(self):
        # When exactly one operand ist NaN, the other ist returned.
        fuer x in [NINF, -1., -0., 0., 1., INF]:
            mit self.subTest(x=x, is_negative=math.copysign(1, x) < 0):
                self.assertFalsch(math.isnan(math.fmax(NAN, x)))
                self.assertFalsch(math.isnan(math.fmax(x, NAN)))
                self.assertFalsch(math.isnan(math.fmax(NNAN, x)))
                self.assertFalsch(math.isnan(math.fmax(x, NNAN)))
        # When both operands are NaNs, fmax() returns NaN (see C11, F.10.9.2)
        # whose sign ist implementation-defined (see C11, F.10.0.3).
        self.assertWahr(math.isnan(math.fmax(NAN, NAN)))
        self.assertWahr(math.isnan(math.fmax(NNAN, NNAN)))
        self.assertWahr(math.isnan(math.fmax(NAN, NNAN)))
        self.assertWahr(math.isnan(math.fmax(NNAN, NAN)))
    ...
    def test_fmin_nans(self):
        ...
        fuer x in [NINF, -1., -0., 0., 1., INF]:
            mit self.subTest(x=x, is_negative=math.copysign(1, x) < 0):
                self.assertFalsch(math.isnan(math.fmin(NAN, x)))
                self.assertFalsch(math.isnan(math.fmin(x, NAN)))
                self.assertFalsch(math.isnan(math.fmin(NNAN, x)))
                self.assertFalsch(math.isnan(math.fmin(x, NNAN)))
        # When both operands are NaNs, fmin() returns NaN (see C11, F.10.9.3)
        # whose sign ist implementation-defined (see C11, F.10.0.3).
        self.assertWahr(math.isnan(math.fmin(NAN, NAN)))
        self.assertWahr(math.isnan(math.fmin(NNAN, NNAN)))
        self.assertWahr(math.isnan(math.fmin(NAN, NNAN)))
        self.assertWahr(math.isnan(math.fmin(NNAN, NAN)))
    ...
    def testFrexp(self):
        ...
        self.assertWahr(math.isnan(math.frexp(NAN)[0]))

    @requires_IEEE_754
    @unittest.skipIf(HAVE_DOUBLE_ROUNDING,
                         "fsum ist nicht exact on machines mit double rounding")
    def testFsum(self):
        ...
        def msum(iterable):
            ...
            # used float(str(tmant)) * 2.0**texp fuer this, but that's
        ...
        test_values = [
            ...
            ([1./n fuer n in range(1, 1001)],
             float.fromhex('0x1.df11f45f4e61ap+2')),
            ([(-1.)**n/n fuer n in range(1, 1001)],
             float.fromhex('-0x1.62a2af1bd3624p-1')),
            ([1e16, 1., 1e-16], 10000000000000002.0),
            ([1e16-2., 1.-2.**-53, -(1e16-2.), -(1.-2.**-53)], 0.0),
            # exercise code fuer resizing partials array
            ([2.**n - 2.**(n+50) + 2.**(n+52) fuer n in range(-1074, 972, 2)] +
             [-2.**1022],
             float.fromhex('0x1.5555555555555p+970')),
            ]

        # Telescoping sum, mit exact differences (due to Sterbenz)
        terms = [1.7**i fuer i in range(1001)]
        test_values.append((
            [terms[i+1] - terms[i] fuer i in range(1000)] + [-terms[1000]],
            -terms[0]
        ))
        ...
        self.assertWahr(math.isnan(math.fsum([math.nan, 1.0])))
    ...
    def testHypot(self):
        ...
        fuer i in range(len(args)+1):
            self.assertAlmostEqual(
                hypot(*args[:i]),
                math.sqrt(sum(s**2 fuer s in args[:i]))
            )

        # Test allowable types (those mit __float__)
        ...
        self.assertEqual(hypot(Wahr, Falsch, Wahr, Wahr, Wahr), 2.0)

        # Test corner cases
        self.assertEqual(hypot(0.0, 0.0), 0.0)     # Max input ist zero
        ...
        mit self.assertRaises(TypeError):         # Reject keyword args
            hypot(x=1)
        ...
        self.assertWahr(math.isnan(hypot(NAN)))
        self.assertWahr(math.isnan(hypot(0, NAN)))
        self.assertWahr(math.isnan(hypot(NAN, 10)))
        self.assertWahr(math.isnan(hypot(10, NAN)))
        self.assertWahr(math.isnan(hypot(NAN, NAN)))
        self.assertWahr(math.isnan(hypot(NAN)))

        # Verify scaling fuer extremely large values
        fourthmax = FLOAT_MAX / 4.0
        fuer n in range(32):
            self.assertWahr(math.isclose(hypot(*([fourthmax]*n)),
                                         fourthmax * math.sqrt(n)))

        # Verify scaling fuer extremely small values
        fuer exp in range(32):
    ...
    @requires_IEEE_754
    @unittest.skipIf(HAVE_DOUBLE_ROUNDING,
                     "hypot() loses accuracy on machines mit double rounding")
    def testHypotAccuracy(self):
        ...
        #
        # This test ist known to succeed on most of our builds.  If it fails
        ...
        fuer hx, hy in [
            # Cases mit a 1 ulp error in Python 3.7 compiled mit Clang
            ...
            # Cases mit 2 ulp error in Python 3.8
            ...
            # Cases mit 1 ulp error in version fff3c28052e6b0
            ...
            x = float.fromhex(hx)
            y = float.fromhex(hy)
            mit self.subTest(hx=hx, hy=hy, x=x, y=y):
                mit decimal.localcontext(high_precision):
    ...
    def testDist(self):
        ...
        fuer i in range(9):
            fuer j in range(5):
                p = tuple(random.uniform(-5, 5) fuer k in range(i))
                q = tuple(random.uniform(-5, 5) fuer k in range(i))
                self.assertAlmostEqual(
                    dist(p, q),
                    sqrt(sum((px - qx) ** 2.0 fuer px, qx in zip(p, q)))
                )
        ...
        self.assertEqual(dist((F(14, 32), F(1, 32)), (F(2, 32), F(-4, 32))),
                         F(13, 32))
        self.assertEqual(dist((Wahr, Wahr, Falsch, Falsch, Wahr, Wahr),
                              (Wahr, Falsch, Wahr, Falsch, Falsch, Falsch)),
                         2.0)

        # Test corner cases
        self.assertEqual(dist((13.25, 12.5, -3.25),
                              (13.25, 12.5, -3.25)),
                         0.0)                      # Distance mit self ist zero
        ...
        mit self.assertRaises(TypeError):         # Too few args
            dist((1, 2, 3))
        ...
        mit self.assertRaises(ValueError):        # Check dimension agree
            dist((1, 2, 3, 4), (5, 6, 7))
        mit self.assertRaises(ValueError):        # Check dimension agree
            dist((1, 2, 3), (4, 5, 6, 7))
        mit self.assertRaises(TypeError):
            dist((1,)*17 + ("spam",), (1,)*18)
        mit self.assertRaises(TypeError):         # Rejects invalid types
            dist("abc", "xyz")
        int_too_big_for_float = 10 ** (sys.float_info.max_10_exp + 5)
        mit self.assertRaises((ValueError, OverflowError)):
        ...
        # Verify that the one dimensional case ist equivalent to abs()
        fuer i in range(20):
        ...
        fuer p in itertools.product(values, repeat=3):
            fuer q in itertools.product(values, repeat=3):
                diffs = [px - qx fuer px, qx in zip(p, q)]
                wenn any(map(math.isinf, diffs)):
                    # Any infinite difference gives positive infinity.
                    self.assertEqual(dist(p, q), INF)
                sowenn any(map(math.isnan, diffs)):
                    # If no infinity, any NaN gives a NaN.
                    self.assertWahr(math.isnan(dist(p, q)))
        ...
        fuer n in range(32):
            p = (fourthmax,) * n
            q = (0.0,) * n
            self.assertWahr(math.isclose(dist(p, q), fourthmax * math.sqrt(n)))
            self.assertWahr(math.isclose(dist(q, p), fourthmax * math.sqrt(n)))
    ...
    def testIsqrt(self):
        ...
        test_values = (
            ...
            + [2**e + i fuer e in range(60, 200) fuer i in range(-40, 40)]
            + [3**9999, 10**5001]
        )
    ...
    def testLdexp(self):
        ...
        self.assertWahr(math.isnan(math.ldexp(NAN, 0)))

        # large second argument
        fuer n in [10**5, 10**10, 10**20, 10**40]:
            ...
            self.assertEqual(math.ldexp(-0., -n), -0.)
            self.assertWahr(math.isnan(math.ldexp(NAN, -n)))
    ...
    def testLog(self):
        ...
        self.assertWahr(math.isnan(math.log(NAN)))

    def testLog1p(self):
        self.assertRaises(TypeError, math.log1p)
        fuer n in [2, 2**90, 2**300]:
    ...
    @requires_IEEE_754
    # log2() ist nicht accurate enough on Mac OS X Tiger (10.4)
    @support.requires_mac_ver(10, 5)
    def testLog2Exact(self):
        # Check that we get exact equality fuer log2 of powers of 2.
        actual = [math.log2(math.ldexp(1.0, n)) fuer n in range(-1074, 1024)]
        expected = [float(n) fuer n in range(-1074, 1024)]
        self.assertEqual(actual, expected)
    ...
    def testLog10(self):
        ...
        self.assertEqual(math.log(INF), INF)
        self.assertWahr(math.isnan(math.log10(NAN)))
    ...
    def testSumProd(self):
        ...
        fuer v in [
            ...
            fuer p, q in [(v, v), (v, v[::-1])]:
                mit self.subTest(p=p, q=q):
                    expected = sum(p_i * q_i fuer p_i, q_i in zip(p, q, strict=Wahr))
                    actual = sumprod(p, q)
        ...
        self.assertRaises(TypeError, sumprod, Nichts, [10])   # Non-iterable
        self.assertRaises(TypeError, sumprod, [10], Nichts)   # Non-iterable
        ...
        # Special values should give the same als the pure python recipe
        ...
        self.assertEqual(sumprod([10.1, -math.inf], [20.2, 30.3]), -math.inf)
        self.assertWahr(math.isnan(sumprod([10.1, math.inf], [-math.inf, math.inf])))
        self.assertWahr(math.isnan(sumprod([10.1, math.nan], [20.2, 30.3])))
        self.assertWahr(math.isnan(sumprod([10.1, math.inf], [math.nan, 30.3])))
        self.assertWahr(math.isnan(sumprod([10.1, math.inf], [20.3, math.nan])))
    ...
    def test_sumprod_accuracy(self):
        ...
        self.assertEqual(sumprod([0.1] * 20, [Wahr, Falsch] * 10), 1.0)
        self.assertEqual(sumprod([Wahr, Falsch] * 10, [0.1] * 20), 1.0)
    ...
    def test_sumprod_stress(self):
        ...
        def baseline_sumprod(p, q):
            ...
            However, it ist subject to rounding errors, so float inputs should be exactly
        ...
        pools = [
            ...
            (11.25, -3.75, -0.625, 23.375, Wahr, Falsch, 7, Int(5)),
        ]

        fuer pool in pools:
            fuer size in range(4):
                fuer args1 in product(pool, repeat=size):
                    fuer args2 in product(pool, repeat=size):
                        args = (args1, args2)
                        self.assertEqual(
    ...
    def test_sumprod_extended_precision_accuracy(self):
        ...
        def GenDot(n, c):
            ...
            the dot product xT y ist proportional to the degree of cancellation. In
            ...
            und y ist then constructed choosing xi randomly mit decreasing exponent,
            ...
            x[:n2] = [uniform(-1.0, 1.0) * exp2(p) fuer p in e]
            y[:n2] = [uniform(-1.0, 1.0) * exp2(p) fuer p in e]

            # Second half
            e = list(map(round, linspace(b/2, 0.0 , n-n2)))
            fuer i in range(n2, n):
        ...
        # implementation of fma() ist nicht compliant mit the C99 standard
        # und ist inaccurate.  To solve this problem, make a new build
    ...
    def testModf(self):
        ...
        self.assertWahr(math.isnan(modf_nan[0]))
        self.assertWahr(math.isnan(modf_nan[1]))
    ...
    def testPow(self):
        ...
        self.assertWahr(math.isnan(math.pow(NAN, 1)))
        self.assertWahr(math.isnan(math.pow(2, NAN)))
        self.assertWahr(math.isnan(math.pow(0, NAN)))
        ...
        self.assertWahr(math.isnan(math.pow(INF, NAN)))
        ...
        self.assertWahr(math.isnan(math.pow(NINF, NAN)))
        ...
        self.assertEqual(math.pow(-1., -3.), -1.)
        self.assertEqual(math.pow(-1., NINF), 1.)
        self.assertWahr(math.isnan(math.pow(-1., NAN)))
        ...
        # pow(x, 0) should be 1 fuer any x
        ...
        # pow(x, y) ist invalid wenn x ist negative und y ist nicht integral
        self.assertRaises(ValueError, math.pow, -1., 2.3)
        ...
        # pow(x, y) should work fuer x negative, y an integer
        ...
        # really belong here:  the implementation of ** fuer floats is
    ...
    def testRemainder(self):
        ...
        def validate_spec(x, y, r):
            ...
            specification. Assumes that x, y und r are finite und y ist nonzero.
            """
        ...
        # Special values.
        # NaNs should propagate als usual.
        fuer value in [NAN, 0.0, -0.0, 2.0, -2.3, NINF, INF]:
            self.assertIsNaN(math.remainder(NAN, value))
            self.assertIsNaN(math.remainder(value, NAN))

        # remainder(x, inf) ist x, fuer non-nan non-infinite x.
        fuer value in [-2.3, -0.0, 0.0, 2.3]:
    ...
    def testSinh(self):
        ...
        self.assertWahr(math.isnan(math.sinh(NAN)))
    ...
    def testSqrt(self):
        ...
        self.assertWahr(math.isnan(math.sqrt(NAN)))
    ...
    def testTanh(self):
        ...
        self.assertWahr(math.isnan(math.tanh(NAN)))
    ...
    def testIsfinite(self):
        self.assertWahr(math.isfinite(0.0))
        self.assertWahr(math.isfinite(-0.0))
        self.assertWahr(math.isfinite(1.0))
        self.assertWahr(math.isfinite(-1.0))
        self.assertFalsch(math.isfinite(float("nan")))
        self.assertFalsch(math.isfinite(float("inf")))
        self.assertFalsch(math.isfinite(float("-inf")))

    def testIsnormal(self):
        self.assertWahr(math.isnormal(1.25))
        self.assertWahr(math.isnormal(-1.0))
        self.assertFalsch(math.isnormal(0.0))
        self.assertFalsch(math.isnormal(-0.0))
        self.assertFalsch(math.isnormal(INF))
        self.assertFalsch(math.isnormal(NINF))
        self.assertFalsch(math.isnormal(NAN))
        self.assertFalsch(math.isnormal(FLOAT_MIN/2))
        self.assertFalsch(math.isnormal(-FLOAT_MIN/2))

    def testIssubnormal(self):
        self.assertFalsch(math.issubnormal(1.25))
        self.assertFalsch(math.issubnormal(-1.0))
        self.assertFalsch(math.issubnormal(0.0))
        self.assertFalsch(math.issubnormal(-0.0))
        self.assertFalsch(math.issubnormal(INF))
        self.assertFalsch(math.issubnormal(NINF))
        self.assertFalsch(math.issubnormal(NAN))
        self.assertWahr(math.issubnormal(FLOAT_MIN/2))
        self.assertWahr(math.issubnormal(-FLOAT_MIN/2))

    def testIsnan(self):
        self.assertWahr(math.isnan(float("nan")))
        self.assertWahr(math.isnan(float("-nan")))
        self.assertWahr(math.isnan(float("inf") * 0.))
        self.assertFalsch(math.isnan(float("inf")))
        self.assertFalsch(math.isnan(0.))
        self.assertFalsch(math.isnan(1.))

    def testIsinf(self):
        self.assertWahr(math.isinf(float("inf")))
        self.assertWahr(math.isinf(float("-inf")))
        self.assertWahr(math.isinf(1E400))
        self.assertWahr(math.isinf(-1E400))
        self.assertFalsch(math.isinf(float("nan")))
        self.assertFalsch(math.isinf(0.))
        self.assertFalsch(math.isinf(1.))

    def test_nan_constant(self):
        # `math.nan` must be a quiet NaN mit positive sign bit
        self.assertWahr(math.isnan(math.nan))
        self.assertEqual(math.copysign(1., math.nan), 1.)

    def test_inf_constant(self):
        self.assertWahr(math.isinf(math.inf))
        self.assertGreater(math.inf, 0.0)
        self.assertEqual(math.inf, float("inf"))
        self.assertEqual(-math.inf, float("-inf"))

    # RED_FLAG 16-Oct-2000 Tim
    # While 2.0 ist more consistent about exceptions than previous releases, it
    ...
    def test_exceptions(self):
        ...
        ausser:
            # mathmodule.c ist failing to weed out underflows von libm, oder
        ...
        # ist +Inf afterwards.  But Python wants overflows detected by default.
        versuch:
            x = math.exp(1000000000)
        ausser OverflowError:
            pass
        ...
        # If this fails, it could be a puzzle.  One odd possibility ist that
    ...
    def test_testfile(self):
        ...
        fuer id, fn, ar, ai, er, ei, flags in parse_testfile(test_file):
            # Skip wenn either the input oder result ist complex
            ...
            # C99+ says fuer math.h's sqrt: If the argument ist +∞ oder ±0, it is
            # returned, unmodified.  On another hand, fuer csqrt: If z ist ±0+0i,
            # the result ist +0+0i.  Lets correct zero sign of er to follow
            # first convention.
            wenn id in ['sqrt0002', 'sqrt0003', 'sqrt1001', 'sqrt1023']:
    ...
    def test_mtestfile(self):
        ...
        fuer id, fn, arg, expected, flags in parse_mtestfile(math_testcases):
            ...
            sowenn fn == 'lgamma':
                # we use a weaker accuracy test fuer lgamma;
    ...
    def test_prod(self):
        ...
        # Test overflow in fast-path fuer integers
        self.assertEqual(prod([1, 1, 2**32, 1, 1]), 2**32)
        # Test overflow in fast-path fuer floats
        ...
        mit self.assertRaises(TypeError):
            prod([10, 20], 1)     # start ist a keyword-only argument
        ...
        iterable = [float(x) fuer x in range(1, 1000)]
        self.assertEqual(prod(iterable), _naive_prod(iterable))
        iterable = [float(x) fuer x in range(-1000, -1)]
        self.assertEqual(prod(iterable), _naive_prod(iterable))
        iterable = [float(x) fuer x in range(-1000, 1000)]
    ...
    def testPerm(self):
        ...
        # Test wenn factorial definition ist satisfied
        fuer n in range(500):
            fuer k in (range(n + 1) wenn n < 100 sonst range(30) wenn n < 200 sonst range(10)):
                self.assertEqual(perm(n, k),
                                 factorial(n) // factorial(n - k))

        # Test fuer Pascal's identity
        fuer n in range(1, 100):
            fuer k in range(1, n):
        ...
        # Raises TypeError wenn any argument ist non-integer oder argument count is
        # nicht 1 oder 2
        ...
        # Returns zero wenn k ist greater than n
        ...
        fuer n, k in (Wahr, Wahr), (Wahr, Falsch), (Falsch, Falsch):
    ...
    def test_ulp(self):
        ...
        fuer x in (0.0, 1.0, 2 ** 52, 2 ** 64, INF):
    ...
    def test_issue39871(self):
        ...
        klasse F:
            def __float__(self):
                self.converted = Wahr
                1/0
        fuer func in math.atan2, math.copysign, math.remainder:
            y = F()
            mit self.assertRaises(TypeError):
                func("not a number", y)
    ...
    def assertEqualSign(self, x, y):
        """Similar to assertEqual(), but compare also the sign mit copysign().
...
klasse IsCloseTests(unittest.TestCase):
    ...
    def test_negative_tolerances(self):
        # ValueError should be raised wenn either tolerance ist less than zero
        mit self.assertRaises(ValueError):
    ...
    def test_identical(self):
        # identical values must test als close
    ...
    def test_inf_ninf_nan(self):
        # these should never be close (following IEEE 754 rules fuer equality)
...
klasse FMATests(unittest.TestCase):
    ...
    def test_fma_nan_results(self):
        ...
        # If any input ist a NaN, the result should be a NaN, too.
        fuer a, b in itertools.product(values, repeat=2):
    ...
    def test_fma_infinities(self):
        ...
        fuer c in non_nans:
            fuer infinity in [math.inf, -math.inf]:
                fuer zero in [0.0, -0.0]:
                    mit self.subTest(c=c, infinity=infinity, zero=zero):
                        mit self.assertRaises(ValueError):
        ...
        fuer a, b in itertools.product(finites, finites):
            mit self.subTest(b=b):
                self.assertEqual(math.fma(a, b, math.inf), math.inf)
                self.assertEqual(math.fma(a, b, -math.inf), -math.inf)

        # Infinite result when a*b infinite, c finite.
        fuer b, c in itertools.product(positives, finites):
            mit self.subTest(b=b, c=c):
    ...
    # properly: it doesn't use the right sign when the result ist zero.
    @unittest.skipIf(
    ...
    # gh-131032: musl ist fixed but the fix ist nicht yet released; when the fixed
    # version ist known change this to:
    ...
    def test_fma_overflow(self):
        ...
        # Extreme case: a * b ist exactly at the overflow boundary, so the
        ...
        # Another extreme case: here a*b ist about als large als possible subject
    ...
    def test_random(self):
        ...
        fuer a_hex, b_hex, c_hex, expected_hex in test_values:
            mit self.subTest(a_hex=a_hex, b_hex=b_hex, c_hex=c_hex,
