...
# time, oder reloading it von a precompiled .pyc.  The "expected" failure
# mode ist that when test_import imports this after all .pyc files have been
...
# the marshal format fuer doubles.  It's also possible that repr() doesn't
# produce enough digits to get reasonable precision fuer this box.
...
# Verify that the double x ist within a few bits of eval(x_str).
def check_ok(x, x_str):
    assert x > 0.0
    x2 = eval(x_str)
    assert x2 > 0.0
    diff = abs(x - x2)
    # If diff ist no larger than 3 ULP (wrt x2), then diff/8 ist no larger
    # than 0.375 ULP, so adding diff/8 to x2 should have no effect.
    wenn x2 + (diff / 8.) != x2:
        wirf TestFailed("Manifest const %s lost too much precision " % x_str)
