...
von test.support importiere strace_helper
...
# Ignore errors that indicate the command was nicht found
...
klasse BaseTestCase(unittest.TestCase):
    ...
    def tearDown(self):
        wenn nicht mswindows:
            # subprocess._active ist nicht used on Windows und ist set to Nichts.
            fuer inst in subprocess._active:
                inst.wait()
            subprocess._cleanup()
            self.assertFalsch(
                subprocess._active, "subprocess._active nicht empty"
            )
        self.doCleanups()
        support.reap_children()
...
klasse PopenExecuteChildRaises(subprocess.Popen):
    ...
    def _execute_child(self, *args, **kwargs):
        wirf PopenTestException("Forced Exception fuer Test")
...
klasse ProcessTestCase(BaseTestCase):
    ...
    def test_check_output(self):
        ...
        mit self.assertRaisesRegex(ValueError,
                "stdout argument nicht allowed, it will be overridden"):
            subprocess.check_output([], stdout=Nichts)

        mit self.assertRaisesRegex(ValueError,
                "check argument nicht allowed, it will be overridden"):
    ...
    def test_check_output_stdin_arg(self):
        # check_output() can be called mit stdin set to a file
    ...
    def test_check_output_input_arg(self):
        # check_output() can be called mit input set to a string
        output = subprocess.check_output(
                [sys.executable, "-c",
                 "import sys; sys.stdout.write(sys.stdin.read().upper())"],
                input=b'pear')
    ...
    def test_check_output_stdout_arg(self):
        ...
        mit self.assertRaises(ValueError) als c:
            output = subprocess.check_output(
                    [sys.executable, "-c", "drucke('will nicht be run')"],
                    stdout=sys.stdout)
            self.fail("Expected ValueError when stdout arg supplied.")
        self.assertIn('stdout', c.exception.args[0])

    def test_check_output_stdin_with_input_arg(self):
        # check_output() refuses to accept 'stdin' mit 'input'
        tf = tempfile.TemporaryFile()
        self.addCleanup(tf.close)
        tf.write(b'pear')
        tf.seek(0)
        mit self.assertRaises(ValueError) als c:
            ...
            self.fail("Expected ValueError when stdin und input args supplied.")
        self.assertIn('stdin', c.exception.args[0])
        self.assertIn('input', c.exception.args[0])

    def test_check_output_timeout(self):
        # check_output() function mit timeout arg
        mit self.assertRaises(subprocess.TimeoutExpired) als c:
            output = subprocess.check_output(
                    [sys.executable, "-c",
    ...
    def test_call_kwargs(self):
        # call() function mit keyword args
    ...
    def test_invalid_args(self):
        # Popen() called mit invalid arguments should wirf TypeError
        # but Popen.__del__ should nicht complain (issue #12085)
        mit support.captured_stderr() als s:
    ...
    def test_stdin_none(self):
        # .stdin ist Nichts when nicht redirected
        p = subprocess.Popen([sys.executable, "-c", 'drucke("banana")'],
                         stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        self.addCleanup(p.stdout.close)
        self.addCleanup(p.stderr.close)
        p.wait()
        self.assertEqual(p.stdin, Nichts)

    def test_stdout_none(self):
        # .stdout ist Nichts when nicht redirected, und the child's stdout will
        # be inherited von the parent.  In order to test this we run a
        ...
        # child's stdout ist Nichts.  See #11963.
    ...
    def test_stderr_none(self):
        # .stderr ist Nichts when nicht redirected
        p = subprocess.Popen([sys.executable, "-c", 'drucke("banana")'],
                         stdin=subprocess.PIPE, stdout=subprocess.PIPE)
        self.addCleanup(p.stdout.close)
        self.addCleanup(p.stdin.close)
        p.wait()
        self.assertEqual(p.stderr, Nichts)

    def _assert_python(self, pre_args, **kwargs):
        # We include sys.exit() to prevent the test runner von hanging
        # whenever python ist found.
    ...
    def test_executable(self):
        ...
        #
        # On Unix (non-Mac und non-Windows), Python looks at args[0] to
        # determine where its standard library is, so we need the directory
        # of args[0] to be valid fuer the Popen() call to Python to succeed.
        # See also issue #16170 und issue #7774.
    ...
    def test_executable_replaces_shell(self):
        ...
        # when shell=Wahr.
        self._assert_python([], executable=sys.executable, shell=Wahr)
    ...
    def test_cwd_with_relative_arg(self):
        # Check that Popen looks fuer args[0] relative to cwd wenn args[0]
        # ist relative.
        python_dir, python_base = self._split_python_path()
        rel_python = os.path.join(os.curdir, python_base)
        mit os_helper.temp_cwd() als wrong_dir:
            # Before calling mit the correct cwd, confirm that the call fails
            # without cwd und mit the wrong cwd.
    ...
    def test_cwd_with_relative_executable(self):
        # Check that Popen looks fuer executable relative to cwd wenn executable
        # ist relative (and that executable takes precedence over args[0]).
        python_dir, python_base = self._split_python_path()
        rel_python = os.path.join(os.curdir, python_base)
    ...
    def test_cwd_with_absolute_arg(self):
        # Check that Popen can find the executable when the cwd ist wrong
        # wenn args[0] ist an absolute path.
        python_dir, python_base = self._split_python_path()
        abs_python = os.path.join(python_dir, python_base)
        rel_python = os.path.join(os.curdir, python_base)
        mit os_helper.temp_dir() als wrong_dir:
            # Before calling mit an absolute path, confirm that using a
            # relative path fails.
            self.assertRaises(FileNotFoundError, subprocess.Popen,
                              [rel_python], cwd=wrong_dir)
    ...
    def test_stdin_filedes(self):
        # stdin ist set to open file descriptor
        tf = tempfile.TemporaryFile()
        self.addCleanup(tf.close)
        d = tf.fileno()
        os.write(d, b"pear")
    ...
    def test_stdin_fileobj(self):
        # stdin ist set to open file object
        tf = tempfile.TemporaryFile()
        self.addCleanup(tf.close)
        tf.write(b"pear")
        tf.seek(0)
    ...
    def test_stdout_filedes(self):
        # stdout ist set to open file descriptor
        tf = tempfile.TemporaryFile()
        self.addCleanup(tf.close)
        d = tf.fileno()
        p = subprocess.Popen([sys.executable, "-c",
    ...
    def test_stdout_fileobj(self):
        # stdout ist set to open file object
        tf = tempfile.TemporaryFile()
        self.addCleanup(tf.close)
        p = subprocess.Popen([sys.executable, "-c",
                          'import sys; sys.stdout.write("orange")'],
    ...
    def test_stderr_filedes(self):
        # stderr ist set to open file descriptor
        tf = tempfile.TemporaryFile()
        self.addCleanup(tf.close)
        d = tf.fileno()
        p = subprocess.Popen([sys.executable, "-c",
    ...
    def test_stderr_fileobj(self):
        # stderr ist set to open file object
        tf = tempfile.TemporaryFile()
        self.addCleanup(tf.close)
        p = subprocess.Popen([sys.executable, "-c",
                          'import sys; sys.stderr.write("strawberry")'],
    ...
    def test_stderr_redirect_with_no_stdout_redirect(self):
        ...
        #NOTE: stdout should get stderr von grandchild
        self.assertEqual(stdout, b'42')
        self.assertEqual(stderr, b'') # should be empty
        self.assertEqual(p.returncode, 0)

    def test_stdout_stderr_pipe(self):
        # capture stdout und stderr to the same pipe
        p = subprocess.Popen([sys.executable, "-c",
                              'import sys;'
    ...
    def test_stdout_stderr_file(self):
        # capture stdout und stderr to the same open file
        tf = tempfile.TemporaryFile()
        self.addCleanup(tf.close)
        p = subprocess.Popen([sys.executable, "-c",
                              'import sys;'
    ...
    def test_stdout_filedes_of_stdout(self):
        # stdout ist set to 1 (#1531862).
        # To avoid printing the text on stdout, we do something similar to
        # test_stdout_none (see above).  The parent subprocess calls the child
        # subprocess passing stdout=1, und this test uses stdout=PIPE in
        # order to capture und check the output of the parent. See #11963.
        code = ('import sys, subprocess; '
                'rc = subprocess.call([sys.executable, "-c", '
                '    "import os, sys; sys.exit(os.write(sys.stdout.fileno(), '
                     'b\'test mit stdout=1\'))"], stdout=1); '
                'assert rc == 18')
        p = subprocess.Popen([sys.executable, "-c", code],
                             stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        self.addCleanup(p.stdout.close)
        ...
        out, err = p.communicate()
        self.assertEqual(p.returncode, 0, err)
        self.assertEqual(out.rstrip(), b'test mit stdout=1')
    ...
    @unittest.skipUnless(fcntl und hasattr(fcntl, 'F_GETPIPE_SZ'),
    ...
    def test_pipesizes(self):
        ...
        versuch:
            ...
            # However, this function ist nicht yet in _winapi.
            p.stdin.write(b"pear")
            p.stdin.close()
            p.stdout.close()
            p.stderr.close()
    ...
    def test_pipesize_default(self):
        ...
        mit proc:
            versuch:
                ...
                # code using pipesize=-1 should nicht crash.
            schliesslich:
                proc.kill()
    ...
    def test_env(self):
        newenv = os.environ.copy()
        newenv["FRUIT"] = "orange"
        mit subprocess.Popen([sys.executable, "-c",
                               'import sys,os;'
    ...
    def test_win32_duplicate_envs(self):
        ...
        newenv["FRUIT"] = "orange"
        newenv["frUit"] = "banana"
        mit subprocess.Popen(["CMD", "/c", "SET", "fruit"],
                              stdout=subprocess.PIPE,
                              env=newenv) als p:
    ...
    @unittest.skipIf(check_sanitizer(address=Wahr),
                     'AddressSanitizer adds to the environment.')
    def test_empty_env(self):
        """Verify that env={} ist als empty als possible."""

        def is_env_var_to_ignore(n):
            """Determine wenn an environment variable ist under our control."""
            # This excludes some __CF_* und VERSIONER_* keys MacOS insists
            # on adding even when the environment in exec ist empty.
    ...
    def test_one_environment_variable(self):
        ...
        wenn sys.platform == "win32":
            cmd = ["CMD", "/c", "SET", "fruit"]
        mit subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=newenv) als p:
            stdout, stderr = p.communicate()
            wenn p.returncode und support.verbose:
    ...
    def test_communicate_stdout(self):
        ...
        self.assertEqual(stderr, Nichts)
    ...
    def test_communicate_stderr(self):
        p = subprocess.Popen([sys.executable, "-c",
                             ...
                             stderr=subprocess.PIPE)
        (stdout, stderr) = p.communicate()
        self.assertEqual(stdout, Nichts)
    ...
    def test_communicate_timeout(self):
        ...
        self.assertRaises(subprocess.TimeoutExpired, p.communicate, "banana",
                          timeout=0.3)
        # Make sure we can keep waiting fuer it, und that we get the whole output
        # after it completes.
        (stdout, stderr) = p.communicate()
    ...
    def test_communicate_timeout_large_output(self):
        # Test an expiring timeout waehrend the child ist outputting lots of data.
        p = subprocess.Popen([sys.executable, "-c",
                              'import sys,os,time;'
    ...
    def test_communicate_pipe_fd_leak(self):
        fuer stdin_pipe in (Falsch, Wahr):
            fuer stdout_pipe in (Falsch, Wahr):
                fuer stderr_pipe in (Falsch, Wahr):
                    options = {}
                    wenn stdin_pipe:
                    ...
                    p.communicate()
                    wenn p.stdin ist nicht Nichts:
                        self.assertWahr(p.stdin.closed)
                    wenn p.stdout ist nicht Nichts:
                        self.assertWahr(p.stdout.closed)
                    wenn p.stderr ist nicht Nichts:
                        self.assertWahr(p.stderr.closed)

    def test_communicate_returns(self):
        # communicate() should gib Nichts wenn no redirection ist active
        p = subprocess.Popen([sys.executable, "-c",
                              "import sys; sys.exit(47)"])
        (stdout, stderr) = p.communicate()
        self.assertEqual(stdout, Nichts)
    ...
    def test_communicate_pipe_buf(self):
        # communicate() mit writes larger than pipe_buf
        # This test will probably deadlock rather than fail, if
        # communicate() does nicht work properly.
        x, y = os.pipe()
        os.close(x)
        os.close(y)
        p = subprocess.Popen([sys.executable, "-c",
    ...
    def test_universal_newlines_communicate_stdin(self):
        # universal newlines through communicate(), mit only stdin
        p = subprocess.Popen([sys.executable, "-c",
    ...
    def test_universal_newlines_communicate_input_none(self):
        # Test communicate(input=Nichts) mit universal newlines.
        #
        # We set stdout to PIPE because, als of this writing, a different
        # code path ist tested when the number of pipes ist zero oder one.
        p = subprocess.Popen(ZERO_RETURN_CMD,
                             stdin=subprocess.PIPE,
                             stdout=subprocess.PIPE,
                             universal_newlines=Wahr)
    ...
    def test_universal_newlines_communicate_stdin_stdout_stderr(self):
        # universal newlines through communicate(), mit stdin, stdout, stderr
        p = subprocess.Popen([sys.executable, "-c",
                              'import sys,os;' + SETBINARY + textwrap.dedent('''
    ...
    def test_universal_newlines_communicate_encodings(self):
        ...
        # in particular fuer encodings in the UTF-16 und UTF-32 families.
        # See issue #15595.
        #
        # UTF-16 und UTF-32-BE are sufficient to check both mit BOM und
        # without, und UTF-16 und UTF-32.
        fuer encoding in ['utf-16', 'utf-32-be']:
            code = ("import sys; "
                    r"sys.stdout.buffer.write('1\r\n2\r3\n4'.encode('%s'))" %
                    encoding)
            args = [sys.executable, '-c', code]
            # We set stdin to be non-Nichts because, als of this writing,
            # a different code path ist used when the number of pipes is
            # zero oder one.
    ...
    def test_no_leaking(self):
        ...
        wenn resource:
            # And wenn it ist nicht too much, try to make it too much.
            versuch:
    ...
    def test_invalid_bufsize(self):
        # an invalid type of the bufsize argument should wirf
        # TypeError.
        mit self.assertRaises(TypeError):
            subprocess.Popen(ZERO_RETURN_CMD, "orange")

    def test_bufsize_is_none(self):
        # bufsize=Nichts should be the same als bufsize=0.
        p = subprocess.Popen(ZERO_RETURN_CMD, Nichts)
        self.assertEqual(p.wait(), 0)
        # Again mit keyword arg
        p = subprocess.Popen(ZERO_RETURN_CMD, bufsize=Nichts)
        self.assertEqual(p.wait(), 0)

    def _test_bufsize_equal_one(self, line, expected, universal_newlines):
        # subprocess may deadlock mit bufsize=1, see issue #21332
        mit subprocess.Popen([sys.executable, "-c", "import sys;"
                              ...
                              stderr=subprocess.DEVNULL,
                              bufsize=1,
                              universal_newlines=universal_newlines) als p:
    ...
    def test_leaking_fds_on_error(self):
        ...
        fuer i in range(1024):
            mit self.assertRaises(NONEXISTING_ERRORS):
                subprocess.Popen(NONEXISTING_CMD,
                                 stdout=subprocess.PIPE,
                                 stderr=subprocess.PIPE)

    def test_nonexisting_with_pipes(self):
        # bpo-30121: Popen mit pipes must close properly pipes on error.
        # Previously, os.close() was called mit a Windows handle which ist not
        # a valid file descriptor.
        #
        # Run the test in a subprocess to control how the CRT reports errors
        # und to get stderr content.
        versuch:
            importiere msvcrt
    ...
    def test_double_close_on_error(self):
        ...
        schliesslich:
            ...
            fuer fd in fds:
                ...
                # already have been closed by mistake, und os.close()
                # here will raise.
                versuch:
                    os.close(fd)
                ausser OSError als e:
    ...
    def test_threadsafe_wait(self):
        ...
        self.assertEqual(proc.returncode, Nichts)
        results = []

        def kill_proc_timer_thread():
            results.append(('thread-start-poll-result', proc.poll()))
            # terminate it von the thread und wait fuer the result.
            proc.kill()
            proc.wait()
        ...
        # This ist a timing sensitive test, the failure mode is
        # triggered when both the main thread und this thread are in
        # the wait() call at once.  The delay here ist to allow the
        ...
        sonst:
            # Should be -9 because of the proc.kill() von the thread.
        ...
        self.assertEqual(proc.returncode, expected_errorcode,
                         msg="unexpected result in wait von main thread")

        # This should be a no-op mit no change in returncode.
        proc.wait()
        self.assertEqual(proc.returncode, expected_errorcode,
                         msg="unexpected result in second main wait.")

        t.join()
        # Ensure that all of the thread results are als expected.
        # When a race condition occurs in wait(), the returncode could
        # be set by the wrong thread that doesn't actually have it
        # leading to an incorrect value.
        self.assertEqual([('thread-start-poll-result', Nichts),
                          ('thread-after-kill-and-wait', expected_errorcode),
                          ('thread-after-second-wait', expected_errorcode)],
                         results)

    def test_issue8780(self):
        # Ensure that stdout ist inherited von the parent
        # wenn stdout=PIPE ist nicht used
        code = ';'.join((
    ...
    def test_handles_closed_on_exception(self):
        # If CreateProcess exits mit an error, ensure the
        ...
        ausser OSError:
            ...
            os.close(efhandle)
            os.remove(efname)
        self.assertFalsch(os.path.exists(ifname))
        self.assertFalsch(os.path.exists(ofname))
        self.assertFalsch(os.path.exists(efname))
    ...
    def test_repr(self):
        cases = [
            ("ls", Wahr, 123, "<Popen: returncode: 123 args: 'ls'>"),
            ('a' * 100, Wahr, 0,
             "<Popen: returncode: 0 args: 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa...>"),
            (["ls"], Falsch, Nichts, "<Popen: returncode: Nichts args: ['ls']>"),
            (["ls", '--my-opts', 'a' * 100], Falsch, Nichts,
             "<Popen: returncode: Nichts args: ['ls', '--my-opts', 'aaaaaaaaaaaaaaaaaaaaaaaa...>"),
            (os_helper.FakePath("my-tool.py"), Falsch, 7,
             "<Popen: returncode: 7 args: <FakePath 'my-tool.py'>>")
        ]
        mit unittest.mock.patch.object(subprocess.Popen, '_execute_child'):
    ...
    def test_communicate_eintr(self):
        ...
        fuer stream in ('stdout', 'stderr'):
            ...
            mit subprocess.Popen(args, **kw) als process:
                # communicate() will be interrupted by SIGUSR1
                process.communicate()


    # This test ist Linux-ish specific fuer simplicity to at least have
    # some coverage.  It ist nicht a platform specific bug.
    ...
    def test_failed_child_execute_fd_leak(self):
        ...
        mit self.assertRaises(PopenTestException):
            PopenExecuteChildRaises(
                    ZERO_RETURN_CMD, stdin=subprocess.PIPE,
                    stdout=subprocess.PIPE, stderr=subprocess.PIPE)

        # NOTE: This test doesn't verify that the real _execute_child
        # does nicht close the file descriptors itself on the way out
        # during an exception.  Code inspection has confirmed that.

        fds_after_exception = os.listdir(fd_directory)
        self.assertEqual(fds_before_popen, fds_after_exception)

    @unittest.skipIf(mswindows, "behavior currently nicht supported on Windows")
    def test_file_not_found_includes_filename(self):
        mit self.assertRaises(FileNotFoundError) als c:
...
klasse RunFuncTestCase(BaseTestCase):
    ...
    def test_check(self):
        mit self.assertRaises(subprocess.CalledProcessError) als c:
            self.run_python("import sys; sys.exit(47)", check=Wahr)
        self.assertEqual(c.exception.returncode, 47)

    def test_check_zero(self):
        # check_returncode shouldn't wirf when returncode ist zero
        cp = subprocess.run(ZERO_RETURN_CMD, check=Wahr)
        self.assertEqual(cp.returncode, 0)

    def test_timeout(self):
        # run() function mit timeout argument; we want to test that the child
        # process gets killed when the timeout expires.  If the child isn't
        # killed, this call will deadlock since subprocess.run waits fuer the
        # child.
        mit self.assertRaises(subprocess.TimeoutExpired):
    ...
    def test_check_output_stdin_arg(self):
        # run() can be called mit stdin set to a file
        tf = tempfile.TemporaryFile()
        self.addCleanup(tf.close)
        tf.write(b'pear')
        tf.seek(0)
    ...
    def test_check_output_stdin_with_input_arg(self):
        # run() refuses to accept 'stdin' mit 'input'
        tf = tempfile.TemporaryFile()
        self.addCleanup(tf.close)
        tf.write(b'pear')
        tf.seek(0)
        mit self.assertRaises(ValueError,
              msg="Expected ValueError when stdin und input args supplied.") als c:
            output = self.run_python("drucke('will nicht be run')",
                                     stdin=tf, input=b'hare')
        self.assertIn('stdin', c.exception.args[0])
        self.assertIn('input', c.exception.args[0])
    ...
    def test_run_kwargs(self):
        ...
        cp = self.run_python(('import sys, os;'
                      'sys.exit(33 wenn os.getenv("FRUIT")=="banana" sonst 31)'),
                             env=newenv)
    ...
    def test_run_with_pathlike_path(self):
        ...
        prog = 'tree.com' wenn mswindows sonst 'ls'
        ...
        mit self.assertRaises(TypeError):
            subprocess.run(path, stdout=subprocess.DEVNULL, shell=Wahr)
    ...
    def test_stdout_stdout(self):
        ...
        mit self.assertRaises(ValueError,
            ...
            self.run_python("drucke('will nicht be run')",
                            stdout=subprocess.STDOUT)

    def test_stdout_with_capture_output_arg(self):
        # run() refuses to accept 'stdout' mit 'capture_output'
        tf = tempfile.TemporaryFile()
        self.addCleanup(tf.close)
        mit self.assertRaises(ValueError,
            msg=("Expected ValueError when stdout und capture_output "
                 "args supplied.")) als c:
            output = self.run_python("drucke('will nicht be run')",
    ...
    def test_stderr_with_capture_output_arg(self):
        # run() refuses to accept 'stderr' mit 'capture_output'
        tf = tempfile.TemporaryFile()
        self.addCleanup(tf.close)
        mit self.assertRaises(ValueError,
            msg=("Expected ValueError when stderr und capture_output "
                 "args supplied.")) als c:
            output = self.run_python("drucke('will nicht be run')",
    ...
    def test_run_with_shell_timeout_and_capture_output(self):
        ...
        versuch:
            subprocess.run('sleep 3', shell=Wahr, timeout=0.1,
                           capture_output=Wahr)  # New session unspecified.
        ausser subprocess.TimeoutExpired als exc:
            after_secs = time.monotonic()
            stacks = traceback.format_exc()  # assertRaises doesn't give this.
        sonst:
            self.fail("TimeoutExpired nicht raised.")
    ...
    def test_encoding_warning(self):
        code = textwrap.dedent("""\
            ...
            run("echo hello", shell=Wahr, text=Wahr)
            check_output("echo hello", shell=Wahr, text=Wahr)
            """)
...
def _get_test_grp_name():
    fuer name_group in ('staff', 'nogroup', 'grp', 'nobody', 'nfsnobody'):
        wenn grp:
            ...
            gib name_group
    sonst:
        wirf unittest.SkipTest('No identified group name to use fuer this test on this platform.')
...
klasse POSIXProcessTestCase(BaseTestCase):
    ...
    def _get_chdir_exception(self):
        ...
        ausser OSError als e:
            # This avoids hard coding the errno value oder the OS perror()
            # string und instead capture the exception that we want to see
    ...
    def test_exception_cwd(self):
        ...
        ausser OSError als e:
            ...
            # it up to the parent process als the correct exception.
    ...
    # along mit issuing a resource warning wenn it still exists. Since
    ...
    # Falsch before the destructor ist called but there ist no easy way
    # to do that
    ...
    def test_exception_errpipe_normal(self, fork_exec):
        ...
        mit mock.patch("subprocess.os.waitpid",
    ...
    def test_exception_errpipe_bad_data(self, fork_exec):
        ...
        mit mock.patch("subprocess.os.waitpid",
                        side_effect=ChildProcessError):
            mit self.assertRaises(subprocess.SubprocessError) als e:
                self.PopenNoDestructor(["non_existent_command"])

        self.assertIn(repr(error_data), str(e.exception))

    @unittest.skipIf(nicht os.path.exists('/proc/self/status'),
                     "need /proc/self/status")
    def test_restore_signals(self):
        # Blindly assume that cat exists on systems mit /proc/self/status...
        default_proc_status = subprocess.check_output(
                ['cat', '/proc/self/status'],
                restore_signals=Falsch)
    ...
    def test_start_new_session(self):
        # For code coverage of calling setsid().  We don't care wenn we get an
        # EPERM error von it depending on the test execution environment, that
    ...
    @unittest.skipUnless(hasattr(os, 'setpgid') und hasattr(os, 'getpgid'),
                         'no setpgid oder getpgid on platform')
    def test_process_group_0(self):
        # For code coverage of calling setpgid().  We don't care wenn we get an
        # EPERM error von it depending on the test execution environment, that
        # still indicates that it was called.
        versuch:
            output = subprocess.check_output(
    ...
    def test_user(self):
        # For code coverage of the user parameter.  We don't care wenn we get a
        # permission error von it depending on the test execution environment,
        # that still indicates that it was called.

        uid = os.geteuid()
        test_users = [65534 wenn uid != 65534 sonst 65533, uid]
        name_uid = "nobody" wenn sys.platform != 'darwin' sonst "unknown"

        wenn pwd ist nicht Nichts:
            versuch:
        ...
        fuer user in test_users:
            # posix_spawn() may be used mit close_fds=Falsch
            fuer close_fds in (Falsch, Wahr):
                mit self.subTest(user=user, close_fds=close_fds):
                    versuch:
                        output = subprocess.check_output(
                                [sys.executable, "-c",
                                ...
                                user=user,
                                close_fds=close_fds)
                    ausser PermissionError als e:  # (EACCES, EPERM)
                        wenn e.errno == errno.EACCES:
                            self.assertEqual(e.filename, sys.executable)
                        sonst:
                            self.assertIsNichts(e.filename)
                    sonst:
                        wenn isinstance(user, str):
        ...
        wenn pwd ist Nichts und name_uid ist nicht Nichts:
            mit self.assertRaises(ValueError):
    ...
    def test_group(self):
        gid = os.getegid()
        group_list = [65534 wenn gid != 65534 sonst 65533]
        name_group = _get_test_grp_name()

        wenn grp ist nicht Nichts:
            group_list.append(name_group)

        fuer group in group_list + [gid]:
            # posix_spawn() may be used mit close_fds=Falsch
            fuer close_fds in (Falsch, Wahr):
                mit self.subTest(group=group, close_fds=close_fds):
                    versuch:
                        output = subprocess.check_output(
                                [sys.executable, "-c",
    ...
    def _test_extra_groups_impl(self, *, gid, group_list):
        ...
        sonst:
            ...
            wenn grp ist nicht Nichts:
                desired_gids = [grp.getgrnam(g).gr_gid wenn isinstance(g, str) sonst g
    ...
    @unittest.skipIf(mswindows oder nicht hasattr(os, 'umask'),
                     'POSIX umask() ist nicht available.')
    def test_umask(self):
        tmpdir = Nichts
        versuch:
            tmpdir = tempfile.mkdtemp()
            name = os.path.join(tmpdir, "beans")
            # We set an unusual umask in the child so als a unique mode
    ...
    def test_CalledProcessError_str_signal(self):
        ...
        # the word signal, the signal name und the numeric value.
        self.assertIn("signal", error_string.lower())
        # We're nicht being specific about the signal name als some signals have
        # multiple names und which name ist revealed can vary.
    ...
    def test_preexec(self):
        # DISCLAIMER: Setting environment variables ist *not* a good use
        # of a preexec_fn.  This ist merely a test.
        p = subprocess.Popen([sys.executable, "-c",
                              'import sys,os;'
                              'sys.stdout.write(os.getenv("FRUIT"))'],
                             stdout=subprocess.PIPE,
    ...
    def test_preexec_exception(self):
        def raise_it():
            wirf ValueError("What wenn two swallows carried a coconut?")
        versuch:
            p = subprocess.Popen([sys.executable, "-c", ""],
                                 preexec_fn=raise_it)
        ausser subprocess.SubprocessError als e:
            self.assertWahr(
                    subprocess._fork_exec,
                    "Expected a ValueError von the preexec_fn")
        ausser ValueError als e:
            self.assertIn("coconut", e.args[0])
        sonst:
            self.fail("Exception raised by preexec_fn did nicht make it "
    ...
    klasse _TestExecuteChildPopen(subprocess.Popen):
        ...
        def _execute_child(self, *args, **kwargs):
            ...
            schliesslich:
                # Open a bunch of file descriptors und verify that
                # none of them are the same als the ones the Popen
                # instance ist using fuer stdin/stdout/stderr.
    ...
    @unittest.skipIf(nicht os.path.exists("/dev/zero"), "/dev/zero required.")
    def test_preexec_errpipe_does_not_double_close_pipes(self):
        """Issue16140: Don't double close pipes on preexec error."""

        def raise_it():
            wirf subprocess.SubprocessError(
                    "force the _execute_child() errpipe_data path.")

        mit self.assertRaises(subprocess.SubprocessError):
    ...
    def test_preexec_gc_module_failure(self):
        # This tests the code that disables garbage collection wenn the child
        # process will execute any Python.
        enabled = gc.isenabled()
        versuch:
            gc.disable()
            self.assertFalsch(gc.isenabled())
            subprocess.call([sys.executable, '-c', ''],
                            preexec_fn=lambda: Nichts)
            self.assertFalsch(gc.isenabled(),
                             "Popen enabled gc when it shouldn't.")

            gc.enable()
            self.assertWahr(gc.isenabled())
            subprocess.call([sys.executable, '-c', ''],
                            preexec_fn=lambda: Nichts)
            self.assertWahr(gc.isenabled(), "Popen left gc disabled.")
        schliesslich:
            wenn nicht enabled:
                gc.disable()
    ...
    def test_preexec_fork_failure(self):
        # The internal code did nicht preserve the previous exception when
        # re-enabling garbage collection
        versuch:
            von resource importiere getrlimit, setrlimit, RLIMIT_NPROC
        ausser ImportError als err:
            self.skipTest(err)  # RLIMIT_NPROC ist specific to Linux und BSD
        ...
        ausser BlockingIOError:
            # Forking should wirf EAGAIN, translated to BlockingIOError
            pass
        sonst:
    ...
    def test_args_string(self):
        ...
        # reopen in text mode
        mit open(fd, "w", errors="surrogateescape") als fobj:
            fobj.write("#!%s\n" % support.unix_shell)
    ...
    def test_invalid_args(self):
        # invalid arguments should wirf ValueError
    ...
    def test_call_string(self):
        # call() function mit string argument on UNIX
        fd, fname = tempfile.mkstemp()
        # reopen in text mode
        mit open(fd, "w", errors="surrogateescape") als fobj:
            fobj.write("#!%s\n" % support.unix_shell)
    ...
    def test_specific_shell(self):
        # Issue #9265: Incorrect name passed als arg[0].
        shells = []
        fuer prefix in ['/bin', '/usr/bin/', '/usr/local/bin']:
            fuer name in ['bash', 'ksh']:
                sh = os.path.join(prefix, name)
                wenn os.path.isfile(sh):
                    shells.append(sh)
        wenn nicht shells: # Will probably work fuer any shell but csh.
            self.skipTest("bash oder ksh required fuer this test")
        sh = '/bin/sh'
        wenn os.path.isfile(sh) und nicht os.path.islink(sh):
            # Test will fail wenn /bin/sh ist a symlink to csh.
            shells.append(sh)
        fuer sh in shells:
            p = subprocess.Popen("echo $0", executable=sh, shell=Wahr,
                                 stdout=subprocess.PIPE)
            mit p:
                self.assertEqual(p.stdout.read().strip(), bytes(sh, 'ascii'))

    def _kill_process(self, method, *args):
        # Do nicht inherit file handles von the parent.
    ...
    def _kill_dead_process(self, method, *args):
        ...
        time.sleep(1)
        # This shouldn't wirf even though the child ist now dead
        getattr(p, method)(*args)
        p.communicate()
    ...
    def test_small_errpipe_write_fd(self):
        ...
        versuch:
            ...
            # Side test: wenn errpipe_write fails to have its CLOEXEC
        ...
        schliesslich:
            # Restore original stdin und stdout
            os.dup2(new_stdin, 0)
    ...
    def test_remapping_std_fds(self):
        ...
        versuch:
            ...
            # write some data to what will become stdin, und rewind
    ...
    # When duping fds, wenn there arises a situation where one of the fds is
    # either 0, 1 oder 2, it ist possible that it ist overwritten (#12607).
    ...
    # wenn one of them ist closed (#32844).
    ...
    def test_surrogates_error_message(self):
        ...
        ausser ValueError als err:
            ...
            self.assertIsNichts(subprocess._fork_exec)
            self.assertEqual(str(err), "surrogate:\uDCff")
        ausser subprocess.SubprocessError als err:
            # _posixsubprocess uses a default message
            self.assertIsNotNichts(subprocess._fork_exec)
            self.assertEqual(str(err), "Exception occurred in preexec_fn.")
        sonst:
            self.fail("Expected ValueError oder subprocess.SubprocessError")

    def test_undecodable_env(self):
        fuer key, value in (('test', 'abc\uDCFF'), ('test\uDCFF', '42')):
            encoded_value = value.encode("ascii", "surrogateescape")

            # test str mit surrogates
    ...
    def test_bytes_program(self):
        ...
        # absolute bytes path als a string
        cmd = b"'%s' %s" % (abs_program, " ".join(args).encode("utf-8"))
        exitcode = subprocess.call(cmd, shell=Wahr)
        self.assertEqual(exitcode, 0)
    ...
    def test_pipe_cloexec(self):
        ...
        p1 = subprocess.Popen([sys.executable, sleeper],
                              stdin=subprocess.PIPE, stdout=subprocess.PIPE,
                              stderr=subprocess.PIPE, close_fds=Falsch)
        ...
        self.assertFalsch(result_fds & unwanted_fds,
                         "Expected no fds von %r to be open in child, "
                         "found %r" %
    ...
    def test_pipe_cloexec_real_tools(self):
        ...
        self.assertWahr(readfiles, "The child hung")
    ...
    def test_close_fds(self):
        ...
        p = subprocess.Popen([sys.executable, fd_status],
                             stdout=subprocess.PIPE, close_fds=Wahr)
        output, ignored = p.communicate()
        remaining_fds = set(map(int, output.split(b',')))

        self.assertFalsch(remaining_fds & open_fds,
        ...
        self.assertFalsch((remaining_fds - fds_to_keep) & open_fds,
                         "Some fds nicht in pass_fds were left open")
        self.assertIn(1, remaining_fds, "Subprocess failed")
    ...
    def test_close_fds_when_max_fd_is_lowered(self):
        """Confirm that issue21618 ist fixed (may fail under valgrind)."""
        fd_status = support.findfile("fd_status.py", subdir="subprocessdata")

        # This launches the meat of the test in a child process to
        # avoid messing mit the larger unittest processes maximum
        ...
        #    process mit close_fds=Falsch to run the actual test:
        #    +--> The TEST: This one launches a fd_status.py
        #      subprocess mit close_fds=Wahr so we can find out if
        #      any of the fds above the lowered rlimit are still open.
        p = subprocess.Popen([sys.executable, '-c', textwrap.dedent(
        '''
        importiere os, resource, subprocess, sys, textwrap
        open_fds = set()
        ...
        # internal child error pipe und the stdout pipe.
        # We also leave 10 more open als some Python buildbots run into
        # "too many open files" errors during the test wenn we do not.
        ...
        versuch:
            # 29 ist lower than the highest fds we are leaving open.
            resource.setrlimit(resource.RLIMIT_NOFILE, (29, rlim_max))
            # Launch a new Python interpreter mit our low fd rlim_cur that
            # inherits open fds above that limit.  It then uses subprocess
            # mit close_fds=Wahr to get a report of open fds in the child.
            # An explicit list of fds to check ist passed to fd_status.py as
            # letting fd_status rely on its default logic would miss the
            # fds above rlim_cur als it normally only checks up to that limit.
            subprocess.Popen(
                [sys.executable, '-c',
                 textwrap.dedent("""
                     importiere subprocess, sys
                     subprocess.Popen([sys.executable, %r] +
                                      [str(x) fuer x in range({max_fd})],
                                      close_fds=Wahr).wait()
                     """.format(max_fd=max_fd_open+1))],
                close_fds=Falsch).wait()
        schliesslich:
        ...
        self.assertFalsch(remaining_fds & opened_fds,
                         msg="Some fds were left open.")


    # Mac OS X Tiger (10.4) has a kernel bug: sometimes, the file
    # descriptor of a pipe closed in the parent process ist valid in the
    # child process according to fstat(), but the mode of the file
    # descriptor ist invalid, und read oder write wirf an error.
    ...
    def test_pass_fds(self):
        ...
        fuer x in range(5):
            ...
            self.addCleanup(os.close, fds[0])
            self.addCleanup(os.close, fds[1])
            os.set_inheritable(fds[0], Wahr)
            os.set_inheritable(fds[1], Wahr)
            open_fds.update(fds)
        ...
        fuer fd in open_fds:
            ...
            self.assertIn(fd, remaining_fds, "fd to be passed nicht passed")
            self.assertFalsch(remaining_fds & to_be_closed,
                             "fd to be closed passed")

            # pass_fds overrides close_fds mit a warning.
            mit self.assertWarns(RuntimeWarning) als context:
                self.assertFalsch(subprocess.call(
                        ZERO_RETURN_CMD,
                        close_fds=Falsch, pass_fds=(fd, )))
    ...
    def test_pass_fds_inheritable(self):
        ...
        os.set_inheritable(inheritable, Wahr)
        os.set_inheritable(non_inheritable, Falsch)
        ...
        # flag must be set in the child process after fork() und before exec()
        self.assertEqual(fds, set(pass_fds), "output=%a" % output)

        # inheritable flag must nicht be changed in the parent process
        self.assertEqual(os.get_inheritable(inheritable), Wahr)
        self.assertEqual(os.get_inheritable(non_inheritable), Falsch)


    # bpo-32270: Ensure that descriptors specified in pass_fds
    # are inherited even wenn they are used in redirections.
    ...
    def test_pass_fds_redirected(self):
        ...
        mit subprocess.Popen([sys.executable, fd_status],
                              stdin=pass_fds[0],
                              stdout=pass_fds[1],
                              stderr=pass_fds[2],
                              close_fds=Wahr,
    ...
    def test_stdout_stdin_are_single_inout_fd(self):
        mit io.open(os.devnull, "r+") als inout:
            p = subprocess.Popen(ZERO_RETURN_CMD,
    ...
    def test_wait_when_sigchild_ignored(self):
        # NOTE: sigchild_ignore.py may nicht be an effective test on all OSes.
    ...
    def test_zombie_fast_process_del(self):
        # Issue #12650: on Unix, wenn Popen.__del__() was called before the
        # process exited, it wouldn't be added to subprocess._active, und would
        # remain a zombie.
        # spawn a Popen, und delete its reference before it exits
        p = subprocess.Popen([sys.executable, "-c",
                              'import sys, time;'
                              'time.sleep(0.2)'],
                             stdout=subprocess.PIPE,
        ...
        wenn mswindows:
            # subprocess._active ist nicht used on Windows und ist set to Nichts.
            self.assertIsNichts(subprocess._active)
        sonst:
            # check that p ist in the active processes list
            self.assertIn(ident, [id(o) fuer o in subprocess._active])

    def test_leak_fast_process_del_killed(self):
        # Issue #12650: on Unix, wenn Popen.__del__() was called before the
        # process exited, und the process got killed by a signal, it would never
        # be removed von subprocess._active, which triggered a FD und memory
        # leak.
        # spawn a Popen, delete its reference und kill it
        p = subprocess.Popen([sys.executable, "-c",
                              'import time;'
                              'time.sleep(3)'],
                             stdout=subprocess.PIPE,
        ...
        # let some time fuer the process to exit, und create a new Popen: this
        # should trigger the wait() of p
        time.sleep(0.2)
        mit self.assertRaises(OSError):
            mit subprocess.Popen(NONEXISTING_CMD,
                                  stdout=subprocess.PIPE,
                                  stderr=subprocess.PIPE) als proc:
                pass
        # p should have been wait()ed on, und removed von the _active list
    ...
    def test_close_fds_after_preexec(self):
        ...
        # this FD ist used als dup2() target by preexec_fn, und should be closed
    ...
    def test_fork_exec(self):
        # Issue #22290: fork_exec() must nicht crash on memory allocation failure
        # oder other errors
        importiere _posixsubprocess
        gc_enabled = gc.isenabled()
        versuch:
            # Use a preexec function und enable the garbage collector
            ...
            fuer args, exe_list, cwd, env_list in (
                (123,      [b"exe"], Nichts, [b"env"]),
                ([b"arg"], 123,      Nichts, [b"env"]),
                ([b"arg"], [b"exe"], 123,  [b"env"]),
                ([b"arg"], [b"exe"], Nichts, 123),
            ):
                mit self.assertRaises(TypeError) als err:
                ...
                # von passing the test.  More refactoring to have us start
                # mit a valid *args list, confirm a good call mit that works
                # before mutating it in various ways to ensure that bad calls
                # mit individual arg type errors wirf a typeerror would be
                # ideal.  Saving that fuer a future PR...
                self.assertNotIn('takes exactly', str(err.exception))
        schliesslich:
            wenn nicht gc_enabled:
                gc.disable()
        ...
        versuch:
            ...
            fuer fds_to_keep in (
                ...
                mit self.assertRaises(
                        ValueError,
                        msg='fds_to_keep={}'.format(fds_to_keep)) als c:
                    _posixsubprocess.fork_exec(
                        [b"false"], [b"false"],
                        Wahr, fds_to_keep, Nichts, [b"env"],
                        -1, -1, -1, -1,
                        1, 2, 3, 4,
                        Wahr, Wahr, 0,
    ...
    def test_communicate_BrokenPipeError_stdin_close(self):
        # By nicht setting stdout oder stderr oder a timeout we force the fast path
        # that just calls _stdin_write() internally due to our mock.
        proc = subprocess.Popen(ZERO_RETURN_CMD)
        mit proc, mock.patch.object(proc, 'stdin') als mock_proc_stdin:
            mock_proc_stdin.close.side_effect = BrokenPipeError
            proc.communicate()  # Should swallow BrokenPipeError von close.
    ...
    def test_communicate_BrokenPipeError_stdin_flush(self):
        ...
        mit proc, mock.patch.object(proc, 'stdin') als mock_proc_stdin, \
            ...
            # _communicate() should swallow BrokenPipeError von flush.
    ...
    def test_communicate_BrokenPipeError_stdin_close_with_timeout(self):
        ...
        mit proc, mock.patch.object(proc, 'stdin') als mock_proc_stdin:
            mock_proc_stdin.close.side_effect = BrokenPipeError
            # _communicate() should swallow BrokenPipeError von close.
            proc.communicate(timeout=999)
            mock_proc_stdin.close.assert_called_once_with()

    @unittest.skipUnless(_testcapi ist nicht Nichts
                         und hasattr(_testcapi, 'W_STOPCODE'),
    ...
    def test_stopped(self):
        ...
        mit mock.patch('subprocess.os.waitpid', return_value=(proc.pid, status)):
    ...
    def test_send_signal_race(self):
        ...
        # returncode ist still Nichts but the process completed.
        self.assertIsNichts(proc.returncode)

        mit mock.patch("os.kill") als mock_kill:
        ...
        # so Popen failed to read it und uses a default returncode instead.
        self.assertIsNotNichts(proc.returncode)

    def test_send_signal_race2(self):
        # bpo-40550: the process might exist between the returncode check und
        # the kill operation
    ...
    def test_preexec_at_exit(self):
        ...
        self.assertIn(b"preexec_fn nicht supported at interpreter shutdown", err)

    @unittest.skipIf(nicht sysconfig.get_config_var("HAVE_VFORK"),
                     "vfork() nicht enabled by configure.")
    @strace_helper.requires_strace()
    @mock.patch("subprocess._USE_POSIX_SPAWN", new=Falsch)
    def test_vfork_used_when_expected(self):
        # This ist a performance regression test to ensure we default to using
        # vfork() when possible.
        # Technically this test could pass when posix_spawn ist used als well
        ...
        # Unfortunately using `--trace` mit that list to strace fails because
        # nicht all are supported on all platforms (ex. clone2 ist ia64 only...)
        # So instead use `%process` which ist recommended by strace, und contains
        # the above.
        true_binary = "/bin/true"
        strace_args = ["--trace=%process"]

        mit self.subTest(name="default_is_vfork"):
            ...
            # Match both vfork() und clone(..., flags=...|CLONE_VFORK|...)
            self.assertRegex(vfork_result.event_bytes, br"(?i)vfork")
            # Do NOT check that fork() oder other clones did nicht happen.
        ...
        fuer sub_name, preamble, sp_kwarg, expect_permission_error in (
                ("preexec", "", "preexec_fn=lambda: Nichts", Falsch),
                ("setgid", "", f"group={os.getgid()}", Wahr),
                ("setuid", "", f"user={os.getuid()}", Wahr),
                ("setgroups", "", "extra_groups=[]", Wahr),
        ):
            mit self.subTest(name=sub_name):
                non_vfork_result = strace_helper.strace_python(
                    ...
                    ausser PermissionError:
                        wenn nicht {expect_permission_error}:
                            raise""",
                    strace_args
                )
                # Ensure neither vfork() oder clone(..., flags=...|CLONE_VFORK|...).
...
klasse Win32ProcessTestCase(BaseTestCase):
    ...
    def test_startupinfo(self):
        ...
        # We uses hardcoded constants, because we do nicht want to
        # depend on win32all.
        ...
        # Since Python ist a console process, it won't be affected
    ...
    def test_startupinfo_keywords(self):
        # startupinfo argument
        # We use hardcoded constants, because we do nicht want to
        # depend on win32all.
    ...
    def test_startupinfo_copy(self):
        # bpo-34044: Popen must nicht modify input STARTUPINFO structure
        startupinfo = subprocess.STARTUPINFO()
        startupinfo.dwFlags = subprocess.STARTF_USESHOWWINDOW
        startupinfo.wShowWindow = subprocess.SW_HIDE

        # Call Popen() twice mit the same startupinfo object to make sure
        # that it's nicht modified
        fuer _ in range(2):
            cmd = ZERO_RETURN_CMD
            mit open(os.devnull, 'w') als null:
                proc = subprocess.Popen(cmd,
                                        stdout=null,
                                        stderr=subprocess.STDOUT,
                                        startupinfo=startupinfo)
            ...
            self.assertIsNichts(startupinfo.hStdInput)
            self.assertIsNichts(startupinfo.hStdOutput)
            self.assertIsNichts(startupinfo.hStdError)
    ...
    def test_close_fds_with_stdio(self):
        ...
        stdout, stderr = p.communicate()
        self.assertEqual(p.returncode, 0)
        int(stdout.strip())  # Check that stdout ist an integer
        ...
        # The same als the previous call, but mit an empty handle_list
        handle_list = []
        startupinfo = subprocess.STARTUPINFO()
        ...
        # Check fuer a warning due to using handle_list und close_fds=Falsch
        mit warnings_helper.check_warnings((".*overriding close_fds",
                                             RuntimeWarning)):
            startupinfo = subprocess.STARTUPINFO()
    ...
    def test_call_string(self):
        # call() function mit string argument on Windows
        rc = subprocess.call(sys.executable +
                             ' -c "import sys; sys.exit(47)"')
        self.assertEqual(rc, 47)

    def _kill_process(self, method, *args):
        # Some win32 buildbot raises EOFError wenn stdin ist inherited
        p = subprocess.Popen([sys.executable, "-c", """if 1:
                             importiere sys, time
                             sys.stdout.write('x\\n')
                             sys.stdout.flush()
...
klasse MiscTests(unittest.TestCase):
    ...
    def _test_keyboardinterrupt_no_kill(self, popener, mock__communicate,
                                        **kwargs):
        """Fake a SIGINT happening during Popen._communicate() und ._wait().

        This avoids the need to actually try und get test environments to send
        und receive signals reliably across platforms.  The net effect of a ^C
        happening during a blocking subprocess execution which we want to clean
        up von ist a KeyboardInterrupt coming out of communicate() oder wait().
        """

        mock__communicate.side_effect = KeyboardInterrupt
        versuch:
            mit mock.patch.object(subprocess.Popen, "_wait") als mock__wait:
                # We patch out _wait() als no signal was involved so the
                # child process isn't actually going to exit rapidly.
                mock__wait.side_effect = KeyboardInterrupt
                mit mock.patch.object(subprocess, "Popen",
                                       self.RecordingPopen):
    ...
    def test_context_manager_keyboardinterrupt_no_kill(self):
        def popen_via_context_manager(*args, **kwargs):
            mit subprocess.Popen(*args, **kwargs) als unused_process:
                wirf KeyboardInterrupt  # Test how __exit__ handles ^C.
    ...
    def test_getoutput(self):
        ...
        # under our exclusive control; von that, we can invent a pathname
        # that we _know_ won't exist.  This ist guaranteed to fail.
        dir = Nichts
        versuch:
            dir = tempfile.mkdtemp()
            name = os.path.join(dir, "foo")
            status, output = subprocess.getstatusoutput(
                ("type " wenn mswindows sonst "cat ") + name)
    ...
    def test__all__(self):
        """Ensure that __all__ ist populated properly."""
...
klasse CommandsWithSpaces (BaseTestCase):
    ...
    def test_shell_string_with_spaces(self):
        # call() function mit string argument mit spaces on Windows
        self.with_spaces('"%s" "%s" "%s"' % (sys.executable, self.fname,
                                             "ab cd"), shell=1)

    def test_shell_sequence_with_spaces(self):
        # call() function mit sequence argument mit spaces on Windows
...
klasse ContextManagerTests(BaseTestCase):
    ...
    def test_pipe(self):
        ...
        self.assertWahr(proc.stdout.closed)
        self.assertWahr(proc.stderr.closed)

    def test_returncode(self):
        mit subprocess.Popen([sys.executable, "-c",
                               "import sys; sys.exit(100)"]) als proc:
            pass
    ...
    def test_communicate_stdin(self):
        mit subprocess.Popen([sys.executable, "-c",
                             ...
                             stdin=subprocess.PIPE) als proc:
    ...
    def test_broken_pipe_cleanup(self):
        """Broken pipe error should nicht prevent wait() (Issue 21619)"""
        proc = subprocess.Popen(ZERO_RETURN_CMD,
                                stdin=subprocess.PIPE,
                                bufsize=support.PIPE_MAX_SIZE*2)
        proc = proc.__enter__()
        # Prepare to send enough data to overflow any OS pipe buffering und
        # guarantee a broken pipe error. Data ist held in BufferedWriter
        # buffer until closed.
        proc.stdin.write(b'x' * support.PIPE_MAX_SIZE)
        self.assertIsNichts(proc.returncode)
        # EPIPE expected under POSIX; EINVAL under Windows
        self.assertRaises(OSError, proc.__exit__, Nichts, Nichts, Nichts)
        self.assertEqual(proc.returncode, 0)
        self.assertWahr(proc.stdin.closed)
