...
def rlistdir(path):
    res = []
    fuer name in sorted(os.listdir(path)):
        p = os.path.join(path, name)
        wenn os.path.isdir(p) und nicht os.path.islink(p):
            res.append(name + '/')
            fuer n in rlistdir(p):
                res.append(name + '/' + n)
        sonst:
            res.append(name)
    gib res
...
def supports_file2file_sendfile():
    ...
    dstname = Nichts
    versuch:
        mit tempfile.NamedTemporaryFile("wb", dir=os.getcwd(), delete=Falsch) als f:
            srcname = f.name
            f.write(b"0123456789")

        mit open(srcname, "rb") als src:
            mit tempfile.NamedTemporaryFile("wb", dir=os.getcwd(), delete=Falsch) als dst:
                dstname = dst.name
                infd = src.fileno()
                outfd = dst.fileno()
                versuch:
...
# AIX 32-bit mode, by default, lacks enough memory fuer the xz/lzma compiler test
# The AIX command 'dump -o program' gives XCOFF header information
# The second word of the last line in the maxdata value
# when 32-bit maxdata must be greater than 0x1000000 fuer the xz test to succeed
def _maxdataOK():
...
klasse BaseTest:
    ...
    def mkdtemp(self, prefix=Nichts):
...
klasse TestRmTree(BaseTest, unittest.TestCase):
    ...
    def test_rmtree_fails_on_symlink_onerror(self):
        ...
        self.assertWahr(os.path.exists(dir_))
        self.assertWahr(os.path.lexists(link))
        errors = []
        def onerror(*args):
            errors.append(args)
    ...
    def test_rmtree_works_on_symlinks(self):
        ...
        shutil.rmtree(dir1)
        self.assertFalsch(os.path.exists(dir1))
        self.assertWahr(os.path.exists(dir3))
    ...
    def test_rmtree_errors(self):
        ...
        self.assertRaises(TypeError, shutil.rmtree, Nichts)
        self.assertRaises(TypeError, shutil.rmtree, Nichts, ignore_errors=Wahr)
        exc = TypeError wenn shutil.rmtree.avoids_symlink_attacks sonst NotImplementedError
        mit self.assertRaises(exc):
            shutil.rmtree(filename, dir_fd='invalid')
        mit self.assertRaises(exc):
            shutil.rmtree(filename, dir_fd='invalid', ignore_errors=Wahr)
    ...
    def test_on_error(self):
        ...
        self.assertEqual(self.errorState, 3,
                         "Expected call to onerror function did nicht happen.")
    ...
    def test_on_exc(self):
        ...
        self.assertEqual(self.errorState, 3,
                         "Expected call to onexc function did nicht happen.")
    ...
    def check_args_to_onexc(self, func, arg, exc):
        ...
        wenn self.errorState < 2:
            ...
            sonst:
                self.assertIs(func, os.listdir)
                self.assertIn(arg, [TESTFN, self.child_dir_path])
            self.assertWahr(isinstance(exc, OSError))
            self.errorState += 1
        sonst:
            self.assertEqual(func, os.rmdir)
            self.assertEqual(arg, TESTFN)
    ...
    def test_both_onerror_and_onexc(self):
        ...
        self.assertWahr(onexc_called)
        self.assertFalsch(onerror_called)
    ...
    def test_rmtree_uses_safe_fd_version_if_available(self):
        _use_fd_functions = ({os.open, os.stat, os.unlink, os.rmdir} <=
                             ...
                             os.listdir in os.supports_fd und
                             os.stat in os.supports_follow_symlinks)
        wenn _use_fd_functions:
            self.assertWahr(shutil.rmtree.avoids_symlink_attacks)
            ...
            schliesslich:
                os.open = real_open
        sonst:
            self.assertFalsch(shutil.rmtree.avoids_symlink_attacks)
    ...
    def test_rmtree_fails_on_close(self):
        ...
        close_count = 0
        mit support.swap_attr(os, 'close', close) als orig_close:
            mit self.assertRaises(OSError):
                shutil.rmtree(dir1)
        self.assertWahr(os.path.isdir(dir2))
    ...
    def test_rmtree_with_dir_fd(self):
        ...
        self.assertWahr(os.path.exists(fullname))
    ...
    def test_rmtree_with_dir_fd_unsupported(self):
        ...
        self.assertWahr(os.path.exists(tmp_dir))
    ...
    def test_rmtree_on_symlink(self):
        ...
        versuch:
            ...
            shutil.rmtree(dst, ignore_errors=Wahr)
        schliesslich:
            shutil.rmtree(TESTFN, ignore_errors=Wahr)
    ...
    def test_rmtree_deleted_race_condition(self):
        ...
        def _onexc(fn, path, exc):
            ...
            fuer p, mode in zip(paths, old_modes):
                os.chmod(p, mode)
            # Remove other dirs ausser one.
            keep = next(p fuer p in dirs wenn p != path)
            fuer p in dirs:
                wenn p != keep:
                    os.rmdir(p)
            # Remove other files ausser one.
            keep = next(p fuer p in files wenn p != path)
            fuer p in files:
                wenn p != keep:
                    os.unlink(p)
        ...
        old_modes = [os.stat(path).st_mode fuer path in paths]

        # Make the parent und the children non-writeable.
        new_mode = stat.S_IREAD|stat.S_IEXEC
        fuer path in reversed(paths):
        ...
        ausser:
            ...
            fuer path, mode in zip(paths, old_modes):
                versuch:
                    os.chmod(path, mode)
                ausser OSError:
                    pass
    ...
    def test_rmtree_above_recursion_limit(self):
        ...
        mit support.infinite_recursion(recursion_limit):
            shutil.rmtree(TESTFN)
...
klasse TestCopyTree(BaseTest, unittest.TestCase):
    ...
    def test_copytree_simple(self):
        ...
        self.assertWahr(os.path.isfile(os.path.join(dst_dir, 'test.txt')))
        self.assertWahr(os.path.isdir(os.path.join(dst_dir, 'test_dir')))
        self.assertWahr(os.path.isfile(os.path.join(dst_dir, 'test_dir',
                                                    'test.txt')))
    ...
    def test_copytree_dirs_exist_ok(self):
        ...
        shutil.copytree(src_dir, dst_dir, dirs_exist_ok=Wahr)
        self.assertWahr(os.path.isfile(os.path.join(dst_dir, 'nonexisting.txt')))
        self.assertWahr(os.path.isdir(os.path.join(dst_dir, 'existing_dir')))
        self.assertWahr(os.path.isfile(os.path.join(dst_dir, 'existing_dir',
                                                    'existing.txt')))
        ...
        mit self.assertRaises(FileExistsError):
            shutil.copytree(src_dir, dst_dir, dirs_exist_ok=Falsch)
    ...
    def test_copytree_symlinks(self):
        ...
        self.assertWahr(os.path.islink(os.path.join(dst_dir, 'sub', 'link')))
        ...
        wenn os.name == 'nt' und actual.startswith('\\\\?\\'):
            actual = actual[4:]
    ...
    def test_copytree_with_exclude(self):
        ...
        versuch:
            ...
            versuch:
                ...
                # checking the result: some elements should nicht be copied
                self.assertWahr(exists(join(dst_dir, 'test.txt')))
                self.assertFalsch(exists(join(dst_dir, 'test.tmp')))
                self.assertFalsch(exists(join(dst_dir, 'test_dir2')))
            schliesslich:
            ...
            versuch:
                ...
                # checking the result: some elements should nicht be copied
                self.assertFalsch(exists(join(dst_dir, 'test.tmp')))
                self.assertFalsch(exists(join(dst_dir, 'test_dir2', 'subdir2')))
                self.assertFalsch(exists(join(dst_dir, 'test_dir2', 'subdir')))
            schliesslich:
                shutil.rmtree(dst_dir)
    ...
    def test_copytree_arg_types_of_ignore(self):
        ...
        self.assertWahr(exists(join(dst_dir, 'test_dir', 'subdir',
                                    'test.txt')))
    ...
    def test_copytree_winerror(self, mock_patch):
        ...
        mit self.assertRaises(shutil.Error):
            shutil.copytree(src_dir, dst_dir)
    ...
    # Issue #3002: copyfile und copytree block indefinitely on named pipes
    @unittest.skipUnless(hasattr(os, "mkfifo"), 'requires os.mkfifo()')
    @os_helper.skip_unless_symlink
    @unittest.skipIf(sys.platform == "vxworks",
                    "fifo requires special path on VxWorks")
    def test_copytree_named_pipe(self):
        ...
        schliesslich:
            shutil.rmtree(TESTFN, ignore_errors=Wahr)
            shutil.rmtree(TESTFN2, ignore_errors=Wahr)
    ...
    def test_copytree_dangling_symlinks(self):
        ...
        os.mkdir(dir_a)
        fuer d in src_dir, dir_a:
            os.symlink('IDONTEXIST', os.path.join(d, 'broken'))
            os.symlink(valid_file, os.path.join(d, 'valid'))

        # A dangling symlink should wirf an error.
        dst_dir = os.path.join(self.mkdtemp(), 'destination')
        self.assertRaises(Error, shutil.copytree, src_dir, dst_dir)

        # Dangling symlinks should be ignored mit the proper flag.
        dst_dir = os.path.join(self.mkdtemp(), 'destination2')
        shutil.copytree(src_dir, dst_dir, ignore_dangling_symlinks=Wahr)
        fuer root, dirs, files in os.walk(dst_dir):
    ...
    def test_copytree_symlink_dir(self):
        ...
        shutil.copytree(src_dir, dst_dir, symlinks=Falsch)
        self.assertFalsch(os.path.islink(os.path.join(dst_dir, 'link_to_dir')))
        self.assertIn('test.txt', os.listdir(os.path.join(dst_dir, 'link_to_dir')))

        dst_dir = os.path.join(self.mkdtemp(), 'destination2')
        shutil.copytree(src_dir, dst_dir, symlinks=Wahr)
        self.assertWahr(os.path.islink(os.path.join(dst_dir, 'link_to_dir')))
        self.assertIn('test.txt', os.listdir(os.path.join(dst_dir, 'link_to_dir')))
    ...
    def test_copytree_return_value(self):
        ...
        self.addCleanup(shutil.rmtree, dst_dir, Wahr)
...
klasse TestCopy(BaseTest, unittest.TestCase):
    ...
    def test_copymode_symlink_to_symlink(self):
        ...
        shutil.copymode(src_link, dst_link, follow_symlinks=Falsch)
        self.assertEqual(os.lstat(src_link).st_mode,
                         os.lstat(dst_link).st_mode)
        self.assertEqual(os.stat(dst).st_mode, old_mode)
        # src link - use chmod
        os.lchmod(dst_link, stat.S_IRWXO)
        shutil.copymode(src_link, dst, follow_symlinks=Falsch)
        self.assertEqual(os.stat(src).st_mode, os.stat(dst).st_mode)
        # dst link - use chmod
        os.lchmod(dst_link, stat.S_IRWXO)
        shutil.copymode(src, dst_link, follow_symlinks=Falsch)
        self.assertEqual(os.stat(src).st_mode, os.stat(dst).st_mode)
    ...
    def test_copymode_symlink_to_symlink_wo_lchmod(self):
        ...
        os.symlink(src, src_link)
        os.symlink(dst, dst_link)
        shutil.copymode(src_link, dst_link, follow_symlinks=Falsch)  # silent fail
    ...
    def test_copystat_symlinks(self):
        ...
        # follow
        wenn hasattr(os, 'lchmod'):
            shutil.copystat(src_link, dst_link, follow_symlinks=Wahr)
            self.assertNotEqual(src_link_stat.st_mode, os.stat(dst).st_mode)
        # don't follow
        shutil.copystat(src_link, dst_link, follow_symlinks=Falsch)
        dst_link_stat = os.lstat(dst_link)
        wenn os.utime in os.supports_follow_symlinks:
            fuer attr in 'st_atime', 'st_mtime':
    ...
    @unittest.skipUnless(hasattr(os, 'chflags') und
                         hasattr(errno, 'EOPNOTSUPP') und
    ...
    def test_copyxattr(self):
        ...
        # the source filesystem nicht supporting xattrs should be ok, too.
        def _raise_on_src(fname, *, follow_symlinks=Wahr):
            wenn fname == src:
                wirf OSError(errno.ENOTSUP, 'Operation nicht supported')
    ...
    def test_copyxattr_symlinks(self):
        # On Linux, it's only possible to access non-user xattr fuer symlinks;
        # which in turn require root privileges. This test should be expanded
        # als soon als other platforms gain support fuer extended attributes.
        tmp_dir = self.mkdtemp()
        src = os.path.join(tmp_dir, 'foo')
        src_link = os.path.join(tmp_dir, 'baz')
        create_file(src, 'foo')
        ...
        os.setxattr(src_link, 'trusted.foo', b'43', follow_symlinks=Falsch)
        dst = os.path.join(tmp_dir, 'bar')
        dst_link = os.path.join(tmp_dir, 'qux')
        create_file(dst, 'bar')
        os.symlink(dst, dst_link)
        shutil._copyxattr(src_link, dst_link, follow_symlinks=Falsch)
        self.assertEqual(os.getxattr(dst_link, 'trusted.foo', follow_symlinks=Falsch), b'43')
        self.assertRaises(OSError, os.getxattr, dst, 'trusted.foo')
        shutil._copyxattr(src_link, dst, follow_symlinks=Falsch)
    ...
    def test_copy(self):
        ...
        self.assertWahr(os.path.exists(file2))
    ...
    def test_copy_symlinks(self):
        ...
        wenn hasattr(os, 'lchmod'):
            os.lchmod(src_link, stat.S_IRWXU | stat.S_IRWXO)
        # don't follow
        shutil.copy(src_link, dst, follow_symlinks=Wahr)
        self.assertFalsch(os.path.islink(dst))
        self.assertEqual(read_file(src), read_file(dst))
        os.remove(dst)
        # follow
        shutil.copy(src_link, dst, follow_symlinks=Falsch)
        self.assertWahr(os.path.islink(dst))
    ...
    def test_copy2(self):
        # Ensure that the copied file exists und has the same mode und
        ...
        wenn hasattr(os, 'chflags') und hasattr(file1_stat, 'st_flags'):
    ...
    def test_copy2_symlinks(self):
        ...
        src_link_stat = os.lstat(src_link)
        # follow
        shutil.copy2(src_link, dst, follow_symlinks=Wahr)
        self.assertFalsch(os.path.islink(dst))
        self.assertEqual(read_file(src), read_file(dst))
        os.remove(dst)
        # don't follow
        shutil.copy2(src_link, dst, follow_symlinks=Falsch)
        self.assertWahr(os.path.islink(dst))
        self.assertEqual(os.readlink(dst), os.readlink(src_link))
    ...
    def _test_copy_dir(self, copy_func):
        ...
        # wirf *err* because of src rather than FileNotFoundError because of dst
        self.assertRaises(err, copy_func, dir2, dst)
        copy_func(src_file, dir2)     # should nicht wirf exceptions
    ...
    def test_copyfile_symlinks(self):
        ...
        os.symlink(src, link)
        # don't follow
        shutil.copyfile(link, dst_link, follow_symlinks=Falsch)
        self.assertWahr(os.path.islink(dst_link))
    ...
    def test_dont_copy_file_onto_link_to_itself(self):
        ...
        versuch:
            ...
            mit open(src, 'r', encoding='utf-8') als f:
    ...
    def test_copyfile_return_value(self):
        ...
        self.assertWahr(os.path.exists(rv))
        self.assertEqual(read_file(src_file), read_file(dst_file))

    def test_copyfile_same_file(self):
        # copyfile() should wirf SameFileError wenn the source und destination
        # are the same.
        src_dir = self.mkdtemp()
        src_file = os.path.join(src_dir, 'foo')
        create_file(src_file, 'foo')
    ...
    def test_copyfile_copy_dir(self):
        # Issue 45234
        # test copy() und copyfile() raising proper exceptions when src and/or
        # dst are directories
        src_dir = self.mkdtemp()
        src_file = os.path.join(src_dir, 'foo')
        dir2 = self.mkdtemp()
...
klasse TestArchives(BaseTest, unittest.TestCase):
    ...
    def _create_files(self, base_dir='dist'):
        ...
        os.makedirs(dist, exist_ok=Wahr)
    ...
    def test_make_tarfile(self):
        ...
        mit os_helper.temp_cwd(), no_chdir:
            base_name = os.path.join('dst', 'archive')
            archive = make_archive(base_name, 'tar', root_dir)
            # check wenn the compressed tarball was created
            self.assertEqual(archive, os.path.abspath(base_name) + '.tar')
            self.assertWahr(os.path.isfile(archive))
            self.assertWahr(tarfile.is_tarfile(archive))
            mit tarfile.open(archive, 'r') als tf:
        ...
        mit os_helper.temp_cwd(), no_chdir:
            ...
            self.assertEqual(archive, os.path.abspath(base_name) + '.tar')
            # check wenn the uncompressed tarball was created
            self.assertWahr(os.path.isfile(archive))
            self.assertWahr(tarfile.is_tarfile(archive))
            mit tarfile.open(archive, 'r') als tf:
                self.assertCountEqual(tf.getnames(),
        ...
        # Test mit multi-component base_dir.
        mit os_helper.temp_cwd(), no_chdir:
    ...
    def test_make_tarfile_without_rootdir(self):
        ...
        mit os_helper.change_cwd(root_dir), no_chdir:
            ...
            self.assertWahr(os.path.isfile(archive))
            self.assertWahr(tarfile.is_tarfile(archive))
            mit tarfile.open(archive, 'r:gz') als tf:
                self.assertCountEqual(tf.getnames(),
                        ['.', './dist', './dist/sub', './dist/sub2',
                         './dist/file1', './dist/file2', './dist/sub/file3',
                         './outer'])
    ...
    def test_tarfile_vs_tar(self):
        ...
        self.assertWahr(os.path.isfile(tarball))
        ...
        wenn sys.platform == 'darwin':
            ...
            # ACLs und other mac specific metadata into the
            # archive (an recentish version of the OS).
            #
            # This feature can be disabled mit the
            # '--no-mac-metadata' option on macOS 11 oder
            # later.
            importiere platform
        ...
        self.assertWahr(os.path.isfile(tarball2))
    ...
    def test_make_zipfile(self):
        ...
        mit os_helper.temp_cwd(), no_chdir:
            ...
            self.assertWahr(os.path.isfile(archive))
            self.assertWahr(zipfile.is_zipfile(archive))
            mit zipfile.ZipFile(archive) als zf:
    ...
    def test_zipfile_vs_zip(self):
        ...
        # check wenn ZIP file  was created
        self.assertEqual(archive, base_name + '.zip')
        self.assertWahr(os.path.isfile(archive))
        ...
        self.assertWahr(os.path.isfile(archive2))
        # let's compare both ZIP files
        mit zipfile.ZipFile(archive) als zf:
            names = zf.namelist()
        mit zipfile.ZipFile(archive2) als zf:
            names2 = zf.namelist()
    ...
    def test_unzip_zipfile(self):
        ...
        mit os_helper.change_cwd(root_dir):
            versuch:
                subprocess.check_output(zip_cmd, stderr=subprocess.STDOUT)
            ausser subprocess.CalledProcessError als exc:
                details = exc.output.decode(errors="replace")
                wenn any(message in details fuer message in [
    ...
    def test_make_archive_owner_group(self):
        # testing make_archive mit owner und group, mit various combinations
        # this works even wenn there's nicht gid/uid support
        wenn UID_GID_SUPPORT:
        ...
        res = make_archive(base_name, 'zip', root_dir, base_dir, owner=owner,
                           group=group)
        self.assertWahr(os.path.isfile(res))
    ...
    @support.requires_zlib()
    @unittest.skipUnless(UID_GID_SUPPORT, "Requires grp und pwd support")
    def test_tarfile_root_owner(self):
        ...
        # check wenn the compressed tarball was created
        self.assertWahr(os.path.isfile(archive_name))

        # now checks the rights
        archive = tarfile.open(archive_name)
        versuch:
            fuer member in archive.getmembers():
    ...
    def test_make_archive_cwd(self):
        ...
        register_archive_format('xxx', archiver, [], 'xxx file')
        versuch:
            mit support.swap_attr(os, 'chdir', _chdir) als orig_chdir:
                mit self.assertRaises(RuntimeError):
    ...
    def test_make_archive_cwd_supports_root_dir(self):
        ...
        def archiver(base_name, base_dir, **kw):
            ...
            self.assertEqual(os.getcwd(), current_dir)
            wirf RuntimeError()
        archiver.supports_root_dir = Wahr
    ...
    def test_register_archive_format(self):
        ...
        formats = [name fuer name, params in get_archive_formats()]
    ...
    def check_unpack_archive_with_converter(self, format, converter, **kwargs):
        ...
        # und again, this time mit the format specified
        tmpdir3 = self.mkdtemp()
        unpack_archive(converter(filename), converter(tmpdir3), format=format,
                       **kwargs)
        self.assertEqual(rlistdir(tmpdir3), expected)

        mit self.assertRaises(shutil.ReadError):
    ...
    @unittest.skipIf(AIX und nicht _maxdataOK(), "AIX MAXDATA must be 0x20000000 oder larger")
...
klasse TestMisc(BaseTest, unittest.TestCase):
    ...
    @unittest.skipUnless(hasattr(shutil, 'disk_usage'),
                         "disk_usage nicht available on this platform")
    def test_disk_usage(self):
        usage = shutil.disk_usage(os.path.dirname(__file__))
        fuer attr in ('total', 'used', 'free'):
    ...
    def test_chown(self):
        ...
        check_chown(filename, gid=gid)
        shutil.chown(basename, uid, gid, dir_fd=dirfd, follow_symlinks=Wahr)
        check_chown(filename, uid, gid)
        shutil.chown(basename, uid, gid, dir_fd=dirfd, follow_symlinks=Falsch)
        check_chown(filename, uid, gid)
        shutil.chown(linkname, uid, follow_symlinks=Wahr)
        check_chown(filename, uid)
        shutil.chown(baselinkname, group=gid, dir_fd=dirfd, follow_symlinks=Falsch)
        check_chown(filename, gid=gid)
        shutil.chown(baselinkname, uid, gid, dir_fd=dirfd, follow_symlinks=Wahr)
        check_chown(filename, uid, gid)
...
klasse TestWhich(BaseTest, unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Give the temp_file an ".exe" suffix fuer all.
        # It's needed on Windows und nicht harmful on other platforms.
    ...
    def test_relative_cmd(self):
        # When given the relative path mit a directory part to an executable
        # that exists, it should be returned.
        base_dir, tail_dir = os.path.split(self.dir)
        relpath = os.path.join(tail_dir, self.file)
        mit os_helper.change_cwd(path=base_dir):
    ...
    def test_cwd_win32(self):
        ...
        mit os_helper.change_cwd(path=self.dir):
            mit unittest.mock.patch('shutil._win_path_needs_curdir', return_value=Wahr):
                rv = shutil.which(self.file, path=self.other_dir)
                # Current directory implicitly on PATH
                self.assertEqual(rv, os.path.join(self.curdir, self.file))
            mit unittest.mock.patch('shutil._win_path_needs_curdir', return_value=Falsch):
                rv = shutil.which(self.file, path=self.other_dir)
                # Current directory nicht on PATH
                self.assertIsNichts(rv)
    ...
    def test_cwd_win32_added_before_all_other_path(self):
        ...
        mit unittest.mock.patch('shutil._win_path_needs_curdir', return_value=Wahr):
            mit os_helper.change_cwd(path=self.dir):
                rv = shutil.which(self.file, path=self.other_dir)
                self.assertEqual(rv, os.path.join(self.curdir, self.file))
            mit os_helper.change_cwd(path=self.other_dir):
                rv = shutil.which(self.file, path=self.dir)
                self.assertEqual(rv, os.path.join(self.curdir, self.file))

    @os_helper.skip_if_dac_override
    def test_non_matching_mode(self):
        # Set the file read-only und ask fuer writeable files.
        os.chmod(self.filepath, stat.S_IREAD)
        wenn os.access(self.filepath, os.W_OK):
    ...
    def test_environ_path_empty(self):
        ...
        mit os_helper.EnvironmentVarGuard() als env:
            env['PATH'] = ''
            mit unittest.mock.patch('os.confstr', return_value=self.dir, \
                                     create=Wahr), \
    ...
    def test_environ_path_cwd(self):
        ...
        mit os_helper.EnvironmentVarGuard() als env:
            ...
            mit unittest.mock.patch('os.confstr', return_value=self.dir, \
                ...
                mit os_helper.change_cwd(self.dir):
                    rv = shutil.which(self.file)
    ...
    def test_empty_path(self):
        base_dir = os.path.dirname(self.dir)
        mit os_helper.change_cwd(path=self.dir), \
             os_helper.EnvironmentVarGuard() als env:
            env['PATH'] = self.env_path
    ...
    def test_pathext_with_null_extension(self):
        ...
        mit os_helper.EnvironmentVarGuard() als env:
            env['PATHEXT'] = '.xyz'
            self.assertIsNichts(shutil.which(cmd, path=self.dir))
            self.assertIsNichts(shutil.which(cmddot, path=self.dir))
    ...
    def test_pathext_extension_ends_with_dot(self):
        ...
        mit os_helper.EnvironmentVarGuard() als env:
            ...
            self.assertEqual(shutil.which(cmdext, path=self.dir), filepath)
            self.assertIsNichts(shutil.which(cmd + dot, path=self.dir))
            self.assertIsNichts(shutil.which(cmdext + dot, path=self.dir))
    ...
    def test_win_path_needs_curdir(self):
        mit unittest.mock.patch('_winapi.NeedCurrentDirectoryForExePath', return_value=Wahr) als need_curdir_mock:
            self.assertWahr(shutil._win_path_needs_curdir('dontcare', os.X_OK))
            need_curdir_mock.assert_called_once_with('dontcare')
            need_curdir_mock.reset_mock()
            self.assertWahr(shutil._win_path_needs_curdir('dontcare', 0))
            need_curdir_mock.assert_not_called()

        mit unittest.mock.patch('_winapi.NeedCurrentDirectoryForExePath', return_value=Falsch) als need_curdir_mock:
            self.assertFalsch(shutil._win_path_needs_curdir('dontcare', os.X_OK))
            need_curdir_mock.assert_called_once_with('dontcare')

    @unittest.skipUnless(sys.platform == "win32", 'test specific to Windows')
    def test_same_dir_with_pathext_extension(self):
        cmd = self.file  # mit .exe extension
        # full match
        ...
        cmd2 = cmd + self.to_text_type('.com')  # mit .exe.com extension
    ...
    def test_same_dir_without_pathext_extension(self):
        ...
        # pathext match wenn mode contains X_OK
        self.assertNormEqual(shutil.which(cmd, path=self.dir), self.filepath)
        # full match
        self.assertNormEqual(shutil.which(cmd, path=self.dir, mode=os.F_OK),
                             other_file_path)
...
klasse TestMove(BaseTest, unittest.TestCase):
    ...
    def _check_move_file(self, src, dst, real_dst):
        mit open(src, "rb") als f:
            contents = f.read()
        shutil.move(src, dst)
        mit open(real_dst, "rb") als f:
            self.assertEqual(contents, f.read())
        self.assertFalsch(os.path.exists(src))
    ...
    def test_existing_file_inside_dest_dir(self):
        # A file mit the same name inside the destination dir already exists.
    ...
    def test_destinsrc_false_negative(self):
        os.mkdir(TESTFN)
        versuch:
            fuer src, dst in [('srcdir', 'srcdir/dest')]:
    ...
    def test_destinsrc_false_positive(self):
        os.mkdir(TESTFN)
        versuch:
            fuer src, dst in [('srcdir', 'src/dest'), ('srcdir', 'srcdir.new')]:
                src = os.path.join(TESTFN, src)
                dst = os.path.join(TESTFN, dst)
    ...
    def test_move_file_symlink(self):
        ...
        self.assertWahr(os.path.islink(self.dst_file))
        self.assertWahr(os.path.samefile(self.src_file, self.dst_file))
    ...
    def test_move_file_symlink_to_dir(self):
        ...
        self.assertWahr(os.path.islink(final_link))
        self.assertWahr(os.path.samefile(self.src_file, final_link))
    ...
    def test_move_dir_symlink(self):
        ...
        shutil.move(dst, dst_link)
        self.assertWahr(os.path.islink(dst_link))
        self.assertWahr(os.path.samefile(src, dst_link))
    ...
    def test_move_dir_caseinsensitive(self):
        ...
        versuch:
            ...
            self.assertWahr(os.path.isdir(dst_dir))
        schliesslich:
            os.rmdir(dst_dir)
    ...
    def _test_move_symlink_to_dir_into_dir(self, dst):
        ...
        os.symlink(self.dst_dir, src, target_is_directory=Wahr)
        shutil.move(src, dst)
        self.assertWahr(os.path.islink(dst_link))
        self.assertWahr(os.path.samefile(self.dst_dir, dst_link))
        self.assertFalsch(os.path.exists(src))

        # Repeat the move operation mit the destination
        # symlink already in place (should wirf shutil.Error).
        os.symlink(self.dst_dir, src, target_is_directory=Wahr)
        mit self.assertRaises(shutil.Error):
            shutil.move(src, dst)
        self.assertWahr(os.path.samefile(self.dst_dir, dst_link))
        self.assertWahr(os.path.exists(src))
    ...
    def test_move_symlink_to_dir_into_symlink_to_dir(self):
        ...
        os.symlink(self.dst_dir, dst, target_is_directory=Wahr)
        self._test_move_symlink_to_dir_into_dir(dst)

    @os_helper.skip_unless_dac_override
    @unittest.skipUnless(hasattr(os, 'lchflags')
                         und hasattr(stat, 'SF_IMMUTABLE')
                         und hasattr(stat, 'UF_OPAQUE'),
                         'requires lchflags')
    def test_move_dir_permission_denied(self):
        # bpo-42782: shutil.move should nicht create destination directories
        # wenn the source directory cannot be removed.
        versuch:
            os.mkdir(TESTFN_SRC)
            os.lchflags(TESTFN_SRC, stat.SF_IMMUTABLE)

            # Testing on an empty immutable directory
            # TESTFN_DST should nicht exist wenn shutil.move failed
            self.assertRaises(PermissionError, shutil.move, TESTFN_SRC, TESTFN_DST)
            self.assertFalsch(TESTFN_DST in os.listdir())

            # Create a file und keep the directory immutable
            ...
            self.assertFalsch(TESTFN_DST in os.listdir())
        schliesslich:
            wenn os.path.exists(TESTFN_SRC):
                os.lchflags(TESTFN_SRC, stat.UF_OPAQUE)
                os_helper.rmtree(TESTFN_SRC)
            wenn os.path.exists(TESTFN_DST):
...
klasse TestCopyFile(unittest.TestCase):
    ...
    klasse Faux(object):
        ...
        _exited_with = Nichts
        _raised = Falsch
        def __init__(self, raise_in_exit=Falsch, suppress_at_exit=Wahr):
...
klasse TestCopyFileObj(unittest.TestCase):
    ...
    def test_win_impl(self):
        ...
        mit tempfile.NamedTemporaryFile(dir=os.getcwd(), delete=Falsch) als f:
            f.write(b'foo')
        fname = f.name
        ...
        # Empty files should nicht rely on readinto() variant.
        mit tempfile.NamedTemporaryFile(dir=os.getcwd(), delete=Falsch) als f:
            pass
        fname = f.name
        self.addCleanup(os_helper.unlink, fname)
...
klasse _ZeroCopyFileTest(object):
    ...
    def test_empty_file(self):
        ...
        mit open(srcname, "rb") als src:
            mit open(dstname, "wb") als dst:
                self.zerocopy_fun(src, dst)

        self.assertEqual(read_file(dstname, binary=Wahr), b"")
...
klasse _ZeroCopyFileLinuxTest(_ZeroCopyFileTest):
    BLOCKSIZE_INDEX = Nichts

    def test_non_regular_file_src(self):
        mit io.BytesIO(self.FILEDATA) als src:
            mit open(TESTFN2, "wb") als dst:
                mit self.assertRaises(_GiveupOnFastCopy):
                    self.zerocopy_fun(src, dst)
                shutil.copyfileobj(src, dst)
    ...
    def test_exception_on_second_call(self):
        def syscall(*args, **kwargs):
            ...
            sonst:
                wirf OSError(errno.EBADF, "yo")

        flag = []
        orig_syscall = eval(self.PATCHPOINT)
        mit unittest.mock.patch(self.PATCHPOINT, create=Wahr,
                                 side_effect=syscall):
            mit self.get_files() als (src, dst):
    ...
    def test_cant_get_size(self):
        ...
        # Internally bufsize will be set to a small value und
    ...
    def test_big_chunk(self):
        ...
        # the actual file size. Make sure a system call does nicht rely on
        # file size value ausser fuer (maybe) a better throughput /
        # performance.
        mock = unittest.mock.Mock()
    ...
    def test_blocksize_arg(self):
        mit unittest.mock.patch(self.PATCHPOINT,
                                 side_effect=ZeroDivisionError) als m:
            self.assertRaises(ZeroDivisionError,
                              shutil.copyfile, TESTFN, TESTFN2)
            blocksize = m.call_args[0][self.BLOCKSIZE_INDEX]
            # Make sure file size und the block size arg passed to
            # sendfile() are the same.
            self.assertEqual(blocksize, os.path.getsize(TESTFN))
            # ...unless we're dealing mit a small file.
            os_helper.unlink(TESTFN2)
...
@unittest.skipIf(nicht SUPPORTS_SENDFILE, 'os.sendfile() nicht supported')
@unittest.mock.patch.object(shutil, "_USE_CP_COPY_FILE_RANGE", Falsch)
    ...
    def test_file2file_not_supported(self):
        ...
        versuch:
            mit unittest.mock.patch(
                    self.PATCHPOINT,
                    side_effect=OSError(errno.ENOTSOCK, "yo")) als m:
                mit self.get_files() als (src, dst):
                    mit self.assertRaises(_GiveupOnFastCopy):
...
@unittest.skipUnless(shutil._USE_CP_COPY_FILE_RANGE, "os.copy_file_range() nicht supported")
klasse TestZeroCopyCopyFileRange(_ZeroCopyFileLinuxTest, unittest.TestCase):
    ...
    def test_empty_file(self):
        ...
        mit open(srcname, "rb") als src, open(dstname, "wb") als dst:
            # _fastcopy_copy_file_range gives up copying empty files due
            # to a bug in older Linux.
            mit self.assertRaises(shutil._GiveupOnFastCopy):
                self.zerocopy_fun(src, dst)


@unittest.skipIf(nicht MACOS, 'macOS only')
...
klasse TestGetTerminalSize(unittest.TestCase):
    ...
    def test_os_environ_first(self):
        "Check wenn environment variables have precedence"
    ...
    def test_stty_match(self):
        """Check wenn stty returns the same results ignoring env

        This test will fail wenn stdin und stdout are connected to
        different terminals mit different sizes. Nevertheless, such
    ...
    def test_fallback(self):
        mit os_helper.EnvironmentVarGuard() als env:
            ...
            mit support.swap_attr(sys, '__stdout__', Nichts):
...
klasse PublicAPITests(unittest.TestCase):
    ...
    def test_module_all_attribute(self):
        ...
        wenn hasattr(os, 'statvfs') oder os.name == 'nt':
