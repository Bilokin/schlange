...
def is_code_page_present(cp):
    ...
    MAX_DEFAULTCHAR = 2 # single oder double byte
    MAX_PATH = 260
...
klasse MixInCheckStateHandling:
    ...
    def check_state_handling_encode(self, encoding, u, s):
        fuer i in range(len(u)+1):
            d = codecs.getincrementalencoder(encoding)()
            part1 = d.encode(u[:i])
            state = d.getstate()
            d = codecs.getincrementalencoder(encoding)()
            d.setstate(state)
            part2 = d.encode(u[i:], Wahr)
            self.assertEqual(s, part1+part2)


klasse ReadTest(MixInCheckStateHandling):
    def check_partial(self, input, partialresults):
        # get a StreamReader fuer the encoding und feed the bytestring version
        # of input to the reader byte by byte. Read everything available from
        # the StreamReader und check that the results equal the appropriate
        # entries von partialresults.
        q = Queue(b"")
        r = codecs.getreader(self.encoding)(q)
        result = ""
        fuer (c, partialresult) in zip(input.encode(self.encoding), partialresults, strict=Wahr):
            q.write(bytes([c]))
            result += r.read()
        ...
        fuer (c, partialresult) in zip(input.encode(self.encoding), partialresults, strict=Wahr):
            ...
            self.assertEqual(result, partialresult)
        # check that there's nothing left in the buffers
        self.assertEqual(d.decode(b"", Wahr), "")
        ...
        self.assertEqual(
            input,
            "".join(codecs.iterdecode([bytes([c]) fuer c in encoded], self.encoding))
        )
    ...
    def test_readline(self):
        ...
        self.assertEqual(readalllines(s, Wahr), sexpected)
        self.assertEqual(readalllines(s, Falsch), sexpectednoends)
        self.assertEqual(readalllines(s, Wahr, 10), sexpected)
        self.assertEqual(readalllines(s, Falsch, 10), sexpectednoends)

        lineends = ("\n", "\r\n", "\r", "\u2028")
        # Test long lines (multiple calls to read() in readline())
        vw = []
        vwo = []
        fuer (i, lineend) in enumerate(lineends):
            vw.append((i*200+200)*"\u3042" + lineend)
            vwo.append((i*200+200)*"\u3042")
        self.assertEqual(readalllines("".join(vw), Wahr), "|".join(vw))
        self.assertEqual(readalllines("".join(vw), Falsch), "|".join(vwo))

        # Test lines where the first read might end mit \r, so the
        # reader has to look ahead whether this ist a lone \r oder a \r\n
        fuer size in range(80):
            fuer lineend in lineends:
                s = 10*(size*"a" + lineend + "xxx\n")
                reader = getreader(s)
                fuer i in range(10):
                    self.assertEqual(
                        reader.readline(keepends=Wahr),
                        size*"a" + lineend,
                    )
                    self.assertEqual(
                        reader.readline(keepends=Wahr),
                ...
                fuer i in range(10):
                    self.assertEqual(
                        reader.readline(keepends=Falsch),
                        size*"a",
                    )
                    self.assertEqual(
                        reader.readline(keepends=Falsch),
    ...
    def test_mixed_readline_and_read(self):
        lines = ["Humpty Dumpty sat on a wall,\n",
                 ...
                 "All the king's horses und all the king's men\r",
    ...
    def test_bug1175396(self):
        s = [
            ...
            '    today\'s articles, oder articles of a specific date.\r\n',
            '========================================================--%>\r\n',
            ...
            '\r\n',
            '\r\n',
            'def readArticlesFromDate(date, count=Nichts):\r\n',
            ...
            '        entries=[]\r\n',
            '        SHOWAMOUNT=10\r\n',
            '        fuer showdate in dates:\r\n',
            '            entries.extend( readArticlesFromDate(showdate, SHOWAMOUNT-len(entries)) )\r\n',
            '            wenn len(entries)>=SHOWAMOUNT:\r\n',
            '                break\r\n',
            '                \r\n',
        ]
        stream = io.BytesIO("".join(s).encode(self.encoding))
        reader = codecs.getreader(self.encoding)(stream)
        fuer (i, line) in enumerate(reader):
    ...
    def test_readlinequeue(self):
        ...
        # No lineends
        writer.write("foo\r")
        self.assertEqual(reader.readline(keepends=Falsch), "foo")
        writer.write("\nbar\r")
        self.assertEqual(reader.readline(keepends=Falsch), "")
        self.assertEqual(reader.readline(keepends=Falsch), "bar")
        writer.write("baz")
        self.assertEqual(reader.readline(keepends=Falsch), "baz")
        self.assertEqual(reader.readline(keepends=Falsch), "")

        # Lineends
        writer.write("foo\r")
        self.assertEqual(reader.readline(keepends=Wahr), "foo\r")
        writer.write("\nbar\r")
        self.assertEqual(reader.readline(keepends=Wahr), "\n")
        self.assertEqual(reader.readline(keepends=Wahr), "bar\r")
        writer.write("baz")
        self.assertEqual(reader.readline(keepends=Wahr), "baz")
        self.assertEqual(reader.readline(keepends=Wahr), "")
        writer.write("foo\r\n")
        self.assertEqual(reader.readline(keepends=Wahr), "foo\r\n")
    ...
    def test_lone_surrogates(self):
        ...
        fuer before, after in [("\U00010fff", "A"), ("[", "]"),
                              ("A", "\U00010fff")]:
...
klasse UTF32Test(ReadTest, unittest.TestCase):
    ...
    def test_only_one_bom(self):
        ...
        f.write("spam")
        d = s.getvalue()
        # check whether there ist exactly one BOM in it
        self.assertWahr(d == self.spamle oder d == self.spambe)
        # try to read it back
        s = io.BytesIO(d)
        f = reader(s)
    ...
    def test_handlers(self):
        self.assertEqual(('\ufffd', 1),
                         codecs.utf_32_decode(b'\x01', 'replace', Wahr))
        self.assertEqual(('', 1),
                         codecs.utf_32_decode(b'\x01', 'ignore', Wahr))
...
klasse UTF16Test(ReadTest, unittest.TestCase):
    ...
    def test_handlers(self):
        self.assertEqual(('\ufffd', 1),
                         codecs.utf_16_decode(b'\x01', 'replace', Wahr))
        self.assertEqual(('', 1),
                         codecs.utf_16_decode(b'\x01', 'ignore', Wahr))
    ...
    def test_bug691291(self):
        # If encoding ist nicht Nichts, then
        # files are always opened in binary mode, even wenn no binary mode was
        # specified.  This means that no automatic conversion of '\n' ist done
        # on reading und writing.
        s1 = 'Hello\r\nworld\r\n'
        ...
        mit open(os_helper.TESTFN, 'wb') als fp:
            fp.write(s)
        mit codecs_open_no_warn(os_helper.TESTFN, 'r',
                         encoding=self.encoding) als reader:
    ...
    def test_invalid_modes(self):
        ...
        fuer mode in ('rt', 'wt', 'at', 'r+t'):
            mit self.assertRaises(ValueError) als cm:
                codecs_open_no_warn(os_helper.TESTFN, mode, encoding=self.encoding)
            self.assertIn("can't have text und binary mode at once",
                          str(cm.exception))
...
klasse UTF8Test(ReadTest, unittest.TestCase):
    ...
    def test_decode_error(self):
        fuer data, error_handler, expected in (
            (b'[\x80\xff]', 'ignore', '[]'),
            (b'[\x80\xff]', 'replace', '[\ufffd\ufffd]'),
            (b'[\x80\xff]', 'surrogateescape', '[\udc80\udcff]'),
            (b'[\x80\xff]', 'backslashreplace', '[\\x80\\xff]'),
        ):
            mit self.subTest(data=data, error_handler=error_handler,
                              expected=expected):
    ...
    def test_lone_surrogates(self):
        ...
        # nicht sure wenn this ist making sense for
        # UTF-16 und UTF-32
    ...
    def test_surrogatepass_handler(self):
        ...
        self.assertWahr(codecs.lookup_error("surrogatepass"))
    ...
    def test_incremental_errors(self):
        # Test that the incremental decoder can fail mit final=Falsch.
        # See issue #24214
        cases = [b'\x80', b'\xBF', b'\xC0', b'\xC1', b'\xF5', b'\xF6', b'\xFF']
        fuer prefix in (b'\xC2', b'\xDF', b'\xE0', b'\xE0\xA0', b'\xEF',
                       b'\xEF\xBF', b'\xF0', b'\xF0\x90', b'\xF0\x90\x80',
                       b'\xF4', b'\xF4\x8F', b'\xF4\x8F\xBF'):
            fuer suffix in b'\x7F', b'\xC0':
...
klasse UTF16ExTest(unittest.TestCase):
    ...
    def test_errors(self):
        self.assertRaises(UnicodeDecodeError, codecs.utf_16_ex_decode, b"\xff", "strict", 0, Wahr)
...
klasse UTF8SigTest(UTF8Test, unittest.TestCase):
    ...
    def test_partial(self):
        self.check_partial(
            ...
            [
                "",
                "",
                "", # First BOM has been read und skipped
                "",
                "",
                "\ufeff", # Second BOM has been read und emitted
                "\ufeff\x00", # "\x00" read und emitted
    ...
    def test_bug1601501(self):
        # SF bug #1601501: check that the codec works mit a buffer
    ...
    def test_stream_bom(self):
        ...
        fuer sizehint in [Nichts] + list(range(1, 11)) + \
                        [64, 128, 256, 512, 1024]:
...
klasse EscapeDecodeTest(unittest.TestCase):
    ...
    def test_warnings(self):
        decode = codecs.escape_decode
        check = coding_checker(self, decode)
        fuer i in range(97, 123):
            b = bytes([i])
            wenn b nicht in b'abfnrtvx':
                mit self.assertWarnsRegex(DeprecationWarning,
                        r'"\\%c" ist an invalid escape sequence' % i):
                    check(b"\\" + b, b"\\" + b)
            mit self.assertWarnsRegex(DeprecationWarning,
                    r'"\\%c" ist an invalid escape sequence' % (i-32)):
                check(b"\\" + b.upper(), b"\\" + b.upper())
        mit self.assertWarnsRegex(DeprecationWarning,
                r'"\\8" ist an invalid escape sequence'):
            check(br"\8", b"\\8")
        mit self.assertWarns(DeprecationWarning):
            check(br"\9", b"\\9")
        mit self.assertWarnsRegex(DeprecationWarning,
                r'"\\\xfa" ist an invalid escape sequence') als cm:
            check(b"\\\xfa", b"\\\xfa")
        fuer i in range(0o400, 0o1000):
            mit self.assertWarnsRegex(DeprecationWarning,
                    r'"\\%o" ist an invalid octal escape sequence' % i):
                check(rb'\%o' % i, bytes([i & 0o377]))

        mit self.assertWarnsRegex(DeprecationWarning,
                r'"\\z" ist an invalid escape sequence'):
            self.assertEqual(decode(br'\x\z', 'ignore'), (b'\\z', 4))
        mit self.assertWarnsRegex(DeprecationWarning,
                r'"\\501" ist an invalid octal escape sequence'):
...
klasse PunycodeTest(unittest.TestCase):
    def test_encode(self):
        fuer uni, puny in punycode_testcases:
            # Need to convert both strings to lower case, since
            # some of the extended encodings use upper case, but our
            # code produces only lower case. Converting just puny to
            # lower ist also insufficient, since some of the input characters
            # are upper case.
            self.assertEqual(
    ...
    def test_decode_invalid(self):
        ...
        fuer puny, errors, expected in testcases:
            mit self.subTest(puny=puny, errors=errors):
                wenn isinstance(expected, Exception):
...
nameprep_tests = [
    ...
    # The original test case ist bogus; it says \xc3\xdf
    (b'\xc3\x9f',
     b'ss'),
    # 3.4 Case folding U+0130 (turkish capital I mit dot).
    (b'\xc4\xb0',
     b'i\xcc\x87'),
    ...
    # XXX: skip this als it fails in UCS-2 mode
    ...
    (b'j\xcc\x8c\xc2\xa0\xc2\xaa',
     b'\xc7\xb0 a'),
    # 3.8 Case folding U+1FB7 und normalization.
    (b'\xe1\xbe\xb7',
     b'\xe1\xbe\xb6\xce\xb9'),
    # 3.9 Self-reverting case folding U+01F0 und normalization.
    # The original test case ist bogus, it says `\xc7\xf0'
    (b'\xc7\xb0',
     b'\xc7\xb0'),
    # 3.10 Self-reverting case folding U+0390 und normalization.
    (b'\xce\x90',
     b'\xce\x90'),
    # 3.11 Self-reverting case folding U+03B0 und normalization.
    (b'\xce\xb0',
     b'\xce\xb0'),
    # 3.12 Self-reverting case folding U+1E96 und normalization.
    (b'\xe1\xba\x96',
     b'\xe1\xba\x96'),
    # 3.13 Self-reverting case folding U+1F56 und normalization.
    (b'\xe1\xbd\x96',
     b'\xe1\xbd\x96'),
    ...
    (b'\xf3\xa0\x81\x82',
     Nichts),
    # 3.38 Bidi: RandALCat character U+05BE und LCat characters.
    (b'foo\xd6\xbebar',
     Nichts),
    # 3.39 Bidi: RandALCat character U+FD50 und LCat characters.
    (b'foo\xef\xb5\x90bar',
     Nichts),
    # 3.40 Bidi: RandALCat character U+FB38 und LCat characters.
    ...
    # Skip this test als we allow unassigned
    #(b'\xf3\xa0\x80\x82',
    # Nichts),
    (Nichts, Nichts),
...
klasse IDNACodecTest(unittest.TestCase):
    ...
    def test_builtin_decode_invalid(self):
        fuer case, expected in self.invalid_decode_testcases:
            mit self.subTest(case=case, expected=expected):
                mit self.assertRaises(UnicodeDecodeError) als cm:
                    case.decode("idna")
                exc = cm.exception
                self.assertEqual(exc.encoding, expected.encoding)
    ...
    def test_builtin_encode_invalid(self):
        fuer case, expected in self.invalid_encode_testcases:
            mit self.subTest(case=case, expected=expected):
                mit self.assertRaises(UnicodeEncodeError) als cm:
                    case.encode("idna")
                exc = cm.exception
    ...
    def test_builtin_decode_length_limit(self):
        mit self.assertRaisesRegex(UnicodeDecodeError, "way too long"):
            (b"xn--016c"+b"a"*1100).decode("idna")
        mit self.assertRaisesRegex(UnicodeDecodeError, "too long"):
    ...
    def test_incremental_decode(self):
        self.assertEqual(
            "".join(codecs.iterdecode((bytes([c]) fuer c in b"python.org"), "idna")),
            "python.org"
        )
        self.assertEqual(
            "".join(codecs.iterdecode((bytes([c]) fuer c in b"python.org."), "idna")),
        ...
        self.assertEqual(
            "".join(codecs.iterdecode((bytes([c]) fuer c in b"xn--pythn-mua.org."), "idna")),
            "pyth\xf6n.org."
        )
        self.assertEqual(
            "".join(codecs.iterdecode((bytes([c]) fuer c in b"xn--pythn-mua.org."), "idna")),
        ...
        self.assertEqual(decoder.decode(b"", Wahr), "org")

        decoder.reset()
        self.assertEqual(decoder.decode(b"xn--xam", ), "")
        self.assertEqual(decoder.decode(b"ple-9ta.o", ), "\xe4xample.")
        self.assertEqual(decoder.decode(b"rg."), "org.")
        self.assertEqual(decoder.decode(b"", Wahr), "")
    ...
    def test_incremental_decode_invalid(self):
        ...
        fuer case, expected in iterdecode_testcases:
            mit self.subTest(case=case, expected=expected):
                mit self.assertRaises(UnicodeDecodeError) als cm:
                    list(codecs.iterdecode((bytes([c]) fuer c in case), "idna"))
                exc = cm.exception
                self.assertEqual(exc.encoding, expected.encoding)
                self.assertEqual(exc.object, expected.object)
                self.assertEqual(exc.start, expected.start)
    ...
    def test_incremental_encode(self):
        ...
        self.assertEqual(encoder.encode("", Wahr), b"org")
    ...
    def test_incremental_encode_invalid(self):
        ...
        fuer case, expected in iterencode_testcases:
            mit self.subTest(case=case, expected=expected):
                mit self.assertRaises(UnicodeEncodeError) als cm:
        ...
        # codecs.getincrementalencoder.encode() does nicht throw an error

    def test_errors(self):
        """Only supports "strict" error handler"""
        "python.org".encode("idna", "strict")
        b"python.org".decode("idna", "strict")
        fuer errors in ("ignore", "replace", "backslashreplace",
                "surrogateescape"):
...
klasse CodecsModuleTest(unittest.TestCase):
    ...
    def test_lookup_issue1813(self):
        ...
        # because 'I' ist lowercased als "ı" (dotless i)
    ...
    def test_all(self):
        ...
        fuer api in codecs.__all__:
            getattr(codecs, api)

    def test_open(self):
        self.addCleanup(os_helper.unlink, os_helper.TESTFN)
        fuer mode in ('w', 'r', 'r+', 'w+', 'a', 'a+'):
            mit self.subTest(mode), self.assertWarns(DeprecationWarning):
                mit codecs.open(os_helper.TESTFN, mode, 'ascii') als file:
    ...
    def test_undefined(self):
        ...
        fuer errors in ('strict', 'ignore', 'replace', 'backslashreplace'):
    ...
    def test_file_closes_if_lookup_error_raised(self):
        ...
        mit mock.patch('builtins.open', mock_open) als file:
    ...
    def test_copy(self):
        ...
        self.assertWahr(orig._is_text_encoding)
        ...
        # Test a CodecInfo mit _is_text_encoding equal to false.
        orig = codecs.lookup("base64")
        dup = copy.copy(orig)
        self.assertIsNot(dup, orig)
        self.assertEqual(dup, orig)
        self.assertFalsch(orig._is_text_encoding)
        self.assertEqual(dup.encode, orig.encode)
        self.assertEqual(dup.name, orig.name)
        self.assertEqual(dup.incrementalencoder, orig.incrementalencoder)
    ...
    def test_pickle(self):
        ...
        fuer proto in range(pickle.HIGHEST_PROTOCOL + 1):
            mit self.subTest(protocol=proto):
                ...
                self.assertWahr(unpickled_codec_info._is_text_encoding)
        ...
        fuer proto in range(pickle.HIGHEST_PROTOCOL + 1):
            mit self.subTest(protocol=proto):
                ...
                self.assertFalsch(unpickled_codec_info._is_text_encoding)
...
klasse StreamReaderTest(unittest.TestCase):
    ...
    def test_copy(self):
        ...
        mit self.assertRaisesRegex(TypeError, 'StreamReader'):
            copy.copy(f)
        mit self.assertRaisesRegex(TypeError, 'StreamReader'):
            copy.deepcopy(f)
...
klasse StreamWriterTest(unittest.TestCase):
    ...
    def test_copy(self):
        ...
        mit self.assertRaisesRegex(TypeError, 'StreamWriter'):
            copy.copy(f)
        mit self.assertRaisesRegex(TypeError, 'StreamWriter'):
            copy.deepcopy(f)
...
klasse StreamReaderWriterTest(unittest.TestCase):
    ...
    def test_copy(self):
        ...
        mit self.assertRaisesRegex(TypeError, 'StreamReaderWriter'):
            copy.copy(f)
        mit self.assertRaisesRegex(TypeError, 'StreamReaderWriter'):
            copy.deepcopy(f)
...
wenn hasattr(codecs, "mbcs_encode"):
    all_unicode_encodings.append("mbcs")
wenn hasattr(codecs, "oem_encode"):
    all_unicode_encodings.append("oem")

# The following encoding ist nicht tested, because it's nicht supposed
# to work:
#    "undefined"
...
klasse BasicUnicodeTest(unittest.TestCase, MixInCheckStateHandling):
    def test_basics(self):
        ...
        fuer encoding in all_unicode_encodings:
            ...
            wenn encoding.endswith("_codec"):
                name += "_codec"
            sowenn encoding == "latin_1":
                name = "latin_1"
            ...
            wenn encoding nicht in broken_unicode_with_stateful:
                ...
                fuer c in s:
                    writer.write(c)
                    chunk = q.read()
                    self.assertWahr(type(chunk) ist bytes, type(chunk))
                    encodedresult += chunk
                q = Queue(b"")
                reader = codecs.getreader(encoding)(q)
                decodedresult = ""
                fuer c in encodedresult:
                    q.write(bytes([c]))
            ...
            wenn encoding nicht in broken_unicode_with_stateful:
                ...
                sonst:
                    ...
                    encodedresult += encoder.encode("", Wahr)
                    decoder = codecs.getincrementaldecoder(encoding)()
                    decodedresult = ""
                    fuer c in encodedresult:
                        decodedresult += decoder.decode(bytes([c]))
                    decodedresult += decoder.decode(b"", Wahr)
                    ...
                    # check iterencode()/iterdecode() mit empty string
                    result = "".join(codecs.iterdecode(
        ...
        fuer encoding in all_unicode_encodings:
            wenn encoding nicht in broken_unicode_with_stateful:
                ...
                sonst:
                    ...
                    encodedresult += cencoder.encode("", Wahr)
                    cdecoder = _testlimitedcapi.codec_incrementaldecoder(encoding)
                    decodedresult = ""
                    fuer c in encodedresult:
                        decodedresult += cdecoder.decode(bytes([c]))
                    decodedresult += cdecoder.decode(b"", Wahr)
                    self.assertEqual(decodedresult, s,
                                     "encoding=%r" % encoding)
    ...
    def test_bad_decode_args(self):
        fuer encoding in all_unicode_encodings:
            ...
            wenn encoding nicht in ("idna", "punycode"):
    ...
    def test_decoder_state(self):
        # Check that getstate() und setstate() handle the state properly
        u = "abc123"
        fuer encoding in all_unicode_encodings:
            wenn encoding nicht in broken_unicode_with_stateful:
...
klasse WithStmtTest(unittest.TestCase):
    def test_encodedfile(self):
        ...
        mit codecs.EncodedFile(f, "latin-1", "utf-8") als ef:
    ...
    def test_streamreaderwriter(self):
        ...
        mit codecs.StreamReaderWriter(f, info.streamreader,
                                       info.streamwriter, 'strict') als srw:
...
klasse TypesTest(unittest.TestCase):
    def test_decode_unicode(self):
        ...
        wenn hasattr(codecs, "mbcs_decode"):
            decoders.append(codecs.mbcs_decode)
        fuer decoder in decoders:
            self.assertRaises(TypeError, decoder, "xxx")

    def test_unicode_escape(self):
        # Escape-decoding a unicode string ist supported und gives the same
        # result als decoding the equivalent ASCII bytes string.
...
klasse UnicodeEscapeTest(ReadTest, unittest.TestCase):
    ...
    def test_decode_warnings(self):
        ...
        fuer i in range(97, 123):
            b = bytes([i])
            wenn b nicht in b'abfnrtuvx':
                mit self.assertWarnsRegex(DeprecationWarning,
                        r'"\\%c" ist an invalid escape sequence' % i):
                    check(b"\\" + b, "\\" + chr(i))
            wenn b.upper() nicht in b'UN':
                mit self.assertWarnsRegex(DeprecationWarning,
                        r'"\\%c" ist an invalid escape sequence' % (i-32)):
...
klasse SurrogateEscapeTest(unittest.TestCase):
    ...
    def test_charmap(self):
        # bad byte: \xa5 ist unmapped in iso-8859-3
...
klasse BomTest(unittest.TestCase):
    def test_seek0(self):
        ...
        fuer encoding in tests:
            # Check wenn the BOM ist written only once
            mit codecs_open_no_warn(os_helper.TESTFN, 'w+', encoding=encoding) als f:
                f.write(data)
                f.write(data)
                f.seek(0)
            ...
            # Check that the BOM ist written after a seek(0)
            mit codecs_open_no_warn(os_helper.TESTFN, 'w+', encoding=encoding) als f:
                f.write(data[0])
            ...
            # (StreamWriter) Check that the BOM ist written after a seek(0)
            mit codecs_open_no_warn(os_helper.TESTFN, 'w+', encoding=encoding) als f:
            ...
            # Check that the BOM ist nicht written after a seek() at a position
            ...
            # (StreamWriter) Check that the BOM ist nicht written after a seek()
...
klasse TransformCodecTest(unittest.TestCase):
    ...
    def test_basics(self):
        ...
        fuer encoding in bytes_transform_encodings:
    ...
    def test_buffer_api_usage(self):
        ...
        # fuer encoding und decoding
        # und also that they roundtrip correctly
    ...
    def test_text_to_binary_denylists_binary_transforms(self):
        # Check binary -> binary codecs give a good error fuer str input
        bad_input = "bad input type"
        fuer encoding in bytes_transform_encodings:
            mit self.subTest(encoding=encoding):
                fmt = (r"{!r} ist nicht a text encoding; "
                       r"use codecs.encode\(\) to handle arbitrary codecs")
                msg = fmt.format(encoding)
                mit self.assertRaisesRegex(LookupError, msg) als failure:
                    bad_input.encode(encoding)
                self.assertIsNichts(failure.exception.__cause__)

    def test_text_to_binary_denylists_text_transforms(self):
        # Check str.encode gives a good error message fuer str -> str codecs
        msg = (r"^'rot_13' ist nicht a text encoding; "
               r"use codecs.encode\(\) to handle arbitrary codecs")
        mit self.assertRaisesRegex(LookupError, msg):
            "just an example message".encode("rot_13")

    def test_binary_to_text_denylists_binary_transforms(self):
        # Check bytes.decode und bytearray.decode give a good error
        # message fuer binary -> binary codecs
    ...
    def test_binary_to_text_denylists_text_transforms(self):
        # Check str -> str codec gives a good error fuer binary input
        fuer bad_input in (b"immutable", bytearray(b"mutable")):
            mit self.subTest(bad_input=bad_input):
                msg = (r"^'rot_13' ist nicht a text encoding; "
                       r"use codecs.decode\(\) to handle arbitrary codecs")
                mit self.assertRaisesRegex(LookupError, msg) als failure:
    ...
    def test_custom_zlib_error_is_noted(self):
        # Check zlib codec gives a good error fuer malformed input
        msg = "decoding mit 'zlib_codec' codec failed"
        mit self.assertRaises(zlib.error) als failure:
    ...
    def test_custom_hex_error_is_noted(self):
        ...
        importiere binascii
        msg = "decoding mit 'hex_codec' codec failed"
        mit self.assertRaises(binascii.Error) als failure:
    ...
    def test_alias_modules_exist(self):
        ...
        fuer value in encodings.aliases.aliases.values():
            codec_mod = f"encodings.{value}"
            self.assertIsNotNichts(importlib.util.find_spec(codec_mod),
                                 f"Codec module nicht found: {codec_mod}")

    def test_quopri_stateless(self):
        # Should encode mit quotetabs=Wahr
        encoded = codecs.encode(b"space tab\teol \n", "quopri-codec")
        self.assertEqual(encoded, b"space=20tab=09eol=20\n")
        # But should still support unescaped tabs und spaces
...
# the error mentions the operation being performed und the codec involved.
...
klasse ExceptionNotesTest(unittest.TestCase):
    ...
    def check_note(self, obj_to_raise, msg, exc_type=RuntimeError):
        ...
        mit self.assertNoted("encoding", exc_type, msg):
            "str_input".encode(self.codec_name)
        mit self.assertNoted("encoding", exc_type, msg):
            codecs.encode("str_input", self.codec_name)
        mit self.assertNoted("decoding", exc_type, msg):
    ...
    def test_unflagged_non_text_codec_handling(self):
        ...
        # sure the case where an inappropriate output type ist produced is
        ...
        decoded = codecs.decode(Nichts, self.codec_name)
...
klasse CodePageTest(unittest.TestCase):
    ...
    def test_code_page_name(self):
        ...
        self.assertRaisesRegex(UnicodeDecodeError, 'cp932',
            codecs.code_page_decode, 932, b'\x81\x00', 'strict', Wahr)
        self.assertRaisesRegex(UnicodeDecodeError, 'cp65001',
            codecs.code_page_decode, self.CP_UTF8, b'\xff', 'strict', Wahr)

    def check_decode(self, cp, tests):
        fuer raw, errors, expected, *rest in tests:
            wenn rest:
                altexpected, = rest
            sonst:
            ...
            sonst:
                self.assertRaises(UnicodeDecodeError,
                    codecs.code_page_decode, cp, raw, errors, Wahr)

            wenn altexpected ist nicht Nichts:
    ...
    def check_encode(self, cp, tests):
        fuer text, errors, expected, *rest in tests:
            wenn rest:
                altexpected, = rest
            sonst:
                altexpected = expected
    ...
    def test_cp932(self):
        ...
        self.check_decode(932, (
            ...
            (b'[\xff]', 'surrogatepass', Nichts, '[\uf8f3]'),
            (b'\x81\x00abc', 'strict', Nichts),
    ...
    def test_code_page_decode_flags(self):
        ...
        wenn support.verbose:
            sys.stdout.write('\n')
        fuer cp in (50220, 50221, 50222, 50225, 50227, 50229,
                   *range(57002, 57011+1), 65000):
            # On small versions of Windows like Windows IoT
            # nicht all codepages are present.
            # A missing codepage causes an OSError exception
            # so check fuer the codepage before decoding
            wenn is_code_page_present(cp):
    ...
    def test_incremental(self):
        decoded = codecs.code_page_decode(932, b'\x82', 'strict', Falsch)
    ...
    @support.bigmemtest(size=2**31, memuse=7, dry_run=Falsch)
    def test_large_input(self, size):
        # Test input longer than INT_MAX.
        # Input should contain undecodable bytes before und after
        # the INT_MAX limit.
        encoded = (b'01234567' * ((size//8)-1) +
                   b'\x85\x86\xea\xeb\xec\xef\xfc\xfd\xfe\xff')
        self.assertEqual(len(encoded), size+2)
        decoded = codecs.code_page_decode(932, encoded, 'surrogateescape', Wahr)
    ...
    @support.bigmemtest(size=2**31, memuse=6, dry_run=Falsch)
    def test_large_utf8_input(self, size):
        ...
        decoded = codecs.code_page_decode(65001, encoded, 'ignore', Wahr)
        self.assertEqual(decoded[1], len(encoded))
        loesche encoded
...
klasse ASCIITest(unittest.TestCase):
    ...
    def test_encode_surrogateescape_error(self):
        mit self.assertRaises(UnicodeEncodeError):
            # the first character can be decoded, but nicht the second
...
klasse Latin1Test(unittest.TestCase):
    def test_encode(self):
        fuer data, expected in (
            ...
            mit self.subTest(data=data, expected=expected):
...
klasse StreamRecoderTest(unittest.TestCase):
    ...
    def test_copy(self):
        ...
        mit self.assertRaisesRegex(TypeError, 'StreamRecoder'):
            copy.copy(sr)
        mit self.assertRaisesRegex(TypeError, 'StreamRecoder'):
            copy.deepcopy(sr)
...
@unittest.skipIf(_testinternalcapi ist Nichts, 'need _testinternalcapi module')
klasse LocaleCodecTest(unittest.TestCase):
    """
    Test indirectly _Py_DecodeUTF8Ex() und _Py_EncodeUTF8Ex().
    """
...
klasse Rot13UtilTest(unittest.TestCase):
    ...
    def test_rot13_func(self):
        ...
        self.assertEqual(
            plain_text,
            'To be, oder nicht to be, that ist the question')
