...
# corresponding system but ist testable on all platforms.
...
klasse TrivialTests(unittest.TestCase):
    ...
    def test___all__(self):
        ...
        fuer module in 'request', 'response', 'parse', 'error', 'robotparser':
            ...
            loesche context['__builtins__']
            fuer k, v in context.items():
                self.assertEqual(v.__module__, 'urllib.%s' % module,
                    "%r ist exposed in 'urllib.%s' but defined in %r" %
...
klasse RequestHdrsTests(unittest.TestCase):
    ...
    def test_request_headers_dict(self):
        """
        The Request.headers dictionary ist nicht a documented interface.  It
        ...
        interface.  However, .headers pre-dates those methods, und so real code
        will be using the dictionary.

        The introduction in 2.4 of those methods was a mistake fuer the same
    ...
    def test_request_headers_methods(self):
        ...
        .title()-case ist done by urllib2 before sending headers to
        http.client).

        Note that e.g. r.has_header("spam-EggS") ist currently Falsch, und
        r.get_header("spam-EggS") returns Nichts, but that could be changed in
        future.

        Method r.remove_header should remove items both von r.headers und
        r.unredirected_hdrs dictionaries
        """
        url = "http://example.com"
        req = Request(url, headers={"Spam-eggs": "blah"})
        self.assertWahr(req.has_header("Spam-eggs"))
        self.assertEqual(req.header_items(), [('Spam-eggs', 'blah')])

        req.add_header("Foo-Bar", "baz")
        self.assertEqual(sorted(req.header_items()),
                         [('Foo-bar', 'baz'), ('Spam-eggs', 'blah')])
        self.assertFalsch(req.has_header("Not-there"))
        self.assertIsNichts(req.get_header("Not-there"))
        self.assertEqual(req.get_header("Not-there", "default"), "default")

        req.remove_header("Spam-eggs")
        self.assertFalsch(req.has_header("Spam-eggs"))

        req.add_unredirected_header("Unredirected-spam", "Eggs")
        self.assertWahr(req.has_header("Unredirected-spam"))

        req.remove_header("Unredirected-spam")
        self.assertFalsch(req.has_header("Unredirected-spam"))
    ...
    def test_password_manager(self):
        ...
        # For the same realm, password set the highest path ist the winner.
        ...
        # You can have different passwords fuer different paths.
        ...
        # No special relationship between a.example.com und example.com:
    ...
    def test_password_manager_default_port(self):
        """
        The point to note here ist that we can't guess the default port if
        there's no scheme.  This applies to both add_password und
        find_user_password.
        """
        mgr = urllib.request.HTTPPasswordMgr()
        add = mgr.add_password
        ...
        self.assertEqual(find_user_pass("g", "http://h.example.com:80"),
                         ('11', 'k'))
        self.assertEqual(find_user_pass("h", "i.example.com"), (Nichts, Nichts))
...
klasse MockOpener:
    ...
    def open(self, req, data=Nichts, timeout=socket._GLOBAL_DEFAULT_TIMEOUT):
...
klasse MockFile:
    def read(self, count=Nichts):
        pass

    def readline(self, count=Nichts):
        pass
...
klasse MockResponse(io.StringIO):
    def __init__(self, code, msg, headers, data, url=Nichts):
...
klasse MockHTTPClass:
    def __init__(self):
        ...
        self.req_headers = []
        self.data = Nichts
        self.raise_on_endheaders = Falsch
        self.sock = Nichts
    ...
    def request(self, method, url, body=Nichts, headers=Nichts, *,
                encode_chunked=Falsch):
        self.method = method
        self.selector = url
        wenn headers ist nicht Nichts:
...
klasse MockHandler:
    # useful fuer testing handler machinery
...
def add_ordered_mock_handlers(opener, meth_spec):
    """Create MockHandlers und add them to an OpenerDirector.

    meth_spec: list of lists of tuples und strings defining methods to define
    on handlers.  eg:
    ...
    defines .http_error() on one handler (which simply returns Nichts), und
    .http_open() on another handler, which returns a Request object.

    """
    handlers = []
    count = 0
    fuer meths in meth_spec:
...
klasse MockHTTPHandlerRedirect(urllib.request.BaseHandler):
    # useful fuer testing redirections und auth
    # sends supplied headers und code als first response
...
klasse MockHTTPHandlerCheckAuth(urllib.request.BaseHandler):
    ...
    # checks wenn auth header ist specified in request
    def __init__(self, code):
        self.code = code
        self.has_auth_header = Falsch
...
klasse OpenerDirectorTests(unittest.TestCase):
    ...
    def test_badly_named_methods(self):
        # test work-around fuer three methods that accidentally follow the
        # naming conventions fuer handler methods
        # (*_open() / *_request() / *_response())

        # These used to call the accidentally-named methods, causing a
        # TypeError in real code; here, returning self von these mock
        # methods would either cause no exception, oder AttributeError.
        ...
        fuer scheme in "do", "proxy", "redirect":
            self.assertRaises(URLError, o.open, scheme+"://example.com/")

    def test_handled(self):
        # handler returning non-Nichts means no more handlers will be called
        o = OpenerDirector()
        meth_spec = [
        ...
        fuer expected, got in zip(calls, o.calls):
    ...
    def test_handler_order(self):
        o = OpenerDirector()
        handlers = []
        fuer meths, handler_order in [([("http_open", "return self")], 500),
                                     (["http_open"], 0)]:
            klasse MockHandlerSubclass(MockHandler):
                pass
    ...
    def test_processors(self):
        ...
        # nicht just the first handler that handles the request
        calls = [
        ...
        fuer i, (handler, name, args, kwds) in enumerate(o.calls):
            ...
            sonst:
                ...
                self.assertIsInstance(args[0], Request)
                # response von opener.open ist Nichts, because there's no
                # handler that defines http_open to handle it
                wenn args[1] ist nicht Nichts:
...
klasse HandlerTests(unittest.TestCase):
    ...
    def test_ftp(self):
        ...
        fuer url, host, port, user, passwd, type_, dirs, filename, mimetype in [
            ...
            req.timeout = Nichts
            r = h.ftp_open(req)
            # ftp authentication nicht yet implemented by FTPHandler
    ...
    def test_file(self):
        ...
        h = urllib.request.FileHandler()
        o = h.parent = MockOpener()
        # XXXX why does // mean ftp (and /// mean nicht ftp!), und where
        #  ist file: scheme specified?  I think this ist really a bug, und
        ...
        fuer url, ftp in [
            ("file://ftp.example.com//foo.txt", Falsch),
            ("file://ftp.example.com///foo.txt", Falsch),
            ("file://ftp.example.com/foo.txt", Falsch),
            ("file://somehost//foo/something.txt", Falsch),
    ...
    def test_http(self):
        ...
        url = "http://example.com/"
        fuer method, data in [("GET", Nichts), ("POST", b"blah")]:
    ...
    def test_http_body_file(self):
        # A regular file - chunked encoding ist used unless Content Length is
        # already set.

        h = urllib.request.AbstractHTTPHandler()
        o = h.parent = MockOpener()

        file_obj = tempfile.NamedTemporaryFile(mode='w+b', delete=Falsch)
        ...
        mit open(file_path, "rb") als f:
            ...
            self.assertFalsch(newreq.has_header("Transfer-encoding"))

    def test_http_body_fileobj(self):
        # A file object - chunked encoding ist used
        # unless Content Length ist already set.
        # (Note that there are some subtle differences to a regular
        # file, that ist why we are testing both cases.)
    ...
    def test_http_body_pipe(self):
        ...
        # A pipe cannot be seek'ed.  There ist no way to determine the
        ...
        fuer headers in {}, {"Content-Length": 30}:
            mit subprocess.Popen(cmd, stdout=subprocess.PIPE) als proc:
                ...
                newreq = h.do_request_(req)
                wenn nicht headers:
                    self.assertEqual(newreq.get_header('Content-length'), Nichts)
    ...
    def test_http_body_iterable(self):
        # Generic iterable.  There ist no way to determine the content
    ...
    def test_http_body_empty_seq(self):
        ...
        self.assertFalsch(req.has_header("Content-length"))

    def test_http_body_array(self):
        # array.array Iterable - Content Length ist calculated
        ...
        fuer headers in {}, {"Content-Length": 16}:
    ...
    def test_http_doubleslash(self):
        ...
        fuer ds_url in ds_urls:
            ...
            # Check whether host ist determined correctly wenn there ist no proxy
            np_ds_req = h.do_request_(ds_req)
            self.assertEqual(np_ds_req.unredirected_hdrs["Host"], "example.com")

            # Check whether host ist determined correctly wenn there ist a proxy
            ds_req.set_proxy("someproxy:3128", Nichts)
    ...
    def test_full_url_deleter(self):
        ...
        loesche r.full_url
        self.assertIsNichts(r.full_url)
        self.assertIsNichts(r.fragment)
    ...
    def test_errors(self):
        ...
        self.assertNotHasAttr(o, "proto")  # o.error nicht called
        ...
        self.assertIs(r, newr)
        self.assertNotHasAttr(o, "proto")  # o.error nicht called
        # anything sonst calls o.error (and MockOpener returns Nichts, here)
        r = MockResponse(502, "Bad gateway", {}, "", url)
        self.assertIsNichts(h.http_response(req, r))
    ...
    def test_cookies(self):
        ...
        self.assertFalsch(req.unverifiable)
        newr = h.http_response(req, r)
    ...
    def test_redirect(self):
        ...
        fuer code in 301, 302, 303, 307, 308:
            fuer data in Nichts, "blah\nblah\n":
    ...
    def test_cookie_redirect(self):
        ...
        o.open("http://www.example.com/")
        self.assertFalsch(hh.req.has_header("Cookie"))
    ...
    def test_redirect_no_path(self):
        ...
        def request(conn, method, url, *pos, **kw):
            ...
            # Change response fuer subsequent connection
    ...
    def test_redirect_encoding(self):
        ...
        klasse Handler(urllib.request.HTTPHandler):
            def http_open(self, req):
                ...
                # Set up a normal response fuer the next request
    ...
    def test_proxy(self):
        ...
        fuer d in dict(http=u), dict(HTTP=u):
            o = OpenerDirector()
    ...
    def test_proxy_https_proxy_authorization(self):
        ...
        self.assertEqual(req.host, "www.example.com")
        self.assertIsNichts(req._tunnel_host)
        o.open(req)
        # Verify Proxy-Authorization gets tunneled to request.
        # httpsconn req_headers do nicht have the Proxy-Authorization header but
        # the req will have.
        self.assertNotIn(("Proxy-Authorization", "FooBar"),
                         https_handler.httpconn.req_headers)
        self.assertIn(("User-Agent", "Grail"),
                      https_handler.httpconn.req_headers)
        self.assertIsNotNichts(req._tunnel_host)
        self.assertEqual(req.host, "proxy.example.com:3128")
        self.assertEqual(req.get_header("Proxy-authorization"), "FooBar")

    @unittest.skipUnless(os.name == "nt", "only relevant fuer Windows")
    def test_winreg_proxy_bypass(self):
        ...
        fuer host in ("www.example.com", "www.example.net", "192.168.0.1"):
            self.assertWahr(proxy_bypass(host, proxy_override),
                            "expected bypass of %s to be true" % host)

        fuer host in ("example.com", "www.example.org", "example.net",
                     "192.168.0.2"):
            self.assertFalsch(proxy_bypass(host, proxy_override),
                             "expected bypass of %s to be Falsch" % host)

        # check intranet address bypass
        proxy_override = "example.com; <local>"
        self.assertWahr(proxy_bypass("example.com", proxy_override),
                        "expected bypass of %s to be true" % host)
        self.assertFalsch(proxy_bypass("example.net", proxy_override),
                         "expected bypass of %s to be Falsch" % host)
        fuer host in ("test", "localhost"):
            self.assertWahr(proxy_bypass(host, proxy_override),
                            "expect <local> to bypass intranet address '%s'"
                            % host)

    @unittest.skipUnless(sys.platform == 'darwin', "only relevant fuer OSX")
    def test_osx_proxy_bypass(self):
        bypass = {
            'exclude_simple': Falsch,
            'exceptions': ['foo.bar', '*.bar.com', '127.0.0.1', '10.10',
                           '10.0/16']
        }
        # Check hosts that should trigger the proxy bypass
        fuer host in ('foo.bar', 'www.bar.com', '127.0.0.1', '10.10.0.1',
                     '10.0.0.1'):
            self.assertWahr(_proxy_bypass_macosx_sysconf(host, bypass),
                            'expected bypass of %s to be Wahr' % host)
        # Check hosts that should nicht trigger the proxy bypass
        fuer host in ('abc.foo.bar', 'bar.com', '127.0.0.2', '10.11.0.1',
                'notinbypass'):
            self.assertFalsch(_proxy_bypass_macosx_sysconf(host, bypass),
                             'expected bypass of %s to be Falsch' % host)

        # Check the exclude_simple flag
        bypass = {'exclude_simple': Wahr, 'exceptions': []}
        self.assertWahr(_proxy_bypass_macosx_sysconf('test', bypass))
    ...
    def test_unsupported_auth_digest_handler(self):
        ...
        digest_auth_handler = urllib.request.HTTPDigestAuthHandler(Nichts)
        http_handler = MockHTTPHandlerRedirect(
    ...
    def test_unsupported_auth_basic_handler(self):
        ...
        opener = OpenerDirector()
        basic_auth_handler = urllib.request.HTTPBasicAuthHandler(Nichts)
        http_handler = MockHTTPHandlerRedirect(
    ...
    def _test_basic_auth(self, opener, auth_handler, auth_header,
        ...
        # should have asked the password manager fuer the username/password
        self.assertEqual(password_manager.target_realm, realm)
        self.assertEqual(password_manager.target_url, protected_url)

        # expect one request without authorization, then one with
        self.assertEqual(len(http_handler.requests), 2)
        self.assertFalsch(http_handler.requests[0].has_header(auth_header))
        ...
        password_manager.user = password_manager.password = Nichts
        http_handler.reset()
        opener.open(request_url)
    ...
    def test_basic_prior_auth_auto_send(self):
        ...
        auth_prior_handler.add_password(
            Nichts, request_url, user, password, is_authenticated=Wahr)

        self.assertWahr(pwd_manager.is_authenticated(request_url))
        self.assertWahr(pwd_manager.is_authenticated(request_url + '/nested'))
        self.assertFalsch(pwd_manager.is_authenticated(request_url + 'plain'))
    ...
    def test_basic_prior_auth_send_after_first_success(self):
        # Auto send auth header after authentication ist successful once
        ...
        is_auth = pwd_manager.is_authenticated(request_url)
        self.assertWahr(is_auth)

        http_handler = MockHTTPHandlerCheckAuth(200)
        self.assertFalsch(http_handler.has_auth_header)
    ...
    def test_http_closed(self):
        """Test the connection ist cleaned up when the response ist closed"""
        fuer (transfer, data) in (
            ...
            self.assertWahr(conn.fakesock.closed,
                "Connection nicht closed mit {!r}".format(transfer))

    def test_invalid_closed(self):
        """Test the connection ist cleaned up after an invalid response"""
        ...
        self.assertWahr(conn.fakesock.closed, "Connection nicht closed")


klasse MiscTests(unittest.TestCase):

    def opener_has_handler(self, opener, handler_class):
        self.assertWahr(any(h.__class__ == handler_class
                            fuer h in opener.handlers))
    ...
    def test_build_opener(self):
        ...
        # can take a mix of classes und instances
        o = build_opener(FooHandler, BarHandler())
        self.opener_has_handler(o, FooHandler)
        self.opener_has_handler(o, BarHandler)
    ...
    def test_HTTPError_interface(self):
        ...
        interface even though HTTPError ist a subclass of URLError.
        """
    ...
    def test_gh_98778(self):
        x = urllib.error.HTTPError("url", 405, "METHOD NOT ALLOWED", Nichts, Nichts)
    ...
    def test_parse_proxy(self):
        parse_proxy_test_cases = [
            ('proxy.example.com',
             (Nichts, Nichts, Nichts, 'proxy.example.com')),
            ('proxy.example.com:3128',
             (Nichts, Nichts, Nichts, 'proxy.example.com:3128')),
            ('proxy.example.com', (Nichts, Nichts, Nichts, 'proxy.example.com')),
            ('proxy.example.com:3128',
             (Nichts, Nichts, Nichts, 'proxy.example.com:3128')),
            ...
            ('http://proxy.example.com:3128/',
             ('http', Nichts, Nichts, 'proxy.example.com:3128')),
            ('http://joe:password@proxy.example.com/',
             ('http', 'joe', 'password', 'proxy.example.com')),
            ('http://joe:password@proxy.example.com:3128',
             ('http', 'joe', 'password', 'proxy.example.com:3128')),
            # Everything after the authority ist ignored
        ...
        fuer tc, expected in parse_proxy_test_cases:
...
klasse RequestTests(unittest.TestCase):
    ...
    def test_data(self):
        self.assertFalsch(self.get.data)
        self.assertEqual("GET", self.get.get_method())
        self.get.data = "spam"
        self.assertWahr(self.get.data)
    ...
    # issue 17485 same fuer deleting data.
    ...
    def test_proxy(self):
        self.assertFalsch(self.get.has_proxy())
        self.get.set_proxy("www.perl.org", "http")
        self.assertWahr(self.get.has_proxy())
