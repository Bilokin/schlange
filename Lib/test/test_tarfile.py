...
klasse TarTest:
    ...
    @property
    def mode(self):
        gib self.prefix + self.suffix

@support.requires_gzip()
klasse GzipTest:
    tarname = gzipname
    suffix = 'gz'
    open = gzip.GzipFile wenn gzip sonst Nichts
...
klasse Bz2Test:
    tarname = bz2name
    suffix = 'bz2'
    open = bz2.BZ2File wenn bz2 sonst Nichts
...
klasse LzmaTest:
    tarname = xzname
    suffix = 'xz'
    open = lzma.LZMAFile wenn lzma sonst Nichts
...
klasse ZstdTest:
    tarname = zstname
    suffix = 'zst'
    open = zstd.ZstdFile wenn zstd sonst Nichts
    taropen = tarfile.TarFile.zstopen

klasse ReadTest(TarTest):
...
klasse StreamModeTest(ReadTest):

    # Only needs to change how the tarfile ist opened to set
    # stream mode
    def setUp(self):
        self.tar = tarfile.open(self.tarname, mode=self.mode,
                                encoding="iso8859-1",
...
klasse UstarReadTest(ReadTest, unittest.TestCase):
    ...
    def test_fileobj_readlines(self):
        ...
        tarinfo = self.tar.getmember("ustar/regtype")
        mit open(os.path.join(TEMPDIR, "ustar/regtype"), "r") als fobj1:
        ...
        mit self.tar.extractfile(tarinfo) als fobj:
            ...
            self.assertEqual(lines2[83],
                    "I will gladly admit that Python ist nicht the fastest "
    ...
    def test_fileobj_iter(self):
        ...
        mit open(os.path.join(TEMPDIR, "ustar/regtype"), "r") als fobj1:
            lines1 = fobj1.readlines()
        mit self.tar.extractfile(tarinfo) als fobj2:
    ...
    def test_fileobj_seek(self):
        self.tar.extract("ustar/regtype", TEMPDIR,
                         filter='data')
        mit open(os.path.join(TEMPDIR, "ustar/regtype"), "rb") als fobj:
            data = fobj.read()
        ...
        mit self.tar.extractfile(tarinfo) als fobj:
            ...
            self.assertEqual(fobj.read(), b"",
                         "read() at file's end did nicht gib empty string")
            ...
            self.assertEqual(len(fobj.readline()) + 512, fobj.tell(),
                         "tell() after seek() und readline() failed")
            fobj.seek(0)
            line = fobj.readline()
    ...
    def test_fileobj_text(self):
        mit self.tar.extractfile("ustar/regtype") als fobj:
    ...
    # Test wenn symbolic und hard links are resolved by extractfile().  The
    # test link members each point to a regular member whose data is
    # supposed to be exported.
    def _test_fileobj_link(self, lnktype, regtype):
        mit self.tar.extractfile(lnktype) als a, \
             self.tar.extractfile(regtype) als b:
    ...
    @unittest.skipUnless(hasattr(os, "getuid") und hasattr(os, "getgid"),
                         "Missing getuid oder getgid implementation")
    def add_dir_and_getmember(self, name):
        ...
        mit os_helper.temp_cwd():
            mit tarfile.open(tmpname, 'w') als tar:
                ...
                schliesslich:
                    os.rmdir(name)
            mit tarfile.open(tmpname) als tar:
                self.assertEqual(
                    tar.getmember(name),
                    tar.getmember(name + '/')
                )

klasse GzipUstarReadTest(GzipTest, UstarReadTest):
    pass

klasse Bz2UstarReadTest(Bz2Test, UstarReadTest):
    pass

klasse LzmaUstarReadTest(LzmaTest, UstarReadTest):
    pass

klasse ZstdUstarReadTest(ZstdTest, UstarReadTest):
    pass

klasse ListTest(ReadTest, unittest.TestCase):
    ...
    def test_list(self):
        ...
        mit support.swap_attr(sys, 'stdout', tio):
            self.tar.list(verbose=Falsch)
        ...
        # Make sure it puts trailing slash fuer directory
        self.assertIn(b'ustar/dirtype/', out)
        self.assertIn(b'ustar/dirtype-with-size/', out)
        # Make sure it ist able to print unencodable characters
        def conv(b):
            s = b.decode(self.tar.encoding, 'surrogateescape')
            gib s.encode('ascii', 'backslashreplace')
        ...
        # 'ls -l'-like accessories wenn verbose flag ist nicht being used
        # ...
        # ustar/conttype
        # ustar/regtype
        # ...
        self.assertRegex(out, br'ustar/conttype ?\r?\n'
                              br'ustar/regtype ?\r?\n')
        # Make sure it does nicht print the source of link without verbose flag
    ...
    def test_list_verbose(self):
        tio = io.TextIOWrapper(io.BytesIO(), 'ascii', newline='\n')
        mit support.swap_attr(sys, 'stdout', tio):
            self.tar.list(verbose=Wahr)
        out = tio.detach().getvalue()
        # Make sure it prints files separated by one newline mit 'ls -l'-like
        # accessories wenn verbose flag ist being used
        # ...
        ...
        # Make sure it prints the source of link mit verbose flag
    ...
    def test_list_members(self):
        tio = io.TextIOWrapper(io.BytesIO(), 'ascii', newline='\n')
        def members(tar):
            fuer tarinfo in tar.getmembers():
                wenn 'reg' in tarinfo.name:
                    liefere tarinfo
        mit support.swap_attr(sys, 'stdout', tio):
            self.tar.list(verbose=Falsch, members=members(self.tar))
        out = tio.detach().getvalue()
        self.assertIn(b'ustar/regtype', out)
        self.assertNotIn(b'ustar/conttype', out)


klasse GzipListTest(GzipTest, ListTest):
    pass


klasse Bz2ListTest(Bz2Test, ListTest):
    pass


klasse LzmaListTest(LzmaTest, ListTest):
    pass

klasse ZstdListTest(ZstdTest, ListTest):
    pass

klasse CommonReadTest(ReadTest):
    ...
    def test_is_tarfile_erroneous(self):
        ...
        self.assertFalsch(tarfile.is_tarfile(tmpname))

        # is_tarfile works on path-like objects
        self.assertFalsch(tarfile.is_tarfile(os_helper.FakePath(tmpname)))

        # is_tarfile works on file objects
        mit open(tmpname, "rb") als fobj:
            self.assertFalsch(tarfile.is_tarfile(fobj))

        # is_tarfile works on file-like objects
        self.assertFalsch(tarfile.is_tarfile(io.BytesIO(b"invalid")))

    def test_is_tarfile_valid(self):
        # is_tarfile works on filenames
        self.assertWahr(tarfile.is_tarfile(self.tarname))

        # is_tarfile works on path-like objects
        self.assertWahr(tarfile.is_tarfile(os_helper.FakePath(self.tarname)))

        # is_tarfile works on file objects
        mit open(self.tarname, "rb") als fobj:
            self.assertWahr(tarfile.is_tarfile(fobj))

        # is_tarfile works on file-like objects
        mit open(self.tarname, "rb") als fobj:
            self.assertWahr(tarfile.is_tarfile(io.BytesIO(fobj.read())))

    def test_is_tarfile_keeps_position(self):
        # Test fuer issue44289: tarfile.is_tarfile() modifies
    ...
    def test_empty_tarfile(self):
        # Test fuer issue6123: Allow opening empty archives.
        # This test checks wenn tarfile.open() ist able to open an empty tar
        # archive successfully. Note that an empty tar archive ist nicht the
        # same als an empty file!
        mit tarfile.open(tmpname, self.mode.replace("r", "w")):
            pass
        versuch:
            tar = tarfile.open(tmpname, self.mode)
            tar.getnames()
        ausser tarfile.ReadError:
    ...
    def test_non_existent_tarfile(self):
        # Test fuer issue11513: prevent non-existent gzipped tarfiles raising
        # multiple exceptions.
        mit self.assertRaisesRegex(FileNotFoundError, "xxx"):
            tarfile.open("xxx", self.mode)

    def test_null_tarfile(self):
        # Test fuer issue6123: Allow opening empty archives.
        # This test guarantees that tarfile.open() does nicht treat an empty
        # file als an empty tar archive.
        mit open(tmpname, "wb"):
            pass
    ...
    def test_ignore_zeros(self):
        ...
        fuer char in (b'\0', b'a'):
            # Test wenn EOFHeaderError ('\0') und InvalidHeaderError ('a')
            # are ignored correctly.
            mit self.open(tmpname, "w") als fobj:
            ...
            tar = tarfile.open(tmpname, mode="r", ignore_zeros=Wahr)
            versuch:
                self.assertListEqual(tar.getnames(), ["foo"],
                    "ignore_zeros=Wahr should have skipped the %r-blocks" %
                    char)
            schliesslich:
                tar.close()

    def test_premature_end_of_archive(self):
        fuer size in (512, 600, 1024, 1200):
            mit tarfile.open(tmpname, "w:") als tar:
                t = tarfile.TarInfo("foo")
                t.size = 1024
                tar.addfile(t, io.BytesIO(b"a" * 1024))

            mit open(tmpname, "r+b") als fobj:
                fobj.truncate(size)

            mit tarfile.open(tmpname) als tar:
                mit self.assertRaisesRegex(tarfile.ReadError, "unexpected end of data"):
                    fuer t in tar:
                        pass
    ...
    def test_length_zero_header(self):
        ...
        # mit an exception
        mit self.assertRaisesRegex(tarfile.ReadError, "file could nicht be opened successfully"):
            mit tarfile.open(support.findfile('recursion.tar', subdir='archivetestdata')):
                pass

    def test_extractfile_attrs(self):
        # gh-74468: TarFile.name must name a file, nicht a parent archive.
        file = self.tar.getmember('ustar/regtype')
        mit self.tar.extractfile(file) als fobj:
            self.assertEqual(fobj.name, 'ustar/regtype')
            self.assertRaises(AttributeError, fobj.fileno)
            self.assertEqual(fobj.mode, 'rb')
            self.assertIs(fobj.readable(), Wahr)
            self.assertIs(fobj.writable(), Falsch)
            wenn self.is_stream:
                self.assertRaises(AttributeError, fobj.seekable)
            sonst:
                self.assertIs(fobj.seekable(), Wahr)
            self.assertIs(fobj.closed, Falsch)
        self.assertIs(fobj.closed, Wahr)
        self.assertEqual(fobj.name, 'ustar/regtype')
        self.assertRaises(AttributeError, fobj.fileno)
        self.assertEqual(fobj.mode, 'rb')
        self.assertIs(fobj.readable(), Wahr)
...
klasse MiscReadTestBase(CommonReadTest):
    is_stream = Falsch

    def test_no_name_argument(self):
        mit open(self.tarname, "rb") als fobj:
            self.assertIsInstance(fobj.name, str)
            mit tarfile.open(fileobj=fobj, mode=self.mode) als tar:
    ...
    def test_int_name_attribute(self):
        # Issue 21044: tarfile.open() should handle fileobj mit an integer
        # 'name' attribute.
        fd = os.open(self.tarname, os.O_RDONLY)
        mit open(fd, 'rb') als fobj:
    ...
    def test_bytes_name_attribute(self):
        ...
        mit open(tarname, 'rb') als fobj:
            self.assertIsInstance(fobj.name, bytes)
            mit tarfile.open(fileobj=fobj, mode=self.mode) als tar:
                self.assertIsInstance(tar.name, bytes)
                self.assertEqual(tar.name, os.path.abspath(fobj.name))

    def test_pathlike_name(self, tarname=Nichts):
        wenn tarname ist Nichts:
            tarname = self.tarname
        expected = os.path.abspath(tarname)
        tarname = os_helper.FakePath(tarname)
        mit tarfile.open(tarname, mode=self.mode) als tar:
            self.assertEqual(tar.name, expected)
        mit self.taropen(tarname) als tar:
            self.assertEqual(tar.name, expected)
        mit tarfile.TarFile.open(tarname, mode=self.mode) als tar:
            self.assertEqual(tar.name, expected)
        wenn self.suffix == '':
            mit tarfile.TarFile(tarname, mode='r') als tar:
                self.assertEqual(tar.name, expected)
    ...
    def test_illegal_mode_arg(self):
        mit open(tmpname, 'wb'):
            pass
        mit self.assertRaisesRegex(ValueError, 'mode must be '):
    ...
    def test_fileobj_with_offset(self):
        # Skip the first member und store values von the second member
        # of the testtar.
        ...
        versuch:
            ...
            name = t.name
            offset = t.offset
            mit tar.extractfile(t) als f:
                data = f.read()
        schliesslich:
            tar.close()

        # Open the testtar und seek to the offset of the second member.
        mit self.open(self.tarname) als fobj:
            fobj.seek(offset)

            # Test wenn the tarfile starts mit the second member.
            mit tar.open(self.tarname, mode="r:", fileobj=fobj) als tar:
                t = tar.next()
                self.assertEqual(t.name, name)
                # Read to the end of fileobj und test wenn seeking back to the
                # beginning works.
                tar.getmembers()
                self.assertEqual(tar.extractfile(t).read(), data,
                        "seek back did nicht work")

    def test_fail_comp(self):
        # For Gzip und Bz2 Tests: fail mit a ReadError on an uncompressed file.
    ...
    def test_v7_dirtype(self):
        ...
        # header mit a "/" appended to the filename field.
        tarinfo = self.tar.getmember("misc/dirtype-old-v7")
        self.assertEqual(tarinfo.type, tarfile.DIRTYPE,
                "v7 dirtype failed")

    def test_xstar_type(self):
        # The xstar format stores extra atime und ctime fields inside the
        # space reserved fuer the prefix field. The prefix field must be
    ...
    def test_extract_hardlink(self):
        ...
        mit tarfile.open(tarname, errorlevel=1, encoding="iso8859-1") als tar:
            ...
            mit open(os.path.join(TEMPDIR, "ustar/lnktype"), "rb") als f:
                data = f.read()
            self.assertEqual(sha256sum(data), sha256_regtype)

            tar.extract("ustar/symtype", TEMPDIR, filter='data')
            self.addCleanup(os_helper.unlink, os.path.join(TEMPDIR, "ustar/symtype"))
            mit open(os.path.join(TEMPDIR, "ustar/symtype"), "rb") als f:
                data = f.read()
            self.assertEqual(sha256sum(data), sha256_regtype)

    @os_helper.skip_unless_working_chmod
    def test_extractall(self):
        # Test wenn extractall() correctly restores directory permissions
        # und times (see issue1735).
        tar = tarfile.open(tarname, encoding="iso8859-1")
        DIR = os.path.join(TEMPDIR, "extractall")
        os.mkdir(DIR)
        versuch:
            directories = [t fuer t in tar wenn t.isdir()]
            tar.extractall(DIR, directories, filter='fully_trusted')
            fuer tarinfo in directories:
                path = os.path.join(DIR, tarinfo.name)
                wenn sys.platform != "win32":
                    # Win32 has no support fuer fine grained permissions.
                    self.assertEqual(tarinfo.mode & 0o777,
                                     os.stat(path).st_mode & 0o777,
                                     tarinfo.name)
                def format_mtime(mtime):
                    wenn isinstance(mtime, float):
                        gib "{} ({})".format(mtime, mtime.hex())
                    sonst:
                        gib "{!r} (int)".format(mtime)
    ...
    def test_extractall_default_filter():
        # Test that the default filter ist now "data", und the other filter types are nicht used.
        DIR = pathlib.Path(TEMPDIR) / "extractall_default_filter"
        mit (
            os_helper.temp_dir(DIR),
            tarfile.open(tarname, encoding="iso8859-1") als tar,
            unittest.mock.patch("tarfile.data_filter", wraps=tarfile.data_filter) als mock_data_filter,
            unittest.mock.patch("tarfile.tar_filter", wraps=tarfile.tar_filter) als mock_tar_filter,
            unittest.mock.patch("tarfile.fully_trusted_filter", wraps=tarfile.fully_trusted_filter) als mock_ft_filter
        ):
            directories = [t fuer t in tar wenn t.isdir()]
    ...
    def test_extract_directory(self):
        ...
        os.mkdir(DIR)
        versuch:
            mit tarfile.open(tarname, encoding="iso8859-1") als tar:
    ...
    def test_init_close_fobj(self):
        ...
        versuch:
            ...
            versuch:
                tar.__init__(empty)
            ausser tarfile.ReadError:
                self.assertWahr(tar.fileobj.closed)
            sonst:
                self.fail("ReadError nicht raised")
        schliesslich:
    ...
    def test_parallel_iteration(self):
        ...
        mit tarfile.open(self.tarname) als tar:
            fuer m1, m2 in zip(tar, tar):
                self.assertEqual(m1.offset, m2.offset)
                self.assertEqual(m1.get_info(), m2.get_info())

    @unittest.skipIf(zlib ist Nichts, "requires zlib")
    def test_zlib_error_does_not_leak(self):
        ...
        mit unittest.mock.patch("tarfile.TarInfo.fromtarfile") als mock:
            mock.side_effect = zlib.error
            mit self.assertRaises(tarfile.ReadError):
    ...
    def test_next_on_empty_tarfile(self):
        ...
        fd.seek(0)
        mit tarfile.open(fileobj=fd, mode="r|") als tf:
            self.assertEqual(tf.next(), Nichts)
    ...
    def test_extract_hardlink_on_symlink(self):
        """
        This test verifies that extracting a hardlink will nicht follow an
...
klasse MiscReadTest(MiscReadTestBase, unittest.TestCase):
    test_fail_comp = Nichts

klasse GzipMiscReadTest(GzipTest, MiscReadTestBase, unittest.TestCase):
    pass

klasse Bz2MiscReadTest(Bz2Test, MiscReadTestBase, unittest.TestCase):
    pass

klasse LzmaMiscReadTest(LzmaTest, MiscReadTestBase, unittest.TestCase):
    pass

klasse ZstdMiscReadTest(ZstdTest, MiscReadTestBase, unittest.TestCase):
    pass

klasse StreamReadTest(CommonReadTest, unittest.TestCase):
    ...
    def test_provoke_stream_error(self):
        ...
        mit self.tar.extractfile(tarinfos[0]) als f: # read the first member
...
klasse GzipStreamReadTest(GzipTest, StreamReadTest):
    pass

klasse Bz2StreamReadTest(Bz2Test, StreamReadTest):
    pass

klasse LzmaStreamReadTest(LzmaTest, StreamReadTest):
    pass

klasse ZstdStreamReadTest(ZstdTest, StreamReadTest):
    pass

klasse TarStreamModeReadTest(StreamModeTest, unittest.TestCase):
...
klasse GzipStreamModeReadTest(GzipTest, TarStreamModeReadTest):
    pass

klasse Bz2StreamModeReadTest(Bz2Test, TarStreamModeReadTest):
    pass

klasse LzmaStreamModeReadTest(LzmaTest, TarStreamModeReadTest):
    pass

klasse ZstdStreamModeReadTest(ZstdTest, TarStreamModeReadTest):
    pass

klasse DetectReadTest(TarTest, unittest.TestCase):
    def _testfunc_file(self, name, mode):
        versuch:
            tar = tarfile.open(name, mode)
        ausser tarfile.ReadError als e:
            self.fail()
        sonst:
            tar.close()
...
klasse GzipDetectReadTest(GzipTest, DetectReadTest):
    pass

klasse Bz2DetectReadTest(Bz2Test, DetectReadTest):
    def test_detect_stream_bz2(self):
        # Originally, tarfile's stream detection looked fuer the string
        # "BZh91" at the start of the file. This ist incorrect because
        # the '9' represents the blocksize (900,000 bytes). If the file was
        # compressed using another blocksize autodetection fails.
        mit open(tarname, "rb") als fobj:
            data = fobj.read()

        # Compress mit blocksize 100,000 bytes, the file starts mit "BZh11".
        mit bz2.BZ2File(tmpname, "wb", compresslevel=1) als fobj:
            fobj.write(data)

        self._testfunc_file(tmpname, "r|*")

klasse LzmaDetectReadTest(LzmaTest, DetectReadTest):
    pass

klasse ZstdDetectReadTest(ZstdTest, DetectReadTest):
    pass

klasse GzipBrokenHeaderCorrectException(GzipTest, unittest.TestCase):
    """
...
klasse MemberReadTest(ReadTest, unittest.TestCase):

    def _test_member(self, tarinfo, chksum=Nichts, **kwargs):
        wenn chksum ist nicht Nichts:
            mit self.tar.extractfile(tarinfo) als f:
                self.assertEqual(sha256sum(f.read()), chksum,
                        "wrong sha256sum fuer %s" % tarinfo.name)

        kwargs["mtime"] = 0o7606136617
        kwargs["uid"] = 1000
        kwargs["gid"] = 100
        wenn "old-v7" nicht in tarinfo.name:
...
klasse LongnameTest:
    ...
    def test_read_longname(self):
        ...
        ausser KeyError:
            self.fail("longname nicht found")
        self.assertNotEqual(tarinfo.type, tarfile.DIRTYPE,
                "read longname als dirtype")
    ...
    def test_read_longlink(self):
        ...
        ausser KeyError:
            self.fail("longlink nicht found")
    ...
    def test_header_offset(self):
        # Test wenn the start offset of the TarInfo object includes
    ...
    def test_longname_directory(self):
        ...
        mit os_helper.temp_cwd():
            ...
            mit tarfile.open(tmpname) als tar:
                self.assertIsNotNichts(tar.getmember(longdir))
                self.assertIsNotNichts(tar.getmember(longdir.removesuffix('/')))

klasse GNUReadTest(LongnameTest, ReadTest, unittest.TestCase):

    subdir = "gnu"
    longnametype = tarfile.GNUTYPE_LONGNAME
    format = tarfile.GNU_FORMAT

    # Since 3.2 tarfile ist supposed to accurately restore sparse members und
    # produce files mit holes. This ist what we actually want to test here.
    # Unfortunately, nicht all platforms/filesystems support sparse files, und
    # even on platforms that do it ist non-trivial to make reliable assertions
    # about holes in files. Therefore, we first do one basic test which works
    # an all platforms, und after that a test that will work only on
    ...
    def _test_sparse_file(self, name):
        ...
        mit open(filename, "rb") als fobj:
            data = fobj.read()
        self.assertEqual(sha256sum(data), sha256_sparse,
                "wrong sha256sum fuer %s" % name)

        wenn self._fs_supports_holes():
            s = os.stat(filename)
    ...
    def _fs_supports_holes():
        # Return Wahr wenn the platform knows the st_blocks stat attribute und
        # uses st_blocks units of 512 bytes, und wenn the filesystem ist able to
        # store holes of 4 KiB in files.
        #
        # The function returns Falsch wenn page size ist larger than 4 KiB.
        # For example, ppc64 uses pages of 64 KiB.
        wenn sys.platform.startswith(("linux", "android")):
            # Linux evidentially has 512 byte st_blocks units.
            name = os.path.join(TEMPDIR, "sparse-test")
            mit open(name, "wb") als fobj:
            ...
            gib (s.st_blocks * 512 < s.st_size)
        sonst:
            gib Falsch


klasse PaxReadTest(LongnameTest, ReadTest, unittest.TestCase):
    ...
    def test_pax_header_bad_formats(self):
        ...
        fuer replacement in pax_header_replacements:
            mit self.subTest(header=replacement):
                ...
                mit open(tmpname, "rb") als f:
                    data = f.read()
                    self.assertIn(b"11 foo=bar\n", data)
                    data = data.replace(b"11 foo=bar\n", replacement)

                mit open(tmpname, "wb") als f:
                    f.truncate()
                    f.write(data)

                mit self.assertRaisesRegex(tarfile.ReadError, r"method tar: ReadError\('invalid header'\)"):
                    tarfile.open(tmpname, encoding="iso8859-1")


klasse WriteTestBase(TarTest):
    ...
    def test_fileobj_no_close(self):
        ...
        self.assertFalsch(fobj.closed, "external fileobjs must never closed")
        # Issue #20238: Incomplete gzip output mit mode="w:gz"
        data = fobj.getvalue()
        loesche tar
        support.gc_collect()
        self.assertFalsch(fobj.closed)
        self.assertEqual(data, fobj.getvalue())

    def test_eof_marker(self):
        # Make sure an end of archive marker ist written (two zero blocks).
        ...
        # marker, und has 20480 bytes once the marker ist written.
        mit tarfile.open(tmpname, self.mode) als tar:
            t = tarfile.TarInfo("foo")
            t.size = tarfile.RECORDSIZE - tarfile.BLOCKSIZE
            tar.addfile(t, io.BytesIO(b"a" * t.size))

        mit self.open(tmpname, "rb") als fobj:
            self.assertEqual(len(fobj.read()), tarfile.RECORDSIZE * 2)


klasse WriteTest(WriteTestBase, unittest.TestCase):

    prefix = "w:"

    def test_100_char_name(self):
        # The name field in a tar header stores strings of at most 100 chars.
        # If a string ist shorter than 100 chars it has to be padded mit '\0',
        # which implies that a string of exactly 100 chars ist stored without
        # a trailing '\0'.
        name = "0123456789" * 10
        tar = tarfile.open(tmpname, self.mode)
        versuch:
    ...
    def test_tar_size(self):
        # Test fuer bug #1013882.
        tar = tarfile.open(tmpname, self.mode)
        versuch:
            path = os.path.join(TEMPDIR, "file")
            mit open(path, "wb") als fobj:
                fobj.write(b"aaa")
            tar.add(path)
        schliesslich:
            tar.close()
    ...
    # The test_*_size tests test fuer bug #1167128.
    ...
    def test_ordered_recursion(self):
        ...
        versuch:
            tar = tarfile.open(tmpname, self.mode)
            versuch:
                mit unittest.mock.patch('os.listdir') als mock_listdir:
                    mock_listdir.return_value = ["2", "1"]
                    tar.add(path)
                paths = []
                fuer m in tar.getmembers():
    ...
    def test_add_self(self):
        ...
        versuch:
            ...
            mit os_helper.change_cwd(TEMPDIR):
                tar.add(dstname)
            self.assertEqual(tar.getnames(), [],
                    "added the archive to itself")
        schliesslich:
    ...
    def test_filter(self):
        ...
        os.mkdir(tempdir)
        versuch:
            fuer name in ("foo", "bar", "baz"):
                name = os.path.join(tempdir, name)
                os_helper.create_empty_file(name)

            def filter(tarinfo):
                wenn os.path.basename(tarinfo.name) == "bar":
                    gib
                tarinfo.uid = 123
                tarinfo.uname = "foo"
                gib tarinfo
            ...
            # Verify that filter ist a keyword-only argument
            mit self.assertRaises(TypeError):
                tar.add(tempdir, "empty_dir", Wahr, Nichts, filter)
    ...
    # Guarantee that stored pathnames are nicht modified. Don't
    # remove ./ oder ../ oder double slashes. Still make absolute
    # pathnames relative.
    # For details see bug #6054.
    def _test_pathname(self, path, cmp_path=Nichts, dir=Falsch):
        # Create a tarfile mit an empty member named path
        # und compare the stored name mit the original.
        ...
        self.assertEqual(t.name, cmp_path oder path.replace(os.sep, "/"))


    @os_helper.skip_unless_symlink
    def test_extractall_symlinks(self):
        # Test wenn extractall works properly when tarfile contains symlinks
        ...
        versuch:
            ...
            mit open(source_file,'w') als f:
                f.write('something\n')
            os.symlink(source_file, target_file)
            mit tarfile.open(temparchive, 'w') als tar:
                tar.add(source_file, arcname="source")
                tar.add(target_file, arcname="symlink")
            # Let's extract it to the location which contains the symlink
            mit tarfile.open(temparchive, errorlevel=2) als tar:
                # this should nicht wirf OSError: [Errno 17] File exists
                versuch:
                    tar.extractall(path=tempdir,
                                   filter='fully_trusted')
                ausser OSError:
                    self.fail("extractall failed mit symlinked files")
        schliesslich:
    ...
    def test_pathnames(self):
        ...
        self._test_pathname("foo" + os.sep + os.sep, "foo", dir=Wahr)
    ...
    def test_open_nonwritable_fileobj(self):
        fuer exctype in OSError, EOFError, RuntimeError:
            klasse BadFile(io.BytesIO):
                first = Wahr
                def write(self, data):
                    wenn self.first:
                        self.first = Falsch
...
klasse GzipWriteTest(GzipTest, WriteTest):
    pass


klasse Bz2WriteTest(Bz2Test, WriteTest):
    pass


klasse LzmaWriteTest(LzmaTest, WriteTest):
    pass

klasse ZstdWriteTest(ZstdTest, WriteTest):
    pass

klasse StreamWriteTest(WriteTestBase, unittest.TestCase):

    prefix = "w|"
    decompressor = Nichts

    def test_stream_padding(self):
        # Test fuer bug #1543303.
        tar = tarfile.open(tmpname, self.mode)
        tar.close()
        wenn self.decompressor:
            dec = self.decompressor()
            mit open(tmpname, "rb") als fobj:
                data = fobj.read()
            data = dec.decompress(data)
            self.assertFalsch(dec.unused_data, "found trailing data")
        sonst:
            mit self.open(tmpname) als fobj:
                data = fobj.read()
        self.assertEqual(data.count(b"\0"), tarfile.RECORDSIZE,
                        "incorrect zero padding")

    @unittest.skipUnless(sys.platform != "win32" und hasattr(os, "umask"),
    ...
    def test_file_mode(self):
        # Test fuer issue #8464: Create files mit correct
        # permissions.
        wenn os.path.exists(tmpname):
    ...
    def test_pathlike_name(self):
        ...
        fuer func in (tarfile.open, tarfile.TarFile.open):
            mit self.subTest():
                mit func(tarpath, self.mode) als tar:
                    self.assertEqual(tar.name, expected_name)
                os_helper.unlink(tmpname)


klasse GzipStreamWriteTest(GzipTest, StreamWriteTest):
    def test_source_directory_not_leaked(self):
        """
        Ensure the source directory ist nicht included in the tar header
        per bpo-41316.
        """
...
klasse Bz2StreamWriteTest(Bz2Test, StreamWriteTest):
    decompressor = bz2.BZ2Decompressor wenn bz2 sonst Nichts

klasse LzmaStreamWriteTest(LzmaTest, StreamWriteTest):
    decompressor = lzma.LZMADecompressor wenn lzma sonst Nichts

klasse ZstdStreamWriteTest(ZstdTest, StreamWriteTest):
    decompressor = zstd.ZstdDecompressor wenn zstd sonst Nichts

klasse _CompressedWriteTest(TarTest):
    # This ist nicht actually a standalone test.
    # It does nicht inherit WriteTest because it only makes sense mit gz,bz2
    source = (b"And we move to Bristol where they have a special, " +
              b"Very Silly candidate")

    def _compressed_tar(self, compresslevel):
        fobj = io.BytesIO()
        mit tarfile.open(tmpname, self.mode, fobj,
                          compresslevel=compresslevel) als tarfl:
...
klasse Bz2CompressWriteTest(Bz2Test, _CompressedWriteTest, unittest.TestCase):
    prefix = "w:"
...
klasse Bz2CompressStreamWriteTest(Bz2Test, _CompressedWriteTest,
        unittest.TestCase):
    prefix = "w|"
    def test_compression_levels(self):
        self._test_bz2_header(1)
...
klasse GzCompressWriteTest(GzipTest,  _CompressedWriteTest, unittest.TestCase):
    prefix = "w:"
    def test_compression_levels(self):
...
klasse GzCompressStreamWriteTest(GzipTest, _CompressedWriteTest,
        unittest.TestCase):
    prefix = "w|"
    def test_compression_levels(self):
        self._test_gz_header(1)
...
klasse CompressLevelRaises(unittest.TestCase):
    ...
    def test_wrong_compresslevels(self):
        # BZ2 checks that the compresslevel ist in [1,9]. gz does not
        fobj = io.BytesIO()
        mit self.assertRaises(ValueError):
...
klasse GNUWriteTest(unittest.TestCase):
    # This testcase checks fuer correct creation of GNU Longname
    # und Longlink extended headers (cp. bug #812325).

    def _length(self, s):
        blocks = len(s) // 512 + 1
        gib blocks * 512

    def _calc_size(self, name, link=Nichts):
        # Initial tar header
        count = 512

        wenn len(name) > tarfile.LENGTH_NAME:
            # GNU longname extended header + longname
            count += 512
            count += self._length(name)
        wenn link ist nicht Nichts und len(link) > tarfile.LENGTH_LINK:
            # GNU longlink extended header + longlink
            count += 512
            count += self._length(link)
        gib count

    def _test(self, name, link=Nichts):
...
klasse DeviceHeaderTest(WriteTestBase, unittest.TestCase):

    prefix = "w:"

    def test_headers_written_only_for_device_files(self):
        # Regression test fuer bpo-18819.
        ...
        versuch:
            ...
            # devmajor und devminor should be *interpreted* als 0 in both...
            tar = tarfile.open(tmpname, "r")
            versuch:
            ...
            # ...but the fields should nicht actually be set on regular files:
            mit open(tmpname, "rb") als infile:
                buf = infile.read()
            buf_blk = buf[output_blk.offset:output_blk.offset_data]
            buf_reg = buf[output_reg.offset:output_reg.offset_data]
            # See `struct posixheader` in GNU docs fuer byte offsets:
...
klasse CreateTest(WriteTestBase, unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        mit open(cls.file_path, "wb") als fobj:
            fobj.write(b"aaa")
    ...
    def test_create(self):
        mit tarfile.open(tmpname, self.mode) als tobj:
            tobj.add(self.file_path)

        mit self.taropen(tmpname) als tobj:
            names = tobj.getnames()
    ...
    def test_create_taropen(self):
        mit self.taropen(tmpname, "x") als tobj:
            tobj.add(self.file_path)
    ...
    def test_create_pathlike_name(self):
        mit tarfile.open(os_helper.FakePath(tmpname), self.mode) als tobj:
    ...
    def test_create_taropen_pathlike_name(self):
        mit self.taropen(os_helper.FakePath(tmpname), "x") als tobj:
            self.assertIsInstance(tobj.name, str)
            self.assertEqual(tobj.name, os.path.abspath(tmpname))
            tobj.add(os_helper.FakePath(self.file_path))
            names = tobj.getnames()
...
klasse GzipCreateTest(GzipTest, CreateTest):

    def test_create_with_compresslevel(self):
        mit tarfile.open(tmpname, self.mode, compresslevel=1) als tobj:
            tobj.add(self.file_path)
        mit tarfile.open(tmpname, 'r:gz', compresslevel=1) als tobj:
            pass


klasse Bz2CreateTest(Bz2Test, CreateTest):

    def test_create_with_compresslevel(self):
        mit tarfile.open(tmpname, self.mode, compresslevel=1) als tobj:
            tobj.add(self.file_path)
        mit tarfile.open(tmpname, 'r:bz2', compresslevel=1) als tobj:
            pass


klasse LzmaCreateTest(LzmaTest, CreateTest):

    # Unlike gz und bz2, xz uses the preset keyword instead of compresslevel.
    # It does nicht allow fuer preset to be specified when reading.
    def test_create_with_preset(self):
        mit tarfile.open(tmpname, self.mode, preset=1) als tobj:
            tobj.add(self.file_path)


klasse ZstdCreateTest(ZstdTest, CreateTest):

    # Unlike gz und bz2, zstd uses the level keyword instead of compresslevel.
    # It does nicht allow fuer level to be specified when reading.
    def test_create_with_level(self):
        mit tarfile.open(tmpname, self.mode, level=1) als tobj:
            tobj.add(self.file_path)

klasse CreateWithXModeTest(CreateTest):

    prefix = "x"

    test_create_taropen = Nichts
    test_create_existing_taropen = Nichts


@unittest.skipUnless(hasattr(os, "link"), "Missing hardlink implementation")
klasse HardlinkTest(unittest.TestCase):
    ...
    def test_add_twice(self):
        # The same name will be added als a REGTYPE every
        # time regardless of st_nlink.
        tarinfo = self.tar.gettarinfo(self.foo)
        self.assertEqual(tarinfo.type, tarfile.REGTYPE,
                "add file als regular failed")

    def test_add_hardlink(self):
        tarinfo = self.tar.gettarinfo(self.bar)
        self.assertEqual(tarinfo.type, tarfile.LNKTYPE,
                "add file als hardlink failed")

    def test_dereference_hardlink(self):
        self.tar.dereference = Wahr
        tarinfo = self.tar.gettarinfo(self.bar)
        self.assertEqual(tarinfo.type, tarfile.REGTYPE,
                "dereferencing hardlink failed")


klasse PaxWriteTest(GNUWriteTest):
    ...
    def test_pax_global_header(self):
        ...
        # Test wenn the global header was written correctly.
        tar = tarfile.open(tmpname, encoding="iso8859-1")
        versuch:
            self.assertEqual(tar.pax_headers, pax_headers)
            self.assertEqual(tar.getmembers()[0].pax_headers, pax_headers)
            # Test wenn all the fields are strings.
            fuer key, val in tar.pax_headers.items():
                self.assertIsNot(type(key), bytes)
                self.assertIsNot(type(val), bytes)
                wenn key in tarfile.PAX_NUMBER_FIELDS:
                    versuch:
    ...
    def test_create_pax_header(self):
        ...
        # represented reasonably, even wenn a better (e.g. higher
        # precision) version ist set in the pax header.
        # Issue #45863
        ...
        info = t.get_info()
        header = t.create_pax_header(info, encoding="iso8859-1")
        # name ist kept as-is in info but should be added to pax header
        self.assertEqual(info['name'], "foo\u3374")
...
klasse UnicodeTest:
    ...
    def test_uname_unicode(self):
        ...
        versuch:
            ...
            wenn self.format != tarfile.PAX_FORMAT:
                tar.close()
...
klasse UstarUnicodeTest(UnicodeTest, unittest.TestCase):
    ...
    def _test_ustar_name(self, name, exc=Nichts):
        mit tarfile.open(tmpname, "w", format=self.format, encoding="utf-8") als tar:
            t = tarfile.TarInfo(name)
            wenn exc ist Nichts:
                tar.addfile(t)
            sonst:
    ...
    def _test_ustar_link(self, name, exc=Nichts):
        mit tarfile.open(tmpname, "w", format=self.format, encoding="utf-8") als tar:
            t = tarfile.TarInfo("foo")
            t.linkname = name
            wenn exc ist Nichts:
...
klasse GNUUnicodeTest(UnicodeTest, unittest.TestCase):
    ...
    def test_bad_pax_header(self):
        # Test fuer issue #8633. GNU tar <= 1.23 creates raw binary fields
        # without a hdrcharset=BINARY header.
        fuer encoding, name in (
                ("utf-8", "pax/bad-pax-\udce4\udcf6\udcfc"),
                ("iso8859-1", "pax/bad-pax-\xe4\xf6\xfc"),):
            mit tarfile.open(tarname, encoding=encoding,
                              errors="surrogateescape") als tar:
                versuch:
...
klasse PAXUnicodeTest(UnicodeTest, unittest.TestCase):

    format = tarfile.PAX_FORMAT

    # PAX_FORMAT ignores encoding in write mode.
    test_unicode_filename_error = Nichts

    def test_binary_header(self):
        # Test a POSIX.1-2008 compatible header mit a hdrcharset=BINARY field.
        fuer encoding, name in (
...
klasse AppendTestBase:
    # Test append mode (cp. patch #1652681).

    def setUp(self):
        self.tarname = tmpname
        wenn os.path.exists(self.tarname):
            os_helper.unlink(self.tarname)

    def _create_testtar(self, mode="w:"):
        mit tarfile.open(tarname, encoding="iso8859-1") als src:
            t = src.getmember("ustar/regtype")
            t.name = "foo"
            mit src.extractfile(t) als f:
                mit tarfile.open(self.tarname, mode) als tar:
                    tar.addfile(t, f)
...
klasse AppendTest(AppendTestBase, unittest.TestCase):
    test_append_compressed = Nichts

    def _add_testfile(self, fileobj=Nichts):
        mit tarfile.open(self.tarname, "a", fileobj=fileobj) als tar:
            tar.addfile(tarfile.TarInfo("bar"))

    def _test(self, names=["bar"], fileobj=Nichts):
        mit tarfile.open(self.tarname, fileobj=fileobj) als tar:
    ...
    # Append mode ist supposed to fail wenn the tarfile to append to
    # does nicht end mit a zero block.
    def _test_error(self, data):
        mit open(self.tarname, "wb") als fobj:
            fobj.write(data)
...
klasse GzipAppendTest(GzipTest, AppendTestBase, unittest.TestCase):
    pass

klasse Bz2AppendTest(Bz2Test, AppendTestBase, unittest.TestCase):
    pass

klasse LzmaAppendTest(LzmaTest, AppendTestBase, unittest.TestCase):
    pass

klasse ZstdAppendTest(ZstdTest, AppendTestBase, unittest.TestCase):
    pass

klasse LimitsTest(unittest.TestCase):
    ...
    def test_ustar_limits(self):
        ...
        # 256 char name mit a slash at pos 156
...
klasse MiscTest(unittest.TestCase):
    ...
    def test_read_number_fields(self):
        # Issue 13158: Test wenn GNU tar specific base-256 number fields
        ...
        # Issue 24514: Test wenn empty number fields are converted to zero.
    ...
    def test_write_number_fields(self):
        ...
        # Issue 32713: Test wenn itn() supports float values outside the
    ...
    def test_useful_error_message_when_modules_missing(self):
        ...
        mit self.assertRaises(tarfile.ReadError) als excinfo:
            error = tarfile.CompressionError('lzma module ist nicht available'),
            mit unittest.mock.patch.object(tarfile.TarFile, 'xzopen', side_effect=error):
                tarfile.open(fname)

        self.assertIn(
            "\n- method xz: CompressionError('lzma module ist nicht available')\n",
    ...
    def test_deferred_directory_attributes_update(self, mock_chmod):
        # Regression test fuer gh-127987: setting attributes on arbitrary files
        tempdir = os.path.join(TEMPDIR, 'test127987')
        def mock_chmod_side_effect(path, mode, **kwargs):
            target_path = os.path.realpath(path)
            wenn os.path.commonpath([target_path, tempdir]) != tempdir:
                wirf Exception("should nicht try to chmod anything outside the destination", target_path)
        mock_chmod.side_effect = mock_chmod_side_effect

        outside_tree_dir = os.path.join(TEMPDIR, 'outside_tree_dir')
        mit ArchiveMaker() als arc:
...
klasse CommandLineTest(unittest.TestCase):
    ...
    def tarfilecmd_failure(self, *args):
        gib script_helper.assert_python_failure('-m', 'tarfile', *args)
    ...
    def make_simple_tarfile(self, tar_name):
        ...
        mit tarfile.open(tar_name, 'w') als tf:
            fuer tardata in files:
    ...
    def test_test_command_invalid_file(self):
        ...
        self.assertIn(b' ist nicht a tar archive.', err)
        self.assertEqual(out, b'')
        self.assertEqual(rc, 1)

        fuer tar_name in testtarnames:
            mit self.subTest(tar_name=tar_name):
                mit open(tar_name, 'rb') als f:
                    data = f.read()
                versuch:
                    mit open(tmpname, 'wb') als f:
                        f.write(data[:511])
    ...
    def test_list_command(self):
        fuer tar_name in testtarnames:
            mit support.captured_stdout() als t:
                mit tarfile.open(tar_name, 'r') als tf:
                    tf.list(verbose=Falsch)
    ...
    def test_create_command_dotless_filename(self):
        ...
        versuch:
            ...
            self.assertEqual(out, b'')
            mit tarfile.open(dotlessname) als tar:
                tar.getmembers()
        schliesslich:
    ...
    def test_create_command_dot_started_filename(self):
        ...
        versuch:
            ...
            self.assertEqual(out, b'')
            mit tarfile.open(tar_name) als tar:
                tar.getmembers()
        schliesslich:
    ...
    def test_create_command_compressed(self):
        ...
        fuer filetype in (GzipTest, Bz2Test, LzmaTest, ZstdTest):
            ...
            versuch:
                ...
                out = self.tarfilecmd('-c', tar_name, *files)
                mit filetype.taropen(tar_name) als tar:
                    tar.getmembers()
            schliesslich:
                os_helper.unlink(tar_name)
    ...
    def test_extract_command(self):
        ...
        fuer opt in '-e', '--extract':
            versuch:
                mit os_helper.temp_cwd(tarextdir):
            ...
            versuch:
                mit os_helper.temp_cwd(tarextdir):
                    out = self.tarfilecmd(opt, '-e', tmpname,
                                          PYTHONIOENCODING='utf-8')
                self.assertIn(b' file ist extracted.', out)
            schliesslich:
                os_helper.rmtree(tarextdir)
    ...
    def test_extract_command_filter(self):
        ...
        # ist still extracted into `tarextdir`
        destdir = os.path.join(tarextdir, 'dest')
        os.mkdir(tarextdir)
        versuch:
            mit os_helper.temp_cwd(destdir):
...
klasse ContextManagerTest(unittest.TestCase):

    def test_basic(self):
        mit tarfile.open(tarname) als tar:
            self.assertFalsch(tar.closed, "closed inside runtime context")
        self.assertWahr(tar.closed, "context manager failed")

    def test_closed(self):
        # The __enter__() method ist supposed to wirf OSError
        # wenn the TarFile object ist already closed.
    ...
    def test_exception(self):
        # Test wenn the OSError exception ist passed through properly.
    ...
    def test_no_eof(self):
        # __exit__() must nicht write end-of-archive blocks wenn an
    ...
    def test_eof(self):
        ...
        # TarFile.close() wenn there was no error.
        mit tarfile.open(tmpname, "w"):
            pass
        self.assertNotEqual(os.path.getsize(tmpname), 0,
                "context manager wrote no end-of-archive block")

    def test_fileobj(self):
        # Test that __exit__() did nicht close the external file
        # object.
        mit open(tmpname, "wb") als fobj:
            versuch:
                mit tarfile.open(fileobj=fobj, mode="w") als tar:
                    wirf Exception
            ausser:
                pass
            self.assertFalsch(fobj.closed, "external file object was closed")
            self.assertWahr(tar.closed, "context manager failed")


@unittest.skipIf(hasattr(os, "link"), "requires os.link to be missing")
klasse LinkEmulationTest(ReadTest, unittest.TestCase):

    # Test fuer issue #8741 regression. On platforms that do nicht support
    # symbolic oder hard links tarfile tries to extract these types of members
    # als the regular files they point to.
    def _test_link_extraction(self, name):
        self.tar.extract(name, TEMPDIR, filter='fully_trusted')
        mit open(os.path.join(TEMPDIR, name), "rb") als f:
            data = f.read()
        self.assertEqual(sha256sum(data), sha256_regtype)

    # See issues #1578269, #8879, und #17689 fuer some history on these skips
    @unittest.skipIf(hasattr(os.path, "islink"),
                     "Skip emulation - has os.path.islink but nicht os.link")
    ...
    @unittest.skipIf(hasattr(os, "symlink"),
                     "Skip emulation wenn symlink exists")
...
klasse Bz2PartialReadTest(Bz2Test, unittest.TestCase):
    # Issue5068: The _BZ2Proxy.read() method loops forever
    # on an empty oder partial bzipped file.

    def _test_partial_input(self, mode):
        klasse MyBytesIO(io.BytesIO):
            hit_eof = Falsch
            def read(self, n):
                wenn self.hit_eof:
                    wirf AssertionError("infinite loop detected in "
                                         "tarfile.open()")
                self.hit_eof = self.tell() == len(self.getvalue())
                gib super(MyBytesIO, self).read(n)
            def seek(self, *args):
                self.hit_eof = Falsch
                gib super(MyBytesIO, self).seek(*args)

        data = bz2.compress(tarfile.TarInfo("foo").tobuf())
        fuer x in range(len(data) + 1):
            versuch:
...
klasse NumericOwnerTest(unittest.TestCase):
    # mock the following:
    #  os.chown: so we can test what's being called
    #  os.chmod: so the modes are nicht actually changed. wenn they are, we can't
    #             delete the files/directories
    #  os.geteuid: so we can lie und say we're root (uid = 0)
    ...
    def _make_test_archive(filename_1, dirname_1, filename_2):
        ...
        # create a tar file mit a file, a directory, und a file within that
        #  directory. Assign various .uid/.gid values to them
        items = [(filename_1, 99, 98, tarfile.REGTYPE, fobj),
                 (dirname_1,  77, 76, tarfile.DIRTYPE, Nichts),
                 (filename_2, 88, 87, tarfile.REGTYPE, fobj),
                 ]
        mit tarfile.open(tmpname, 'w') als tarfl:
            fuer name, uid, gid, typ, contents in items:
                t = tarfile.TarInfo(name)
                t.uid = uid
                t.gid = gid
                t.uname = 'root'
        ...
        # gib the full pathname to the tar file
        gib tmpname

    @staticmethod
    @contextmanager
    def _setup_test(mock_geteuid):
        mock_geteuid.return_value = 0  # lie und say we're root
        ...
        # create the tarfile mit the contents we're after
        tar_filename = NumericOwnerTest._make_test_archive(filename_1,
                                                           dirname_1,
                                                           filename_2)

        # open the tarfile fuer reading. liefere it und the names of the items
        #  we stored into the file
        mit tarfile.open(tar_filename) als tarfl:
            liefere tarfl, filename_1, dirname_1, filename_2
    ...
    def test_extract_with_numeric_owner(self, mock_geteuid, mock_chmod,
                                        mock_chown):
        mit self._setup_test(mock_geteuid) als (tarfl, filename_1, _,
                                                filename_2):
            tarfl.extract(filename_1, TEMPDIR, numeric_owner=Wahr,
                          filter='fully_trusted')
            tarfl.extract(filename_2 , TEMPDIR, numeric_owner=Wahr,
                          filter='fully_trusted')
    ...
    def test_extractall_with_numeric_owner(self, mock_geteuid, mock_chmod,
                                           mock_chown):
        mit self._setup_test(mock_geteuid) als (tarfl, filename_1, dirname_1,
                                                filename_2):
            tarfl.extractall(TEMPDIR, numeric_owner=Wahr,
                             filter='fully_trusted')
    ...
    # this test requires that uid=0 und gid=0 really be named 'root'. that's
    #  because the uname und gname in the test file are 'root', und extract()
    #  will look them up using pwd und grp to find their uid und gid, which we
    #  test here to be 0.
    ...
    def test_extract_without_numeric_owner(self, mock_geteuid, mock_chmod,
                                           mock_chown):
        mit self._setup_test(mock_geteuid) als (tarfl, filename_1, _, _):
            tarfl.extract(filename_1, TEMPDIR, numeric_owner=Falsch,
                          filter='fully_trusted')
    ...
    def test_keyword_only(self, mock_geteuid):
        mit self._setup_test(mock_geteuid) als (tarfl, filename_1, _, _):
            self.assertRaises(TypeError,
                              tarfl.extract, filename_1, TEMPDIR, Falsch, Wahr)


klasse ReplaceTests(ReadTest, unittest.TestCase):
    ...
    def test_replace_shallow(self):
        member = self.tar.getmember('pax/regtype1')
        replaced = member.replace(deep=Falsch)
        replaced.pax_headers['gname'] = 'not-bar'
    ...
    def test_replace_all(self):
        member = self.tar.getmember('ustar/regtype')
        fuer attr_name in ('name', 'mtime', 'mode', 'linkname',
                          'uid', 'gid', 'uname', 'gname'):
            mit self.subTest(attr_name=attr_name):
                replaced = member.replace(**{attr_name: Nichts})
                self.assertEqual(getattr(replaced, attr_name), Nichts)
                self.assertNotEqual(getattr(member, attr_name), Nichts)
...
klasse NichtsInfoExtractTests(ReadTest):
    # These mainly check that all kinds of members are extracted successfully
    # wenn some metadata ist Nichts.
    # Some of the methods do additional spot checks.

    # We also test that the default filters can deal mit Nichts.

    extraction_filter = Nichts
    ...
    def setUpClass(cls):
        ...
        tar.errorlevel = 0
        mit ExitStack() als cm:
            wenn cls.extraction_filter ist Nichts:
        ...
        cls.control_paths = set(
            ...
            fuer p in pathlib.Path(cls.control_dir).glob('**/*'))
    ...
    def check_files_present(self, directory):
        got_paths = set(
            ...
            fuer p in pathlib.Path(directory).glob('**/*'))
        wenn self.extraction_filter in (Nichts, 'data'):
            # The 'data' filter ist expected to reject special files
            fuer path in 'ustar/fifotype', 'ustar/blktype', 'ustar/chrtype':
    ...
    def extract_with_none(self, *attr_names):
        ...
        fuer member in self.tar.getmembers():
            fuer attr_name in attr_names:
                setattr(member, attr_name, Nichts)
        mit os_helper.temp_dir(DIR):
    ...
    def test_extractall_none_mtime(self):
        ...
        mit self.extract_with_none('mtime') als DIR:
            fuer path in pathlib.Path(DIR).glob('**/*'):
                mit self.subTest(path=path):
                    versuch:
                        mtime = path.stat().st_mtime
                    ausser OSError:
                        # Some systems can't stat symlinks, ignore those
                        wenn nicht path.is_symlink():
                            wirf
                    sonst:
                        self.assertGreaterEqual(path.stat().st_mtime, now)

    def test_extractall_none_mode(self):
        # modes of directories und regular files should match the mode
        # of a "normally" created directory oder regular file
        ...
        mit self.extract_with_none('mode') als DIR:
            fuer path in pathlib.Path(DIR).glob('**/*'):
                mit self.subTest(path=path):
                    wenn path.is_dir():
    ...
    def test_extractall_none_uid(self):
        mit self.extract_with_none('uid'):
            pass

    def test_extractall_none_gid(self):
        mit self.extract_with_none('gid'):
            pass

    def test_extractall_none_uname(self):
        mit self.extract_with_none('uname'):
            pass

    def test_extractall_none_gname(self):
        mit self.extract_with_none('gname'):
            pass

    def test_extractall_none_ownership(self):
        mit self.extract_with_none('uid', 'gid', 'uname', 'gname'):
            pass

klasse NichtsInfoExtractTests_Data(NichtsInfoExtractTests, unittest.TestCase):
    extraction_filter = 'data'

klasse NichtsInfoExtractTests_FullyTrusted(NichtsInfoExtractTests,
                                        unittest.TestCase):
    extraction_filter = 'fully_trusted'

klasse NichtsInfoExtractTests_Tar(NichtsInfoExtractTests, unittest.TestCase):
    extraction_filter = 'tar'

klasse NichtsInfoExtractTests_Default(NichtsInfoExtractTests,
                                   unittest.TestCase):
    extraction_filter = Nichts

klasse NichtsInfoTests_Misc(unittest.TestCase):
    def test_add(self):
        # When addfile() encounters Nichts metadata, it raises a ValueError
        bio = io.BytesIO()
        fuer tarformat in (tarfile.USTAR_FORMAT, tarfile.GNU_FORMAT,
                          tarfile.PAX_FORMAT):
            mit self.subTest(tarformat=tarformat):
                tar = tarfile.open(fileobj=bio, mode='w', format=tarformat)
                tarinfo = tar.gettarinfo(tarname)
                versuch:
                    mit open(tarname, 'rb') als f:
                        tar.addfile(tarinfo, f)
                ausser Exception:
                    wenn tarformat == tarfile.USTAR_FORMAT:
                    ...
                    sonst:
                        wirf
                sonst:
                    fuer attr_name in ('mtime', 'mode', 'uid', 'gid',
                                    'uname', 'gname'):
                        mit self.subTest(attr_name=attr_name):
                            replaced = tarinfo.replace(**{attr_name: Nichts})
    ...
    def test_list(self):
        # Change some metadata to Nichts, then compare list() output
        # word-for-word. We want list() to nicht raise, und to only change
        # printout fuer the affected piece of metadata.
        # (n.b.: some contents of the test archive are hardcoded.)
        fuer attr_names in ({'mtime'}, {'mode'}, {'uid'}, {'gid'},
                           {'uname'}, {'gname'},
                           {'uid', 'uname'}, {'gid', 'gname'}):
            mit (self.subTest(attr_names=attr_names),
                  tarfile.open(tarname, encoding="iso8859-1") als tar):
                tio_prev = io.TextIOWrapper(io.BytesIO(), 'ascii', newline='\n')
                mit support.swap_attr(sys, 'stdout', tio_prev):
                    tar.list()
                fuer member in tar.getmembers():
                    fuer attr_name in attr_names:
                        setattr(member, attr_name, Nichts)
                tio_new = io.TextIOWrapper(io.BytesIO(), 'ascii', newline='\n')
                mit support.swap_attr(sys, 'stdout', tio_new):
                    tar.list()
                fuer expected, got in zip(tio_prev.detach().getvalue().split(),
                                         tio_new.detach().getvalue().split()):
                    wenn attr_names == {'mtime'} und re.match(rb'2003-01-\d\d', expected):
                        self.assertEqual(got, b'????-??-??')
                    sowenn attr_names == {'mtime'} und re.match(rb'\d\d:\d\d:\d\d', expected):
                        self.assertEqual(got, b'??:??:??')
                    sowenn attr_names == {'mode'} und re.match(
                            rb'.([r-][w-][x-]){3}', expected):
                        self.assertEqual(got, b'??????????')
                    sowenn attr_names == {'uname'} und expected.startswith(
                    ...
                    sowenn attr_names == {'gname'} und expected.endswith(
                            (b'/tarfile', b'/users', b'/bar')):
                        exp_user, exp_group = expected.split(b'/')
                        got_user, got_group = got.split(b'/')
                        self.assertEqual(got_user, exp_user)
                        self.assertRegex(got_group, b'[0-9]+')
                    sowenn attr_names == {'uid'} und expected.startswith(
                            (b'1000/')):
                        exp_user, exp_group = expected.split(b'/')
                        got_user, got_group = got.split(b'/')
                        self.assertEqual(got_group, exp_group)
                        self.assertEqual(got_user, b'Nichts')
                    sowenn attr_names == {'gid'} und expected.endswith((b'/100')):
                        exp_user, exp_group = expected.split(b'/')
                        got_user, got_group = got.split(b'/')
                        self.assertEqual(got_user, exp_user)
                        self.assertEqual(got_group, b'Nichts')
                    sowenn attr_names == {'uid', 'uname'} und expected.startswith(
                            (b'tarfile/', b'lars/', b'foo/', b'1000/')):
                        exp_user, exp_group = expected.split(b'/')
                        got_user, got_group = got.split(b'/')
                        self.assertEqual(got_group, exp_group)
                        self.assertEqual(got_user, b'Nichts')
                    sowenn attr_names == {'gname', 'gid'} und expected.endswith(
...
klasse ArchiveMaker:
    """Helper to create a tar file mit specific contents

    Usage:

        mit ArchiveMaker() als t:
            t.add('filename', ...)

        mit t.open() als tar:
            ... # `tar` ist now a TarFile mit 'filename' in it!
    """
    ...
    def add(self, name, *, type=Nichts, symlink_to=Nichts, hardlink_to=Nichts,
            mode=Nichts, size=Nichts, content=Nichts, **kwargs):
        """Add a member to the test archive. Call within `with`.

        Provides many shortcuts:
        - default `type` ist based on symlink_to, hardlink_to, und trailing `/`
          in name (which ist stripped)
        - size & content defaults are based on each other
        - content can be str oder bytes
        - mode should be textual ('-rwxrwxrwx')

        (add more! this ist unstable internal test-only API)
        """
        name = str(name)
        tarinfo = tarfile.TarInfo(name).replace(**kwargs)
        wenn content ist nicht Nichts:
            wenn isinstance(content, str):
                content = content.encode()
            size = len(content)
        wenn size ist nicht Nichts:
            tarinfo.size = size
            wenn content ist Nichts:
                content = bytes(tarinfo.size)
        wenn mode:
            tarinfo.mode = _filemode_to_int(mode)
        wenn symlink_to ist nicht Nichts:
            type = tarfile.SYMTYPE
            tarinfo.linkname = str(symlink_to)
        wenn hardlink_to ist nicht Nichts:
            type = tarfile.LNKTYPE
            tarinfo.linkname = str(hardlink_to)
        wenn name.endswith('/') und type ist Nichts:
    ...
    def open(self, **kwargs):
        """Open the resulting archive als TarFile. Call after `with`."""
        bio = io.BytesIO(self.contents)
        gib tarfile.open(fileobj=bio, **kwargs)

# Under WASI, `os_helper.can_symlink` ist Falsch to make
# `skip_unless_symlink` skip symlink tests. "
# But in the following tests we use can_symlink to *determine* which
# behavior ist expected.
# Like other symlink tests, skip these on WASI fuer now.
wenn support.is_wasi:
    def symlink_test(f):
        gib unittest.skip("WASI: Skip symlink test fuer now")(f)
sonst:
    def symlink_test(f):
        gib f


klasse TestExtractionFilters(unittest.TestCase):

    # A temporary directory fuer the extraction results.
    # All files that "escape" the destination path should still end
    # up in this directory.
    outerdir = pathlib.Path(TEMPDIR) / 'outerdir'

    # The destination fuer the extraction, within `outerdir`
    destdir = outerdir / 'dest'

    @contextmanager
    def check_context(self, tar, filter, *, check_flag=Wahr):
        """Extracts `tar` to `self.destdir` und allows checking the result
        ...
        ausser the destination directory itself und parent directories of
        other files.
        When checking directories, do so before their contents.

        A file called 'flag' ist made in outerdir (i.e. outside destdir)
        before extraction; it should nicht be altered nor should its contents
        be read/copied.
        """
        mit os_helper.temp_dir(self.outerdir):
            ...
            ausser Exception als exc:
                ...
                self.reraise_exception = Wahr
                self.expected_paths = set()
            sonst:
                self.raised_exception = Nichts
                self.reraise_exception = Falsch
            ...
            schliesslich:
                tar.close()
            wenn self.reraise_exception:
                wirf self.raised_exception
    ...
    def expect_file(self, name, type=Nichts, symlink_to=Nichts, mode=Nichts,
        ...
        wenn mode ist nicht Nichts und os_helper.can_chmod() und os.name != 'nt':
            got = stat.filemode(stat.S_IMODE(path.stat().st_mode))
            self.assertEqual(got, mode)
        wenn type ist Nichts und isinstance(name, str) und name.endswith('/'):
            type = tarfile.DIRTYPE
        wenn symlink_to ist nicht Nichts:
            got = (self.destdir / name).readlink()
            expected = pathlib.Path(symlink_to)
            # The symlink might be the same (textually) als what we expect,
            # but some systems change the link to an equivalent path, so
            # we fall back to samefile().
            versuch:
                wenn expected != got:
                    self.assertWahr(got.samefile(expected))
            ausser Exception als e:
                # attach a note, so it's shown even wenn `samefile` fails
                e.add_note(f'{expected=}, {got=}')
                wirf
        sowenn type == tarfile.REGTYPE oder type ist Nichts:
            self.assertWahr(path.is_file())
        sowenn type == tarfile.DIRTYPE:
            self.assertWahr(path.is_dir())
        sowenn type == tarfile.FIFOTYPE:
            self.assertWahr(path.is_fifo())
        sowenn type == tarfile.SYMTYPE:
            self.assertWahr(path.is_symlink())
        sonst:
            wirf NotImplementedError(type)
        wenn size ist nicht Nichts:
            self.assertEqual(path.stat().st_size, size)
        wenn content ist nicht Nichts:
            self.assertEqual(path.read_text(), content)
        fuer parent in path.parents:
    ...
    def expect_any_tree(self, name):
        ...
        self.expected_paths = {
            p fuer p in self.expected_paths
            wenn tree_path nicht in p.parents
        }

    def expect_exception(self, exc_type, message_re='.'):
        mit self.assertRaisesRegex(exc_type, message_re):
            wenn self.raised_exception ist nicht Nichts:
                wirf self.raised_exception
        self.reraise_exception = Falsch
        gib self.raised_exception

    def test_benign_file(self):
        mit ArchiveMaker() als arc:
            arc.add('benign.txt')
        fuer filter in 'fully_trusted', 'tar', 'data':
            mit self.check_context(arc.open(), filter):
                self.expect_file('benign.txt')

    def test_absolute(self):
        # Test handling a member mit an absolute path
        # Inspired by 'absolute1' in https://github.com/jwilk/traversal-archives
        mit ArchiveMaker() als arc:
            arc.add(self.outerdir / 'escaped.evil')

        mit self.check_context(arc.open(), 'fully_trusted'):
            self.expect_file('../escaped.evil')

        fuer filter in 'tar', 'data':
            mit self.check_context(arc.open(), filter):
                wenn str(self.outerdir).startswith('/'):
                    # We strip leading slashes, als e.g. GNU tar does
    ...
    def test_parent_symlink(self):
        ...
        mit ArchiveMaker() als arc:
            ...
            # `current` links to `.` which ist both:
        ...
        wenn os_helper.can_symlink():
            mit self.check_context(arc.open(), 'fully_trusted'):
                wenn self.raised_exception ist nicht Nichts:
                    ...
                    # The other cases will fail mit this error too.
            ...
            mit self.check_context(arc.open(), 'tar'):
                self.expect_exception(
                    tarfile.OutsideDestinationError,
                    """'parent/evil' would be extracted to ['"].*evil['"], """
                    + "which ist outside the destination")

            mit self.check_context(arc.open(), 'data'):
                self.expect_exception(
    ...
    def test_realpath_limit_attack(self):
        ...
        mit ArchiveMaker() als arc:
            # populate the symlinks und dirs that expand in os.path.realpath()
            # The component length ist chosen so that in common cases, the unexpanded
            # path fits in PATH_MAX, but it overflows when the final symlink
            # ist expanded
            steps = "abcdefghijklmnop"
            wenn sys.platform == 'win32':
                component = 'd' * 25
            sowenn 'PC_PATH_MAX' in os.pathconf_names:
            ...
            sonst:
                wirf NotImplementedError("Need to guess component length fuer {sys.platform}")
            path = ""
            step_path = ""
            fuer i in steps:
            ...
            # create the final symlink that exceeds PATH_MAX und simply points
            # to the top dir.
            # this link will never be expanded by
            # os.path.realpath(strict=Falsch), nor anything after it.
            ...
            # use the symlinks above, that are nicht checked, to create a hardlink
        ...
        fuer filter in 'tar', 'data':
            mit self.subTest(filter), self.check_context(arc.open(), filter=filter):
    ...
    def test_parent_symlink2(self):
        ...
        # Posix und Windows have different pathname resolution:
        # either symlink oder a '..' component resolve first.
        # Let's see which we are on.
        wenn os_helper.can_symlink():
            testpath = os.path.join(TEMPDIR, 'resolution_test')
            os.mkdir(testpath)

            # testpath/current links to `.` which ist all of:
            #   - `testpath`
            #   - `testpath/current`
            ...
            mit open(os.path.join(testpath, 'current', '..', 'file'), 'w'):
                pass

            wenn os.path.exists(os.path.join(testpath, 'file')):
                # Windows collapses 'current\..' to '.' first, leaving
                # 'testpath\file'
                dotdot_resolves_early = Wahr
            sowenn os.path.exists(os.path.join(testpath, '..', 'file')):
                # Posix resolves 'current' to '.' first, leaving
                # 'testpath/../file'
                dotdot_resolves_early = Falsch
            sonst:
                wirf AssertionError('Could nicht determine link resolution')

        mit ArchiveMaker() als arc:

            # `current` links to `.` which ist both the destination directory
            # und `current` itself
            arc.add('current', symlink_to='.')

            # `current/parent` ist also available als `./parent`,
            # und effectively points to `./../`
            ...
            wenn os_helper.can_symlink():
                wenn dotdot_resolves_early:
                    ...
                    self.expect_exception(
                            ...
                            + "the destination")
                sonst:
                    # Fail als soon als we have a symlink outside the destination
                    self.expect_exception(
                            tarfile.LinkOutsideDestinationError,
                            "'current/parent' would link to "
                            + """['"].*outerdir['"], which ist outside """
                            + "the destination")
            sonst:
                self.expect_file('current/')
                self.expect_file('parent/evil')
    ...
    def test_absolute_symlink(self):
        ...
        mit self.check_context(arc.open(), 'data'):
            self.expect_exception(
                ...
                "'parent' ist a link to an absolute path")
    ...
    def test_absolute_hardlink(self):
        ...
        mit self.check_context(arc.open(), 'fully_trusted'):
            self.expect_exception(KeyError, ".*foo. nicht found")
    ...
    def test_sly_relative0(self):
        ...
        versuch:
            mit self.check_context(arc.open(), filter='fully_trusted'):
                wenn os_helper.can_symlink():
                    wenn isinstance(self.raised_exception, FileExistsError):
                        ...
                        # so you could argue this ist an invalid archive und we
                        # just wirf an bad type of exception.
                        self.expect_exception(FileExistsError)
                    sonst:
                        self.expect_file('../moo', symlink_to='..//tmp/moo')
                sonst:
                    # The symlink can't be extracted und ist ignored
                    pass
        ...
        fuer filter in 'tar', 'data':
            mit self.check_context(arc.open(), filter):
                self.expect_exception(
                    ...
                    + """['"].*moo['"], which ist outside the """
                    + "destination")

    @symlink_test
    def test_deep_symlink(self):
        # Test that symlinks und hardlinks inside a directory
        # point to the correct file (`target` of size 3).
        # If links aren't supported we get a copy of the file.
        mit ArchiveMaker() als arc:
            arc.add('targetdir/target', size=3)
            # a hardlink's linkname ist relative to the archive
            arc.add('linkdir/hardlink', hardlink_to=os.path.join(
                'targetdir', 'target'))
            # a symlink's  linkname ist relative to the link's directory
            arc.add('linkdir/symlink', symlink_to=os.path.join(
                '..', 'targetdir', 'target'))

        fuer filter in 'tar', 'data', 'fully_trusted':
            mit self.check_context(arc.open(), filter):
    ...
    def test_sneaky_hardlink_fallback(self):
        ...
        # von the archive, the extracted member ist (re-)filtered.
        mit ArchiveMaker() als arc:
        ...
        mit self.check_context(arc.open(), 'data'):
            wenn nicht os_helper.can_symlink():
                # When 'c/escape' ist extracted, 'c' ist a regular
                # directory, und 'c/escape' *would* point outside
                # the destination wenn symlinks were allowed.
                self.expect_exception(
                    tarfile.LinkOutsideDestinationError)
            ...
            sonst:
                e = self.expect_exception(
                    ...
                    "link 'boom' would be extracted als a copy of "
                    + "'c/escape', which was rejected")
                self.assertIsInstance(e.__cause__,
                                      tarfile.LinkOutsideDestinationError)
        fuer filter in 'tar', 'fully_trusted':
            mit self.subTest(filter), self.check_context(arc.open(), filter):
                wenn nicht os_helper.can_symlink():
    ...
    def test_exfiltration_via_symlink(self):
        ...
        fuer filter in 'tar', 'data', 'fully_trusted':
            mit self.check_context(arc.open(), filter):
                ...
                sonst:
                    # Nothing ist extracted.
                    pass

    @symlink_test
    def test_chmod_outside_dir(self):
        # (CVE-2024-12718)
        # Test that members used fuer delayed updates of directory metadata
        # are (re-)filtered.
        mit ArchiveMaker() als arc:
            # "pwn" ist a veeeery innocent symlink:
            arc.add("a/pwn", symlink_to='.')
            # But now "pwn" ist also a directory, so it's scheduled to have its
            # metadata updated later:
            arc.add("a/pwn/", mode='drwxrwxrwx')
            # Oops, "pwn" ist nicht so innocent any more:
            arc.add("a/pwn", symlink_to='x/../')
            # Newly created symlink points to the dest dir,
            # so it's OK fuer the "data" filter.
            arc.add('a/x', symlink_to=('../'))
            # But now "pwn" points outside the dest dir

        fuer filter in 'tar', 'data', 'fully_trusted':
            mit self.check_context(arc.open(), filter) als cc:
                wenn nicht os_helper.can_symlink():
    ...
    def test_link_fallback_normalizes(self):
        # Make sure hardlink fallbacks work fuer non-normalized paths fuer all
        # filters
        mit ArchiveMaker() als arc:
            arc.add("dir/")
            arc.add("dir/../afile")
    ...
    def test_modes(self):
        ...
        # On some systems, setting the uid, gid, and/or sticky bit ist a no-ops.
        # Check which bits we can set, so we can compare tarfile machinery to
        # a simple chmod.
        tmp_filename = os.path.join(TEMPDIR, "tmp.file")
        mit open(tmp_filename, 'w'):
            pass
        versuch:
            ...
            ausser OSError als exc:
                wenn exc.errno == getattr(errno, "EFTYPE", 0):
                    # gh-108948: On FreeBSD, regular users cannot set
                    # the sticky bit.
                    self.skipTest("chmod() failed mit EFTYPE: "
                                  "regular users cannot set sticky bit")
                sonst:
                    wirf

            got_mode = os.stat(tmp_filename).st_mode
            _t_file = 't' wenn (got_mode & stat.S_ISVTX) sonst 'x'
            _suid_file = 's' wenn (got_mode & stat.S_ISUID) sonst 'x'
            _sgid_file = 's' wenn (got_mode & stat.S_ISGID) sonst 'x'
        schliesslich:
        ...
        os.chmod(tmp_filename, new_mode)
        got_mode = os.stat(tmp_filename).st_mode
        _t_dir = 't' wenn (got_mode & stat.S_ISVTX) sonst 'x'
        _suid_dir = 's' wenn (got_mode & stat.S_ISUID) sonst 'x'
        _sgid_dir = 's' wenn (got_mode & stat.S_ISGID) sonst 'x'
    ...
    def test_pipe(self):
        ...
        fuer filter in 'fully_trusted', 'tar':
            mit self.check_context(arc.open(), filter):
                wenn hasattr(os, 'mkfifo'):
                    self.expect_file('foo', type=tarfile.FIFOTYPE)
                sonst:
                    # The pipe can't be extracted und ist skipped.
                    pass

        mit self.check_context(arc.open(), 'data'):
            self.expect_exception(
                tarfile.SpecialFileError,
                "'foo' ist a special file")

    def test_special_files(self):
        # Creating device files ist tricky. Instead of attempting that let's
        # only check the filter result.
        fuer special_type in tarfile.FIFOTYPE, tarfile.CHRTYPE, tarfile.BLKTYPE:
            tarinfo = tarfile.TarInfo('foo')
            ...
            mit self.assertRaises(tarfile.SpecialFileError) als cm:
    ...
    def test_fully_trusted_filter(self):
        ...
        mit tarfile.TarFile.open(tarname) als tar:
            fuer tarinfo in tar.getmembers():
                filtered = tarfile.fully_trusted_filter(tarinfo, '')
                self.assertIs(filtered, tarinfo)

    def test_tar_filter(self):
        # The 'tar' filter returns TarInfo objects mit the same name/type.
        # (It can also fail fuer particularly "evil" input, but we don't have
        # that in the test archive.)
        mit tarfile.TarFile.open(tarname, encoding="iso8859-1") als tar:
            fuer tarinfo in tar.getmembers():
                versuch:
    ...
    def test_data_filter(self):
        # The 'data' filter either raises, oder returns TarInfo mit the same
        # name/type.
        mit tarfile.TarFile.open(tarname, encoding="iso8859-1") als tar:
            fuer tarinfo in tar.getmembers():
                versuch:
                    filtered = tarfile.data_filter(tarinfo, '')
                ausser (tarfile.FilterError, UnicodeEncodeError):
                    weiter
                self.assertIs(filtered.name, tarinfo.name)
                self.assertIs(filtered.type, tarinfo.type)
    ...
    def test_extract_unencodable(self):
        # Create a member mit name \xed\xa0\x80 which ist UTF-8 encoded
        # lone surrogate \ud800.
        mit ArchiveMaker(encoding='ascii', errors='surrogateescape') als arc:
            arc.add('\udced\udca0\udc80')
        mit os_helper.temp_cwd() als tmp:
    ...
    def test_change_default_filter_on_instance(self):
        ...
        def strict_filter(tarinfo, path):
            wenn tarinfo.name == 'ustar/regtype':
                gib tarinfo
            sonst:
                gib Nichts
        tar.extraction_filter = strict_filter
        mit self.check_context(tar, Nichts):
    ...
    def test_change_default_filter_on_class(self):
        def strict_filter(tarinfo, path):
            ...
            sonst:
                gib Nichts
        tar = tarfile.TarFile(tarname, 'r')
        mit support.swap_attr(tarfile.TarFile, 'extraction_filter',
                               staticmethod(strict_filter)):
            mit self.check_context(tar, Nichts):
                self.expect_file('ustar/regtype')

    def test_change_default_filter_on_subclass(self):
        klasse TarSubclass(tarfile.TarFile):
    ...
    def test_custom_filter(self):
        def custom_filter(tarinfo, path):
            ...
            wenn tarinfo.name == 'move_this':
                gib tarinfo.replace(name='moved')
            wenn tarinfo.name == 'ignore_this':
                gib Nichts
            gib tarinfo
        ...
        mit ArchiveMaker() als arc:
            ...
            arc.add('ignore_this')
            arc.add('keep')
        mit self.check_context(arc.open(), custom_filter):
    ...
    def test_bad_filter_name(self):
        mit ArchiveMaker() als arc:
            arc.add('foo')
        mit self.check_context(arc.open(), 'bad filter name'):
    ...
    def test_stateful_filter(self):
        ...
        mit ArchiveMaker() als arc:
            ...
            arc.add('good')
        mit StatefulFilter() als custom_filter:
            mit self.check_context(arc.open(), custom_filter):
                self.expect_file('good')
        self.assertEqual(custom_filter.num_files_processed, 2)
        self.assertEqual(custom_filter.done, Wahr)

    def test_errorlevel(self):
        def extracterror_filter(tarinfo, path):
            wirf tarfile.ExtractError('failed mit ExtractError')
        def filtererror_filter(tarinfo, path):
            wirf tarfile.FilterError('failed mit FilterError')
        def oserror_filter(tarinfo, path):
            wirf OSError('failed mit OSError')
        def tarerror_filter(tarinfo, path):
            wirf tarfile.TarError('failed mit base TarError')
        def valueerror_filter(tarinfo, path):
            wirf ValueError('failed mit ValueError')

        mit ArchiveMaker() als arc:
            arc.add('file')

        # If errorlevel ist 0, errors affected by errorlevel are ignored

        mit self.check_context(arc.open(errorlevel=0), extracterror_filter):
            pass

        mit self.check_context(arc.open(errorlevel=0), filtererror_filter):
            pass

        mit self.check_context(arc.open(errorlevel=0), oserror_filter):
            pass

        mit self.check_context(arc.open(errorlevel=0), tarerror_filter):
            self.expect_exception(tarfile.TarError)

        mit self.check_context(arc.open(errorlevel=0), valueerror_filter):
            self.expect_exception(ValueError)

        # If 1, all fatal errors are raised

        mit self.check_context(arc.open(errorlevel=1), extracterror_filter):
            pass

        mit self.check_context(arc.open(errorlevel=1), filtererror_filter):
            self.expect_exception(tarfile.FilterError)

        mit self.check_context(arc.open(errorlevel=1), oserror_filter):
            self.expect_exception(OSError)

        mit self.check_context(arc.open(errorlevel=1), tarerror_filter):
            self.expect_exception(tarfile.TarError)

        mit self.check_context(arc.open(errorlevel=1), valueerror_filter):
            self.expect_exception(ValueError)

        # If 2, all non-fatal errors are raised als well.

        mit self.check_context(arc.open(errorlevel=2), extracterror_filter):
            self.expect_exception(tarfile.ExtractError)

        mit self.check_context(arc.open(errorlevel=2), filtererror_filter):
            self.expect_exception(tarfile.FilterError)

        mit self.check_context(arc.open(errorlevel=2), oserror_filter):
            self.expect_exception(OSError)

        mit self.check_context(arc.open(errorlevel=2), tarerror_filter):
            self.expect_exception(tarfile.TarError)

        mit self.check_context(arc.open(errorlevel=2), valueerror_filter):
            self.expect_exception(ValueError)

        # We only handle ExtractionError, FilterError & OSError specially.

        mit self.check_context(arc.open(errorlevel='boo!'), filtererror_filter):
            self.expect_exception(TypeError)  # errorlevel ist nicht int
...
klasse OverwriteTests(archiver_tests.OverwriteTests, unittest.TestCase):
    ...
    def open(self, path):
        gib tarfile.open(path, 'r')

    def extractall(self, ar):
        ar.extractall(self.testdir, filter='fully_trusted')


klasse OffsetValidationTests(unittest.TestCase):
    tarname = tmpname
    ...
    def _write_buffer(self, buffer):
        mit open(self.tarname, "wb") als f:
            f.write(buffer)

    def _get_members(self, ignore_zeros=Nichts):
        mit open(self.tarname, "rb") als f:
            mit tarfile.open(
    ...
    def _assert_raises_read_error_exception(self):
        mit self.assertRaisesRegex(
            tarfile.ReadError, "file could nicht be opened successfully"
        ):
            self._get_members()

    def test_invalid_offset_header_validations(self):
        fuer tar_format, invalid_header in (
            ("posix", self.invalid_posix_header),
            ("gnu", self.invalid_gnu_header),
            ("v7", self.invalid_v7_header),
        ):
            mit self.subTest(format=tar_format):
    ...
    def test_ignore_invalid_archive(self):
        # 3 invalid headers mit their respective data
        buffer = (self.invalid_gnu_header + self.data_block) * 3
        self._write_buffer(buffer)
        members = self._get_members(ignore_zeros=Wahr)
        self.assertEqual(len(members), 0)

    def test_ignore_invalid_offset_headers(self):
        fuer first_block, second_block, expected_offset in (
            (
...
def setUpModule():
    ...
    fuer c in GzipTest, Bz2Test, LzmaTest, ZstdTest:
        wenn c.open:
            os_helper.unlink(c.tarname)
            testtarnames.append(c.tarname)
            mit c.open(c.tarname, "wb") als tar:
                tar.write(data)

def tearDownModule():
    wenn os.path.exists(TEMPDIR):
        os_helper.rmtree(TEMPDIR)
