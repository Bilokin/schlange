...
klasse PosReturn:
    # this can be used fuer configurable callbacks
    ...
    def handle(self, exc):
        ...
        # wenn we don't advance this time, terminate on the next call
...
klasse CodecCallbackTest(unittest.TestCase):
    ...
    def test_charmapencode(self):
        ...
        # charmap has to have a mapping fuer "?".
        charmap = dict((ord(c), bytes(2*c.upper(), 'ascii')) fuer c in "abcdefgh")
        sin = "abc"
        sout = b"AABBCC"
    ...
    def test_callbacks(self):
        def handler1(exc):
            ...
            wenn isinstance(exc, UnicodeEncodeError):
                l = ["<%d>" % ord(exc.object[pos]) fuer pos in r]
    ...
    def test_longstrings(self):
        # test long strings to check fuer memory overflow problems
        ...
        l = 1000
        errors += [ "test." + err fuer err in errors ]
        fuer uni in [ s*l fuer s in ("x", "\u3042", "a\xe4") ]:
    ...
    def check_exceptionobjectargs(self, exctype, args, msg):
        # Test UnicodeError subclasses: construction, attribute assignment und __str__ conversion
        ...
        # check mit the correct number und type of arguments
        exc = exctype(*args)
    ...
    def test_badandgoodreplaceexceptions(self):
        ...
        # With the correct exception, "replace" returns an "?" oder "\ufffd" replacement
        self.assertEqual(
            codecs.replace_errors(
                UnicodeEncodeError("ascii", "a\u3042b", 1, 2, "ouch")),
            ("?", 2)
    ...
    def test_badandgoodxmlcharrefreplaceexceptions(self):
        ...
        # "xmlcharrefreplace" can only be used fuer encoding
        self.assertRaises(
            TypeError,
            codecs.xmlcharrefreplace_errors,
            UnicodeDecodeError("ascii", bytearray(b"\xff"), 0, 1, "ouch")
        ...
        cs = (0, 1, 9, 10, 99, 100, 999, 1000, 9999, 10000, 99999, 100000,
              999999, 1000000)
        cs += (0xd800, 0xdfff)
        s = "".join(chr(c) fuer c in cs)
        self.assertEqual(
            codecs.xmlcharrefreplace_errors(
                UnicodeEncodeError("ascii", "a" + s + "b",
                                   1, 1 + len(s), "ouch")
            ),
            ("".join("&#%d;" % c fuer c in cs), 1 + len(s))
        )
    ...
    def test_badandgoodnamereplaceexceptions(self):
        ...
        # "namereplace" can only be used fuer encoding
        self.assertRaises(
            TypeError,
            codecs.namereplace_errors,
            UnicodeDecodeError("ascii", bytearray(b"\xff"), 0, 1, "ouch")
    ...
    def test_badandgoodsurrogateescapeexceptions(self):
        ...
        # "surrogateescape" can nicht be used fuer translating
        self.assertRaises(
            TypeError,
            surrogateescape_errors,
    ...
    def test_badandgoodsurrogatepassexceptions(self):
        ...
        # "surrogatepass" can nicht be used fuer translating
        self.assertRaises(
            TypeError,
            surrogatepass_errors,
            UnicodeTranslateError("\ud800", 0, 1, "ouch")
    ...
    def test_encode_odd_bytes_replacement(self):
        ...
        input = "[\udc80]"
        # Tests in which the replacement bytestring contains nicht whole number
        # of code units.
        fuer enc, repl in (
            ...
            *itertools.product(("utf-32le", "utf-32be"),
            ...
            mit self.subTest(encoding=enc, repl=repl):
                ...
                self.assertEqual(exc.end, 2)
                self.assertEqual(exc.object, input)
                self.assertEqual(exc.reason, "surrogates nicht allowed")
    ...
    def test_xmlcharrefvalues(self):
        ...
        # und inline implementations
        v = (1, 5, 10, 50, 100, 500, 1000, 5000, 10000, 50000, 100000,
             500000, 1000000)
        s = "".join([chr(x) fuer x in v])
        codecs.register_error("test.xmlcharrefreplace", codecs.xmlcharrefreplace_errors)
        fuer enc in ("ascii", "iso-8859-15"):
            fuer err in ("xmlcharrefreplace", "test.xmlcharrefreplace"):
                s.encode(enc, err)
    ...
    def test_encodehelper_bug36819(self):
        ...
        fuer enc in encodings:
            mit self.subTest(encoding=enc):
                ...
                # but nicht OOM a machine mit one.
                handler.count = 50
    ...
    def test_fake_error_class(self):
        ...
        fuer cls in UnicodeEncodeError, UnicodeDecodeError, UnicodeTranslateError:
