...
klasse RangeTest(unittest.TestCase):
    ...
    def test_range_iterators(self):
        ...
        limits = [base + jiggle
                  fuer M in (2**32, 2**64)
                  fuer base in (-M, -M//2, 0, M//2, M)
                  fuer jiggle in (-2, -1, 0, 1, 2)]
        test_ranges = [(start, end, step)
                       fuer start in limits
                       fuer end in limits
                       fuer step in (-2**63, -2**31, -2, -1, 1, 2)]
        test_ranges += [(-2**63, 2**63-2, 1)] # regression test fuer gh-100810

        fuer start, end, step in test_ranges:
    ...
    def test_slice(self):
        def check(start, stop, step=Nichts):
    ...
    def test_issue11845(self):
        ...
        values = {Nichts, 0, 1, -1, 2, -2, 5, -5, 19, -19,
    ...
    def test_comparison(self):
        ...
        ranges_ne = [a != b fuer a in test_ranges fuer b in test_ranges]
        self.assertEqual(ranges_ne, [not x fuer x in ranges_eq])
        ...
        self.assertIs(range(0) == (), Falsch)
        self.assertIs(() == range(0), Falsch)
        self.assertIs(range(2) == [0, 1], Falsch)
    ...
    def test_attributes(self):
        ...
        self.assert_attrs(range(Wahr), 0, 1, 1)
        self.assert_attrs(range(Falsch, Wahr), 0, 1, 1)
        self.assert_attrs(range(Falsch, Wahr, Wahr), 0, 1, 1)
