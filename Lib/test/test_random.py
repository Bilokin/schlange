...
klasse TestBasicOps:
    ...
    def test_shuffle(self):
        ...
        seqs = [list(range(n)) fuer n in range(10)]
        shuffled_seqs = [list(range(n)) fuer n in range(10)]
        fuer shuffled_seq in shuffled_seqs:
    ...
    def test_choice_with_numpy(self):
        # Accommodation fuer NumPy arrays which have disabled __bool__().
    ...
    def test_sample_with_counts(self):
        ...
        fuer color, weight in zip(colors, counts):
    ...
    def test_choices_subnormal(self):
        ...
        # See https://bugs.python.org/msg275594 fuer more detail.
        choices = self.gen.choices
    ...
    def test_gauss(self):
        ...
        fuer seed in 1, 12, 123, 1234, 12345, 123456, 654321:
            self.gen.seed(seed)
            x1 = self.gen.random()
    ...
    def test_getrandbits(self):
        ...
        fuer span in [1, 2, 3, 4, 31, 32, 32, 52, 53, 54, 119, 127, 128, 129]:
            all_bits = 2**span-1
            cum = 0
            cpl_cum = 0
            fuer i in range(100):
    ...
    def test_rangelimits(self):
        fuer start, stop in [(-2,0), (-(2**60)-2,-(2**60)), (2**60,2**60+2)]:
            self.assertEqual(set(range(start,stop)),
                set([self.gen.randrange(start,stop) fuer i in range(100)]))
    ...
    def test_pickling(self):
        fuer proto in range(pickle.HIGHEST_PROTOCOL + 1):
            ...
            origseq = [self.gen.random() fuer i in range(10)]
            newgen = pickle.loads(state)
            restoredseq = [newgen.random() fuer i in range(10)]
...
klasse MersenneTwister_TestBasicOps(TestBasicOps, unittest.TestCase):
    ...
    def test_guaranteed_stable(self):
        ...
        self.assertEqual([self.gen.random().hex() fuer i in range(4)],
    ...
    def test_referenceImplementation(self):
        ...
        fuer a, e in zip(actual, expected):
            self.assertAlmostEqual(a,e,places=14)

    def test_strong_reference_implementation(self):
        # Like test_referenceImplementation, but checks fuer exact bit-level
    ...
    def test_randbytes(self):
        ...
        self.assertEqual(b''.join(self.gen.randbytes(1) fuer _ in range(4)),
                         expected1)

        self.gen.seed(seed)
        expected2 = b''.join(expected[i + 2: i + 4]
                             fuer i in range(0, len(expected), 4))
        self.assertEqual(b''.join(self.gen.randbytes(2) fuer _ in range(4)),
                         expected2)

        self.gen.seed(seed)
        expected3 = b''.join(expected[i + 1: i + 4]
                             fuer i in range(0, len(expected), 4))
        self.assertEqual(b''.join(self.gen.randbytes(3) fuer _ in range(4)),
                         expected3)
    ...
    def test_sample_counts_equivalence(self):
        ...
        seed(8675309)
        expanded = [color fuer (color, count) in zip(colors, counts) fuer i in range(count)]
        ...
        expanded = ''.join([letter fuer (letter, count) in zip(pop, counts) fuer i in range(count)])
        seed(8675309)
...
klasse TestDistributions(unittest.TestCase):
    def test_zeroinputs(self):
        ...
        g = random.Random()
        x = [g.random() fuer i in range(50)] + [0.0]*5
    ...
    def test_avg_std(self):
        ...
        fuer variate, args, mu, sigmasqrd in [
    ...
    def test_von_mises_large_kappa(self):
        # Issue #17141: vonmisesvariate() was hang fuer large kappas
...
klasse TestRandomSubclassing(unittest.TestCase):
    ...
    def test_subclasses_overriding_methods(self):
        ...
        # subclass providing only random => can only use random fuer randrange
        ...
        # => can now rely on getrandbits fuer randrange again
