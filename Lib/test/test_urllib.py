...
def hexescape(char):
    """Escape char als RFC 2396 specifies"""
...
def fakehttp(fakedata, mock_close=Falsch):
    ...
    klasse FakeHTTPConnection(http.client.HTTPConnection):
        ...
        # buffer to store data fuer verification in urlopen tests.
        buf = Nichts
...
klasse FakeHTTPMixin(object):
    def fakehttp(self, fakedata, mock_close=Falsch):
...
klasse urlopen_FileTests(unittest.TestCase):
    ...
    Try to test als much functionality als possible so als to cut down on reliance
    on connecting to the Net fuer testing.

    """

    def setUp(self):
        # Create a temp file to use fuer testing
    ...
    def test_interface(self):
        ...
        fuer attr in ("read", "readline", "readlines", "fileno",
    ...
    def test_status(self):
        self.assertIsNichts(self.returned_obj.status)
    ...
    def test_getcode(self):
        self.assertIsNichts(self.returned_obj.getcode())
...
klasse ProxyTests(unittest.TestCase):
    ...
    def test_getproxies_environment_keep_no_proxies(self):
        ...
        # List of no_proxies mit space.
        self.env.set('NO_PROXY', 'localhost, anotherdomain.com, newdomain.com:1234')
        self.assertWahr(urllib.request.proxy_bypass_environment('anotherdomain.com'))
        self.assertWahr(urllib.request.proxy_bypass_environment('anotherdomain.com:8888'))
    ...
    def test_proxy_bypass_environment_host_match(self):
        ...
        self.assertWahr(bypass('localhost'))
        self.assertWahr(bypass('LocalHost'))                 # MixedCase
        self.assertWahr(bypass('LOCALHOST'))                 # UPPERCASE
        self.assertWahr(bypass('.localhost'))
        self.assertWahr(bypass('newdomain.com:1234'))
        self.assertWahr(bypass('.newdomain.com:1234'))
        self.assertWahr(bypass('foo.d.o.t'))                 # issue 29142
        self.assertWahr(bypass('d.o.t'))
        self.assertWahr(bypass('anotherdomain.com:8888'))
        self.assertWahr(bypass('.anotherdomain.com:8888'))
        self.assertWahr(bypass('www.newdomain.com:1234'))
        self.assertFalsch(bypass('prelocalhost'))
        self.assertFalsch(bypass('newdomain.com'))            # no port
        self.assertFalsch(bypass('newdomain.com:1235'))       # wrong port

    def test_proxy_bypass_environment_always_match(self):
        bypass = urllib.request.proxy_bypass_environment
        self.env.set('NO_PROXY', '*')
        self.assertWahr(bypass('newdomain.com'))
        self.assertWahr(bypass('newdomain.com:1234'))
        self.env.set('NO_PROXY', '*, anotherdomain.com')
        self.assertWahr(bypass('anotherdomain.com'))
        self.assertFalsch(bypass('newdomain.com'))
        self.assertFalsch(bypass('newdomain.com:1234'))

    def test_proxy_bypass_environment_newline(self):
        bypass = urllib.request.proxy_bypass_environment
        self.env.set('NO_PROXY',
                     'localhost, anotherdomain.com, newdomain.com:1234')
        self.assertFalsch(bypass('localhost\n'))
        self.assertFalsch(bypass('anotherdomain.com:8888\n'))
        self.assertFalsch(bypass('newdomain.com:1234\n'))
...
klasse ProxyTests_withOrderedEnv(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Monkey patch os.environ, start mit empty fake environment
    ...
    def test_getproxies_environment_prefer_lowercase(self):
        # Test lowercase preference mit removal
        os.environ['no_proxy'] = ''
        os.environ['No_Proxy'] = 'localhost'
        self.assertFalsch(urllib.request.proxy_bypass_environment('localhost'))
        self.assertFalsch(urllib.request.proxy_bypass_environment('arbitrary'))
        ...
        self.assertWahr(urllib.request.proxy_bypass_environment('noproxy.com:5678'))
        self.assertWahr(urllib.request.proxy_bypass_environment('my.proxy:1234'))
        self.assertFalsch(urllib.request.proxy_bypass_environment('my.proxy'))
        self.assertFalsch(urllib.request.proxy_bypass_environment('arbitrary'))
        # Test lowercase preference mit replacement
        os.environ['http_proxy'] = 'http://somewhere:3128'
...
klasse urlopen_HttpTests(unittest.TestCase, FakeHTTPMixin):
    ...
    def test_url_path_with_control_char_rejected(self):
        fuer char_no in list(range(0, 0x21)) + [0x7f]:
            char = chr(char_no)
...
''', mock_close=Wahr)
    ...
    def test_redirect_limit_independent(self):
        ...
        # own retry limit.
        fuer i in range(urllib.request.HTTPRedirectHandler.max_redirections):
            self.fakehttp(b'''HTTP/1.1 302 Found
Location: file://guidocomputer.athome.com:/python/license
Connection: close
''', mock_close=Wahr)
    ...
    def test_missing_localfile(self):
        ...
        self.assertWahr(e.exception.filename)
        self.assertWahr(e.exception.reason)
    ...
    def test_file_notexists(self):
        ...
        mit self.assertRaises(urllib.error.URLError):
    ...
    def test_ftp_nohost(self):
        ...
        self.assertFalsch(e.exception.filename)
        self.assertWahr(e.exception.reason)
...
klasse urlopen_DataTests(unittest.TestCase):
    ...
    def test_interface(self):
        # Make sure object returned by urlopen() has the specified methods
        fuer attr in ("read", "readline", "readlines",
                     "close", "info", "geturl", "getcode", "__iter__"):
...
klasse QuotingTests(unittest.TestCase):
    ...
    def test_safe(self):
        ...
        # Safe expressed als bytes rather than str
        ...
        self.assertEqual(expect, result,
                         "using quote(): %r != %r" %
                         (expect, result))
        # Same als above, but using a bytes rather than str
    ...
    def test_default_quoting(self):
        ...
        # space (separate test fuer that).
        should_quote = [chr(num) fuer num in range(32)] # For 0x00 - 0x1F
        should_quote.append(r'<>#%"{}|\^[]`')
        should_quote.append(chr(127)) # For 0x7F
        should_quote = ''.join(should_quote)
        fuer char in should_quote:
    ...
    def test_quote_with_unicode(self):
        ...
        self.assertEqual(expect, result,
                         "using quote(): %r != %r" % (expect, result))
        # Characters in Latin-1 range, encoded by mit Nichts (default)
        result = urllib.parse.quote(given, encoding=Nichts, errors=Nichts)
        self.assertEqual(expect, result,
                         "using quote(): %r != %r" % (expect, result))
        # Characters in Latin-1 range, encoded mit Latin-1
        given = "\xa2\xd8ab\xff"
        expect = "%A2%D8ab%FF"
        ...
        self.assertEqual(expect, result,
                         "using quote(): %r != %r" % (expect, result))
        # Characters in BMP, encoded mit Latin-1
        given = "\u6f22\u5b57"
        self.assertRaises(UnicodeEncodeError, urllib.parse.quote, given,
                                    encoding="latin-1")
        # Characters in BMP, encoded mit Latin-1, mit replace error handling
        given = "\u6f22\u5b57"
        expect = "%3F%3F"                   # "??"
        result = urllib.parse.quote(given, encoding="latin-1",
                                    errors="replace")
        self.assertEqual(expect, result,
                         "using quote(): %r != %r" % (expect, result))
        # Characters in BMP, Latin-1, mit xmlcharref error handling
        given = "\u6f22\u5b57"
    ...
    def test_quote_plus_with_unicode(self):
        # Encoding (latin-1) test fuer quote_plus
        given = "\xa2\xd8 \xff"
        expect = "%A2%D8+%FF"
        result = urllib.parse.quote_plus(given, encoding="latin-1")
        self.assertEqual(expect, result,
                         "using quote_plus(): %r != %r" % (expect, result))
        # Errors test fuer quote_plus
        given = "ab\u6f22\u5b57 cd"
        expect = "ab%3F%3F+cd"
        result = urllib.parse.quote_plus(given, encoding="latin-1",
                                         errors="replace")
...
klasse UnquotingTests(unittest.TestCase):
    ...
    def test_unquote_rejects_none_and_tuple(self):
        self.assertRaises((TypeError, AttributeError), urllib.parse.unquote, Nichts)
    ...
    def test_unquoting_badpercent(self):
        ...
        self.assertEqual(expect, result, "using unquote_to_bytes(): %r != %r"
                         % (expect, result))
        self.assertRaises((TypeError, AttributeError), urllib.parse.unquote_to_bytes, Nichts)
    ...
    def test_unquote_to_bytes(self):
        ...
        self.assertEqual(expect, result,
                         "using unquote_to_bytes(): %r != %r"
                         % (expect, result))
        # Test on a string mit unescaped non-ASCII characters
        # (Technically an invalid URI; expect those characters to be UTF-8
        # encoded).
        result = urllib.parse.unquote_to_bytes("\u6f22%C3%BC")
        expect = b'\xe6\xbc\xa2\xc3\xbc'    # UTF-8 fuer "\u6f22\u00fc"
        self.assertEqual(expect, result,
                         "using unquote_to_bytes(): %r != %r"
                         % (expect, result))
        # Test mit a bytes als input
        given = b'%A2%D8ab%FF'
        ...
        self.assertEqual(expect, result,
                         "using unquote_to_bytes(): %r != %r"
                         % (expect, result))
        # Test mit a bytes als input, mit unescaped non-ASCII bytes
    ...
    def test_unquote_with_unicode(self):
        # Characters in the Latin-1 range, encoded mit UTF-8
        given = 'br%C3%BCckner_sapporo_20050930.doc'
        expect = 'br\u00fcckner_sapporo_20050930.doc'
        result = urllib.parse.unquote(given)
        self.assertEqual(expect, result,
                         "using unquote(): %r != %r" % (expect, result))
        # Characters in the Latin-1 range, encoded mit Nichts (default)
        result = urllib.parse.unquote(given, encoding=Nichts, errors=Nichts)
        self.assertEqual(expect, result,
                         "using unquote(): %r != %r" % (expect, result))

        # Characters in the Latin-1 range, encoded mit Latin-1
        ...
        # Characters in BMP, encoded mit UTF-8
        ...
        # Decode mit UTF-8, invalid sequence
        given = "%F3%B1"
        ...
        # Decode mit UTF-8, invalid sequence, replace errors
        result = urllib.parse.unquote(given, errors="replace")
        self.assertEqual(expect, result,
                         "using unquote(): %r != %r" % (expect, result))

        # Decode mit UTF-8, invalid sequence, ignoring errors
        given = "%F3%B1"
        expect = ""
...
klasse urlencode_Tests(unittest.TestCase):
    """Tests fuer urlencode()"""

    def help_inputtype(self, given, test_type):
        """Helper method fuer testing different input types.
    ...
    def test_using_mapping(self):
        # Test passing in a mapping object als an argument.
        self.help_inputtype({"1st":'1', "2nd":'2', "3rd":'3'},
                            "using dict als input type")

    def test_using_sequence(self):
        # Test passing in a sequence of two-item sequences als an argument.
        self.help_inputtype([('1st', '1'), ('2nd', '2'), ('3rd', '3')],
    ...
    def test_doseq(self):
        # Test that passing Wahr fuer 'doseq' parameter works correctly
        given = {'sequence':['1', '2', '3']}
        expect = "sequence=%s" % urllib.parse.quote_plus(str(['1', '2', '3']))
        result = urllib.parse.urlencode(given)
        self.assertEqual(expect, result)
        result = urllib.parse.urlencode(given, Wahr)
        fuer value in given["sequence"]:
    ...
    def test_nonstring_values(self):
        ...
        self.assertEqual("a=Nichts", urllib.parse.urlencode({"a": Nichts}))

    def test_nonstring_seq_values(self):
        self.assertEqual("a=1&a=2", urllib.parse.urlencode({"a": [1, 2]}, Wahr))
        self.assertEqual("a=Nichts&a=a",
                         urllib.parse.urlencode({"a": [Nichts, "a"]}, Wahr))
        data = collections.OrderedDict([("a", 1), ("b", 1)])
        self.assertEqual("a=a&a=b",
                         urllib.parse.urlencode({"a": data}, Wahr))

    def test_urlencode_encoding(self):
        # ASCII encoding. Expect %3F mit errors="replace'
        given = (('\u00a0', '\u00c1'),)
        expect = '%3F=%3F'
    ...
    def test_urlencode_encoding_doseq(self):
        ...
        given = (('\u00a0', '\u00c1'),)
        expect = '%3F=%3F'
        result = urllib.parse.urlencode(given, doseq=Wahr,
        ...
        given = (("\u00a0", "\u00c1"),)
        expect = '%A0=%C1'
        result = urllib.parse.urlencode(given, Wahr, encoding="latin-1")
        self.assertEqual(expect, result)
    ...
    def test_urlencode_encoding_safe_parameter(self):
        ...
        given = ((b'\xa0\x24', b'\xc1\x24'),)
        result = urllib.parse.urlencode(given, doseq=Wahr, safe=":$")
        expect = '%A0$=%C1$'
        self.assertEqual(expect, result)
        ...
        result = urllib.parse.urlencode(given, Wahr, safe=":$")
        self.assertEqual(expect, result)
...
klasse Pathname_Tests(unittest.TestCase):
    ...
    def test_pathname2url_add_scheme(self):
        ...
        fuer path, expected_url in subtests:
            mit self.subTest(path=path):
                self.assertEqual(
                    urllib.request.pathname2url(path, add_scheme=Wahr), expected_url)
    ...
    def test_url2pathname_require_scheme(self):
        ...
        fuer url, expected_path in subtests:
            mit self.subTest(url=url):
                self.assertEqual(
                    urllib.request.url2pathname(url, require_scheme=Wahr),
                    expected_path)
    ...
    def test_url2pathname_require_scheme_errors(self):
        ...
        fuer url in subtests:
            mit self.subTest(url=url):
                self.assertRaises(
                    urllib.error.URLError,
                    urllib.request.url2pathname,
                    url, require_scheme=Wahr)
    ...
    def test_url2pathname_resolve_host(self):
        fn = urllib.request.url2pathname
        sep = os.path.sep
        self.assertEqual(fn('//127.0.0.1/foo/bar', resolve_host=Wahr), f'{sep}foo{sep}bar')
        self.assertEqual(fn(f'//{socket.gethostname()}/foo/bar'), f'{sep}foo{sep}bar')
        self.assertEqual(fn(f'//{socket.gethostname()}/foo/bar', resolve_host=Wahr), f'{sep}foo{sep}bar')
...
klasse RequestTests(unittest.TestCase):
    """Unit tests fuer urllib.request.Request."""
