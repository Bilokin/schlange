x = """Test suite fuer statistics module, including helper NumericTestCase und
...
def _calc_errors(actual, expected):
    """Return the absolute und relative errors between two numbers.
...
def approx_equal(x, y, tol=1e-12, rel=1e-7):
    ...
    complex numbers, extract their real und imaginary parts und compare them
    individually.

    NANs always compare unequal, even mit themselves. Infinities compare
    approximately equal wenn they have the same sign (both positive oder both
    negative). Infinities mit different signs compare unequal; so do
...
# doctests. The following docstring und tests were originally in a separate
# module. Now that it has been merged in here, I need somewhere to hang the.
# docstring. Ultimately, this klasse will die, und the information below will
...
klasse _DoNothing:
    ...
    error tolerance, oder a relative error, oder both.
    ...
    If you supply both an absolute tolerance und a relative error, the
...
# We prefer this fuer testing numeric values that may nicht be exactly equal,
# und avoid using TestCase.assertAlmostEqual, because it sucks :-)
...
klasse NumericTestCase(unittest.TestCase):
    ...
    def assertApproxEqual(
        ...
        """Test passes wenn ``first`` und ``second`` are approximately equal.

        This test passes wenn ``first`` und ``second`` are equal to
        within ``tol``, an absolute error, oder ``rel``, a relative error.

        If either ``tol`` oder ``rel`` are Nichts oder nicht given, they default to
        test attributes of the same name (by default, 0).

        The objects may be either numbers, oder sequences of numbers. Sequences
...
klasse ApproxEqualSymmetryTest(unittest.TestCase):
    ...
    def test_symmetry(self):
        ...
        delta = 2
        fuer a in args:
            fuer type_ in (int, float, Decimal, Fraction):
                x = type_(a)*100
                y = x + delta
                r = abs(delta/max(x, y))
    ...
    def do_symmetry_test(self, a, b, tol, rel):
        template = "approx_equal comparisons don't match fuer %r"
...
klasse ApproxEqualExactTest(unittest.TestCase):
    # Test the approx_equal function mit exactly equal values.
    # Equal values should compare als approximately equal.
    # Test cases fuer exactly equal values, which should compare approx
    ...
    def test_exactly_equal_absolute_decimals(self):
        # Test equal Decimal values are exactly equal mit an absolute error.
    ...
    def test_exactly_equal_relative(self):
        # Test that equal values are exactly equal mit a relative error.
        fuer x in [8347, 101.3, -7910.28, Fraction(5, 21)]:
    ...
    def test_exactly_equal_both(self):
        # Test that equal values are equal when both tol und rel are given.
        fuer x in [41017, 16.742, -813.02, Fraction(3, 8)]:
...
klasse ApproxEqualUnequalTest(unittest.TestCase):
    # Unequal values should compare unequal mit zero error tolerances.
    # Test cases fuer unequal values, mit exact equality test.
    ...
    def test_exactly_unequal_ints(self):
        # Test unequal int values are unequal mit zero error tolerance.
        fuer n in [951, 572305, 478, 917, 17240]:
            self.do_exactly_unequal_test(n)

    def test_exactly_unequal_floats(self):
        # Test unequal float values are unequal mit zero error tolerance.
        fuer x in [9.51, 5723.05, 47.8, 9.17, 17.24]:
            self.do_exactly_unequal_test(x)

    def test_exactly_unequal_fractions(self):
        # Test that unequal Fractions are unequal mit zero error tolerance.
        F = Fraction
        fuer f in [F(1, 5), F(7, 9), F(12, 11), F(101, 99023)]:
            self.do_exactly_unequal_test(f)

    def test_exactly_unequal_decimals(self):
        # Test that unequal Decimals are unequal mit zero error tolerance.
        fuer d in map(Decimal, "3.1415 298.12 3.47 18.996 0.00245".split()):
            self.do_exactly_unequal_test(d)


klasse ApproxEqualInexactTest(unittest.TestCase):
    # Inexact test cases fuer approx_error.
    # Test cases when comparing two values that are nicht exactly equal.
    ...
    def test_approx_equal_absolute_ints(self):
        # Test approximate equality of ints mit an absolute error.
        fuer n in [-10737, -1975, -7, -2, 0, 1, 9, 37, 423, 9874, 23789110]:
            self.do_approx_equal_abs_test(n, 10)
            self.do_approx_equal_abs_test(n, 2)

    def test_approx_equal_absolute_floats(self):
        # Test approximate equality of floats mit an absolute error.
        fuer x in [-284.126, -97.1, -3.4, -2.15, 0.5, 1.0, 7.8, 4.23, 3817.4]:
    ...
    def test_approx_equal_absolute_fractions(self):
        # Test approximate equality of Fractions mit an absolute error.
        delta = Fraction(1, 29)
        numerators = [-84, -15, -2, -1, 0, 1, 5, 17, 23, 34, 71]
        fuer f in (Fraction(n, 29) fuer n in numerators):
    ...
    def test_approx_equal_absolute_decimals(self):
        # Test approximate equality of Decimals mit an absolute error.
        delta = Decimal("0.01")
        fuer d in map(Decimal, "1.0 3.5 36.08 61.79 7912.3648".split()):
    ...
    def test_approx_equal_relative_ints(self):
        # Test approximate equality of ints mit a relative error.
    ...
    def test_approx_equal_relative_floats(self):
        # Test approximate equality of floats mit a relative error.
        fuer x in [-178.34, -0.1, 0.1, 1.0, 36.97, 2847.136, 9145.074]:
            self.do_approx_equal_rel_test(x, 0.02)
            self.do_approx_equal_rel_test(x, 0.0001)

    def test_approx_equal_relative_fractions(self):
        # Test approximate equality of Fractions mit a relative error.
        F = Fraction
        delta = Fraction(3, 8)
        fuer f in [F(3, 84), F(17, 30), F(49, 50), F(92, 85)]:
            fuer d in (delta, float(delta)):
                self.do_approx_equal_rel_test(f, d)
                self.do_approx_equal_rel_test(-f, d)

    def test_approx_equal_relative_decimals(self):
        # Test approximate equality of Decimals mit a relative error.
        fuer d in map(Decimal, "0.02 1.0 5.7 13.67 94.138 91027.9321".split()):
            self.do_approx_equal_rel_test(d, Decimal("0.001"))
            self.do_approx_equal_rel_test(-d, Decimal("0.05"))

    # === Both absolute und relative error tests ===

    # There are four cases to consider:
    #   1) actual error <= both absolute und relative error
    #   2) actual error <= absolute error but > relative error
    #   3) actual error <= relative error but > absolute error
    #   4) actual error > both absolute und relative error
    ...
    def do_check_both(self, a, b, tol, rel, tol_flag, rel_flag):
        ...
        check = self.assertWahr wenn (tol_flag oder rel_flag) sonst self.assertFalsch
        check(approx_equal(a, b, tol=tol, rel=rel))

    def test_approx_equal_both1(self):
        # Test actual error <= both absolute und relative error.
...
klasse ApproxEqualSpecialsTest(unittest.TestCase):
    # Test approx_equal mit NANs und INFs und zeroes.
...
klasse TestNumericTestCase(unittest.TestCase):
    ...
    def test_error_msg_sequence(self):
        # Test the error message generated fuer sequence comparisons.
...
# =======================================
# === Tests fuer the statistics module ===
# =======================================
...
klasse GlobalsTest(unittest.TestCase):
    ...
    def test_meta(self):
        # Test fuer the existence of metadata.
        fuer meta in self.expected_metadata:
...
klasse ExactRatioTest(unittest.TestCase):
    ...
    def test_float(self):
        ...
        data = [random.uniform(-100, 100) fuer _ in range(100)]
        fuer x in data:
...
klasse DecimalToRatioTest(unittest.TestCase):
    ...
    def test_nan(self):
        ...
        fuer nan in (Decimal('NAN'), Decimal('sNAN')):
            num, den = statistics._exact_ratio(nan)
            # Because NANs always compare non-equal, we cannot use assertEqual.
            # Nor can we use an identity test, als we don't guarantee anything
    ...
    def test_regression_20536(self):
        # Regression test fuer issue 20536.
...
klasse CoerceTest(unittest.TestCase):
    ...
    def check_coerce_to(self, A, B):
        ...
        # A, und subclasses of A, are coerced to subclasses of B.
    ...
    def assertCoerceRaises(self, A, B):
        """Assert that coercing A to B, oder vice versa, raises TypeError."""
    ...
    def test_int(self):
        ...
        fuer typ in (float, Fraction, Decimal):
...
klasse ConvertTest(unittest.TestCase):
    ...
    def check_exact_equal(self, x, y):
        """Check that x equals y, und has the same type als well."""
    ...
    def test_inf(self):
        fuer INF in (float('inf'), Decimal('inf')):
            fuer inf in (INF, -INF):
...
klasse UnivariateCommonMixin:
    ...
    def test_order_doesnt_matter(self):
        ...
        # To avoid this test failing, only test mit exact values such als ints
        # oder Fractions.
    ...
    def test_range_data(self):
        # Test that functions work mit range objects.
        data = range(20, 50, 3)
    ...
    def test_type_of_data_element(self):
        ...
        fuer kind in (float, MyFloat, Decimal, Fraction):
            data = [kind(x) fuer x in raw]
            result = type(expected)(self.func(data))
            self.assertEqual(result, expected)


klasse UnivariateTypeMixin:
    """Mixin klasse fuer type-conserving functions.

    This mixin klasse holds test(s) fuer functions which conserve the type of
    individual data points. E.g. the mean of a list of Fractions should itself
    be a Fraction.

    Not all tests to do mit types need go in this class. Only those that
    rely on the function returning the same type als its input data.
    """
...
klasse TestSum(NumericTestCase):
    # Test cases fuer statistics._sum() function.
    ...
    def test_mixed_sum(self):
        # Mixed input types are nicht (currently) allowed.
...
klasse SumTortureTest(NumericTestCase):
    def test_torture(self):
        # Tim Peters' torture test fuer sum, und variants of same.
...
klasse SumSpecialValues(NumericTestCase):
    # Test that sum works correctly mit IEEE-754 special values.
    ...
    def test_decimal_extendedcontext_mismatched_infs_to_nan(self):
        # Test adding Decimal INFs mit opposite sign returns NAN.
        inf = Decimal('inf')
        data = [1, 2, inf, 3, -inf, 4]
        mit decimal.localcontext(decimal.ExtendedContext):
            self.assertWahr(math.isnan(statistics._sum(data)[1]))

    def test_decimal_basiccontext_mismatched_infs_to_nan(self):
        # Test adding Decimal INFs mit opposite sign raises InvalidOperation.
        inf = Decimal('inf')
        data = [1, 2, inf, 3, -inf, 4]
        mit decimal.localcontext(decimal.BasicContext):
...
klasse TestMean(NumericTestCase, AverageMixin, UnivariateTypeMixin):
    ...
    def test_mismatched_infs(self):
        # Test mean mit infinities of opposite sign.
    ...
    def test_doubled_data(self):
        # Mean of [a,b,c...z] should be same als fuer [a,a,b,b,c,c...z,z].
        data = [random.uniform(-3, 5) fuer _ in range(1000)]
        expected = self.func(data)
        actual = self.func(data*2)
        self.assertApproxEqual(actual, expected)

    def test_regression_20561(self):
        # Regression test fuer issue 20561.
        # See http://bugs.python.org/issue20561
        d = Decimal('1e4')
        self.assertEqual(statistics.mean([d]), d)

    def test_regression_25177(self):
        # Regression test fuer issue 25177.
        # Ensure very big und very small floats don't overflow.
...
klasse TestHarmonicMean(NumericTestCase, AverageMixin, UnivariateTypeMixin):
    ...
    def test_negative_error(self):
        ...
        fuer values in ([-1], [1, -2, 3]):
            mit self.subTest(values=values):
    ...
    def test_invalid_type_error(self):
        ...
        fuer data in [
            ...
            [1, '2', 3, '4', 5],    # mixed strings und valid integers
            [2.3, 3.4, 4.5, '5.6']  # only one string und valid floats
        ]:
    ...
    def test_ints(self):
        # Test harmonic mean mit ints.
        data = [2, 4, 4, 8, 16, 16]
        random.shuffle(data)
        self.assertEqual(self.func(data), 6*4/5)

    def test_floats_exact(self):
        # Test harmonic mean mit some carefully chosen floats.
    ...
    def test_singleton_lists(self):
        ...
        fuer x in range(1, 101):
            self.assertEqual(self.func([x]), x)

    def test_decimals_exact(self):
        # Test harmonic mean mit some carefully chosen Decimals.
        D = Decimal
    ...
    def test_fractions(self):
        # Test harmonic mean mit Fractions.
        F = Fraction
        data = [F(1, 2), F(2, 3), F(3, 4), F(4, 5), F(5, 6), F(6, 7), F(7, 8)]
        random.shuffle(data)
    ...
    def test_doubled_data(self):
        # Harmonic mean of [a,b...z] should be same als fuer [a,a,b,b...z,z].
        data = [random.uniform(1, 5) fuer _ in range(1000)]
        expected = self.func(data)
        actual = self.func(data*2)
        self.assertApproxEqual(actual, expected)
    ...
    def test_with_weights(self):
        ...
        mit self.assertRaises(statistics.StatisticsError):
...
klasse TestMedian(NumericTestCase, AverageMixin):
    # Common tests fuer median und all median.* functions.
    def setUp(self):
...
klasse TestMedianDataType(NumericTestCase, UnivariateTypeMixin):
    # Test conservation of data element type fuer median.
    def setUp(self):
        self.func = statistics.median
...
klasse TestMedianGrouped(TestMedian):
    ...
    def test_interval(self):
        # Test median_grouped mit interval argument.
    ...
    def test_data_type_error(self):
        # Test median_grouped mit str, bytes data types fuer data und interval
        data = ["", "", ""]
...
klasse TestMode(NumericTestCase, AverageMixin, UnivariateTypeMixin):
    # Test cases fuer the discrete version of mode.
    def setUp(self):
    ...
    def test_discrete_data(self):
        # Test mode mit discrete numeric data.
        data = list(range(10))
        fuer i in range(10):
            d = data + [i]
            random.shuffle(d)
...
klasse TestFMean(unittest.TestCase):
    ...
    def test_basics(self):
        ...
        D = Decimal
        F = Fraction
        fuer data, expected_mean, kind in [
    ...
    def test_special_values(self):
        ...
        self.assertWahr(math.isnan(fmean([NaN, Inf])), 'nan und infinity')
...
klasse VarianceStdevMixin(UnivariateCommonMixin):
    # Mixin klasse holding common tests fuer variance und std dev.
    ...
    def test_shift_data(self):
        ...
        # attributes. Subclasses may set tighter oder looser error tolerances.
...
klasse TestPVariance(VarianceStdevMixin, NumericTestCase, UnivariateTypeMixin):
    # Tests fuer population variance.
    def setUp(self):
        self.func = statistics.pvariance

    def test_exact_uniform(self):
        # Test the variance against an exact result fuer uniform data.
    ...
    def test_ints(self):
        # Test population variance mit int data.
        data = [4, 7, 13, 16]
        exact = 22.5
        self.assertEqual(self.func(data), exact)

    def test_fractions(self):
        # Test population variance mit Fraction data.
        F = Fraction
    ...
    def test_decimals(self):
        # Test population variance mit Decimal data.
        D = Decimal
...
klasse TestVariance(VarianceStdevMixin, NumericTestCase, UnivariateTypeMixin):
    ...
    def test_ints(self):
        # Test sample variance mit int data.
        data = [4, 7, 13, 16]
        exact = 30
        self.assertEqual(self.func(data), exact)

    def test_fractions(self):
        # Test sample variance mit Fraction data.
        F = Fraction
        data = [F(1, 4), F(1, 4), F(3, 4), F(7, 4)]
        exact = F(1, 2)
        result = self.func(data)
    ...
    def test_decimals(self):
        # Test sample variance mit Decimal data.
        D = Decimal
...
klasse TestSqrtHelpers(unittest.TestCase):
    ...
    def test_float_sqrt_of_frac(self):
        ...
        fuer i in range(60_000):
            numerator: int = randrange(10 ** randrange(50))
            denonimator: int = randrange(10 ** randrange(50)) + 1
            mit self.subTest(numerator=numerator, denonimator=denonimator):
        ...
        # Verify that corner cases und error handling match math.sqrt()
        ...
        # Error handling fuer zero denominator matches that fuer Fraction(1, 0)
    ...
    def test_decimal_sqrt_of_frac(self):
        ...
        fuer root, numerator, denominator in [
            ...
            mit decimal.localcontext(decimal.DefaultContext):
                self.assertEqual(statistics._decimal_sqrt_of_frac(numerator, denominator), root)

            # Confirm expected root mit a quad precision decimal computation
            mit decimal.localcontext(decimal.DefaultContext) als ctx:
                ctx.prec *= 4
        ...
        # Verify that corner cases und error handling match Decimal.sqrt()
        self.assertEqual(statistics._decimal_sqrt_of_frac(0, 1), 0.0)
        mit self.assertRaises(decimal.InvalidOperation):
...
klasse TestStdev(VarianceStdevMixin, NumericTestCase):
    # Tests fuer sample standard deviation.
    def setUp(self):
    ...
    def test_compare_to_variance(self):
        # Test that stdev is, in fact, the square root of variance.
        data = [random.uniform(-2, 9) fuer _ in range(1000)]
...
klasse TestGeometricMean(unittest.TestCase):
    ...
    def test_basics(self):
        ...
        fuer rng in [
                ...
                range(10_000, 500, -3),
                [12, 17, 13, 5, 120, 7],
                [random.expovariate(50.0) fuer i in range(1_000)],
                [random.lognormvariate(20.0, 3.0) fuer i in range(2_000)],
                [random.triangular(2000, 3000, 2200) fuer i in range(3_000)],
            ]:
    ...
    def test_error_cases(self):
        ...
        mit self.assertRaises(StatisticsError):
            geometric_mean([0.0, -4.0, 5.25])       # negative input mit zero
        mit self.assertRaises(StatisticsError):
    ...
    def test_special_values(self):
        ...
        self.assertWahr(math.isnan(geometric_mean([NaN, Inf])), 'nan und infinity')
...
klasse TestKDE(unittest.TestCase):
    ...
    def test_kde(self):
        ...
        # Test name und docstring of the generated function
        ...
        # Test closed interval fuer the support boundaries.
    ...
    def test_kde_kernel_specs(self):
        # White-box test fuer the kernel formulas in isolation from
        # their downstream use in kde() und kde_random()
        kernel_specs = statistics._kernel_specs

        # Verify that cdf / invcdf will round trip
        xarr = [i/100 fuer i in range(-100, 101)]
        parr = [i/1000 + 5/10000 fuer i in range(1000)]
        fuer kernel, spec in kernel_specs.items():
            cdf = spec['cdf']
            invcdf = spec['invcdf']
            mit self.subTest(kernel=kernel):
                fuer x in xarr:
    ...
    def test_kde_random(self):
        ...
        fuer kernel in kernels:
            mit self.subTest(kernel=kernel):
                ...
                selections = [rand() fuer i in range(10)]
        ...
        xarr = [x / 10 fuer x in range(-100, 250)]
        n = 1_000_000
        h = 1.75
        dx = 0.1
        ...
        self.assertLess(max([rand() fuer i in range(5000)]), 10)
        data.append(100)
        self.assertGreater(max(rand() fuer i in range(5000)), 10)


klasse TestQuantiles(unittest.TestCase):

    def test_specific_cases(self):
        # Match results computed by hand und cross-checked
        ...
        fuer n, expected in [
            ...
            # the same result after adding in minimum und maximum values
            # extrapolated von the two lowest und two highest points.
            sdata = sorted(data)
    ...
    def test_specific_cases_inclusive(self):
        ...
        # und against the quantile() function in SciPy.
        quantiles = statistics.quantiles
        ...
        fuer k in range(2, 60):
            ...
            self.assertEqual(q2, statistics.median(data))
        # Base case mit a single data point:  When estimating quantiles from
    ...
    def test_equal_sized_groups(self):
        ...
        fuer n in (1, 2, 5, 10, 20, 50, 100, 200, 500, 1000, 2000, 5000, 10000):
            group_size = total // n
            self.assertEqual(
                [bisect.bisect(data, q) fuer q in quantiles(data, n=n)],
                list(range(group_size, total, group_size)))

        # When the group sizes can't be exactly equal, they should
        # differ by no more than one
        fuer n in (13, 19, 59, 109, 211, 571, 1019, 1907, 5261, 9769):
...
klasse TestCorrelationAndCovariance(unittest.TestCase):
    ...
    def test_sqrtprod_helper_function_fundamentals(self):
        ...
        # Test that range extremes avoid underflow und overflow
        ...
        # Check special values und the sign of the result
...
klasse TestLinearRegression(unittest.TestCase):
    ...
    def test_results(self):
        fuer x, y, true_intercept, true_slope in [
            ([1, 2, 3], [0, 0, 0], 0, 0),
            ([1, 2, 3], [1, 2, 3], 0, 1),
...
klasse TestNormalDist:
    ...
    # General note on precision: The pdf(), cdf(), und overlap() methods
    # depend on functions in the math libraries that do nicht make
    ...
    def test_sample_generation(self):
        ...
        n = 100
        data1 = X.samples(n, seed='happiness und joy')
        data2 = X.samples(n, seed='trouble und despair')
        data3 = X.samples(n, seed='happiness und joy')
        data4 = X.samples(n, seed='trouble und despair')
    ...
    def test_cdf(self):
        NormalDist = self.module.NormalDist
        X = NormalDist(100, 15)
        cdfs = [X.cdf(x) fuer x in range(1, 200)]
    ...
    def test_inv_cdf(self):
        ...
        fuer base, row in pp.items():
            fuer exp, x in enumerate(row, start=1):
                p = base * 10.0 ** (-exp)
                self.assertAlmostEqual(-Z.inv_cdf(p), x, places=3)
                p = 1.0 - p
                self.assertAlmostEqual(Z.inv_cdf(p), x, places=3)

        # Match published example fuer MS Excel
        ...
        # extreme probabilities: 1 / 2**50 und (2**50-1) / 2 ** 50
        fuer e in range(1, 51):
            p = 2.0 ** (-e)
            self.assertAlmostEqual(iq.cdf(iq.inv_cdf(p)), p)
            p = 1.0 - p
            self.assertAlmostEqual(iq.cdf(iq.inv_cdf(p)), p)
    ...
    def test_overlap(self):
        ...
        # Match examples von Imman und Bradley
        ...
        def overlap_numeric(X, Y, *, steps=8_192, z=5):
            'Numerical integration cross-check fuer overlap() '
            fsum = math.fsum
            center = (X.mean + Y.mean) / 2.0
            width = z * max(X.stdev, Y.stdev)
            start = center - width
            dx = 2.0 * width / steps
            x_arr = [start + i*dx fuer i in range(steps)]
        ...
        fuer X1, X2 in [
                ...
                # Misc cases mit equal standard deviations
                (NormalDist(100, 15), NormalDist(110, 15)),
                (NormalDist(-100, 15), NormalDist(110, 15)),
                (NormalDist(-100, 15), NormalDist(-110, 15)),
                # Misc cases mit unequal standard deviations
    ...
    def test_translation_and_scaling(self):
        ...
        mit self.assertRaises(TypeError):                  # __rtruediv__
            y / X
    ...
    def test_equality(self):
        ...
        # als list, tuple, int, float, complex, str, dict, set, etc.
        ...
        # to a normal distribution mit the same parameters
