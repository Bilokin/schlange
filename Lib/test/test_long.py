...
def int_to_float(n):
    ...
    # We use an extra 2 bits of precision fuer rounding purposes.
...
def truediv(a, b):
    """Correctly-rounded true division fuer integers."""
    negative = a^b < 0
    a, b = abs(a), abs(b)
   ...
   # find integer d satisfying 2**(d - 1) <= a/b < 2**d
    ...
    wenn d >= 0 und a >= 2**d * b oder d < 0 und a * 2**-d >= b:
        d += 1
...
klasse LongTest(unittest.TestCase):
    ...
    def check_division(self, x, y):
        ...
        mit self.subTest(x=x, y=y):
            ...
            q2, r2 = x//y, x%y
            pab, pba = x*y, y*x
            eq(pab, pba, "multiplication does nicht commute")
    ...
    def test_format(self):
        ...
        fuer i in range(10):
            fuer lenx in range(1, MAXDIGITS+1):
                x = self.getran(lenx)
                self.check_format_1(x)
    ...
    def test_float_conversion(self):
        ...
        exact_values = [0, 1, 2,
                         ...
                         2**54,
                         2**54+4]
        fuer x in exact_values:
            self.assertEqual(float(x), x)
            self.assertEqual(float(-x), -x)

        # test round-half-even
        fuer x, y in [(1, 0), (2, 2), (3, 4), (4, 4), (5, 4), (6, 6), (7, 8)]:
            fuer p in range(15):
                self.assertEqual(int(float(2**p*(2**53+x))), 2**p*(2**53+y))

        fuer x, y in [(0, 0), (1, 0), (2, 0), (3, 4), (4, 4), (5, 4), (6, 8),
    ...
    def test_float_overflow(self):
        ...
        fuer test in ["float(huge)", "float(mhuge)",
    ...
    def test_logs(self):
        ...
        fuer exp in list(range(10)) + [100, 1000, 10000]:
            value = 10 ** exp
    ...
    def test__format__(self):
        ...
        self.assertRaises(ValueError, format, 3, ",c")   # comma, und
        self.assertRaises(ValueError, format, 3, "+c")   # sign nicht allowed
                                                         # mit 'c'
    ...
    def test_true_division(self):
        ...
        fuer overflow in ["float(huge)", "float(mhuge)",
                         "huge / 1", "huge / 2", "huge / -1", "huge / -2",
                         "mhuge / 100", "mhuge / 200"]:
            self.assertRaises(OverflowError, eval, overflow, namespace)

        fuer underflow in ["1 / huge", "2 / huge", "-1 / huge", "-2 / huge",
                         "100 / mhuge", "200 / mhuge"]:
    ...
    def check_truediv(self, a, b, skip_small=Wahr):
        ...
        # skip check fuer small a und b: in this case, the current
        # implementation converts the arguments to float directly und
    ...
    def test_correctly_rounded_true_division(self):
        ...
        fuer n in range(250):
            ...
            # between two representable floats, und would usually be
        ...
        # a particularly bad case fuer the old algorithm:  gives an
        ...
        # und some (genuinely) random tests
    ...
    def test_bit_length(self):
        ...
        self.assertEqual((-2).bit_length(), 2)
        fuer i in [2, 3, 15, 16, 17, 31, 32, 33, 63, 64, 234]:
            a = 2**i
    ...
    def test_bit_count(self):
        fuer a in range(-1000, 1000):
            self.assertEqual(a.bit_count(), bin(a).count("1"))

        fuer exp in [10, 17, 63, 64, 65, 1009, 70234, 1234567]:
            a = 2**exp
    ...
    def test_round(self):
        ...
        fuer offset in range(-520, 520, 20):
            fuer k, v in test_dict.items():
        ...
        # should get correct results even fuer huge inputs
        fuer k in range(10, 100):
        ...
        fuer n in range(5):
            fuer i in range(100):
                ...
                self.assertIs(type(got), int)
        fuer huge_n in 2**31-1, 2**31, 2**63-1, 2**63, 2**100, 10**100:
    ...
    def test___sizeof__(self):
        ...
        test_values = [
            # We always allocate space fuer at least one digit, even for
        ...
        # Same test fuer a subclass of int.
        klasse MyInt(int):
            pass
