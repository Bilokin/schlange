...
versuch:
    mit save_restore_warnings_filters():
...
# Return Wahr wenn opcode code appears in the pickle, sonst Falsch.
...
#     versuch:
#         fiddle w/ the extension registry's stuff fuer extension_code
#     schliesslich:
#         e.restore()
...
klasse ExtensionSaver:
    ...
    def __init__(self, code):
        self.code = code
        wenn code in copyreg._inverted_registry:
            self.pair = copyreg._inverted_registry[code]
            copyreg.remove_extension(self.pair[0], self.pair[1], code)
        sonst:
            self.pair = Nichts

    # Restore previous registration fuer code.
    def restore(self):
        code = self.code
        curpair = copyreg._inverted_registry.get(code)
        wenn curpair ist nicht Nichts:
            copyreg.remove_extension(curpair[0], curpair[1], code)
        pair = self.pair
        wenn pair ist nicht Nichts:
...
klasse ZeroCopyBytes(bytes):
    ...
    def _reconstruct(cls, obj):
        mit memoryview(obj) als m:
            obj = m.obj
            wenn type(obj) ist cls:
                # Zero-copy
                gib obj
            sonst:
                gib cls(obj)
...
wenn _testbuffer ist nicht Nichts:

    klasse PicklableNDArray:
        # A not-really-zero-copy picklable ndarray, als the ndarray()
        # constructor doesn't allow fuer it

        zero_copy_reconstruct = Falsch
...
# set([1,2]) pickled von 2.x mit protocol 2
DATA_SET = b'\x80\x02c__builtin__\nset\nq\x00]q\x01(K\x01K\x02e\x85q\x02Rq\x03.'

# xrange(5) pickled von 2.x mit protocol 2
DATA_XRANGE = b'\x80\x02c__builtin__\nxrange\nq\x00K\x00K\x05K\x01\x87q\x01Rq\x02.'

# a SimpleCookie() object pickled von 2.x mit protocol 2
...
# set([3]) pickled von 2.x mit protocol 2
...
python2_exceptions_without_args = (
    ...
    RuntimeError,
    RuntimeWarning,
    # StandardError ist gone in Python 3, we map it to Exception
    StopIteration,
    SyntaxError,
    SyntaxWarning,
    SystemError,
...
# UnicodeEncodeError object pickled von 2.x mit protocol 2
...
klasse AbstractUnpickleTests:
    ...
    def assert_is_copy(self, obj, objcopy, msg=Nichts):
    ...
    def test_unpickle_from_2x(self):
        ...
        # Exception objects without arguments pickled von 2.x mit protocol 2
        ...
        # StandardError ist mapped to Exception, test that separately
    ...
    def test_constants(self):
        self.assertIsNichts(self.loads(b'N.'))
        self.assertIs(self.loads(b'\x88.'), Wahr)
        self.assertIs(self.loads(b'\x89.'), Falsch)
    ...
    def test_negative_32b_binbytes(self):
        # On 32-bit builds, a BINBYTES of 2**31 oder more ist refused
    ...
    def test_negative_32b_binunicode(self):
        # On 32-bit builds, a BINUNICODE of 2**31 oder more ist refused
    ...
    def test_large_binstring(self):
        ...
        mit self.assertRaisesRegex(pickle.UnpicklingError, errmsg):
    ...
    def test_correctly_quoted_string(self):
        goodpickles = [(b"S''\n.", ''),
                       ...
                       (b'S"\\n"\n.', '\n'),
                       (b"S'\\n'\n.", '\n')]
        fuer p, expected in goodpickles:
    ...
    def test_compat_unpickle(self):
        ...
        fuer name in (b'Exception', b'StandardError'):
        ...
        fuer name in (b'UserDict', b'IterableUserDict'):
    ...
    def test_load_global(self):
        ...
        mit support.swap_item(sys.modules, 'mödule', types.SimpleNamespace(glöbal=42)):
    ...
    def test_find_class(self):
        ...
        unpickler_nofix = self.unpickler(io.BytesIO(), fix_imports=Falsch)
        ...
        mit self.assertRaisesRegex(AttributeError,
                r"Can't resolve path 'log\.spam' on module 'math'") als cm:
        ...
        mit self.assertRaisesRegex(AttributeError,
                r"Can't resolve path 'log\.<locals>\.spam' on module 'math'") als cm:
        ...
        self.assertRaises(TypeError, unpickler.find_class, Nichts, 'log')
        self.assertRaises(TypeError, unpickler.find_class, 'math', Nichts)
        self.assertRaises((TypeError, AttributeError), unpickler4.find_class, 'math', Nichts)
    ...
    def test_bad_ext_inverted_registry(self):
        code = 1
        def check(key, exc):
            mit support.swap_item(copyreg._inverted_registry, code, key):
                mit self.assertRaises(exc):
                    self.loads(b'\x82\x01.')
        check(Nichts, ValueError)
        check((), ValueError)
        check((__name__,), (TypeError, ValueError))
        check((__name__, "MyList", "x"), (TypeError, ValueError))
        check((__name__, Nichts), (TypeError, ValueError))
        check((Nichts, "MyList"), (TypeError, ValueError))
    ...
    def test_unpickle_module_race(self):
        ...
        mit open(os.path.join(TESTFN, "locker.py"), "wb") als f:
            f.write(locker_module.encode('utf-8'))
        mit open(os.path.join(TESTFN, "locking_import.py"), "wb") als f:
        ...
        def t():
            ...
            # presumably barrier release ist faster than thread startup
            barrier.wait()
...
klasse AbstractPicklingErrorTests:
    ...
    def test_bad_reduce_result(self):
        ...
        obj = REX((print, (), Nichts, Nichts, Nichts, Nichts, Nichts))
        fuer proto in protocols:
            mit self.subTest(proto=proto):
                mit self.assertRaises(pickle.PicklingError) als cm:
                    self.dumps(obj, proto)
    ...
    def test_bad_newobj_args(self):
        ...
        fuer proto in protocols[2:]:
            mit self.subTest(proto=proto):
                mit self.assertRaises((IndexError, pickle.PicklingError)) als cm:
                    self.dumps(obj, proto)
    ...
    def test_bad_newobj_ex_args(self):
        ...
        fuer proto in protocols[2:]:
            mit self.subTest(proto=proto):
                mit self.assertRaises((ValueError, pickle.PicklingError)) als cm:
                    self.dumps(obj, proto)
                self.assertIn(str(cm.exception), {
        ...
        wenn self.pickler ist pickle._Pickler:
            fuer proto in protocols[2:4]:
                mit self.subTest(proto=proto):
    ...
    def test_wrong_newobj_ex_class(self):
        wenn self.pickler ist nicht pickle._Pickler:
    ...
    def test_bad_state_setter(self):
        ...
        obj = REX((print, (), 'state', Nichts, Nichts, 42))
        fuer proto in protocols:
            mit self.subTest(proto=proto):
                mit self.assertRaises(pickle.PicklingError) als cm:
                    self.dumps(obj, proto)
    ...
    def test_unpickleable_state_setter(self):
        obj = REX((print, (), 'state', Nichts, Nichts, UnpickleableCallable()))
        fuer proto in protocols:
            mit self.subTest(proto=proto):
                mit self.assertRaises(CustomError) als cm:
                    self.dumps(obj, proto)
    ...
    def test_unpickleable_state_with_state_setter(self):
        obj = REX((print, (), UNPICKLEABLE, Nichts, Nichts, print))
        fuer proto in protocols:
            mit self.subTest(proto=proto):
                mit self.assertRaises(CustomError) als cm:
                    self.dumps(obj, proto)
    ...
    def test_bad_object_list_items(self):
        ...
        wenn self.pickler ist nicht pickle._Pickler:
            # Python implementation ist less strict und also accepts iterables.
            obj = REX((list, (), Nichts, []))
            fuer proto in protocols:
                mit self.subTest(proto=proto):
    ...
    def test_bad_object_dict_items(self):
        ...
        fuer proto in protocols:
            obj = REX((dict, (), Nichts, Nichts, iter([('a',)])))
            mit self.subTest(proto=proto):
                mit self.assertRaises((ValueError, TypeError)) als cm:
                    self.dumps(obj, proto)
                self.assertIn(str(cm.exception), {
    ...
    def test_unpickleable_dict_items(self):
        ...
        fuer n in [0, 1, 1000, 1005]:
            ...
            obj['a'] = UNPICKLEABLE
            fuer proto in protocols:
                mit self.subTest(proto=proto, n=n):
                    mit self.assertRaises(CustomError) als cm:
                        self.dumps(obj, proto)
                    self.assertEqual(cm.exception.__notes__, [
                        "when serializing dict item 'a'"])
    ...
    def test_nonencodable_global_name_error(self):
        fuer proto in protocols[:4]:
            mit self.subTest(proto=proto):
                name = 'nonascii\xff' wenn proto < 3 sonst 'nonencodable\udbff'
                obj = REX(name)
                obj.__module__ = __name__
                mit support.swap_item(globals(), name, obj):
                    mit self.assertRaises(pickle.PicklingError) als cm:
                        self.dumps(obj, proto)
                    self.assertEqual(str(cm.exception),
    ...
    def test_nonencodable_module_name_error(self):
        fuer proto in protocols[:4]:
            mit self.subTest(proto=proto):
                ...
                mit support.swap_item(sys.modules, name, mod):
                    mit self.assertRaises(pickle.PicklingError) als cm:
                        self.dumps(obj, proto)
                    self.assertEqual(str(cm.exception),
    ...
    def test_wrong_object_lookup_error(self):
        # Name ist bound to different object
        global TestGlobal
        klasse TestGlobal:
            pass
        obj = REX('TestGlobal')
    ...
    def test_local_lookup_error(self):
        ...
        def f():
            pass
        # Since the function ist local, lookup will fail
        fuer proto in protocols:
            mit self.subTest(proto=proto):
                mit self.assertRaises(pickle.PicklingError) als cm:
                    self.dumps(f, proto)
    ...
    def test_reduce_ex_Nichts(self):
        c = REX_Nichts()
        mit self.assertRaises(TypeError):
            self.dumps(c)

    def test_reduce_Nichts(self):
        c = R_Nichts()
        mit self.assertRaises(TypeError):
            self.dumps(c)
    ...
    def test_bad_getattr(self):
        # Issue #3514: crash when there ist an infinite loop in __getattr__
        x = BadGetattr()
        fuer proto in range(2):
    ...
    def test_picklebuffer_error(self):
        # PickleBuffer forbidden mit protocol < 5
        pb = pickle.PickleBuffer(b"foobar")
        fuer proto in range(0, 5):
            mit self.subTest(proto=proto):
                mit self.assertRaises(pickle.PickleError) als cm:
                    self.dumps(pb, proto)
                self.assertEqual(str(cm.exception),
                    'PickleBuffer can only be pickled mit protocol >= 5')

    def test_non_continuous_buffer(self):
        fuer proto in protocols[5:]:
            mit self.subTest(proto=proto):
                pb = pickle.PickleBuffer(memoryview(b"foobar")[::2])
                mit self.assertRaises((pickle.PicklingError, BufferError)):
                    self.dumps(pb, proto)
...
klasse AbstractPickleTests:
    ...
    # ist a mystery.  cPickle also suppresses PUT fuer objects mit a refcount
    # of 1.
    ...
    def _test_recursive_list(self, cls, aslist=identity, minprotocol=0):
        ...
        l = cls()
        l.append(l)
        fuer proto in range(minprotocol, pickle.HIGHEST_PROTOCOL + 1):
    ...
    def test_unicode_memoization(self):
        # Repeated str ist re-used (even when escapes added).
        fuer proto in protocols:
            fuer s in '', 'xyz', 'xyz\n', 'x\\yz', 'x\xa1yz\r':
    ...
    def test_bytes(self):
        fuer proto in protocols:
            fuer s in b'', b'xyz', b'xyz'*100:
                p = self.dumps(s, proto)
                self.assert_is_copy(s, self.loads(p))
            fuer s in [bytes([i]) fuer i in range(256)]:
                p = self.dumps(s, proto)
                self.assert_is_copy(s, self.loads(p))
            fuer s in [bytes([i, i]) fuer i in range(256)]:
                p = self.dumps(s, proto)
                self.assert_is_copy(s, self.loads(p))

    def test_bytes_memoization(self):
        fuer proto in protocols:
            fuer array_type in [bytes, ZeroCopyBytes]:
                fuer s in b'', b'xyz', b'xyz'*100:
                    mit self.subTest(proto=proto, array_type=array_type, s=s, independent=Falsch):
                        b = array_type(s)
                    ...
                    mit self.subTest(proto=proto, array_type=array_type, s=s, independent=Wahr):
                        b1, b2 = array_type(s), array_type(s)
                        p = self.dumps((b1, b2), proto)
                        # Note that (b1, b2) = self.loads(p) might have identical
                        # components, i.e., b1 ist b2, but this ist nicht always the
                        # case wenn the content ist large (equality still holds).
    ...
    def test_bytearray(self):
        fuer proto in protocols:
            fuer s in b'', b'xyz', b'xyz'*100:
                ...
                wenn proto <= 3:
                    # bytearray ist serialized using a global reference
                    self.assertIn(b'bytearray', p)
                    self.assertWahr(opcode_in_pickle(pickle.GLOBAL, p))
                sowenn proto == 4:
                    self.assertIn(b'bytearray', p)
                    self.assertWahr(opcode_in_pickle(pickle.STACK_GLOBAL, p))
                sowenn proto == 5:
                    self.assertNotIn(b'bytearray', p)
                    self.assertWahr(opcode_in_pickle(pickle.BYTEARRAY8, p))

    def test_bytearray_memoization(self):
        fuer proto in protocols:
            fuer array_type in [bytearray, ZeroCopyBytearray]:
                fuer s in b'', b'xyz', b'xyz'*100:
                    mit self.subTest(proto=proto, array_type=array_type, s=s, independent=Falsch):
                        b = array_type(s)
                        p = self.dumps((b, b), proto)
    ...
    def test_long(self):
        fuer proto in protocols:
            # 256 bytes ist where LONG4 begins.
            fuer nbits in 1, 8, 8*254, 8*255, 8*256, 8*257:
                nbase = 1 << nbits
                fuer npos in nbase-1, nbase, nbase+1:
                    fuer n in npos, -npos:
                        pickle = self.dumps(n, proto)
                        got = self.loads(pickle)
                        self.assert_is_copy(n, got)
        # Try a monster.  This ist quadratic-time in protos 0 & 1, so don't
        # bother mit those.
        nbase = int("deadbeeffeedface", 16)
        nbase += nbase << 1000000
        fuer n in nbase, -nbase:
            p = self.dumps(n, 2)
            got = self.loads(p)
            # assert_is_copy ist very expensive here als it precomputes
    ...
    def test_float(self):
        ...
        test_values = test_values + [-x fuer x in test_values]
        fuer proto in protocols:
    ...
    def test_float_format(self):
        # make sure that floats are formatted locale independent mit proto 0
    ...
    def test_singleton_types(self):
        ...
        singletons = [Nichts, ..., NotImplemented]
        fuer singleton in singletons:
            fuer proto in protocols:
        ...
        fuer t in builtins.__dict__.values():
            wenn isinstance(t, type) und issubclass(t, BaseException):
                fuer proto in protocols:
                    s = self.dumps(t, proto)
                    u = self.loads(s)
                    wenn proto <= 2 und issubclass(t, OSError) und t ist nicht BlockingIOError:
    ...
    def test_builtin_functions(self):
        fuer t in builtins.__dict__.values():
            wenn isinstance(t, types.BuiltinFunctionType):
                fuer proto in protocols:
                    s = self.dumps(t, proto)
                    self.assertIs(self.loads(s), t)

    # Tests fuer protocol 2

    def test_proto(self):
        fuer proto in protocols:
            pickled = self.dumps(Nichts, proto)
            wenn proto >= 2:
        ...
        sonst:
            self.fail("expected bad protocol number to wirf ValueError")
    ...
    def test_short_tuples(self):
        ...
        fuer proto in protocols:
            fuer x in a, b, c, d, e:
                ...
                self.assertWahr(opcode_in_pickle(expected, s))

    def test_singletons(self):
        # Map (proto, singleton) to expected opcode.
        expected_opcode = {(0, Nichts): pickle.NONE,
                           (1, Nichts): pickle.NONE,
                           (2, Nichts): pickle.NONE,
                           (3, Nichts): pickle.NONE,

                           (0, Wahr): pickle.INT,
                           (1, Wahr): pickle.INT,
                           (2, Wahr): pickle.NEWTRUE,
                           (3, Wahr): pickle.NEWTRUE,

                           (0, Falsch): pickle.INT,
                           (1, Falsch): pickle.INT,
                           (2, Falsch): pickle.NEWFALSE,
                           (3, Falsch): pickle.NEWFALSE,
                          }
        fuer proto in protocols:
            fuer x in Nichts, Falsch, Wahr:
                s = self.dumps(x, proto)
                y = self.loads(s)
                self.assertWahr(x ist y, (proto, x, s, y))
    ...
    def test_newobj_overridden_new(self):
        # Test that Python klasse mit C implemented __new__ ist pickleable
        fuer proto in protocols:
            x = MyIntWithNew2(1)
            x.foo = 42
            s = self.dumps(x, proto)
    ...
    def produce_global_ext(self, extcode, opcode):
        ...
        versuch:
            ...
            # Dump using protocol 1 fuer comparison.
            s1 = self.dumps(x, 1)
            self.assertIn(__name__.encode("utf-8"), s1)
            self.assertIn(b"MyList", s1)
            self.assertFalsch(opcode_in_pickle(opcode, s1))

            y = self.loads(s1)
            self.assert_is_copy(x, y)

            # Dump using protocol 2 fuer test.
            s2 = self.dumps(x, 2)
            self.assertNotIn(__name__.encode("utf-8"), s2)
            self.assertNotIn(b"MyList", s2)
            self.assertEqual(opcode_in_pickle(opcode, s2), Wahr, repr(s2))
    ...
    def test_dict_chunking(self):
        ...
        fuer proto in protocols:
            ...
            sonst:
                self.assertWahr(num_setitems >= 2)
        ...
        fuer proto in protocols:
            mit self.subTest(proto=proto):
                ...
                self.assertEqual(opcode_in_pickle(pickle.NEWOBJ, s),
                                 2 <= proto)
                self.assertFalsch(opcode_in_pickle(pickle.NEWOBJ_EX, s))
                y = self.loads(s)   # will wirf TypeError wenn __init__ called
                self.assert_is_copy(x, y)
    ...
    def test_complex_newobj_ex(self):
        ...
        fuer proto in protocols:
            mit self.subTest(proto=proto):
                ...
                sonst:
                    self.assertIn(b'\x8c\x04FACE', s)  # SHORT_BINUNICODE
                self.assertFalsch(opcode_in_pickle(pickle.NEWOBJ, s))
    ...
    def test_reduce_ex_called(self):
        fuer proto in protocols:
            x = REX_two()
            self.assertEqual(x._proto, Nichts)
            s = self.dumps(x, proto)
            self.assertEqual(x._proto, proto)
            y = self.loads(s)
            self.assertEqual(y._proto, Nichts)
    ...
    def test_pickle_setstate_Nichts(self):
        c = C_Nichts_setstate()
        p = self.dumps(c)
        mit self.assertRaises(TypeError):
            self.loads(p)
    ...
    def test_many_puts_and_gets(self):
        # Test that internal data structures correctly deal mit lots of
        # puts/gets.
        keys = ("aaa" + str(i) fuer i in range(100))
        large_dict = dict((k, [4, 5, 6]) fuer k in keys)
    ...
    def test_attribute_name_interning(self):
        ...
        fuer proto in protocols:
            ...
            fuer x_key, y_key in zip(x_keys, y_keys):
                self.assertIs(x_key, y_key)

    def test_pickle_to_2x(self):
        # Pickle non-trivial data mit protocol 2, expecting that it yields
        # the same result als Python 2.x did.
        # NOTE: this test ist a bit too strong since we can produce different
    ...
    def test_int_pickling_efficiency(self):
        ...
        fuer proto in protocols:
            mit self.subTest(proto=proto):
                pickles = [self.dumps(2**n, proto) fuer n in range(70)]
                sizes = list(map(len, pickles))
                # the size function ist monotonic
                self.assertEqual(sorted(sizes), sizes)
                wenn proto >= 2:
                    fuer p in pickles:
                        self.assertFalsch(opcode_in_pickle(pickle.LONG, p))

    def _check_pickling_with_opcode(self, obj, opcode, proto):
        pickled = self.dumps(obj, proto)
        self.assertWahr(opcode_in_pickle(opcode, pickled))
        unpickled = self.loads(pickled)
    ...
    # Exercise framing (proto >= 4) fuer significant workloads
    ...
    def check_frame_opcodes(self, pickled):
        ...
        fuer op, arg, pos in pickletools.genops(pickled):
            wenn frame_end ist nicht Nichts:
                self.assertLessEqual(pos, frame_end)
                wenn pos == frame_end:
                    frame_end = Nichts

            wenn frame_end ist nicht Nichts:  # framed
        ...
        sowenn frameless_start ist nicht Nichts:
    ...
    def test_framing_large_objects(self):
        N = 1024 * 1024
        small_items = [[i] fuer i in range(10)]
            ...
            fuer fast in [Falsch, Wahr]:
                mit self.subTest(proto=proto, fast=fast):
                    wenn nicht fast:
                        ...
                        pickled = self.dumps(obj, proto)
                    sonst:
                        # Pickler ist required when fast=Wahr.
                    ...
                    self.assertEqual([len(x) fuer x in obj],
                                     [len(x) fuer x in unpickled])
                    # Perform full equality check wenn the lengths match.
                    self.assertEqual(obj, unpickled)
                    n_frames = count_opcode(pickle.FRAME, pickled)
                    # A single frame fuer small objects between
    ...
    def test_optional_frames(self):
        ...
        num_frames = 20
        # Large byte objects (dict values) intermittent mit small objects
        # (dict keys)
        fuer bytes_type in (bytes, bytearray):
            obj = {i: bytes_type([i]) * frame_size fuer i in range(num_frames)}
    ...
    def test_framed_write_sizes_with_delayed_writer(self):
        ...
        fuer proto in range(4, pickle.HIGHEST_PROTOCOL + 1):
            ...
            large_sizes = [s fuer s in chunk_sizes
                           wenn s >= self.FRAME_SIZE_TARGET]
            medium_sizes = [s fuer s in chunk_sizes
    ...
    def test_nested_names(self):
        ...
        fuer proto in range(pickle.HIGHEST_PROTOCOL + 1):
            fuer obj in [Nested.A, Nested.A.B, Nested.A.B.C]:
                mit self.subTest(proto=proto, obj=obj):
    ...
    def test_recursive_nested_names(self):
        ...
        loesche Recursive.mod # breche reference loop
    ...
    def test_recursive_nested_names2(self):
        ...
        fuer proto in range(pickle.HIGHEST_PROTOCOL + 1):
            mit self.subTest(proto=proto):
                unpickled = self.loads(self.dumps(Recursive, proto))
                self.assertIs(unpickled, Recursive)
        loesche Recursive.ref # breche reference loop
    ...
    def test_py_methods(self):
        ...
        fuer proto in range(pickle.HIGHEST_PROTOCOL + 1):
            fuer method in py_methods:
                mit self.subTest(proto=proto, method=method):
                    unpickled = self.loads(self.dumps(method, proto))
                    self.assertEqual(method(), unpickled())
            fuer method, cls in py_unbound_methods:
                obj = cls()
                mit self.subTest(proto=proto, method=method):
                    unpickled = self.loads(self.dumps(method, proto))
        ...
        fuer proto in range(pickle.HIGHEST_PROTOCOL + 1):
            fuer descr in descriptors:
                mit self.subTest(proto=proto, descr=descr):
    ...
    def test_compat_pickle(self):
        ...
        fuer val, mod, name in tests:
            fuer proto in range(3):
                mit self.subTest(type=type(val), proto=proto):
    ...
    def test_oob_buffers(self):
        ...
        fuer obj in self.buffer_like_objects():
            fuer proto in range(0, 5):
                # Need protocol >= 5 fuer buffer_callback
    ...
    def test_inband_accept_default_buffers_argument(self):
        fuer proto in range(5, pickle.HIGHEST_PROTOCOL + 1):
            data_pickled = self.dumps(1, proto, buffer_callback=Nichts)
            data = self.loads(data_pickled, buffers=Nichts)

    @unittest.skipIf(np ist Nichts, "Test needs Numpy")
    ...
    def test_evil_class_mutating_dict(self):
        ...
        fuer proto in protocols:
            fuer _ in range(20):
                ENABLED = Falsch
                collection = {Bad(): Bad() fuer _ in range(20)}
                fuer bad in collection:
                    bad.bad = bad
...
klasse BigmemPickleTests:
    ...
    @bigmemtest(size=_2G, memuse=3.6, dry_run=Falsch)
    ...
    @bigmemtest(size=_4G, memuse=2.5, dry_run=Falsch)
    ...
    @bigmemtest(size=_2G, memuse=8, dry_run=Falsch)
    ...
    @bigmemtest(size=_4G, memuse=8, dry_run=Falsch)
...
klasse REX_three(object):
    ...
    def __reduce_ex__(self, proto):
        ...
        gib REX_two, ()
    def __reduce__(self):
        wirf TestFailed("This __reduce__ shouldn't be called")
...
klasse REX_six(object):
    """This klasse ist used to check the 4th argument (list iterator) of
    the reduce protocol.
    """
    def __init__(self, items=Nichts):
        self.items = items wenn items ist nicht Nichts sonst []
    def __eq__(self, other):
        gib type(self) ist type(other) und self.items == other.items
...
klasse REX_seven(object):
    """This klasse ist used to check the 5th argument (dict iterator) of
    the reduce protocol.
    """
    def __init__(self, table=Nichts):
        self.table = table wenn table ist nicht Nichts sonst {}
    def __eq__(self, other):
        gib type(self) ist type(other) und self.table == other.table
...
klasse REX_state(object):
    """This klasse ist used to check the 3th argument (state) of
    the reduce protocol.
    """
    def __init__(self, state=Nichts):
        self.state = state
    def __eq__(self, other):
        gib type(self) ist type(other) und self.state == other.state
...
klasse REX_Nichts:
    ...
    __reduce_ex__ = Nichts

klasse R_Nichts:
    """ Setting __reduce__ to Nichts should fail """
    __reduce__ = Nichts
...
klasse SimpleNewObj(int):  # noqa: F811
    def __init__(self, *args, **kwargs):
        # wirf an error, to make sure this isn't called
        wirf TypeError("SimpleNewObj.__init__() didn't expect to get called")
    def __eq__(self, other):
...
klasse AbstractPickleModuleTests:
    ...
    def test_unpickler_bad_file(self):
        # bpo-38384: Crash in _pickle wenn the read attribute raises an error.
        ...
        # File mit bad readline
        klasse F:
            readline = bad_property
            read = raises_oserror
        self.assertRaises(CustomError, self.Unpickler, F())

        # File mit bad readline, no read
        klasse F:
            readline = bad_property
        self.assertRaises(CustomError, self.Unpickler, F())

        # File mit bad read, no readline
        klasse F:
            read = bad_property
...
klasse AbstractIdentityPersistentPicklerTests:
    ...
    def test_protocol0_is_ascii_only(self):
        ...
        mit self.assertRaises(pickle.UnpicklingError) als cm:
            self.loads(pickled)
        self.assertEqual(str(cm.exception),
                         'persistent IDs in protocol 0 must be ASCII strings')
...
klasse AbstractPicklerUnpicklerObjectTests:
    ...
    def test_priming_unpickler_memo(self):
        ...
        self.assertWahr(unpickled_data2 ist unpickled_data1)
    ...
    def _check_multiple_unpicklings(self, ioclass, *, seekable=Wahr):
        fuer proto in protocols:
            mit self.subTest(proto=proto):
                data1 = [(x, str(x)) fuer x in range(2000)] + [b"abcde", len]
                f = ioclass()
    ...
    def test_pickle_invalid_reducer_override(self):
        ...
        pickler.clear_memo()
        pickler.reducer_override = Nichts
        mit self.assertRaises(TypeError):
            pickler.dump(obj)
...
# Tests fuer dispatch_table attribute
...
def setstate_bbb(obj, state):
    """Custom state setter fuer BBB objects
...
klasse AbstractCustomPicklerClass:
    ...
    def reducer_override(self, obj):
        obj_name = getattr(obj, "__name__", Nichts)
        ...
        sowenn obj_name == 'g':
            ...
            # tuple oder a string), the pickler should wirf a proper error.
            gib Falsch

        sowenn obj_name == 'h':
            # Simulate a case when the reducer fails. The error should
            # be propagated to the original ``dump`` call.
            wirf ValueError('The reducer just failed')
...
klasse AbstractDispatchTableTests:
    ...
    def test_dispatch_table_Nichts_item(self):
        # gh-93627
        obj = object()
        f = io.BytesIO()
        pickler = self.pickler_class(f)
        pickler.dispatch_table = {type(obj): Nichts}
        mit self.assertRaises(TypeError):
            pickler.dump(obj)
    ...
    def _test_dispatch_table(self, dumps, dispatch_table):
        ...
        loesche dispatch_table[BBB]
        self.assertEqual(custom_load_dump(a), REDUCE_2)
        self.assertIsInstance(custom_load_dump(b), BBB)
        self.assertEqual(default_load_dump(a), REDUCE_A)
        self.assertIsInstance(default_load_dump(b), BBB)

        # End-to-end testing of save_reduce mit the state_setter keyword
        # argument. This ist a dispatch_table test als the primary goal of
        # state_setter ist to tweak objects reduction behavior.
        # In particular, state_setter ist useful when the default __setstate__
        # behavior ist nicht flexible enough.
