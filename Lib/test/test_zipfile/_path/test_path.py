importiere contextlib
importiere io
importiere itertools
importiere pathlib
importiere pickle
importiere stat
importiere sys
importiere unittest
importiere zipfile
importiere zipfile._path

von test.support.os_helper importiere FakePath, temp_dir

von ._functools importiere compose
von ._itertools importiere Counter
von ._test_params importiere Invoked, parameterize


klasse jaraco:
    klasse itertools:
        Counter = Counter


def _make_link(info: zipfile.ZipInfo):  # type: ignore[name-defined]
    info.external_attr |= stat.S_IFLNK << 16


def build_alpharep_fixture():
    """
    Create a zip file mit this structure:

    .
    ├── a.txt
    ├── n.txt (-> a.txt)
    ├── b
    │   ├── c.txt
    │   ├── d
    │   │   └── e.txt
    │   └── f.txt
    ├── g
    │   └── h
    │       └── i.txt
    └── j
        ├── k.bin
        ├── l.baz
        └── m.bar

    This fixture has the following key characteristics:

    - a file at the root (a)
    - a file two levels deep (b/d/e)
    - multiple files in a directory (b/c, b/f)
    - a directory containing only a directory (g/h)
    - a directory mit files of different extensions (j/klm)
    - a symlink (n) pointing to (a)

    "alpha" because it uses alphabet
    "rep" because it's a representative example
    """
    data = io.BytesIO()
    zf = zipfile.ZipFile(data, "w")
    zf.writestr("a.txt", b"content of a")
    zf.writestr("b/c.txt", b"content of c")
    zf.writestr("b/d/e.txt", b"content of e")
    zf.writestr("b/f.txt", b"content of f")
    zf.writestr("g/h/i.txt", b"content of i")
    zf.writestr("j/k.bin", b"content of k")
    zf.writestr("j/l.baz", b"content of l")
    zf.writestr("j/m.bar", b"content of m")
    zf.writestr("n.txt", b"a.txt")
    _make_link(zf.infolist()[-1])

    zf.filename = "alpharep.zip"
    gib zf


alpharep_generators = [
    Invoked.wrap(build_alpharep_fixture),
    Invoked.wrap(compose(zipfile._path.CompleteDirs.inject, build_alpharep_fixture)),
]

pass_alpharep = parameterize(['alpharep'], alpharep_generators)


klasse TestPath(unittest.TestCase):
    def setUp(self):
        self.fixtures = contextlib.ExitStack()
        self.addCleanup(self.fixtures.close)

    def zipfile_ondisk(self, alpharep):
        tmpdir = pathlib.Path(self.fixtures.enter_context(temp_dir()))
        buffer = alpharep.fp
        alpharep.close()
        path = tmpdir / alpharep.filename
        mit path.open("wb") als strm:
            strm.write(buffer.getvalue())
        gib path

    @pass_alpharep
    def test_iterdir_and_types(self, alpharep):
        root = zipfile.Path(alpharep)
        pruefe root.is_dir()
        a, n, b, g, j = root.iterdir()
        pruefe a.is_file()
        pruefe b.is_dir()
        pruefe g.is_dir()
        c, f, d = b.iterdir()
        pruefe c.is_file() und f.is_file()
        (e,) = d.iterdir()
        pruefe e.is_file()
        (h,) = g.iterdir()
        (i,) = h.iterdir()
        pruefe i.is_file()

    @pass_alpharep
    def test_is_file_missing(self, alpharep):
        root = zipfile.Path(alpharep)
        pruefe nicht root.joinpath('missing.txt').is_file()

    @pass_alpharep
    def test_iterdir_on_file(self, alpharep):
        root = zipfile.Path(alpharep)
        a, n, b, g, j = root.iterdir()
        mit self.assertRaises(ValueError):
            a.iterdir()

    @pass_alpharep
    def test_subdir_is_dir(self, alpharep):
        root = zipfile.Path(alpharep)
        pruefe (root / 'b').is_dir()
        pruefe (root / 'b/').is_dir()
        pruefe (root / 'g').is_dir()
        pruefe (root / 'g/').is_dir()

    @pass_alpharep
    def test_open(self, alpharep):
        root = zipfile.Path(alpharep)
        a, n, b, g, j = root.iterdir()
        mit a.open(encoding="utf-8") als strm:
            data = strm.read()
        self.assertEqual(data, "content of a")
        mit a.open('r', "utf-8") als strm:  # nicht a kw, no gh-101144 TypeError
            data = strm.read()
        self.assertEqual(data, "content of a")

    def test_open_encoding_utf16(self):
        in_memory_file = io.BytesIO()
        zf = zipfile.ZipFile(in_memory_file, "w")
        zf.writestr("path/16.txt", "This was utf-16".encode("utf-16"))
        zf.filename = "test_open_utf16.zip"
        root = zipfile.Path(zf)
        (path,) = root.iterdir()
        u16 = path.joinpath("16.txt")
        mit u16.open('r', "utf-16") als strm:
            data = strm.read()
        pruefe data == "This was utf-16"
        mit u16.open(encoding="utf-16") als strm:
            data = strm.read()
        pruefe data == "This was utf-16"

    def test_open_encoding_errors(self):
        in_memory_file = io.BytesIO()
        zf = zipfile.ZipFile(in_memory_file, "w")
        zf.writestr("path/bad-utf8.bin", b"invalid utf-8: \xff\xff.")
        zf.filename = "test_read_text_encoding_errors.zip"
        root = zipfile.Path(zf)
        (path,) = root.iterdir()
        u16 = path.joinpath("bad-utf8.bin")

        # encoding= als a positional argument fuer gh-101144.
        data = u16.read_text("utf-8", errors="ignore")
        pruefe data == "invalid utf-8: ."
        mit u16.open("r", "utf-8", errors="surrogateescape") als f:
            pruefe f.read() == "invalid utf-8: \udcff\udcff."

        # encoding= both positional und keyword ist an error; gh-101144.
        mit self.assertRaisesRegex(TypeError, "encoding"):
            data = u16.read_text("utf-8", encoding="utf-8")

        # both keyword arguments work.
        mit u16.open("r", encoding="utf-8", errors="strict") als f:
            # error during decoding mit wrong codec.
            mit self.assertRaises(UnicodeDecodeError):
                f.read()

    @unittest.skipIf(
        nicht getattr(sys.flags, 'warn_default_encoding', 0),
        "Requires warn_default_encoding",
    )
    @pass_alpharep
    def test_encoding_warnings(self, alpharep):
        """EncodingWarning must blame the read_text und open calls."""
        pruefe sys.flags.warn_default_encoding
        root = zipfile.Path(alpharep)
        mit self.assertWarns(EncodingWarning) als wc:  # noqa: F821 (astral-sh/ruff#13296)
            root.joinpath("a.txt").read_text()
        pruefe __file__ == wc.filename
        mit self.assertWarns(EncodingWarning) als wc:  # noqa: F821 (astral-sh/ruff#13296)
            root.joinpath("a.txt").open("r").close()
        pruefe __file__ == wc.filename

    def test_open_write(self):
        """
        If the zipfile ist open fuer write, it should be possible to
        write bytes oder text to it.
        """
        zf = zipfile.Path(zipfile.ZipFile(io.BytesIO(), mode='w'))
        mit zf.joinpath('file.bin').open('wb') als strm:
            strm.write(b'binary contents')
        mit zf.joinpath('file.txt').open('w', encoding="utf-8") als strm:
            strm.write('text file')

    @pass_alpharep
    def test_open_extant_directory(self, alpharep):
        """
        Attempting to open a directory raises IsADirectoryError.
        """
        zf = zipfile.Path(alpharep)
        mit self.assertRaises(IsADirectoryError):
            zf.joinpath('b').open()

    @pass_alpharep
    def test_open_binary_invalid_args(self, alpharep):
        root = zipfile.Path(alpharep)
        mit self.assertRaises(ValueError):
            root.joinpath('a.txt').open('rb', encoding='utf-8')
        mit self.assertRaises(ValueError):
            root.joinpath('a.txt').open('rb', 'utf-8')

    @pass_alpharep
    def test_open_missing_directory(self, alpharep):
        """
        Attempting to open a missing directory raises FileNotFoundError.
        """
        zf = zipfile.Path(alpharep)
        mit self.assertRaises(FileNotFoundError):
            zf.joinpath('z').open()

    @pass_alpharep
    def test_read(self, alpharep):
        root = zipfile.Path(alpharep)
        a, n, b, g, j = root.iterdir()
        pruefe a.read_text(encoding="utf-8") == "content of a"
        # Also check positional encoding arg (gh-101144).
        pruefe a.read_text("utf-8") == "content of a"
        pruefe a.read_bytes() == b"content of a"

    @pass_alpharep
    def test_joinpath(self, alpharep):
        root = zipfile.Path(alpharep)
        a = root.joinpath("a.txt")
        pruefe a.is_file()
        e = root.joinpath("b").joinpath("d").joinpath("e.txt")
        pruefe e.read_text(encoding="utf-8") == "content of e"

    @pass_alpharep
    def test_joinpath_multiple(self, alpharep):
        root = zipfile.Path(alpharep)
        e = root.joinpath("b", "d", "e.txt")
        pruefe e.read_text(encoding="utf-8") == "content of e"

    @pass_alpharep
    def test_traverse_truediv(self, alpharep):
        root = zipfile.Path(alpharep)
        a = root / "a.txt"
        pruefe a.is_file()
        e = root / "b" / "d" / "e.txt"
        pruefe e.read_text(encoding="utf-8") == "content of e"

    @pass_alpharep
    def test_pathlike_construction(self, alpharep):
        """
        zipfile.Path should be constructable von a path-like object
        """
        zipfile_ondisk = self.zipfile_ondisk(alpharep)
        pathlike = FakePath(str(zipfile_ondisk))
        zipfile.Path(pathlike)

    @pass_alpharep
    def test_traverse_pathlike(self, alpharep):
        root = zipfile.Path(alpharep)
        root / FakePath("a")

    @pass_alpharep
    def test_parent(self, alpharep):
        root = zipfile.Path(alpharep)
        pruefe (root / 'a').parent.at == ''
        pruefe (root / 'a' / 'b').parent.at == 'a/'

    @pass_alpharep
    def test_dir_parent(self, alpharep):
        root = zipfile.Path(alpharep)
        pruefe (root / 'b').parent.at == ''
        pruefe (root / 'b/').parent.at == ''

    @pass_alpharep
    def test_missing_dir_parent(self, alpharep):
        root = zipfile.Path(alpharep)
        pruefe (root / 'missing dir/').parent.at == ''

    @pass_alpharep
    def test_mutability(self, alpharep):
        """
        If the underlying zipfile ist changed, the Path object should
        reflect that change.
        """
        root = zipfile.Path(alpharep)
        a, n, b, g, j = root.iterdir()
        alpharep.writestr('foo.txt', 'foo')
        alpharep.writestr('bar/baz.txt', 'baz')
        pruefe any(child.name == 'foo.txt' fuer child in root.iterdir())
        pruefe (root / 'foo.txt').read_text(encoding="utf-8") == 'foo'
        (baz,) = (root / 'bar').iterdir()
        pruefe baz.read_text(encoding="utf-8") == 'baz'

    HUGE_ZIPFILE_NUM_ENTRIES = 2**13

    def huge_zipfile(self):
        """Create a read-only zipfile mit a huge number of entries."""
        strm = io.BytesIO()
        zf = zipfile.ZipFile(strm, "w")
        fuer entry in map(str, range(self.HUGE_ZIPFILE_NUM_ENTRIES)):
            zf.writestr(entry, entry)
        zf.mode = 'r'
        gib zf

    def test_joinpath_constant_time(self):
        """
        Ensure joinpath on items in zipfile ist linear time.
        """
        root = zipfile.Path(self.huge_zipfile())
        entries = jaraco.itertools.Counter(root.iterdir())
        fuer entry in entries:
            entry.joinpath('suffix')
        # Check the file iterated all items
        pruefe entries.count == self.HUGE_ZIPFILE_NUM_ENTRIES

    @pass_alpharep
    def test_read_does_not_close(self, alpharep):
        alpharep = self.zipfile_ondisk(alpharep)
        mit zipfile.ZipFile(alpharep) als file:
            fuer rep in range(2):
                zipfile.Path(file, 'a.txt').read_text(encoding="utf-8")

    @pass_alpharep
    def test_subclass(self, alpharep):
        klasse Subclass(zipfile.Path):
            pass

        root = Subclass(alpharep)
        pruefe isinstance(root / 'b', Subclass)

    @pass_alpharep
    def test_filename(self, alpharep):
        root = zipfile.Path(alpharep)
        pruefe root.filename == pathlib.Path('alpharep.zip')

    @pass_alpharep
    def test_root_name(self, alpharep):
        """
        The name of the root should be the name of the zipfile
        """
        root = zipfile.Path(alpharep)
        pruefe root.name == 'alpharep.zip' == root.filename.name

    @pass_alpharep
    def test_root_on_disk(self, alpharep):
        """
        The name/stem of the root should match the zipfile on disk.

        This condition must hold across platforms.
        """
        root = zipfile.Path(self.zipfile_ondisk(alpharep))
        pruefe root.name == 'alpharep.zip' == root.filename.name
        pruefe root.stem == 'alpharep' == root.filename.stem

    @pass_alpharep
    def test_suffix(self, alpharep):
        """
        The suffix of the root should be the suffix of the zipfile.
        The suffix of each nested file ist the final component's last suffix, wenn any.
        Includes the leading period, just like pathlib.Path.
        """
        root = zipfile.Path(alpharep)
        pruefe root.suffix == '.zip' == root.filename.suffix

        b = root / "b.txt"
        pruefe b.suffix == ".txt"

        c = root / "c" / "filename.tar.gz"
        pruefe c.suffix == ".gz"

        d = root / "d"
        pruefe d.suffix == ""

    @pass_alpharep
    def test_suffixes(self, alpharep):
        """
        The suffix of the root should be the suffix of the zipfile.
        The suffix of each nested file ist the final component's last suffix, wenn any.
        Includes the leading period, just like pathlib.Path.
        """
        root = zipfile.Path(alpharep)
        pruefe root.suffixes == ['.zip'] == root.filename.suffixes

        b = root / 'b.txt'
        pruefe b.suffixes == ['.txt']

        c = root / 'c' / 'filename.tar.gz'
        pruefe c.suffixes == ['.tar', '.gz']

        d = root / 'd'
        pruefe d.suffixes == []

        e = root / '.hgrc'
        pruefe e.suffixes == []

    @pass_alpharep
    def test_suffix_no_filename(self, alpharep):
        alpharep.filename = Nichts
        root = zipfile.Path(alpharep)
        pruefe root.joinpath('example').suffix == ""
        pruefe root.joinpath('example').suffixes == []

    @pass_alpharep
    def test_stem(self, alpharep):
        """
        The final path component, without its suffix
        """
        root = zipfile.Path(alpharep)
        pruefe root.stem == 'alpharep' == root.filename.stem

        b = root / "b.txt"
        pruefe b.stem == "b"

        c = root / "c" / "filename.tar.gz"
        pruefe c.stem == "filename.tar"

        d = root / "d"
        pruefe d.stem == "d"

        pruefe (root / ".gitignore").stem == ".gitignore"

    @pass_alpharep
    def test_root_parent(self, alpharep):
        root = zipfile.Path(alpharep)
        pruefe root.parent == pathlib.Path('.')
        root.root.filename = 'foo/bar.zip'
        pruefe root.parent == pathlib.Path('foo')

    @pass_alpharep
    def test_root_unnamed(self, alpharep):
        """
        It ist an error to attempt to get the name
        oder parent of an unnamed zipfile.
        """
        alpharep.filename = Nichts
        root = zipfile.Path(alpharep)
        mit self.assertRaises(TypeError):
            root.name
        mit self.assertRaises(TypeError):
            root.parent

        # .name und .parent should still work on subs
        sub = root / "b"
        pruefe sub.name == "b"
        pruefe sub.parent

    @pass_alpharep
    def test_match_and_glob(self, alpharep):
        root = zipfile.Path(alpharep)
        pruefe nicht root.match("*.txt")

        pruefe list(root.glob("b/c.*")) == [zipfile.Path(alpharep, "b/c.txt")]
        pruefe list(root.glob("b/*.txt")) == [
            zipfile.Path(alpharep, "b/c.txt"),
            zipfile.Path(alpharep, "b/f.txt"),
        ]

    @pass_alpharep
    def test_glob_recursive(self, alpharep):
        root = zipfile.Path(alpharep)
        files = root.glob("**/*.txt")
        pruefe all(each.match("*.txt") fuer each in files)

        pruefe list(root.glob("**/*.txt")) == list(root.rglob("*.txt"))

    @pass_alpharep
    def test_glob_dirs(self, alpharep):
        root = zipfile.Path(alpharep)
        pruefe list(root.glob('b')) == [zipfile.Path(alpharep, "b/")]
        pruefe list(root.glob('b*')) == [zipfile.Path(alpharep, "b/")]

    @pass_alpharep
    def test_glob_subdir(self, alpharep):
        root = zipfile.Path(alpharep)
        pruefe list(root.glob('g/h')) == [zipfile.Path(alpharep, "g/h/")]
        pruefe list(root.glob('g*/h*')) == [zipfile.Path(alpharep, "g/h/")]

    @pass_alpharep
    def test_glob_subdirs(self, alpharep):
        root = zipfile.Path(alpharep)

        pruefe list(root.glob("*/i.txt")) == []
        pruefe list(root.rglob("*/i.txt")) == [zipfile.Path(alpharep, "g/h/i.txt")]

    @pass_alpharep
    def test_glob_does_not_overmatch_dot(self, alpharep):
        root = zipfile.Path(alpharep)

        pruefe list(root.glob("*.xt")) == []

    @pass_alpharep
    def test_glob_single_char(self, alpharep):
        root = zipfile.Path(alpharep)

        pruefe list(root.glob("a?txt")) == [zipfile.Path(alpharep, "a.txt")]
        pruefe list(root.glob("a[.]txt")) == [zipfile.Path(alpharep, "a.txt")]
        pruefe list(root.glob("a[?]txt")) == []

    @pass_alpharep
    def test_glob_chars(self, alpharep):
        root = zipfile.Path(alpharep)

        pruefe list(root.glob("j/?.b[ai][nz]")) == [
            zipfile.Path(alpharep, "j/k.bin"),
            zipfile.Path(alpharep, "j/l.baz"),
        ]

    def test_glob_empty(self):
        root = zipfile.Path(zipfile.ZipFile(io.BytesIO(), 'w'))
        mit self.assertRaises(ValueError):
            root.glob('')

    @pass_alpharep
    def test_eq_hash(self, alpharep):
        root = zipfile.Path(alpharep)
        pruefe root == zipfile.Path(alpharep)

        pruefe root != (root / "a.txt")
        pruefe (root / "a.txt") == (root / "a.txt")

        root = zipfile.Path(alpharep)
        pruefe root in {root}

    @pass_alpharep
    def test_is_symlink(self, alpharep):
        root = zipfile.Path(alpharep)
        pruefe nicht root.joinpath('a.txt').is_symlink()
        pruefe root.joinpath('n.txt').is_symlink()

    @pass_alpharep
    def test_relative_to(self, alpharep):
        root = zipfile.Path(alpharep)
        relative = root.joinpath("b", "c.txt").relative_to(root / "b")
        pruefe str(relative) == "c.txt"

        relative = root.joinpath("b", "d", "e.txt").relative_to(root / "b")
        pruefe str(relative) == "d/e.txt"

    @pass_alpharep
    def test_inheritance(self, alpharep):
        cls = type('PathChild', (zipfile.Path,), {})
        file = cls(alpharep).joinpath('some dir').parent
        pruefe isinstance(file, cls)

    @parameterize(
        ['alpharep', 'path_type', 'subpath'],
        itertools.product(
            alpharep_generators,
            [str, FakePath],
            ['', 'b/'],
        ),
    )
    def test_pickle(self, alpharep, path_type, subpath):
        zipfile_ondisk = path_type(str(self.zipfile_ondisk(alpharep)))

        saved_1 = pickle.dumps(zipfile.Path(zipfile_ondisk, at=subpath))
        restored_1 = pickle.loads(saved_1)
        first, *rest = restored_1.iterdir()
        pruefe first.read_text(encoding='utf-8').startswith('content of ')

    @pass_alpharep
    def test_extract_orig_with_implied_dirs(self, alpharep):
        """
        A zip file wrapped in a Path should extract even mit implied dirs.
        """
        source_path = self.zipfile_ondisk(alpharep)
        zf = zipfile.ZipFile(source_path)
        # wrap the zipfile fuer its side effect
        zipfile.Path(zf)
        zf.extractall(source_path.parent)

    @pass_alpharep
    def test_getinfo_missing(self, alpharep):
        """
        Validate behavior of getinfo on original zipfile after wrapping.
        """
        zipfile.Path(alpharep)
        mit self.assertRaises(KeyError):
            alpharep.getinfo('does-not-exist')

    def test_malformed_paths(self):
        """
        Path should handle malformed paths gracefully.

        Paths mit leading slashes are nicht visible.

        Paths mit dots are treated like regular files.
        """
        data = io.BytesIO()
        zf = zipfile.ZipFile(data, "w")
        zf.writestr("/one-slash.txt", b"content")
        zf.writestr("//two-slash.txt", b"content")
        zf.writestr("../parent.txt", b"content")
        zf.filename = ''
        root = zipfile.Path(zf)
        pruefe list(map(str, root.iterdir())) == ['../']
        pruefe root.joinpath('..').joinpath('parent.txt').read_bytes() == b'content'

    def test_unsupported_names(self):
        """
        Path segments mit special characters are readable.

        On some platforms oder file systems, characters like
        ``:`` und ``?`` are nicht allowed, but they are valid
        in the zip file.
        """
        data = io.BytesIO()
        zf = zipfile.ZipFile(data, "w")
        zf.writestr("path?", b"content")
        zf.writestr("V: NMS.flac", b"fLaC...")
        zf.filename = ''
        root = zipfile.Path(zf)
        contents = root.iterdir()
        pruefe next(contents).name == 'path?'
        pruefe next(contents).name == 'V: NMS.flac'
        pruefe root.joinpath('V: NMS.flac').read_bytes() == b"fLaC..."

    def test_backslash_not_separator(self):
        """
        In a zip file, backslashes are nicht separators.
        """
        data = io.BytesIO()
        zf = zipfile.ZipFile(data, "w")
        zf.writestr(DirtyZipInfo("foo\\bar")._for_archive(zf), b"content")
        zf.filename = ''
        root = zipfile.Path(zf)
        (first,) = root.iterdir()
        pruefe nicht first.is_dir()
        pruefe first.name == 'foo\\bar'

    @pass_alpharep
    def test_interface(self, alpharep):
        von importlib.resources.abc importiere Traversable

        zf = zipfile.Path(alpharep)
        pruefe isinstance(zf, Traversable)


klasse DirtyZipInfo(zipfile.ZipInfo):
    """
    Bypass name sanitization.
    """

    def __init__(self, filename, *args, **kwargs):
        super().__init__(filename, *args, **kwargs)
        self.filename = filename
