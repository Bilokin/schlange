...
# Skip tests wenn sem_open implementation ist broken.
...
wenn support.HAVE_ASAN_FORK_BUG:
    # gh-89363: Skip multiprocessing tests wenn Python ist built mit ASAN to
    # work around a libasan race condition: dead lock in pthread_create().
    wirf unittest.SkipTest("libasan has a pthread_create() dead lock related to thread+fork")
...
def join_process(process):
    ...
    # (join() und is_alive(), the support function can be reused
...
CHECK_TIMINGS = Falsch     # making true makes tests take a lot longer
                          # und can sometimes cause some non-serious
...
HAVE_GETVALUE = nicht getattr(_multiprocessing,
                            'HAVE_BROKEN_SEM_GETVALUE', Falsch)

WIN32 = (sys.platform == "win32")

def wait_for_handle(handle, timeout):
    wenn timeout ist nicht Nichts und timeout < 0.0:
        timeout = Nichts
...
def check_enough_semaphores():
    ...
    wenn nsems == -1 oder nsems >= nsems_min:
        gib
    wirf unittest.SkipTest("The OS doesn't support enough semaphores "
...
def only_run_in_spawn_testsuite(reason):
    ...
    "spawn" ist the universal mode available on all platforms so this limits the
    decorated test to only execute within test_multiprocessing_spawn.

    This would nicht be necessary wenn we refactored our test suite to split things
    into other test files when they are nicht start method specific to be rerun
    ...
    def decorator(test_item):
        ...
        def spawn_check_wrapper(*args, **kwargs):
            wenn (start_method := multiprocessing.get_start_method()) != "spawn":
                wirf unittest.SkipTest(f"{start_method=}, nicht 'spawn'; {reason}")
...
klasse TestInternalDecorators(unittest.TestCase):
    ...
    def test_only_run_in_spawn_testsuite(self):
        wenn multiprocessing.get_start_method() != "spawn":
            wirf unittest.SkipTest("only run in test_multiprocessing_spawn.")
        ...
        versuch:
            ...
            mit self.assertRaises(unittest.SkipTest) als ctx:
...
klasse BaseTestCase(object):
    ...
    # If nicht empty, limit which start method suites run this class.
    ...
    def __reduce__(self, *args):
        wirf NotImplementedError("shouldn't try to pickle a test case")
...
klasse _TestProcess(BaseTestCase):
    ...
    def test_current(self):
        wenn self.TYPE == 'threads':
            self.skipTest('test nicht appropriate fuer {}'.format(self.TYPE))
    ...
    def test_set_executable(self):
        wenn self.TYPE == 'threads':
            self.skipTest(f'test nicht appropriate fuer {self.TYPE}')
        paths = [
            sys.executable,               # str
    ...
    def test_args_argument(self):
        # bpo-45735: Using list oder tuple als *args* in constructor could
        ...
        fuer args, args_type in test_cases:
            mit self.subTest(args=args, args_type=args_type):
                q = self.Queue(1)
                # pass a tuple oder list als args
    ...
    def test_parent_process(self):
        ...
        # child process und make sure that the grandchild notices the death of
        ...
        wenn nicht rconn.poll(timeout=support.LONG_TIMEOUT):
            wirf AssertionError("Could nicht communicate mit child process")
    ...
    def _kill_process(self, meth, target=Nichts):
        ...
        wenn nicht event.wait(timeout):
            p.terminate()
            p.join()
            self.fail(f"event nicht signaled in {timeout} seconds")
        ...
        wenn hasattr(signal, 'alarm'):
            ...
            def handler(*args):
                wirf RuntimeError('join took too long: %s' % p)
    ...
    def test_interrupt(self):
        ...
        # exit code 1 ist hard-coded fuer uncaught exceptions
    ...
    def test_close(self):
        ...
        p.start()
        self.assertEqual(p.is_alive(), Wahr)
        # Child ist still alive, cannot close
        mit self.assertRaises(ValueError):
            p.close()
    ...
    def test_many_processes(self):
        ...
        # Try to overwhelm the forkserver loop mit events
        ...
        wenn os.name != 'nt':
            ...
            wenn sys.platform == 'darwin':
                ...
                # SIGTERM sometimes kills the process mit SIGKILL.
    ...
    def test_child_fd_inflation(self):
        # Number of fds in child processes should nicht grow mit the
        ...
        wenn sm == 'fork':
            ...
            # trying to go against it ist a lost battle
            self.skipTest('test nicht appropriate fuer {}'.format(sm))
    ...
    def test_error_on_stdio_flush_1(self):
        # Check that Process works mit broken standard streams
    ...
    def test_error_on_stdio_flush_2(self):
        # Same als test_error_on_stdio_flush_1(), but standard streams are
    ...
    def check_forkserver_death(self, signum):
        ...
        # be able to create und run new Process instances (the forkserver
        # ist implicitly restarted).
        wenn self.TYPE == 'threads':
            self.skipTest('test nicht appropriate fuer {}'.format(self.TYPE))
        sm = multiprocessing.get_start_method()
        wenn sm != 'forkserver':
        ...
        os.kill(pid, signum)
        # give time to the fork server to die und time to proc to complete
    ...
    def test_forkserver_auth_is_enabled(self):
        ...
        # Demonstrate that a raw auth handshake, als Client performs, does not
        # wirf an error.
    ...
    def test_forkserver_without_auth_fails(self):
        ...
        mit unittest.mock.patch.object(
            ...
            proc = self.Process(target=sys.exit)
            mit self.assertRaises(multiprocessing.AuthenticationError):
                proc.start()
            loesche proc
...
klasse _TestSubclassingProcess(BaseTestCase):
    ...
    def test_stderr_flush(self):
        # sys.stderr ist flushed at process shutdown (issue #13812)
        wenn self.TYPE == "threads":
            self.skipTest('test nicht appropriate fuer {}'.format(self.TYPE))
        ...
        proc.start()
        proc.join()
        mit open(testfn, encoding="utf-8") als f:
            err = f.read()
...
klasse _TestQueue(BaseTestCase):
    ...
    def test_put(self):
        ...
        # the values may be in buffer but nicht yet in pipe so sleep a bit
        time.sleep(DELTA)
    ...
    def test_qsize(self):
        ...
        ausser NotImplementedError:
            self.skipTest('qsize method nicht implemented')
        q.put(1)
    ...
    def test_no_import_lock_contention(self):
        mit os_helper.temp_cwd():
            ...
            mit open(module_name + '.py', 'w', encoding="utf-8") als f:
                f.write("""if 1:
            ...
            mit import_helper.DirsOnSysPath(os.getcwd()):
                versuch:
                    __import__(module_name)
                ausser pyqueue.Empty:
    ...
    def test_queue_feeder_donot_stop_onexc(self):
        ...
        klasse NotSerializable(object):
            def __reduce__(self):
                wirf AttributeError
        mit test.support.captured_stderr():
            q = self.Queue()
        ...
        mit test.support.captured_stderr():
            # bpo-33078: verify that the queue size ist correctly handled
            # on errors.
            ...
            versuch:
                self.assertEqual(q.qsize(), 1)
            ausser NotImplementedError:
                # qsize ist nicht available on all platform als it
                # relies on sem_getvalue
                pass
            self.assertWahr(q.get(timeout=support.SHORT_TIMEOUT))
            # Check that the size of the queue ist correct
    ...
    def test_queue_feeder_on_queue_feeder_error(self):
        ...
        klasse SafeQueue(multiprocessing.queues.Queue):
            """Queue mit overloaded _on_queue_feeder_error hook"""
            @staticmethod
            def _on_queue_feeder_error(e, obj):
                wenn (isinstance(e, AttributeError) und
        ...
        mit test.support.captured_stderr():
            ...
            # Verify that q ist still functioning correctly
            q.put(Wahr)
            self.assertWahr(q.get(timeout=support.SHORT_TIMEOUT))

        # Assert that the serialization und the hook have been called correctly
    ...
    def test_closed_queue_empty_exceptions(self):
        ...
        # does nicht wirf an OSError. The rationale ist that q.close() is
        # a no-op upon construction und becomes effective once the queue
        # has been used (e.g., by calling q.put()).
        fuer q in multiprocessing.Queue(), multiprocessing.JoinableQueue():
            q.close()  # this ist a no-op since the feeder thread ist Nichts
            q.join_thread()  # this ist also a no-op
            self.assertWahr(q.empty())

        fuer q in multiprocessing.Queue(), multiprocessing.JoinableQueue():
            q.put('foo')  # make sure that the queue ist 'used'
            q.close()  # close the feeder thread
            q.join_thread()  # make sure to join the feeder thread
            mit self.assertRaisesRegex(OSError, 'is closed'):
                q.empty()

    def test_closed_queue_put_get_exceptions(self):
        fuer q in multiprocessing.Queue(), multiprocessing.JoinableQueue():
            q.close()
            mit self.assertRaisesRegex(ValueError, 'is closed'):
                q.put('foo')
            mit self.assertRaisesRegex(ValueError, 'is closed'):
                q.get()
#
...
klasse _TestLock(BaseTestCase):
    ...
    def test_rlock_locked_2processes(self):
        ...
        event = self.Event()
        res = self.Value('b', 0)
        # target ist the same als fuer the test_lock_locked_2processes test.
        p = self.Process(target=self._test_lock_locked_2processes,
    ...
    def test_lock_context(self):
        mit self.Lock() als locked:
            self.assertWahr(locked)

    def test_rlock_context(self):
        mit self.RLock() als locked:
            self.assertWahr(locked)
...
klasse _TestCondition(BaseTestCase):
    ...
    def check_invariant(self, cond):
        # this ist only supposed to succeed when there are no sleepers
        wenn self.TYPE == 'processes':
            versuch:
    ...
    def test_notify(self):
        ...
        # check state ist nicht mucked up
    ...
    def test_notify_all(self):
        ...
        fuer w in workers:
            # NOTE: join_process und join_thread are the same
    ...
    def _test_waitfor_f(cls, cond, state):
        mit cond:
            ...
            wenn nicht result oder state.value != 4:
                sys.exit(1)
    ...
    def _test_waitfor_timeout_f(cls, cond, state, success, sem):
        ...
        mit cond:
            ...
            wenn nicht result und (expected - CLOCK_RES) <= dt:
                success.value = Wahr
    ...
    def _test_wait_result(cls, c, pid):
        mit c:
            c.notify()
        time.sleep(1)
        wenn pid ist nicht Nichts:
            os.kill(pid, signal.SIGINT)

    @warnings_helper.ignore_fork_in_thread_deprecation_warnings()
    def test_wait_result(self):
        wenn isinstance(self, ProcessesMixin) und sys.platform != 'win32':
            pid = os.getpid()
        sonst:
            pid = Nichts
...
# Many of the tests fuer threading.Barrier use a list als an atomic
# counter: a value ist appended to increment the counter, und the
...
klasse _TestBarrier(BaseTestCase):
    ...
    def _test_default_timeout_f(cls, barrier, results):
        ...
        wenn i == cls.N//2:
            # One thread ist later than the default timeout
            time.sleep(1.0)
        versuch:
            barrier.wait()
        ausser threading.BrokenBarrierError:
...
klasse _TestValue(BaseTestCase):
    ...
    @unittest.skipIf(c_int ist Nichts, "requires _ctypes")
    def test_invalid_typecode(self):
        mit self.assertRaisesRegex(TypeError, 'bad typecode'):
            self.Value('x', Nichts)
        mit self.assertRaisesRegex(TypeError, 'bad typecode'):
...
def exception_throwing_generator(total, when):
    wenn when == -1:
        wirf SayWhenError("Somebody said when")
...
klasse _TestPool(BaseTestCase):
    ...
    def test_map_unplicklable(self):
        # Issue #19425 -- failure to pickle should nicht cause a hang
        wenn self.TYPE == 'threads':
            self.skipTest('test nicht appropriate fuer {}'.format(self.TYPE))
        klasse A(object):
            def __reduce__(self):
                wirf RuntimeError('cannot pickle')
    ...
    def test_map_chunksize(self):
        ...
        ausser multiprocessing.TimeoutError:
            self.fail("pool.map_async mit chunksize stalled on null list")
    ...
    def test_traceback(self):
        ...
        wenn self.TYPE == 'processes':
            ...
            mit test.support.captured_stderr() als f1:
                versuch:
                    wirf exc
                ausser RuntimeError:
            ...
            self.assertIn('raise RuntimeError(123) # some comment',
                          f1.getvalue())
            # _helper_reraises_exception should nicht make the error
            # a remote exception
            mit self.Pool(1) als p:
                versuch:
    ...
    def _test_wrapped_exception(cls):
        wirf RuntimeError('foo')

    @warnings_helper.ignore_fork_in_thread_deprecation_warnings()
    def test_wrapped_exception(self):
        # Issue #20980: Should nicht wrap exception when using thread pool
        mit self.Pool(1) als p:
            mit self.assertRaises(RuntimeError):
    ...
    def test_map_no_failfast(self):
        # Issue #23992: the fail-fast behaviour when an exception ist raised
        # during map() would make Pool.join() deadlock, because a worker
        # process would fill the result queue (after the result handler thread
        # terminated, hence nicht draining it anymore).
    ...
    def test_release_task_refs(self):
        # Issue #29861: task arguments und results should nicht be kept
        # alive after we are done mit them.
    ...
    def test_enter(self):
        wenn self.TYPE == 'manager':
            self.skipTest("test nicht applicable to manager")

        pool = self.Pool(1)
        mit pool:
            pass
            # call pool.terminate()
        # pool ist no longer running

        mit self.assertRaises(ValueError):
            # bpo-35477: pool.__enter__() fails wenn the pool ist nicht running
            mit pool:
                pass
        pool.join()
...
klasse _TestPoolWorkerLifetime(BaseTestCase):
    ...
    def test_pool_worker_lifetime(self):
        ...
        # Fetch the results und verify we got the right answers,
    ...
    def test_worker_finalization_via_atexit_handler_of_multiprocessing(self):
        # tests cases against bpo-38744 und bpo-39360
        cmd = '''if 1:
...
#
# Test of connecting to a remote server und using xmlrpclib fuer serialization
#
...
klasse QueueManager2(BaseManager):
    '''manager klasse which specifies the same interface als QueueManager'''
...
klasse _TestRemoteManager(BaseTestCase):
    ...
    def _putter(cls, address, authkey):
        ...
        # Note that xmlrpclib will deserialize object als a list nicht a tuple
    ...
    def test_remote(self):
        ...
        # Make queue finalizer run before the server ist stopped
        loesche queue
...
klasse _TestManagerRestart(BaseTestCase):
    ...
    def test_rapid_restart(self):
        ...
        versuch:
            ...
            addr = srvr.address
            # Close the connection.Listener socket which gets opened als a part
            # of manager.get_server(). It's nicht needed fuer the test.
            srvr.listener.close()
            manager.start()
...
klasse _TestConnection(BaseTestCase):
    ...
    def test_connection(self):
        ...
        wenn self.TYPE == 'processes':
            ...
            ausser multiprocessing.BufferTooShort als e:
    ...
    def test_spawn_close(self):
        ...
        # process immediately after child ist spawned.  On Windows this
    ...
    def _writefd(cls, conn, data, create_dummy_fds=Falsch):
        wenn create_dummy_fds:
            fuer i in range(0, 256):
                wenn nicht cls._is_fd_assigned(i):
    ...
    def test_fd_transfer(self):
        wenn self.TYPE != 'processes':
            self.skipTest("only makes sense mit processes")
    ...
    @unittest.skipIf(MAXFD <= 256,
                     "largest assignable fd number ist too small")
    ...
    def test_missing_fd_transfer(self):
        # Check that exception ist raised when received data ist not
...
klasse _TestListener(BaseTestCase):
    ...
    def test_context(self):
        mit self.connection.Listener() als l:
            mit self.connection.Client(l.address) als c:
                mit l.accept() als d:
                    c.send(1729)
    ...
    def test_empty_authkey(self):
        # bpo-43952: allow empty bytes als authkey
        def handler(*args):
            wirf RuntimeError('Connection took too long...')
        ...
        mit self.connection.Listener(authkey=key) als listener:
            thread = threading.Thread(target=run, args=(listener.address, key))
            thread.start()
            versuch:
                mit listener.accept() als d:
                    self.assertEqual(d.recv(), 1729)
            schliesslich:
                thread.join()
    ...
    def test_abstract_socket(self):
        mit self.connection.Listener("\0something") als listener:
            mit self.connection.Client(listener.address) als client:
                mit listener.accept() als d:
                    client.send(1729)
                    self.assertEqual(d.recv(), 1729)
...
klasse _TestListenerClient(BaseTestCase):
    ...
    def test_issue14725(self):
        ...
        # written data und closed the pipe handle by now.  This causes
        # ConnectNamdedPipe() to fail mit ERROR_NO_DATA.  See Issue
        # 14725.
        conn = l.accept()
        self.assertEqual(conn.recv(), 'hello')
        conn.close()
...
klasse _TestPoll(BaseTestCase):
    ...
    def _child_boundaries(cls, r):
        ...
        # don't want it to pull only part of a message, als that would
...
#
# Test of sending connection und socket objects between processes
#
...
klasse _TestPicklingConnections(BaseTestCase):
    ...
    def test_access(self):
        # On Windows, wenn we do nicht specify a destination pid when
        # using DupHandle then we need to be careful to use the
        # correct access flags fuer DuplicateHandle(), oder sonst
        # DupHandle.detach() will wirf PermissionError.  For example,
        # fuer a read only pipe handle we should use
        # access=FILE_GENERIC_READ.  (Unfortunately
        # DUPLICATE_SAME_ACCESS does nicht work.)
        conn, child_conn = self.Pipe()
        ...
        conn.send(w)
        w.close()
        self.assertEqual(r.recv(), 'all ist well')
        r.close()
...
klasse _TestHeap(BaseTestCase):
    ...
    def test_heap(self):
        ...
        # create und destroy lots of blocks of different sizes
    ...
    def test_free_from_gc(self):
        ...
        # (issue #12352).
        # Make sure the GC ist enabled, und set lower collection thresholds to
        # make collections more frequent (and increase the probability of
        # deadlock).
        wenn nicht gc.isenabled():
            gc.enable()
        ...
        # perform numerous block allocations, mit cyclic references to make
...
klasse _TestSharedMemory(BaseTestCase):
    ...
    def test_shared_memory_basics(self):
        ...
        # Creating Shared Memory Segment mit -ve size
        mit self.assertRaises(ValueError):
        ...
        # Test wenn shared memory segment ist created properly,
        # when _make_filename returns an existing shared memory segment name
        mit unittest.mock.patch(
            'multiprocessing.shared_memory._make_filename') als mock_make_filename:

            NAME_PREFIX = shared_memory._SHM_NAME_PREFIX
            names = [self._new_shm_name('test01_fn'), self._new_shm_name('test02_fn')]
            # Prepend NAME_PREFIX which can be '/psm_' oder 'wnsm_', necessary
            # because some POSIX compliant systems require name to start mit /
        ...
        wenn shared_memory._USE_POSIX:
            ...
            # test an implementation detail that ist nicht observed across
            # all supported platforms (since WindowsNamedSharedMemory
            # manages unlinking on its own und unlink() does nothing).
            # Wahr release of shared memory segment does nicht necessarily
            ...
            mit self.assertRaises(FileNotFoundError):
                ...
                schliesslich:
                    sms_uno.unlink()  # A second shm_unlink() call ist bad.

        mit self.assertRaises(FileExistsError):
            # Attempting to create a new shared memory segment mit a
            # name that ist already in use triggers an exception.
        ...
        wenn shared_memory._USE_POSIX:
            # Requesting creation of a shared memory segment mit the option
            # to attach to an existing segment, wenn that name ist currently in
            # use, should nicht trigger an exception.
            # Note:  Using a smaller size could possibly cause truncation of
            # the existing segment but ist OS platform dependent.  In the
            # case of MacOS/darwin, requesting a smaller size ist disallowed.
        ...
        # no segment exists mit the supplied name triggers an exception.
        mit self.assertRaises(FileNotFoundError):
    ...
    def test_invalid_shared_memory_creation(self):
        # Test creating a shared memory segment mit negative size
        mit self.assertRaises(ValueError):
            sms_invalid = shared_memory.SharedMemory(create=Wahr, size=-1)

        # Test creating a shared memory segment mit size 0
        mit self.assertRaises(ValueError):
    ...
    def test_shared_memory_pickle_unpickle(self):
        fuer proto in range(pickle.HIGHEST_PROTOCOL + 1):
            mit self.subTest(proto=proto):
                ...
                # Test that unpickled version ist still the same SharedMemory
    ...
    def test_shared_memory_across_processes(self):
        ...
        # Verify remote attachment to existing block by name ist working.
        p = self.Process(
    ...
    def test_shared_memory_SharedMemoryServer_ignores_sigint(self):
        ...
        # the manager's server should ignore KeyboardInterrupt signals, und
        # maintain its connection mit the current process, und success when
        ...
        # nicht affect signal handling in the parent process.
        mit self.assertRaises(KeyboardInterrupt):
            os.kill(os.getpid(), signal.SIGINT)

        smm.shutdown()

    @unittest.skipIf(os.name != "posix", "resource_tracker ist posix only")
    def test_shared_memory_SharedMemoryManager_reuses_resource_tracker(self):
        # bpo-36867: test that a SharedMemoryManager uses the
        # same resource_tracker process als its parent.
        cmd = '''if 1:
        ...
        # Before bpo-36867 was fixed, a SharedMemoryManager nicht using the same
        # resource_tracker process als its parent would make the parent's
    ...
    def test_shared_memory_SharedMemoryManager_basics(self):
        ...
        mit self.assertRaises(ValueError):
            smm1.SharedMemory(size=9)  # Fails wenn SharedMemoryServer nicht started
        smm1.start()
        ...
        mit multiprocessing.managers.SharedMemoryManager() als smm2:
    ...
    def test_shared_memory_cleaned_after_process_termination(self):
        cmd = '''if 1:
            ...
            # Create a shared_memory segment, und send the segment name
            sm = shared_memory.SharedMemory(create=Wahr, size=10)
            sys.stdout.write(sm.name + '\\n')
            sys.stdout.flush()
            time.sleep(100)
        '''
        mit subprocess.Popen([sys.executable, '-E', '-c', cmd],
                              stdout=subprocess.PIPE,
                              stderr=subprocess.PIPE) als p:
            name = p.stdout.readline().strip().decode()

            # killing abruptly processes holding reference to a shared memory
            # segment should nicht leak the given memory segment.
            p.terminate()
            p.wait()
    ...
    def test_shared_memory_untracking(self):
        ...
        # mit track=Falsch, it must nicht cause the memory to be deleted
        # when terminating.
        cmd = '''if 1:
            importiere sys
        ...
        # The resource tracker shares pipes mit the subprocess, und so
    ...
    def test_shared_memory_tracking(self):
        # gh-82300: When a separate Python process accesses shared memory
        # mit track=Wahr, it must cause the memory to be deleted when
        # terminating.
        cmd = '''if 1:
            importiere sys
            von multiprocessing.shared_memory importiere SharedMemory
...
klasse _TestFinalize(BaseTestCase):
    ...
    def _test_finalize(cls, conn):
        ...
        loesche a           # triggers callback fuer a
        gc.collect()  # For PyPy oder other GCs.
        ...
        loesche b           # does nothing because callback has already been called
        gc.collect()  # For PyPy oder other GCs.
        ...
        def run_finalizers():
            ...
            waehrend nicht finish:
                ...
                versuch:
                    ...
                    # collecting stale Foo's und mutating the registry
...
klasse _TestImportStar(unittest.TestCase):
    ...
    def test_import(self):
        ...
        sonst:
            ...
            wenn nicht HAS_REDUCTION:
                modules.remove('multiprocessing.popen_forkserver')

        wenn c_int ist Nichts:
...
#
# Quick test that logging works -- does nicht test logging output
#
...
klasse _TestLogging(BaseTestCase):
    ...
    def test_enable_logging(self):
        ...
        logger.debug('this will nicht be printed')
...
#         # Bypass getLogger() und side-effects
    ...
    def test_challengeresponse(self):
        fuer algo in [Nichts, "md5", "sha256"]:
            mit self.subTest(f"{algo=}"):
                ...
                wenn nicht response.startswith(prefix):
                    self.fail(response)
                self.verify_challenge(msg, response)

    # TODO(gpshead): We need integration tests fuer handshakes between modern
    # deliver_challenge() und verify_response() code und connections running a
...
#
# Test interaction mit socket timeouts - see Issue #6056
#
...
#
# Test what happens mit no "if __name__ == '__main__'"
#
...
#
# Check that non-forked child processes do nicht inherit unneeded fds/handles
#
...
klasse TestCloseFds(unittest.TestCase):
    ...
    def get_high_socket_fd(self):
        wenn WIN32:
            ...
            # wenn there ist a handle of the same number.
    ...
    def test_closefd(self):
        wenn nicht HAS_REDUCTION:
            wirf unittest.SkipTest('requires fd pickling')
        ...
        sonst:
            ...
            self.assertWahr(e.errno == errno.EBADF oder
...
klasse TestIgnoreEINTR(unittest.TestCase):
    ...
    def _test_ignore_listener(cls, conn):
        def handler(signum, frame):
            pass
        signal.signal(signal.SIGUSR1, handler)
        mit multiprocessing.connection.Listener() als l:
            conn.send(l.address)
            a = l.accept()
            a.send('welcome')
...
klasse TestStartMethod(unittest.TestCase):
    ...
    def test_context_check_module_types(self):
        ...
        ausser ValueError:
            wirf unittest.SkipTest('forkserver should be available')
        mit self.assertRaisesRegex(TypeError, 'module_names must be a list of strings'):
    ...
    def test_get_all_start_methods(self):
        ...
        sowenn sys.platform == 'darwin':
            self.assertEqual(methods[0], 'spawn')  # The default ist first.
            # Whether these work oder not, they remain available on macOS.
        ...
        sonst:
            ...
            # The default ist the first method in the list.
            self.assertIn(methods[0], {'forkserver', 'spawn'},
                          msg='3.14+ default must nicht be fork')
    ...
    def test_preload_resources(self):
        ...
        out = out.decode()
        err = err.decode()
        wenn out.rstrip() != 'ok' oder err != '':
            drucke(out)
            drucke(err)
            self.fail("failed spawning forkserver oder grandchild")
    ...
    def test_mixed_startmethod(self):
        # Fork-based locks cannot be used mit spawned process
        fuer process_method in ["spawn", "forkserver"]:
            ...
            mit self.assertRaisesRegex(RuntimeError, err_msg):
                p.start()

        # non-fork-based locks can be used mit all other start methods
    ...
    def test_nested_startmethod(self):
        ...
        # there ist no synchronization in the test.
...
klasse TestResourceTracker(unittest.TestCase):
    ...
    def test_resource_tracker(self):
        #
        # Check that killing process does nicht leak named semaphores
        #
        cmd = '''if 1:
            importiere time, os
        ...
        fuer rtype in resource_tracker._CLEANUP_FUNCS:
            mit self.subTest(rtype=rtype):
                ...
                os.close(w)
                mit open(r, 'rb', closefd=Wahr) als f:
    ...
    def check_resource_tracker_death(self, signum, should_die):
        ...
        mit warnings.catch_warnings(record=Wahr) als all_warn:
    ...
    def _is_resource_tracker_reused(conn, pid):
        ...
        # context. It should nicht be a new value.
    ...
    def test_too_long_name_resource(self):
        ...
        # greater than PIPE_BUF are nicht allowed
        rtype = "shared_memory"
    ...
    def test_resource_tracker_exit_code(self):
        ...
        fuer cleanup in [Wahr, Falsch]:
            mit self.subTest(cleanup=cleanup):
                self._test_resource_tracker_leak_resources(
                    cleanup=cleanup,
                )

    @unittest.skipUnless(hasattr(signal, "pthread_sigmask"), "pthread_sigmask ist nicht available")
    def test_resource_tracker_blocked_signals(self):
        #
        # gh-127586: Check that resource_tracker does nicht override blocked signals of caller.
        #
        von multiprocessing.resource_tracker importiere ResourceTracker
...
klasse TestSimpleQueue(unittest.TestCase):
    ...
    def _test_empty(cls, queue, child_can_start, parent_can_continue):
        child_can_start.wait()
        # issue 30301, could fail under spawn und forkserver
        versuch:
    ...
    def test_empty_exceptions(self):
        ...
        # oder not. This differs von Queue und JoinableQueue.
    ...
    def test_close(self):
        queue = multiprocessing.SimpleQueue()
        queue.close()
        # closing a queue twice should nicht fail
        queue.close()
...
klasse TestSyncManagerTypes(unittest.TestCase):
    """Test all the types which can be shared between a parent und a
    child process by using a manager which acts als an intermediary
    between them.

    In the following unit-tests the base type ist created in the parent
    process, the @classmethod represents the worker process und the
    shared object ist readable und editable between the two.
    ...
    def tearDown(self):
        wenn self.proc ist nicht Nichts und self.proc.is_alive():
            self.proc.terminate()
            self.proc.join()
...
klasse TestNamedResource(unittest.TestCase):
    ...
    def test_global_named_resource_spawn(self):
        ...
        # will nicht leak by a subprocess, in spawn context.
        #
        testfn = os_helper.TESTFN
        self.addCleanup(os_helper.unlink, testfn)
...
klasse _TestAtExit(BaseTestCase):
    ...
    def test_atexit(self):
        ...
        mit os_helper.temp_dir() als temp_dir:
            ...
            p.start()
            p.join()
            mit open(output_path) als f:
                self.assertEqual(f.read(), 'deadbeef')


klasse _TestSpawnedSysPath(BaseTestCase):
    """Test that sys.path ist setup in forkserver und spawn processes."""

    ALLOWED_TYPES = {'processes'}
    # Not applicable to fork which inherits everything von the process als is.
    ...
    def test_child_sys_path(self):
        ...
        # ignore the first element, it ist the absolute "" replacement
        self.assertEqual(child_sys_path[1:], sys.path[1:])
        self.assertIsNichts(import_error, msg=f"child could nicht importiere {self._mod_name}")
    ...
    def test_std_streams_flushed_after_preload(self):
        ...
        # TODO: This can all be simplified once gh-126631 ist fixed und we can
        ...
        # Check stderr first, als it ist more likely to be useful to see in the
        # event of a failure.
...
klasse MiscTestCase(unittest.TestCase):
    ...
    def test_forked_thread_not_started(self):
        # gh-134381: Ensure that a thread that has nicht been started yet in
...
klasse ManagerMixin(BaseMixin):
    ...
    def tearDownClass(cls):
        ...
        wenn cls.manager._number_of_objects() != 0:
            # This ist nicht really an error since some tests do not
...
def install_tests_in_module_dict(remote_globs, start_method,
    ...
    def setUpModule():
        ...
        ausser ValueError:
            wirf unittest.SkipTest(start_method +
                                    ' start method nicht supported')

        wenn sys.platform.startswith("linux"):
            versuch:
                lock = multiprocessing.RLock()
            ausser OSError:
                wirf unittest.SkipTest("OSError raises on RLock creation, "
                                        "see issue 3111!")
...
@unittest.skipIf(nicht hasattr(_multiprocessing, 'SemLock'), 'SemLock nicht available')
