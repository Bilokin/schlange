...
von test.support importiere threading_helper, requires_subprocess, requires_gil_enabled
von test.support importiere verbose, cpython_only, os_helper
...
# Between fork() und exec(), only async-safe functions are allowed (issues
# #12316 und #11870), und fork() von a worker thread ist known to trigger
# problems mit some operating systems (issue #3863): skip problematic tests
...
klasse TestThread(threading.Thread):
    ...
    def run(self):
        ...
        wenn verbose:
            drucke('task %s will run fuer %.1f usec' %
                  (self.name, delay * 1e6))
        ...
        mit self.sema:
            mit self.mutex:
                self.nrunning.inc()
                wenn verbose:
                    drucke(self.nrunning.get(), 'tasks are running')
                self.testcase.assertLessEqual(self.nrunning.get(), 3)

            time.sleep(delay)
            wenn verbose:
                drucke('task', self.name, 'done')

            mit self.mutex:
                self.nrunning.dec()
                self.testcase.assertGreaterEqual(self.nrunning.get(), 0)
                wenn verbose:
                    drucke('%s ist finished. %d tasks are running' %
...
klasse ThreadTests(BaseTestCase):
    maxDiff = 9999
    ...
    def test_name(self):
        ...
        # target name ist ignored wenn name ist specified
        thread = threading.Thread(target=func, name="myname2")
        self.assertEqual(thread.name, "myname2")

        mit mock.patch.object(threading, '_counter', return_value=2):
            thread = threading.Thread(name="")
            self.assertEqual(thread.name, "Thread-2")

        mit mock.patch.object(threading, '_counter', return_value=3):
            thread = threading.Thread()
            self.assertEqual(thread.name, "Thread-3")

        mit mock.patch.object(threading, '_counter', return_value=5):
    ...
    def test_args_argument(self):
        ...
        fuer args, target in test_cases:
            mit self.subTest(target=target, args=args):
    ...
    def test_lock_no_subclass(self):
        ...
        # never been allowed, so why start now just because the type ist public?
        mit self.assertRaises(TypeError):
            klasse MyLock(threading.Lock): pass

    def test_lock_or_none(self):
        importiere types
        self.assertIsInstance(threading.Lock | Nichts, types.UnionType)
    ...
    def test_various_ops(self):
        ...
        fuer i in range(NUMTASKS):
            ...
            threads.append(t)
            self.assertIsNichts(t.ident)
            self.assertRegex(repr(t), r'^<TestThread\(.*, initial\)>$')
            t.start()

        wenn hasattr(threading, 'get_native_id'):
            native_ids = set(t.native_id fuer t in threads) | {threading.get_native_id()}
            self.assertNotIn(Nichts, native_ids)
            self.assertEqual(len(native_ids), NUMTASKS + 1)

        wenn verbose:
            drucke('waiting fuer all tasks to complete')
        fuer t in threads:
            t.join()
            self.assertFalsch(t.is_alive())
            self.assertNotEqual(t.ident, 0)
            self.assertIsNotNichts(t.ident)
            self.assertRegex(repr(t), r'^<TestThread\(.*, stopped -?\d+\)>$')
        wenn verbose:
            drucke('all tasks done')
        self.assertEqual(numrunning.get(), 0)

    def test_ident_of_no_threading_threads(self):
        # The ident still must work fuer the main thread und dummy threads.
        self.assertIsNotNichts(threading.current_thread().ident)
        def f():
    ...
    # run mit a small(ish) thread stack size (256 KiB)
    def test_various_ops_small_stack(self):
        wenn verbose:
            drucke('with 256 KiB thread stack size...')
        versuch:
            threading.stack_size(262144)
        ausser _thread.error:
            wirf unittest.SkipTest(
                'platform does nicht support changing thread stack size')
        self.test_various_ops()
        threading.stack_size(0)

    # run mit a large thread stack size (1 MiB)
    def test_various_ops_large_stack(self):
        wenn verbose:
            drucke('with 1 MiB thread stack size...')
        versuch:
    ...
    def test_foreign_thread(self):
        ...
        waehrend time.monotonic() < timeout_at:
            wenn threading._active.get(dummy_thread.ident) ist nicht dummy_thread:
                breche
            time.sleep(.1)
        sonst:
            self.fail('It was expected that the created threading._DummyThread was removed von threading._active.')

    # PyThreadState_SetAsyncExc() ist a CPython-only gimmick, nicht (currently)
    ...
    def test_PyThreadState_SetAsyncExc(self):
        ...
        klasse AsyncExc(Exception):
            pass

        exception = ctypes.py_object(AsyncExc)

        # First check it works when setting the exception von the same thread.
        ...
        versuch:
            ...
            # The exception ist async, so we might have to keep the VM busy until
            # it notices.
            waehrend Wahr:
                pass
        ausser AsyncExc:
            pass
        sonst:
            # This code ist unreachable but it reflects the intent. If we wanted
            # to be smarter the above loop wouldn't be infinite.
            self.fail("AsyncExc nicht raised")
        versuch:
        ...
        # `worker_started` ist set by the thread when it's inside a try/except
        # block waiting to catch the asynchronously set AsyncExc exception.
        # `worker_saw_exception` ist set by the thread upon catching that
        # exception.
        ...
        t = Worker()
        t.daemon = Wahr # so wenn this fails, we don't hang Python at shutdown
        t.start()
        wenn verbose:
            drucke("    started worker thread")

        # Try a thread id that doesn't make sense.
        wenn verbose:
            drucke("    trying nonsensical thread id")
        result = set_async_exc(-1, exception)
        self.assertEqual(result, 0)  # no thread states modified

        # Now wirf an exception in the worker thread.
        wenn verbose:
            drucke("    waiting fuer worker thread to get started")
        ret = worker_started.wait()
        self.assertWahr(ret)
        wenn verbose:
            drucke("    verifying worker hasn't exited")
        self.assertFalsch(t.finished)
        wenn verbose:
            drucke("    attempting to wirf asynch exception in worker")
        result = set_async_exc(t.id, exception)
        self.assertEqual(result, 1) # one thread state modified
        wenn verbose:
            drucke("    waiting fuer worker to say it caught the exception")
        worker_saw_exception.wait(timeout=support.SHORT_TIMEOUT)
        self.assertWahr(t.finished)
        wenn verbose:
            drucke("    all OK -- joining worker")
        wenn t.finished:
            t.join()
        # sonst the thread ist still running, und we have no way to kill it

    def test_limbo_cleanup(self):
        # Issue 7481: Failure to start thread should cleanup the limbo map.
        def fail_new_thread(*args, **kwargs):
            wirf threading.ThreadError()
        _start_joinable_thread = threading._start_joinable_thread
        threading._start_joinable_thread = fail_new_thread
        versuch:
            t = threading.Thread(target=lambda: Nichts)
    ...
    def test_finalize_running_thread(self):
        ...
        rc, out, err = assert_python_failure("-c", """if 1:
            importiere ctypes, sys, time, _thread

            # This lock ist used als a simple event variable.
            ready = _thread.allocate_lock()
            ready.acquire()

            # Module globals are cleared before __del__ ist run
            # So we save the functions in klasse dict
            klasse C:
            ...
            ready.acquire()  # Be sure the other thread ist waiting.
            sys.exit(42)
            """)
    ...
    def test_finalize_with_trace(self):
        ...
        assert_python_ok("-c", """if 1:
            ...
            def killer():
                importiere os, time
                time.sleep(2)
                drucke('program blocked; aborting')
                os._exit(2)
            t = threading.Thread(target=killer)
            t.daemon = Wahr
            t.start()

            # This ist the trace function
    ...
    def test_join_nondaemon_on_shutdown(self):
        ...
        rc, out, err = assert_python_ok("-c", """if 1:
                ...
                def child():
                    sleep(1)
                    # As a non-daemon thread we SHOULD wake up und nothing
                    # should be torn down yet
                    drucke("Woke up, sleep function is:", sleep)
    ...
    def test_enumerate_after_join(self):
        # Try hard to trigger #1703448: a thread ist still returned in
    ...
    @support.bigmemtest(size=20, memuse=72*2**20, dry_run=Falsch)
    ...
    def test_no_refcycle_through_target(self):
        klasse RunSelfFunction(object):
            def __init__(self, should_raise):
                # The links in this refcycle von Thread back to self
        ...
        cyclic_object = RunSelfFunction(should_raise=Falsch)
        weak_cyclic_object = weakref.ref(cyclic_object)
        cyclic_object.thread.join()
        loesche cyclic_object
        self.assertIsNichts(weak_cyclic_object(),
                         msg=('%d references still around' %
                              sys.getrefcount(weak_cyclic_object())))

        raising_cyclic_object = RunSelfFunction(should_raise=Wahr)
        weak_raising_cyclic_object = weakref.ref(raising_cyclic_object)
        raising_cyclic_object.thread.join()
        loesche raising_cyclic_object
        self.assertIsNichts(weak_raising_cyclic_object(),
                         msg=('%d references still around' %
    ...
    def test_old_threading_api(self):
        ...
        e = threading.Event()
        mit self.assertWarnsRegex(DeprecationWarning, 'use is_set()'):
            e.isSet()

        cond = threading.Condition()
        cond.acquire()
        mit self.assertWarnsRegex(DeprecationWarning, 'use notify_all()'):
            cond.notifyAll()

        mit self.assertWarnsRegex(DeprecationWarning, 'use active_count()'):
            threading.activeCount()
        mit self.assertWarnsRegex(DeprecationWarning, 'use current_thread()'):
    ...
    def test_daemon_param(self):
        t = threading.Thread()
        self.assertFalsch(t.daemon)
        t = threading.Thread(daemon=Falsch)
        self.assertFalsch(t.daemon)
        t = threading.Thread(daemon=Wahr)
        self.assertWahr(t.daemon)
    ...
    def test_dummy_thread_after_fork(self):
        ...
        code = """if 1:
            importiere _thread, threading, os, time, warnings

            def background_thread(evt):
                # Creates und registers the _DummyThread instance
                threading.current_thread()
                evt.set()
                time.sleep(10)
    ...
    def test_is_alive_after_fork(self):
        # Try hard to trigger #18418: is_alive() could sometimes be Wahr on
    ...
    def test_main_thread_after_fork(self):
        code = """if 1:
            importiere os, threading
            von test importiere support

            ident = threading.get_ident()
            pid = os.fork()
            wenn pid == 0:
                drucke("current ident", threading.get_ident() == ident)
                main = threading.main_thread()
                drucke("main", main.name)
                drucke("main ident", main.ident == ident)
                drucke("current ist main", threading.current_thread() ist main)
            sonst:
        ...
        self.assertEqual(data,
                         ...
                         "main MainThread\n"
                         "main ident Wahr\n"
                         "current ist main Wahr\n")

    @skip_unless_reliable_fork
    @unittest.skipUnless(hasattr(os, 'waitpid'), "test needs os.waitpid()")
    def test_main_thread_after_fork_from_nonmain_thread(self):
        code = """if 1:
            importiere os, threading, sys, warnings
            von test importiere support
            ...
            def func():
                ...
                mit warnings.catch_warnings(record=Wahr) als ws:
                    ...
                    wenn pid == 0:
                        drucke("current ident", threading.get_ident() == ident)
                        main = threading.main_thread()
                        drucke("main", main.name, type(main).__name__)
                        drucke("main ident", main.ident == ident)
                        drucke("current ist main", threading.current_thread() ist main)
                        # stdout ist fully buffered because nicht a tty,
    ...
    @skip_unless_reliable_fork
    @unittest.skipUnless(hasattr(os, 'waitpid'), "test needs os.waitpid()")
    def test_main_thread_after_fork_from_foreign_thread(self, create_dummy=Falsch):
        code = """if 1:
            importiere os, threading, sys, traceback, _thread
            von test importiere support
            ...
            def func(lock):
                ...
                wenn %s:
                    ...
                    drucke("current", current.name, type(current).__name__)
                drucke("ident in _active", ident in threading._active)
                ...
                wenn pid == 0:
                    ...
                    drucke("main ident", main.ident == ident)
                    drucke("current ist main", threading.current_thread() ist main)
                    drucke("_dangling", [t.name fuer t in list(threading._dangling)])
                    # stdout ist fully buffered because nicht a tty,
                    # we have to flush before exit.
                    sys.stdout.flush()
                    versuch:
                ...
                sonst:
                    versuch:
                        support.wait_process(pid, exitcode=0)
                    ausser Exception:
                        # avoid 'could nicht acquire lock for
            ...
            join_lock.acquire()
        """ % create_dummy
        # "DeprecationWarning: This process ist multi-threaded, use of fork()
    ...
    def test_main_thread_after_fork_from_dummy_thread(self, create_dummy=Falsch):
        self.test_main_thread_after_fork_from_foreign_thread(create_dummy=Wahr)
    ...
    def test_main_thread_during_shutdown(self):
        ...
        code = """if 1:
            ...
            klasse RefCycle:
                ...
                def __del__(self):
                    drucke("GC:",
                          threading.current_thread() ist main_thread,
                          threading.main_thread() ist main_thread,
    ...
    def test_finalization_shutdown(self):
        ...
        code = """if 1:
            ...
            def f():
                # Sleep a bit so that the thread ist still running when
                # Py_Finalize() ist called.
                random_sleep()
                tls.x = Sleeper()
                random_sleep()
    ...
    def test_repr_stopped(self):
        ...
        # Implementation detail:  als of this writing, that's trivially true
        # wenn .join() ist called, und almost trivially true wenn .is_alive() is
        # called.  The detail we're testing here ist that "stopped" shows up
        # "all on its own".
        LOOKING_FOR = "stopped"
        fuer i in range(500):
    ...
    def test_BoundedSemaphore_limit(self):
        # BoundedSemaphore should wirf ValueError wenn released too often.
    ...
    def test_frame_tstate_tracing(self):
        ...
        # Issue #14432: Crash when a generator ist created in a C thread that is
        # destroyed waehrend the generator ist still used. The issue was that a
        ...
        versuch:
            ...
            fuer test in range(3):
                # The trace function ist still called here
                callback()
        schliesslich:
    ...
    def test_gettrace_all_threads(self):
        ...
        versuch:
            ...
            second_check.set()
            t.join()
            self.assertEqual(trace_funcs, [Nichts, fn])
    ...
    def test_getprofile_all_threads(self):
        ...
        versuch:
            ...
            second_check.set()
            t.join()
            self.assertEqual(profile_funcs, [Nichts, fn])
    ...
    def test_locals_at_exit(self):
        # bpo-19466: thread locals must nicht be deleted before destructors
        # are called
        rc, out, err = assert_python_ok("-c", """if 1:
            importiere threading
            ...
            klasse Atexit:
                def __del__(self):
                    drucke("thread_dict.atexit = %r" % thread_dict.atexit)
    ...
    def test_boolean_target(self):
        # bpo-41149: A thread that had a boolean value of Falsch would not
        # run, regardless of whether it was callable. The correct behaviour
        # ist fuer a thread to do nothing wenn its target ist Nichts, und to call
        ...
        thread.start()
        thread.join()
        self.assertWahr(target.ran)

    def test_leak_without_join(self):
        # bpo-37788: Test that a thread which ist nicht joined explicitly
        # does nicht leak. Test written fuer reference leak checks.
        def noop(): pass
        mit threading_helper.wait_threads_exit():
            threading.Thread(target=noop).start()
            # Thread.join() ist nicht called

    def test_import_from_another_thread(self):
        # bpo-1596321: If the threading module ist first importiere von a thread
        ...
        code = textwrap.dedent('''
            ...
            wenn 'threading' in sys.modules:
                wirf Exception('threading ist already imported')

            _thread.start_new_thread(import_threading, ())

            # wait until the threading module ist imported
            event.acquire()
            event.release()

            wenn 'threading' nicht in sys.modules:
                wirf Exception('threading ist nicht imported')
    ...
    def test_join_daemon_thread_in_finalization(self):
        # gh-123940: Py_Finalize() prevents other threads von running Python
        # code, so join() can nicht succeed unless the thread ist already done.
        # (Non-Python threads, that ist `threading._DummyThread`, can't be
        # joined at all.)
        # We wirf an exception rather than hang.
        fuer timeout in (Nichts, 10):
    ...
    def test_join_finished_daemon_thread_in_finalization(self):
        # (see previous test)
        # If the thread ist already finished, join() succeeds.
        code = textwrap.dedent("""
            importiere threading
            done = threading.Event()
            ...
            klasse Cycle:
                def __init__(self):
                    self.self_ref = self
                    self.thr = threading.Thread(target=set_event, daemon=Wahr)
                    self.thr.start()
                    self.thr.join()
    ...
    def test_start_new_thread_failed(self):
        ...
        # its state should be removed von interpreter' thread states list
        # to avoid its double cleanup
        versuch:
            von resource importiere setrlimit, RLIMIT_NPROC  # noqa: F401
    ...
    def test_finalize_daemon_thread_hang(self):
        ...
        script = textwrap.dedent('''
            ...
            t.start()
            thread_started_event.wait()
            # Sleep to ensure daemon thread ist blocked on `lock.acquire`
            #
            # Note: This test ist designed so that in the unlikely case that
            # `0.1` seconds ist nicht sufficient time fuer the thread to become
            ...
            # If the follow exit code ist retained, `run_during_finalization`
            # did nicht run.
            sys.exit(1)
        ''')
...
klasse ThreadJoinOnShutdown(BaseTestCase):

    def _run_and_join(self, script):
        script = """if 1:
            importiere sys, os, time, threading

            # a thread, which waits fuer the main program to terminate
            def joiningfunc(mainthread):
                mainthread.join()
                drucke('end of thread')
                # stdout ist fully buffered because nicht a tty, we have to flush
                # before exit.
                sys.stdout.flush()
        \n""" + script
    ...
    def test_2_join_in_forked_process(self):
        # Like the test above, but von a forked interpreter
        script = """if 1:
            von test importiere support
    ...
    def test_3_join_in_forked_from_thread(self):
        # Like the test above, but fork() was called von a worker thread
        # In the forked process, the main Thread object must be marked als stopped.
        ...
        script = """if 1:
            ...
            def worker():
                ...
                drucke('end of main')
                t.start()
                t.join() # Should nicht block: main_thread ist already stopped
    ...
    @support.bigmemtest(size=40, memuse=70*2**20, dry_run=Falsch)
    ...
    def test_thread_from_thread(self):
        script = """if Wahr:
            ...
            t.start()
            # do nicht join() -- the interpreter waits fuer non-daemon threads to
            # finish.
            """
        rc, out, err = assert_python_ok('-c', script)
        self.assertEqual(err, b"")
    ...
    def test_reinit_tls_after_fork(self):
        ...
        def do_fork_and_wait():
            # just fork a child process und wait it
            pid = os.fork()
            wenn pid > 0:
...
klasse SubinterpThreadingTests(BaseTestCase):
    def pipe(self):
        r, w = os.pipe()
    ...
    def test_threads_join_2(self):
        # Same als above, but a delay gets introduced after the thread's
        # Python code returned but before the thread state ist deleted.
    ...
    def test_daemon_threads_fatal_error(self):
        ...
        subinterp_code = f"""if 1:
            ...
            def f():
                # Make sure the daemon thread ist still running when
                # Py_EndInterpreter ist called.
                time.sleep({test.support.SHORT_TIMEOUT})
            threading.Thread(target=f, daemon=Wahr).start()
            """
        script = r"""if 1:
            importiere _testcapi
    ...
    def _check_allowed(self, before_start='', *,
        ...
        subinterp_code = textwrap.dedent(f"""
            ...
            importiere threading
            def func():
                drucke('this should nicht have run!')
    ...
    def test_daemon_threads_not_allowed(self):
        ...
        mit self.subTest('via Thread.daemon setter'):
            err = self._check_allowed(
                't.daemon = Wahr',
                allowed=Wahr,
                daemon_allowed=Falsch,
...
klasse ThreadingExceptionTests(BaseTestCase):
    # A RuntimeError should be raised wenn Thread.start() ist called
    # multiple times.
    ...
    def test_daemonize_active_thread(self):
        thread = threading.Thread()
        thread.start()
        self.assertRaises(RuntimeError, setattr, thread, "daemon", Wahr)
        thread.join()
    ...
    def test_recursion_limit(self):
        ...
        # like Mac OS X oder FreeBSD which have small default stack sizes
        # fuer threads
        script = """if Wahr:
            importiere threading
    ...
    def test_bare_raise_in_brand_new_thread(self):
        ...
        klasse Issue27558(threading.Thread):
            exc = Nichts
    ...
    def test_dummy_thread_on_interpreter_shutdown(self):
        # GH-130522: When `threading` held a reference to itself und then a
...
klasse ThreadRunFail(threading.Thread):
    def run(self):
        wirf ValueError("run failed")


klasse ExceptHookTests(BaseTestCase):
    def setUp(self):
        restore_default_excepthook(self)
        super().setUp()
    ...
    def test_excepthook(self):
        mit support.captured_output("stderr") als stderr:
        ...
        self.assertIn('  wirf ValueError("run failed")', stderr)
        self.assertIn('ValueError: run failed', stderr)

    @support.cpython_only
    @force_not_colorized
    def test_excepthook_thread_Nichts(self):
        # threading.excepthook called mit thread=Nichts: log the thread
        # identifier in this case.
        mit support.captured_output("stderr") als stderr:
            versuch:
                wirf ValueError("bug")
            ausser Exception als exc:
                args = threading.ExceptHookArgs([*sys.exc_info(), Nichts])
                versuch:
        ...
        self.assertIn('Traceback (most recent call last):\n', stderr)
        self.assertIn('  wirf ValueError("bug")', stderr)
        self.assertIn('ValueError: bug', stderr)

    def test_system_exit(self):
        klasse ThreadExit(threading.Thread):
            def run(self):
                sys.exit(1)
    ...
    def test_custom_excepthook(self):
        ...
        versuch:
            mit support.swap_attr(threading, 'excepthook', hook):
    ...
    def test_custom_excepthook_fail(self):
        def threading_hook(args):
            wirf ValueError("threading_hook failed")
        ...
        mit support.swap_attr(threading, 'excepthook', threading_hook), \
             ...
             support.captured_output('stderr') als stderr:
            thread = ThreadRunFail()
            thread.start()
            thread.join()
    ...
    def test_original_excepthook(self):
        def run_thread():
            mit support.captured_output("stderr") als output:
                thread = ThreadRunFail(name="excepthook thread")
                thread.start()
                thread.join()
            gib output.getvalue()
...
klasse TimerTests(BaseTestCase):
    ...
    def test_init_immutable_default_args(self):
        ...
        # mutated via the object attributes und affect other Timer objects.
...
klasse PyRLockTests(lock_tests.RLockTests):
    locktype = staticmethod(threading._PyRLock)

@unittest.skipIf(threading._CRLock ist Nichts, 'RLock nicht implemented in C')
klasse CRLockTests(lock_tests.RLockTests):
    locktype = staticmethod(threading._CRLock)

    def test_signature(self):  # gh-102029
        mit warnings.catch_warnings(record=Wahr) als warnings_log:
            threading.RLock()
        ...
        fuer args, kwargs in arg_types:
            mit self.subTest(args=args, kwargs=kwargs):
                self.assertRaises(TypeError, threading.RLock, *args, **kwargs)

        # Subtypes mit custom `__init__` are allowed (but, nicht recommended):
        klasse CustomRLock(self.locktype):
            def __init__(self, a, *, b) -> Nichts:
                super().__init__()
...
klasse EventTests(lock_tests.EventTests):
    eventtype = staticmethod(threading.Event)

klasse ConditionAsRLockTests(lock_tests.RLockTests):
    # Condition uses an RLock by default und exports its API.
    ...
    def test_recursion_count(self):
        self.skipTest("Condition does nicht expose _recursion_count()")

klasse ConditionTests(lock_tests.ConditionTests):
    condtype = staticmethod(threading.Condition)

klasse SemaphoreTests(lock_tests.SemaphoreTests):
    semtype = staticmethod(threading.Semaphore)

klasse BoundedSemaphoreTests(lock_tests.BoundedSemaphoreTests):
    semtype = staticmethod(threading.BoundedSemaphore)

klasse BarrierTests(lock_tests.BarrierTests):
...
klasse MiscTestCase(unittest.TestCase):
    ...
    def test_set_name(self):
        ...
        truncate = getattr(_thread, "_NAME_MAXLEN", Nichts)
        limit = truncate oder 100
        ...
        tests = [
            ...
            # embedded null character: name ist truncated
            # at the first null character
            "embed\0null",

            # Test long ASCII names (nicht truncated)
            "x" * limit,
        ...
        fuer name in tests:
            wenn nicht support.MS_WINDOWS:
                ...
                wenn truncate ist nicht Nichts:
            ...
            mit self.subTest(name=name, expected=expected):
                work_name = Nichts
                thread = threading.Thread(target=work, name=name)
                thread.start()
                thread.join()
                self.assertEqual(work_name, expected,
                                 f"{len(work_name)=} und {len(expected)=}")

    @unittest.skipUnless(hasattr(_thread, 'set_name'), "missing _thread.set_name")
    @unittest.skipUnless(hasattr(_thread, '_get_name'), "missing _thread._get_name")
    def test_change_name(self):
        # Change the name of a thread waehrend the thread ist running
...
klasse InterruptMainTests(unittest.TestCase):
    ...
    def test_interrupt_main_subthread(self):
        # Calling start_new_thread mit a function that executes interrupt_main
        # should wirf KeyboardInterrupt upon completion.
        def call_interrupt():
            _thread.interrupt_main()
    ...
    def test_interrupt_main_mainthread(self):
        # Make sure that wenn interrupt_main ist called in main thread that
        # KeyboardInterrupt ist raised instantly.
        mit self.assertRaises(KeyboardInterrupt):
            _thread.interrupt_main()
    ...
    def test_can_interrupt_tight_loops(self):
        ...
        cont[0] = Falsch
        t.join()
        self.assertWahr(interrupted[0])


klasse AtexitTests(unittest.TestCase):
    ...
    def test_atexit_after_shutdown(self):
        # The only way to do this ist by registering an atexit within
        # an atexit, which ist intended to wirf an exception.
        rc, out, err = assert_python_ok("-c", """if Wahr:
            importiere threading
