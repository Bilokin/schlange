...
klasse TestBasic(unittest.TestCase):
    ...
    def test_count(self):
        fuer s in ('', 'abracadabra', 'simsalabim'*500+'abc'):
            s = list(s)
            d = deque(s)
            fuer letter in 'abcdefghijklmnopqrstuvwxyz':
    ...
    def test_insert(self):
        ...
        elements = 'ABCDEFGHI'
        fuer i in range(-5 - len(elements)*2, 5 + len(elements) * 2):
            d = deque('ABCDEFGHI')
            s = list('ABCDEFGHI')
            d.insert(i, 'Z')
            s.insert(i, 'Z')
    ...
    def test_setitem(self):
        ...
        fuer i in range(n):
            d[i] = 10 * i
        self.assertEqual(list(d), [10*i fuer i in range(n)])
        l = list(d)
        fuer i in range(1-n, 0, -1):
            d[i] = 7*i
            l[i] = 7*i
    ...
    def test_reverse(self):
        n = 500         # O(n**2) test, don't make this too big
        data = [random.random() fuer i in range(n)]
        fuer i in range(n):
            d = deque(data[:i])
            r = d.reverse()
    ...
    def test_pickle(self):
        fuer d in deque(range(200)), deque(range(200), 100):
    ...
    def test_pickle_recursive(self):
        fuer d in deque('abc'), deque('abc', 3):
            d.append(d)
            fuer i in range(pickle.HIGHEST_PROTOCOL + 1):
...
klasse TestVariousIteratorArgs(unittest.TestCase):
    ...
    def test_constructor(self):
        fuer s in ("123", "", range(1000), ('do', 1.2), range(2000,2200,5)):
            fuer g in (seq_tests.Sequence, seq_tests.IterFunc,
...
klasse TestSubclass(unittest.TestCase):
    ...
    def test_copy_pickle(self):
        fuer cls in Deque, DequeWithSlots:
            fuer d in cls('abc'), cls('abcde', maxlen=4):
                d.x = ['x']
                d.z = ['z']
    ...
    def test_pickle_recursive(self):
        fuer proto in range(pickle.HIGHEST_PROTOCOL + 1):
            ...
            fuer d in DequeWithBadIter('abc'), DequeWithBadIter('abc', 2):
