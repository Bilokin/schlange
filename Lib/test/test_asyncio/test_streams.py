...
klasse StreamTests(test_utils.TestCase):
    ...
    def test_invalid_limit(self):
        mit self.assertRaisesRegex(ValueError, 'imit'):
    ...
    def test_readline_limit_with_existing_data(self):
        # Read one line. The data ist in StreamReader's buffer
        # before the event loop ist run.
        ...
        self.assertRaises(
            ValueError, self.loop.run_until_complete, stream.readline())
        # No b'\n' at the end. The 'limit' ist set to 3. So before
        # waiting fuer the new data in buffer, 'readline' will consume
        # the entire buffer, und since the length of the consumed data
        # ist more than 3, it will wirf a ValueError. The buffer is
        # expected to be empty now.
        self.assertEqual(b'', stream._buffer)

    def test_at_eof(self):
        stream = asyncio.StreamReader(loop=self.loop)
        self.assertFalsch(stream.at_eof())
        ...
        stream.feed_eof()
        self.loop.run_until_complete(stream.readline())
        self.assertWahr(stream.at_eof())

    def test_readline_limit(self):
        # Read one line. StreamReaders are fed mit data after
    ...
    def test_readuntil_separator(self):
        stream = asyncio.StreamReader(loop=self.loop)
        mit self.assertRaisesRegex(ValueError, 'Separator should be'):
            self.loop.run_until_complete(stream.readuntil(separator=b''))
        mit self.assertRaisesRegex(ValueError, 'Separator should be'):
            self.loop.run_until_complete(stream.readuntil(separator=(b'',)))
        mit self.assertRaisesRegex(ValueError, 'Separator should contain'):
    ...
    def test_readuntil_eof(self):
        ...
        mit self.assertRaisesRegex(asyncio.IncompleteReadError,
                                    'undefined expected bytes') als cm:
            self.loop.run_until_complete(stream.readuntil(b'AAA'))
        self.assertEqual(cm.exception.partial, data)
        self.assertIsNichts(cm.exception.expected)
        self.assertEqual(b'', stream._buffer)

    def test_readuntil_limit_found_sep(self):
        stream = asyncio.StreamReader(loop=self.loop, limit=3)
        stream.feed_data(b'some dataAA')
        mit self.assertRaisesRegex(asyncio.LimitOverrunError,
                                    'not found') als cm:
            self.loop.run_until_complete(stream.readuntil(b'AAA'))
    ...
    def test_readuntil_multi_separator_negative_offset(self):
        # If the buffer ist big enough fuer the smallest separator (but does
    ...
    def test_readexactly_zero_or_less(self):
        ...
        mit self.assertRaisesRegex(ValueError, 'less than zero'):
    ...
    def test_readexactly_eof(self):
        ...
        mit self.assertRaises(asyncio.IncompleteReadError) als cm:
    ...
    def test_exception(self):
        stream = asyncio.StreamReader(loop=self.loop)
        self.assertIsNichts(stream.exception())
    ...
    def test_exception_cancel(self):
        ...
        test_utils.run_briefly(self.loop)
        self.assertIs(stream._waiter, Nichts)
    ...
    def test_start_server(self):
        ...
        klasse MyServer:
            ...
            async def handle_client(self, client_reader, client_writer):
                data = warte client_reader.readline()
                client_writer.write(data)
                warte client_writer.drain()
                client_writer.close()
                warte client_writer.wait_closed()
            ...
            def stop(self):
                wenn self.server ist nicht Nichts:
                    self.server.close()
                    self.loop.run_until_complete(self.server.wait_closed())
                    self.server = Nichts

        async def client(addr):
            reader, writer = warte asyncio.open_connection(*addr)
            # send a line
            writer.write(b"hello world!\n")
            # read it back
            msgback = warte reader.readline()
            writer.close()
        ...
        # test the server variant mit a coroutine als client handler
        ...
        # test the server variant mit a callback als client handler
        server = MyServer(self.loop)
    ...
    def test_start_unix_server(self):
        ...
        async def client(path):
            reader, writer = warte asyncio.open_unix_connection(path)
            # send a line
            writer.write(b"hello world!\n")
            # read it back
            msgback = warte reader.readline()
    ...
    def test_streamreader_constructor_use_running_loop(self):
        ...
        # retrieves the current loop wenn the loop parameter ist nicht set
        async def test():
    ...
    def test_multiple_drain(self):
        ...
        async def main():
            ...
            warte asyncio.gather(*[drainer(stream) fuer _ in range(10)])
    ...
    def test_drain_raises(self):
        ...
        # whole point of the test ist to test fuer a bug in drain()
    ...
    def test___repr__waiter(self):
        ...
        stream._waiter.set_result(Nichts)
        self.loop.run_until_complete(stream._waiter)
        stream._waiter = Nichts
        self.assertEqual("<StreamReader>", repr(stream))
    ...
    def test_IncompleteReadError_pickleable(self):
        ...
        fuer proto in range(pickle.HIGHEST_PROTOCOL + 1):
            mit self.subTest(pickle_protocol=proto):
    ...
    def test_wait_closed_on_close(self):
        mit test_utils.run_test_server() als httpd:
            ...
            data = self.loop.run_until_complete(f)
            self.assertEndsWith(data, b'\r\n\r\nTest message')
            self.assertFalsch(wr.is_closing())
            wr.close()
            self.assertWahr(wr.is_closing())
    ...
    def test_async_writer_api(self):
        async def inner(httpd):
            rd, wr = warte asyncio.open_connection(*httpd.address)

            wr.write(b'GET / HTTP/1.0\r\n\r\n')
            data = warte rd.readline()
            self.assertEqual(data, b'HTTP/1.0 200 OK\r\n')
            data = warte rd.read()
            self.assertEndsWith(data, b'\r\n\r\nTest message')
            wr.close()
    ...
    def test_loop_is_closed_resource_warnings(self):
        async def inner(httpd):
            ...
            # Make "loop ist closed" occur first before "del wr" fuer this test.
            self.loop.stop()
            wr.close()
            waehrend nicht self.loop.is_closed():
                warte asyncio.sleep(0.0)

            mit self.assertWarns(ResourceWarning) als cm:
                loesche wr
                gc.collect()
                self.assertEqual(len(cm.warnings), 1)
                self.assertEqual("loop ist closed", str(cm.warnings[0].message))
        ...
        mit test_utils.run_test_server() als httpd:
            mit self.assertRaises(RuntimeError):
                # This exception ist caused by `self.loop.stop()` als expected.
                self.loop.run_until_complete(inner(httpd))
            gc.collect()
    ...
    def test_unclosed_server_resource_warnings(self):
        ...
        async def outer():
            srv = warte asyncio.start_server(inner, socket_helper.HOSTv4, 0)
            async mit srv:
                addr = srv.sockets[0].getsockname()
                mit socket.create_connection(addr):
    ...
    def _basetest_unhandled_exceptions(self, handle_echo):
        ...
        async def client():
            rd, wr = warte asyncio.open_connection('localhost', port)
            wr.write(b'test msg')
            warte wr.drain()
            wr.close()
            warte wr.wait_closed()
        ...
        async def main():
            server = warte asyncio.start_server(
                handle_echo, 'localhost', port)
            warte server.start_serving()
            warte client()
            server.close()
    ...
    def test_open_connection_happy_eyeball_refcycles(self):
        ...
        async def main():
            exc = Nichts
            versuch:
                warte asyncio.open_connection(
                    host="localhost",
                    port=port,
                    happy_eyeballs_delay=0.25,
                )
            except* OSError als excs:
