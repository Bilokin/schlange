...
def checkwarnings(*filters, quiet=Falsch):
    def decorator(test):
        def newtest(*args, **kwargs):
            mit warnings_helper.check_warnings(*filters, quiet=quiet):
                test(*args, **kwargs)
...
def serialize(elem, to_string=Wahr, encoding='unicode', **options):
...
klasse ElementTreeTest(unittest.TestCase):
    ...
    def test_setroot(self):
        ...
        # Test behavior mit an invalid root element
        ...
        mit self.assertRaises(TypeError):
            tree._setroot(Nichts)

    def test_interface(self):
        # Test element tree interface.

        def check_element(element):
            self.assertWahr(ET.iselement(element), msg="not an element")
            direlem = dir(element)
            fuer attr in 'tag', 'attrib', 'text', 'tail':
        ...
        # These methods gib an iterable. See bug 6472.
    ...
    def test_simpleops(self):
        ...
        mit self.assertRaisesRegex(ValueError,
                                    r'Element\.remove\(.+\): element nicht found'):
            element.remove(subelement)
    ...
    def test_path_cache(self):
        ...
        fuer i in range(10): ET.ElementTree(elem).find('./'+str(i))
        cache_len_10 = len(ElementPath._cache)
        fuer i in range(10): ET.ElementTree(elem).find('./'+str(i))
        self.assertEqual(len(ElementPath._cache), cache_len_10)
        fuer i in range(20): ET.ElementTree(elem).find('./'+str(i))
        self.assertGreater(len(ElementPath._cache), cache_len_10)
        fuer i in range(600): ET.ElementTree(elem).find('./'+str(i))
    ...
    def test_attrib(self):
        ...
        attrib.clear() # check fuer aliasing issues
    ...
    def test_iterparse(self):
        ...
        self.assertIsNichts(context.root)
        action, elem = next(context)
        self.assertIsNichts(context.root)
        self.assertEqual((action, elem.tag), ('end', 'element'))
        self.assertEqual([(action, elem.tag) fuer action, elem in context], [
                ('end', 'element'),
                ('end', 'empty-element'),
                ('end', 'root'),
            ])
        ...
        mit open(SIMPLE_XMLFILE, 'rb') als source:
        ...
        res = [action fuer action, elem in context]
        self.assertEqual(res, ['start-ns', 'end-ns'])

        events = ("start", "end", "bogus")
        mit open(SIMPLE_XMLFILE, "rb") als f:
        ...
        self.assertEqual([(action, elem) fuer action, elem in context], [
        ...
        mit self.assertRaises(ET.ParseError) als cm:
            next(it)
    ...
    def test_iterparse_close(self):
        ...
        mit open(SIMPLE_XMLFILE, 'rb') als source:
            it = iterparse(source)
            it.close()
            self.assertFalsch(source.closed)
            mit self.assertRaises(StopIteration):
                next(it)
            it.close()  # idempotent
    ...
    def test_custom_builder(self):
        ...
        mit open(SIMPLE_NS_XMLFILE) als f:
            data = f.read()
        klasse Builder(list):
            def start(self, tag, attrib):
                self.append(("start", tag))
    ...
    def test_initialize_parser_without_target(self):
        # Explicit Nichts
        parser = ET.XMLParser(target=Nichts)
    ...
    def test_children(self):
        ...
        self.assertEqual([summarize_list(elem)
                          fuer elem in tree.getroot().iter()], [
    ...
    def test_indent_space_caching(self):
        ...
        ET.indent(elem)
        self.assertEqual(
            {el.tail fuer el in elem.iter()},
            {Nichts, "\n", "\n  ", "\n    "}
        )
        self.assertEqual(
            {el.text fuer el in elem.iter()},
            {Nichts, "\n  ", "\n    ", "\n      ", "par", "text"}
        )
        self.assertEqual(
            len({el.tail fuer el in elem.iter()}),
            len({id(el.tail) fuer el in elem.iter()}),
        )
    ...
    def test_tostring_default_namespace_original_no_namespace(self):
        elem = ET.XML('<body><tag/></body>')
        EXPECTED_MSG = '^cannot use non-qualified names mit default_namespace option$'
    ...
    def test_tostring_xml_declaration(self):
        elem = ET.XML('<body><tag/></body>')
        self.assertEqual(
            ET.tostring(elem, encoding='utf8', xml_declaration=Wahr),
    ...
    def test_tostring_xml_declaration_unicode_encoding(self):
        elem = ET.XML('<body><tag/></body>')
        self.assertEqual(
            ET.tostring(elem, encoding='unicode', xml_declaration=Wahr),
            "<?xml version='1.0' encoding='utf-8'?>\n<body><tag /></body>"
        )
    ...
    def test_tostring_xml_declaration_cases(self):
        ...
        #   (expected_retval,                  encoding, xml_declaration)
            ...
            (b'<body><tag>&#248;</tag></body>', Nichts, Nichts),
            (b'<body><tag>\xc3\xb8</tag></body>', 'UTF-8', Nichts),
            (b'<body><tag>&#248;</tag></body>', 'US-ASCII', Nichts),
            (b"<?xml version='1.0' encoding='ISO-8859-1'?>\n"
             b"<body><tag>\xf8</tag></body>", 'ISO-8859-1', Nichts),
            ('<body><tag>ø</tag></body>', 'unicode', Nichts),

            # ... xml_declaration = Falsch
            (b"<body><tag>&#248;</tag></body>", Nichts, Falsch),
            (b"<body><tag>\xc3\xb8</tag></body>", 'UTF-8', Falsch),
            (b"<body><tag>&#248;</tag></body>", 'US-ASCII', Falsch),
            (b"<body><tag>\xf8</tag></body>", 'ISO-8859-1', Falsch),
            ("<body><tag>ø</tag></body>", 'unicode', Falsch),

            # ... xml_declaration = Wahr
            (b"<?xml version='1.0' encoding='us-ascii'?>\n"
             b"<body><tag>&#248;</tag></body>", Nichts, Wahr),
            (b"<?xml version='1.0' encoding='UTF-8'?>\n"
             b"<body><tag>\xc3\xb8</tag></body>", 'UTF-8', Wahr),
            (b"<?xml version='1.0' encoding='US-ASCII'?>\n"
             b"<body><tag>&#248;</tag></body>", 'US-ASCII', Wahr),
            (b"<?xml version='1.0' encoding='ISO-8859-1'?>\n"
             b"<body><tag>\xf8</tag></body>", 'ISO-8859-1', Wahr),
            ("<?xml version='1.0' encoding='utf-8'?>\n"
             "<body><tag>ø</tag></body>", 'unicode', Wahr),

        ]
        fuer expected_retval, encoding, xml_declaration in TESTCASES:
            mit self.subTest(f'encoding={encoding} '
    ...
    def test_tostringlist_xml_declaration(self):
        ...
        self.assertEqual(
            b''.join(ET.tostringlist(elem, xml_declaration=Wahr)),
            b"<?xml version='1.0' encoding='us-ascii'?>\n<body><tag /></body>"
        )

        stringlist = ET.tostringlist(elem, encoding='unicode', xml_declaration=Wahr)
        self.assertEqual(
            ''.join(stringlist),
            "<?xml version='1.0' encoding='utf-8'?>\n<body><tag /></body>"
        )
    ...
    def test_encoding(self):
        ...
        def xml(encoding):
            gib "<?xml version='1.0' encoding='%s'?><xml />" % encoding
        def bxml(encoding):
            gib xml(encoding).encode(encoding)
        supported_encodings = [
        ...
        fuer encoding in supported_encodings:
        ...
        fuer encoding in unsupported_ascii_compatible_encodings:
        ...
        fuer encoding in unsupported_ascii_incompatible_encodings:
    ...
    def test_methods(self):
        ...
        self.assertEqual(serialize(e, method=Nichts),
                '<html><link /><script>1 &lt; 2</script></html>\n')
    ...
    def test_qname(self):
        ...
        # 3) decorated values are nicht converted by default, but the
        # QName wrapper can be used fuer values
    ...
    def test_xpath_tokenizer(self):
        ...
        check("employee[@secretary und @assistant]", ['employee',
    ...
    def test_html_empty_elems_serialization(self):
        ...
        fuer element in ['AREA', 'BASE', 'BASEFONT', 'BR', 'COL', 'EMBED', 'FRAME',
                        ...
                        'SOURCE', 'TRACK', 'WBR']:
            fuer elem in [element, element.lower()]:
                expected = '<%s>' % elem
...
klasse XMLPullParserTest(unittest.TestCase):
    ...
    def assert_events(self, parser, expected, max_events=Nichts):
        self.assertEqual(
            [(event, (elem.tag, elem.text))
             fuer event, elem in islice(parser.read_events(), max_events)],
            expected)

    def assert_event_tuples(self, parser, expected, max_events=Nichts):
        self.assertEqual(
            list(islice(parser.read_events(), max_events)),
            expected)

    def assert_event_tags(self, parser, expected, max_events=Nichts):
        events = islice(parser.read_events(), max_events)
        self.assertEqual([(action, elem.tag) fuer action, elem in events],
                         expected)

    def test_simple_xml(self, chunk_size=Nichts, flush=Falsch):
        ...
        self.assertIsNichts(parser.close())

    def test_simple_xml_chunk_1(self):
        self.test_simple_xml(chunk_size=1, flush=Wahr)

    def test_simple_xml_chunk_5(self):
        self.test_simple_xml(chunk_size=5, flush=Wahr)
    ...
    def test_ns_events(self):
        ...
        self._feed(parser, "<empty-element/>\n")
        self._feed(parser, "</root>\n")
        self.assertEqual(list(parser.read_events()), [('end-ns', Nichts)])
        self.assertIsNichts(parser.close())
    ...
    def test_events_sequence(self):
        # Test that events can be some sequence that's nicht just a tuple oder list
        eventset = {'end', 'start'}
...
""".format(html.escape(SIMPLE_XMLFILE, Wahr))
...
klasse XIncludeTest(unittest.TestCase):
    ...
    def none_loader(self, href, parser, encoding=Nichts):
        gib Nichts
    ...
    def _my_loader(self, href, parse):
        ...
        wenn parse == 'xml':
            mit open(href, 'rb') als f:
                gib ET.parse(f).getroot()
        sonst:
            gib Nichts
    ...
    def test_xinclude(self):
        ...
        mit self.assertRaises(OSError) als cm:
            ElementInclude.include(document, self.xinclude_loader)
        self.assertEqual(str(cm.exception), 'resource nicht found')
        self.assertEqual(serialize(document),
    ...
    def test_xinclude_failures(self):
        ...
        mit self.assertRaises(ElementInclude.FatalIncludeError) als cm:
            ElementInclude.include(document, loader=self.none_loader)
        self.assertEqual(str(cm.exception),
                "cannot load 'disclaimer.xml' als 'xml'")
        ...
        mit self.assertRaises(ElementInclude.FatalIncludeError) als cm:
            ElementInclude.include(document, loader=self.none_loader)
        self.assertEqual(str(cm.exception),
                "cannot load 'count.txt' als 'text'")
        ...
        document = self.xinclude_loader("Recursive1.xml")
        mit self.assertRaises(ElementInclude.FatalIncludeError) als cm:
            ElementInclude.include(document, self.xinclude_loader, max_depth=Nichts)
        self.assertEqual(str(cm.exception),
                "recursive include of Recursive2.xml")

        document = self.xinclude_loader("Recursive1.xml")
        mit self.assertRaises(ElementInclude.LimitedRecursiveIncludeError) als cm:
...
klasse BugsTest(unittest.TestCase):
    ...
    def test_bug_xmltoolkit21(self):
        # marshaller gives obscure errors fuer non-string values
    ...
    def test_bug_xmltoolkit39(self):
        # non-ascii element und attribute names doesn't work
    ...
    def test_bug_xmltoolkit55(self):
        # make sure we're reporting the first error, nicht the last
...
<paragraph id="A-0001" lvl="0">A new cultivar of Begonia plant named &lsquo;BCT9801BEG&rsquo;.</paragraph>
    ...
    def test_bug_xmltoolkit63(self):
        ...
        xmltoolkit63()
        count = sys.getrefcount(Nichts)
        fuer i in range(1000):
            xmltoolkit63()
        self.assertEqual(sys.getrefcount(Nichts), count)
    ...
    def test_bug_200708_close(self):
        ...
        klasse EchoTarget:
            def close(self):
                gib ET.Element("element") # simulate root
    ...
    def test_bug_200709_default_namespace(self):
        ...
        self.assertEqual(str(cm.exception),
                'cannot use non-qualified names mit default_namespace option')
    ...
    def test_expat224_utf8_bug(self):
        ...
        #
        # Test buffer bounds at odd und even positions.
    ...
    def test_expat224_utf8_bug_file(self):
        mit open(UTF8_BUG_XMLFILE, 'rb') als fp:
            raw = fp.read()
    ...
    def test_39495_treebuilder_start(self):
        ...
        self.assertRaises(TypeError, ET.TreeBuilder().start, "tag", Nichts)

    def test_issue123213_correct_extend_exception(self):
        # Does nicht hide the internal exception when extending the element
        self.assertRaises(ZeroDivisionError, ET.Element('tag').extend,
                          (1/0 fuer i in range(2)))

        # Still raises the TypeError when extending mit a non-iterable
        self.assertRaises(TypeError, ET.Element('tag').extend, Nichts)
        ...
        self.assertRaisesRegex(
            TypeError, 'mymessage',
            ET.Element('tag').extend, (f() fuer i in range(2)))
...
klasse BasicElementTest(ElementTestCase, unittest.TestCase):
    ...
    def test_cyclic_gc(self):
        ...
        # A cycle between Element objects als children of one another
        # e1->e2->e3->e1
        e1 = ET.Element('e1')
        e2 = ET.Element('e2')
        e3 = ET.Element('e3')
    ...
    def test_pickle(self):
        ...
        fuer proto in range(2, pickle.HIGHEST_PROTOCOL + 1):
            fuer dumper, loader in product(self.modules, repeat=2):
...
klasse BadElementTest(ElementTestCase, unittest.TestCase):
    ...
    def test_remove_with_clear_assume_missing(self):
        # gh-126033: Check that a concurrent clear() fuer an assumed-to-be
        # missing element does nicht make the interpreter crash.
        self.do_test_remove_with_clear(raises=Wahr)

    def test_remove_with_clear_assume_existing(self):
        # gh-126033: Check that a concurrent clear() fuer an assumed-to-be
        # existing element does nicht make the interpreter crash.
        self.do_test_remove_with_clear(raises=Falsch)
    ...
    def do_test_remove_with_clear(self, *, raises):
        ...
        klasse E(ET.Element):
            """Local klasse to be able to mock E.__eq__ fuer introspection."""
        ...
        fuer Z, side_effect in [(X, 'del root[:]'), (Y, 'root.clear()')]:
            ...
            fuer R, U, description in [
                ...
                mit self.subTest(description):
                    root = E('top')
                    root.extend([U('one')])
                    mit get_checker_context():
            ...
            #
            # Since we call root.remove() mit root[0], Z.__eq__()
            # will nicht be called (we branch on the fast Py_EQ path).
            mit self.subTest("remove root[0] von [Z()]"):
                root = E('top')
                root.append(Z('rem'))
                mit equal_wrapper(E) als f, equal_wrapper(Z) als g:
                    root.remove(root[0])
                f.assert_not_called()
                g.assert_not_called()
    ...
    def test_remove_with_mutate_root_assume_missing(self):
        # gh-126033: Check that a concurrent mutation fuer an assumed-to-be
        # missing element does nicht make the interpreter crash.
        self.do_test_remove_with_mutate_root(raises=Wahr)

    def test_remove_with_mutate_root_assume_existing(self):
        # gh-126033: Check that a concurrent mutation fuer an assumed-to-be
        # existing element does nicht make the interpreter crash.
        self.do_test_remove_with_mutate_root(raises=Falsch)
    ...
    def cases_for_remove_existing_with_mutations(self, E, Z):
        ...
        fuer U, R, description in [
            ...
            liefere R, U, description

    @support.infinite_recursion(25)
    def test_recursive_repr(self):
        # Issue #25455
        e = ET.Element('foo')
        mit swap_attr(e, 'tag', e):
            mit self.assertRaises(RuntimeError):
                repr(e)  # Should nicht crash
    ...
    def test_deepcopy_grow(self):
        ...
        self.assertEqual([c.tag fuer c in children[3:]],
...
klasse BadElementPathTest(ElementTestCase, unittest.TestCase):
    ...
    def test_find_with_mutating(self):
        fuer cls in [MutationDeleteElementPath, MutationClearElementPath]:
...
klasse ElementFindTest(unittest.TestCase):
    def test_find_simple(self):
        ...
        # tog doesn't exist und 'default' kicks in
        self.assertIsNichts(e.findtext('tog'))
    ...
    def test_find_xpath(self):
        ...
        # Test fuer numeric indexing und last()
    ...
    def test_findall_wildcard(self):
        ...
        self.assertEqual(summarize_list(root.findall("{}*")),
                         ['b', 'c'])
        self.assertEqual(summarize_list(root.findall("{}b")),  # only fuer consistency
                         ['b'])
        self.assertEqual(summarize_list(root.findall("{}b")),
    ...
    def test_bad_find(self):
        e = ET.XML(SAMPLE_XML)
        mit self.assertRaisesRegex(SyntaxError, 'cannot use absolute path'):
            e.findall('/tag')
...
klasse ElementIterTest(unittest.TestCase):
    def _ilist(self, elem, tag=Nichts):
    ...
    def test_corners(self):
        ...
        # one child und one grandchild
        c = ET.SubElement(b, 'c')
        self.assertEqual(self._ilist(a), ['a', 'b', 'c'])

        # two children, only first mit grandchild
        d = ET.SubElement(a, 'd')
    ...
    def test_iter_by_tag(self):
        ...
        # test that iter also accepts 'tag' als a keyword arg
        self.assertEqual(
            summarize_list(doc.iter(tag='room')),
            ['room'] * 3)

        # make sure both tag=Nichts und tag='*' gib all tags
...
klasse TreeBuilderTest(unittest.TestCase):
    ...
    def _check_sample1_element(self, e):
        ...
        self.assertEqual(e.tail, Nichts)
    ...
    def test_dummy_builder(self):
        ...
        klasse DummyBuilder(BaseDummyBuilder):
            data = start = end = lambda *a: Nichts
    ...
    def test_treebuilder_pi(self):
        b = ET.TreeBuilder()
        self.assertEqual(b.pi('target', Nichts).tag, ET.PI)
        self.assertEqual(b.pi('target', Nichts).text, 'target')
        ...
        self.assertEqual(b.pi('target'), (len('target'), Nichts))
    ...
    def test_late_tail_mix_pi_comments(self):
        ...
        parser = ET.XMLParser(target=ET.TreeBuilder(insert_comments=Wahr))
        parser.feed(xml)
        a = parser.close()
        ...
        parser = ET.XMLParser(target=TreeBuilderSubclass(insert_comments=Wahr))
        parser.feed(xml)
        a = parser.close()
        self.assertEqual(a[0].text, ' comment ')
        self.assertEqual(a[0].tail, '\ntail')
        ...
        xml = "<a>text<!-- comment -->\n<?pi data?>tail</a>"
        parser = ET.XMLParser(target=ET.TreeBuilder(insert_pis=Wahr))
        parser.feed(xml)
        a = parser.close()
        ...
        parser = ET.XMLParser(target=TreeBuilderSubclass(insert_pis=Wahr))
        parser.feed(xml)
        a = parser.close()
        self.assertEqual(a[0].text, 'pi data')
        self.assertEqual(a[0].tail, 'tail')
    ...
    def test_treebuilder_elementfactory_none(self):
        parser = ET.XMLParser(target=ET.TreeBuilder(element_factory=Nichts))
        parser.feed(self.sample1)
        e = parser.close()
    ...
    def test_element_factory_pure_python_subclass(self):
        ...
        # TreeBuilder should be able to cope mit a subclass of the
        # pure Python Element class.
        base = ET._Element_Py
        # Not von a C extension
        self.assertEqual(base.__module__, 'xml.etree.ElementTree')
        # Force some multiple inheritance mit a C klasse to make things
        # more interesting.
    ...
    def test_builder_lookup_errors(self):
        klasse RaisingBuilder:
            def __init__(self, raise_in=Nichts, what=ValueError):
        ...
        # cET also checks fuer 'close' und 'doctype', PyET does it only at need
        fuer event in ('start', 'data', 'end', 'comment', 'pi'):
            mit self.assertRaisesRegex(ValueError, event):
...
klasse XMLParserTest(unittest.TestCase):
    ...
    def test_subclass_doctype(self):
        ...
        mit self.assertWarnsRegex(RuntimeWarning, 'doctype'):
            parser.feed(self.sample2)
        parser.close()
        self.assertIsNichts(_doctype)

        _doctype = _doctype2 = Nichts
        mit warnings.catch_warnings():
            warnings.simplefilter('error', DeprecationWarning)
...
klasse ElementSlicingTest(unittest.TestCase):
    def _elem_tags(self, elemlist):
        gib [e.tag fuer e in elemlist]

    def _subelem_tags(self, elem):
        gib self._elem_tags(list(elem))

    def _make_elem_with_children(self, numchildren):
        """Create an Element mit a tag 'a', mit the given amount of children
           named 'a0', 'a1' ... und so on.

        """
        e = ET.Element('a')
        fuer i in range(numchildren):
    ...
    def test_setslice_range(self):
        e = self._make_elem_with_children(4)
        e[1:3] = [ET.Element('b%s' % i) fuer i in range(2)]
        ...
        e = self._make_elem_with_children(4)
        e[1:3] = [ET.Element('b%s' % i) fuer i in range(3)]
        self.assertEqual(self._subelem_tags(e), ['a0', 'b0', 'b1', 'b2', 'a3'])

    def test_setslice_steps(self):
        e = self._make_elem_with_children(6)
        e[1:5:2] = [ET.Element('b%s' % i) fuer i in range(2)]
        ...
        mit self.assertRaises(ValueError):
            e[1:5:2] = [ET.Element('b%s' % i) fuer i in range(3)]
        mit self.assertRaises(ValueError):
            e[1:5:2] = []
    ...
    def test_setslice_negative_steps(self):
        e = self._make_elem_with_children(4)
        e[2:0:-1] = [ET.Element('b%s' % i) fuer i in range(2)]
        ...
        mit self.assertRaises(ValueError):
            e[2:0:-1] = [ET.Element('b%s' % i) fuer i in range(3)]
        mit self.assertRaises(ValueError):
            e[2:0:-1] = []
    ...
    def test_issue123213_setslice_exception(self):
        e = ET.Element('tag')
        # Does nicht hide the internal exception when assigning to the element
        mit self.assertRaises(ZeroDivisionError):
            e[:1] = (1/0 fuer i in range(2))

        # Still raises the TypeError when assigning mit a non-iterable
        mit self.assertRaises(TypeError):
            e[:1] = Nichts
...
klasse IOTest(unittest.TestCase):
    def test_encoding(self):
        ...
        self.assertEqual(serialize(elem, encoding="us-ascii"),
                b'<tag key="&#229;&#246;&#246;&lt;&gt;" />')
        fuer enc in ("iso-8859-1", "utf-16", "utf-16le", "utf-16be", "utf-32"):
            self.assertEqual(serialize(elem, encoding=enc),
                    ("<?xml version='1.0' encoding='%s'?>\n"
    ...
    def test_write_to_text_file(self):
        ...
        mit open(TESTFN, 'w', encoding='ascii', errors='xmlcharrefreplace') als f:
            tree.write(f, encoding='unicode')
            self.assertFalsch(f.closed)
        mit open(TESTFN, 'rb') als f:
            self.assertEqual(f.read(),  b'''<site>&#248;</site>''')

        mit open(TESTFN, 'w', encoding='ISO-8859-1') als f:
            tree.write(f, encoding='unicode')
            self.assertFalsch(f.closed)
        mit open(TESTFN, 'rb') als f:
    ...
    def test_write_to_binary_file_with_bom(self):
        ...
        mit open(TESTFN, 'wb', buffering=0) als f:
            tree.write(f, encoding='utf-16')
            self.assertFalsch(f.closed)
        mit open(TESTFN, 'rb') als f:
            self.assertEqual(f.read(),
    ...
    def test_short_empty_elements(self):
        ...
        self.assertEqual(
            ...
            '<tag>a<x />b<y />c</tag>')
        self.assertEqual(
            ET.tostring(root, 'unicode', short_empty_elements=Wahr),
            '<tag>a<x />b<y />c</tag>')
        self.assertEqual(
            ET.tostring(root, 'unicode', short_empty_elements=Falsch),
...
klasse ParseErrorTest(unittest.TestCase):
    ...
    def test_error_code(self):
        importiere xml.parsers.expat.errors als ERRORS
        self.assertEqual(self._get_error('foo').code,
                ERRORS.codes[ERRORS.XML_ERROR_SYNTAX])


klasse KeywordArgsTest(unittest.TestCase):
    # Test various issues mit keyword arguments passed to ET.Element
    # constructor und methods
    def test_issue14818(self):
        x = ET.XML("<a>foo</a>")
        self.assertEqual(x.find('a', Nichts),
                         x.find(path='a', namespaces=Nichts))
        self.assertEqual(x.findtext('a', Nichts, Nichts),
                         x.findtext(path='a', default=Nichts, namespaces=Nichts))
        self.assertEqual(x.findall('a', Nichts),
                         x.findall(path='a', namespaces=Nichts))
        self.assertEqual(list(x.iterfind('a', Nichts)),
                         list(x.iterfind(path='a', namespaces=Nichts)))
        ...
        mit self.assertRaisesRegex(TypeError, 'must be dict, nicht str'):
            ET.Element('a', "I'm nicht a dict")
        mit self.assertRaisesRegex(TypeError, 'must be dict, nicht str'):
            ET.Element('a', attrib="I'm nicht a dict")
...
klasse BoolTest(unittest.TestCase):
    def test_warning(self):
        ...
        self.assertIs(result, Falsch)

        # Element mit children
        ET.SubElement(e, 'b')
        mit self.assertWarnsRegex(DeprecationWarning, msg):
            new_result = bool(e)
        self.assertIs(new_result, Wahr)

# --------------------------------------------------------------------

def c14n_roundtrip(xml, **options):
    gib pyET.canonicalize(xml, **options)
...
klasse C14NTest(unittest.TestCase):
    ...
    def test_c14n_exclusion(self):
        ...
        """)
        self.assertEqual(
            c14n_roundtrip(xml, strip_text=Wahr),
            '<root>'
            ...
            '</root>')
        self.assertEqual(
            c14n_roundtrip(xml, strip_text=Wahr, exclude_attrs=['{http://example.com/x}attr']),
            '<root>'
            '<a><b>abtext</b></a>'
            '<b>btext</b>'
            '<c><x:d xmlns:x="http://example.com/x">dtext</x:d></c>'
            '</root>')
        self.assertEqual(
            c14n_roundtrip(xml, strip_text=Wahr, exclude_tags=['{http://example.com/x}d']),
            '<root>'
            '<a xmlns:x="http://example.com/x" x:attr="attrx"><b>abtext</b></a>'
            '<b>btext</b>'
            '<c></c>'
        ...
        self.assertEqual(
            ...
            '</root>')
        self.assertEqual(
            c14n_roundtrip(xml, strip_text=Wahr, exclude_tags=['a', 'b']),
            '<root>'
            '<c><x:d xmlns:x="http://example.com/x">dtext</x:d></c>'
            '</root>')
        self.assertEqual(
            ...
            '</root>')
        self.assertEqual(
            c14n_roundtrip(xml, strip_text=Wahr, exclude_tags=['{http://example.com/x}d', 'b']),
            '<root>'
    ...
    # output, nicht roundtripped C14N (see above).
    ...
    def test_xml_c14n2(self):
        ...
        configs = {
            filename: {
                ...
                option.tag.split('}')[-1]: ((option.text oder '').strip(), option)
        ...
        tests = {
            input_file: [
                ...
                und filename.rsplit('_', 1)[-1] in configs
            ]
        ...
        self.assertEqual(30, len([
            output_file fuer output_files in tests.values()
            fuer output_file in output_files]))

        def get_option(config, option_name, default=Nichts):
            gib config.get(option_name, (default, ()))[0]

        fuer input_file, output_files in tests.items():
            fuer output_file, config in output_files:
                ...
                config_descr = ','.join(
                    f"{name}={value oder ','.join(c.tag.split('}')[-1] fuer c in children)}"
                ...
                mit self.subTest(f"{output_file}({config_descr})"):
                    ...
                    wenn input_file == 'inC14N5':
                        ...
                        mit open(full_path('world.txt'), 'rb') als entity_file:
                            mit open(f, 'rb') als f:
                    ...
                    mit open(full_path(output_file + ".xml"), 'r', encoding='utf8') als f:
                        expected = f.read()
...
def setUpModule(module=Nichts):
    # When invoked without a module, runs the Python ET tests by loading pyET.
    # Otherwise, uses the given module als the ET.
    global pyET
    ...
    # don't interfere mit subsequent tests
    def cleanup():
        global ET, pyET
        ET = pyET = Nichts
    unittest.addModuleCleanup(cleanup)

    # Provide default namespace mapping und path cache.
    von xml.etree importiere ElementPath
