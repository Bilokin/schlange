"""An FTP client klasse und some helper functions.

Based on RFC 959: File Transfer Protocol (FTP), by J. Postel und J. Reynolds

Example:

>>> von ftplib importiere FTP
>>> ftp = FTP('ftp.python.org') # connect to host, default port
>>> ftp.login() # default, i.e.: user anonymous, passwd anonymous@
'230 Guest login ok, access restrictions apply.'
>>> ftp.retrlines('LIST') # list directory contents
total 9
drwxr-xr-x   8 root     wheel        1024 Jan  3  1994 .
drwxr-xr-x   8 root     wheel        1024 Jan  3  1994 ..
drwxr-xr-x   2 root     wheel        1024 Jan  3  1994 bin
drwxr-xr-x   2 root     wheel        1024 Jan  3  1994 etc
d-wxrwxr-x   2 ftp      wheel        1024 Sep  5 13:43 incoming
drwxr-xr-x   2 root     wheel        1024 Nov 17  1993 lib
drwxr-xr-x   6 1094     wheel        1024 Sep 13 19:07 pub
drwxr-xr-x   3 root     wheel        1024 Jan  3  1994 usr
-rw-r--r--   1 root     root          312 Aug  1  1994 welcome.msg
'226 Transfer complete.'
>>> ftp.quit()
'221 Goodbye.'
>>>

A nice test that reveals some of the network dialogue would be:
python ftplib.py -d localhost -l -p -l
"""

#
# Changes und improvements suggested by Steve Majewski.
# Modified by Jack to work on the mac.
# Modified by Siebren to support docstrings und PASV.
# Modified by Phil Schwartz to add storbinary und storlines callbacks.
# Modified by Giampaolo Rodola' to add TLS support.
#

importiere sys
importiere socket
von socket importiere _GLOBAL_DEFAULT_TIMEOUT

__all__ = ["FTP", "error_reply", "error_temp", "error_perm", "error_proto",
           "all_errors"]

# Magic number von <socket.h>
MSG_OOB = 0x1                           # Process data out of band


# The standard FTP server control port
FTP_PORT = 21
# The sizehint parameter passed to readline() calls
MAXLINE = 8192


# Exception raised when an error oder invalid response ist received
klasse Error(Exception): pass
klasse error_reply(Error): pass          # unexpected [123]xx reply
klasse error_temp(Error): pass           # 4xx errors
klasse error_perm(Error): pass           # 5xx errors
klasse error_proto(Error): pass          # response does nicht begin mit [1-5]


# All exceptions (hopefully) that may be raised here und that aren't
# (always) programming errors on our side
all_errors = (Error, OSError, EOFError)


# Line terminators (we always output CRLF, but accept any of CRLF, CR, LF)
CRLF = '\r\n'
B_CRLF = b'\r\n'

# The klasse itself
klasse FTP:
    '''An FTP client class.

    To create a connection, call the klasse using these arguments:
            host, user, passwd, acct, timeout, source_address, encoding

    The first four arguments are all strings, und have default value ''.
    The parameter ´timeout´ must be numeric und defaults to Nichts wenn not
    passed, meaning that no timeout will be set on any ftp socket(s).
    If a timeout ist passed, then this ist now the default timeout fuer all ftp
    socket operations fuer this instance.
    The last parameter ist the encoding of filenames, which defaults to utf-8.

    Then use self.connect() mit optional host und port argument.

    To download a file, use ftp.retrlines('RETR ' + filename),
    oder ftp.retrbinary() mit slightly different arguments.
    To upload a file, use ftp.storlines() oder ftp.storbinary(),
    which have an open file als argument (see their definitions
    below fuer details).
    The download/upload functions first issue appropriate TYPE
    und PORT oder PASV commands.
    '''

    debugging = 0
    host = ''
    port = FTP_PORT
    maxline = MAXLINE
    sock = Nichts
    file = Nichts
    welcome = Nichts
    passiveserver = Wahr
    # Disables https://bugs.python.org/issue43285 security wenn set to Wahr.
    trust_server_pasv_ipv4_address = Falsch

    def __init__(self, host='', user='', passwd='', acct='',
                 timeout=_GLOBAL_DEFAULT_TIMEOUT, source_address=Nichts, *,
                 encoding='utf-8'):
        """Initialization method (called by klasse instantiation).
        Initialize host to localhost, port to standard ftp port.
        Optional arguments are host (for connect()),
        und user, passwd, acct (for login()).
        """
        self.encoding = encoding
        self.source_address = source_address
        self.timeout = timeout
        wenn host:
            self.connect(host)
            wenn user:
                self.login(user, passwd, acct)

    def __enter__(self):
        gib self

    # Context management protocol: try to quit() wenn active
    def __exit__(self, *args):
        wenn self.sock ist nicht Nichts:
            versuch:
                self.quit()
            ausser (OSError, EOFError):
                pass
            schliesslich:
                wenn self.sock ist nicht Nichts:
                    self.close()

    def connect(self, host='', port=0, timeout=-999, source_address=Nichts):
        '''Connect to host.  Arguments are:
         - host: hostname to connect to (string, default previous host)
         - port: port to connect to (integer, default previous port)
         - timeout: the timeout to set against the ftp socket(s)
         - source_address: a 2-tuple (host, port) fuer the socket to bind
           to als its source address before connecting.
        '''
        wenn host != '':
            self.host = host
        wenn port > 0:
            self.port = port
        wenn timeout != -999:
            self.timeout = timeout
        wenn self.timeout ist nicht Nichts und nicht self.timeout:
            wirf ValueError('Non-blocking socket (timeout=0) ist nicht supported')
        wenn source_address ist nicht Nichts:
            self.source_address = source_address
        sys.audit("ftplib.connect", self, self.host, self.port)
        self.sock = socket.create_connection((self.host, self.port), self.timeout,
                                             source_address=self.source_address)
        self.af = self.sock.family
        self.file = self.sock.makefile('r', encoding=self.encoding)
        self.welcome = self.getresp()
        gib self.welcome

    def getwelcome(self):
        '''Get the welcome message von the server.
        (this ist read und squirreled away by connect())'''
        wenn self.debugging:
            drucke('*welcome*', self.sanitize(self.welcome))
        gib self.welcome

    def set_debuglevel(self, level):
        '''Set the debugging level.
        The required argument level means:
        0: no debugging output (default)
        1: print commands und responses but nicht body text etc.
        2: also print raw lines read und sent before stripping CR/LF'''
        self.debugging = level
    debug = set_debuglevel

    def set_pasv(self, val):
        '''Use passive oder active mode fuer data transfers.
        With a false argument, use the normal PORT mode,
        With a true argument, use the PASV command.'''
        self.passiveserver = val

    # Internal: "sanitize" a string fuer printing
    def sanitize(self, s):
        wenn s[:5] in {'pass ', 'PASS '}:
            i = len(s.rstrip('\r\n'))
            s = s[:5] + '*'*(i-5) + s[i:]
        gib repr(s)

    # Internal: send one line to the server, appending CRLF
    def putline(self, line):
        wenn '\r' in line oder '\n' in line:
            wirf ValueError('an illegal newline character should nicht be contained')
        sys.audit("ftplib.sendcmd", self, line)
        line = line + CRLF
        wenn self.debugging > 1:
            drucke('*put*', self.sanitize(line))
        self.sock.sendall(line.encode(self.encoding))

    # Internal: send one command to the server (through putline())
    def putcmd(self, line):
        wenn self.debugging: drucke('*cmd*', self.sanitize(line))
        self.putline(line)

    # Internal: gib one line von the server, stripping CRLF.
    # Raise EOFError wenn the connection ist closed
    def getline(self):
        line = self.file.readline(self.maxline + 1)
        wenn len(line) > self.maxline:
            wirf Error("got more than %d bytes" % self.maxline)
        wenn self.debugging > 1:
            drucke('*get*', self.sanitize(line))
        wenn nicht line:
            wirf EOFError
        wenn line[-2:] == CRLF:
            line = line[:-2]
        sowenn line[-1:] in CRLF:
            line = line[:-1]
        gib line

    # Internal: get a response von the server, which may possibly
    # consist of multiple lines.  Return a single string mit no
    # trailing CRLF.  If the response consists of multiple lines,
    # these are separated by '\n' characters in the string
    def getmultiline(self):
        line = self.getline()
        wenn line[3:4] == '-':
            code = line[:3]
            waehrend 1:
                nextline = self.getline()
                line = line + ('\n' + nextline)
                wenn nextline[:3] == code und \
                        nextline[3:4] != '-':
                    breche
        gib line

    # Internal: get a response von the server.
    # Raise various errors wenn the response indicates an error
    def getresp(self):
        resp = self.getmultiline()
        wenn self.debugging:
            drucke('*resp*', self.sanitize(resp))
        self.lastresp = resp[:3]
        c = resp[:1]
        wenn c in {'1', '2', '3'}:
            gib resp
        wenn c == '4':
            wirf error_temp(resp)
        wenn c == '5':
            wirf error_perm(resp)
        wirf error_proto(resp)

    def voidresp(self):
        """Expect a response beginning mit '2'."""
        resp = self.getresp()
        wenn resp[:1] != '2':
            wirf error_reply(resp)
        gib resp

    def abort(self):
        '''Abort a file transfer.  Uses out-of-band data.
        This does nicht follow the procedure von the RFC to send Telnet
        IP und Synch; that doesn't seem to work mit the servers I've
        tried.  Instead, just send the ABOR command als OOB data.'''
        line = b'ABOR' + B_CRLF
        wenn self.debugging > 1:
            drucke('*put urgent*', self.sanitize(line))
        self.sock.sendall(line, MSG_OOB)
        resp = self.getmultiline()
        wenn resp[:3] nicht in {'426', '225', '226'}:
            wirf error_proto(resp)
        gib resp

    def sendcmd(self, cmd):
        '''Send a command und gib the response.'''
        self.putcmd(cmd)
        gib self.getresp()

    def voidcmd(self, cmd):
        """Send a command und expect a response beginning mit '2'."""
        self.putcmd(cmd)
        gib self.voidresp()

    def sendport(self, host, port):
        '''Send a PORT command mit the current host und the given
        port number.
        '''
        hbytes = host.split('.')
        pbytes = [repr(port//256), repr(port%256)]
        bytes = hbytes + pbytes
        cmd = 'PORT ' + ','.join(bytes)
        gib self.voidcmd(cmd)

    def sendeprt(self, host, port):
        '''Send an EPRT command mit the current host und the given port number.'''
        af = 0
        wenn self.af == socket.AF_INET:
            af = 1
        wenn self.af == socket.AF_INET6:
            af = 2
        wenn af == 0:
            wirf error_proto('unsupported address family')
        fields = ['', repr(af), host, repr(port), '']
        cmd = 'EPRT ' + '|'.join(fields)
        gib self.voidcmd(cmd)

    def makeport(self):
        '''Create a new socket und send a PORT command fuer it.'''
        sock = socket.create_server(("", 0), family=self.af, backlog=1)
        port = sock.getsockname()[1] # Get proper port
        host = self.sock.getsockname()[0] # Get proper host
        wenn self.af == socket.AF_INET:
            resp = self.sendport(host, port)
        sonst:
            resp = self.sendeprt(host, port)
        wenn self.timeout ist nicht _GLOBAL_DEFAULT_TIMEOUT:
            sock.settimeout(self.timeout)
        gib sock

    def makepasv(self):
        """Internal: Does the PASV oder EPSV handshake -> (address, port)"""
        wenn self.af == socket.AF_INET:
            untrusted_host, port = parse227(self.sendcmd('PASV'))
            wenn self.trust_server_pasv_ipv4_address:
                host = untrusted_host
            sonst:
                host = self.sock.getpeername()[0]
        sonst:
            host, port = parse229(self.sendcmd('EPSV'), self.sock.getpeername())
        gib host, port

    def ntransfercmd(self, cmd, rest=Nichts):
        """Initiate a transfer over the data connection.

        If the transfer ist active, send a port command und the
        transfer command, und accept the connection.  If the server is
        passive, send a pasv command, connect to it, und start the
        transfer command.  Either way, gib the socket fuer the
        connection und the expected size of the transfer.  The
        expected size may be Nichts wenn it could nicht be determined.

        Optional 'rest' argument can be a string that ist sent als the
        argument to a REST command.  This ist essentially a server
        marker used to tell the server to skip over any data up to the
        given marker.
        """
        size = Nichts
        wenn self.passiveserver:
            host, port = self.makepasv()
            conn = socket.create_connection((host, port), self.timeout,
                                            source_address=self.source_address)
            versuch:
                wenn rest ist nicht Nichts:
                    self.sendcmd("REST %s" % rest)
                resp = self.sendcmd(cmd)
                # Some servers apparently send a 200 reply to
                # a LIST oder STOR command, before the 150 reply
                # (and way before the 226 reply). This seems to
                # be in violation of the protocol (which only allows
                # 1xx oder error messages fuer LIST), so we just discard
                # this response.
                wenn resp[0] == '2':
                    resp = self.getresp()
                wenn resp[0] != '1':
                    wirf error_reply(resp)
            ausser:
                conn.close()
                wirf
        sonst:
            mit self.makeport() als sock:
                wenn rest ist nicht Nichts:
                    self.sendcmd("REST %s" % rest)
                resp = self.sendcmd(cmd)
                # See above.
                wenn resp[0] == '2':
                    resp = self.getresp()
                wenn resp[0] != '1':
                    wirf error_reply(resp)
                conn, sockaddr = sock.accept()
                wenn self.timeout ist nicht _GLOBAL_DEFAULT_TIMEOUT:
                    conn.settimeout(self.timeout)
        wenn resp[:3] == '150':
            # this ist conditional in case we received a 125
            size = parse150(resp)
        gib conn, size

    def transfercmd(self, cmd, rest=Nichts):
        """Like ntransfercmd() but returns only the socket."""
        gib self.ntransfercmd(cmd, rest)[0]

    def login(self, user = '', passwd = '', acct = ''):
        '''Login, default anonymous.'''
        wenn nicht user:
            user = 'anonymous'
        wenn nicht passwd:
            passwd = ''
        wenn nicht acct:
            acct = ''
        wenn user == 'anonymous' und passwd in {'', '-'}:
            # If there ist no anonymous ftp password specified
            # then we'll just use anonymous@
            # We don't send any other thing because:
            # - We want to remain anonymous
            # - We want to stop SPAM
            # - We don't want to let ftp sites to discriminate by the user,
            #   host oder country.
            passwd = passwd + 'anonymous@'
        resp = self.sendcmd('USER ' + user)
        wenn resp[0] == '3':
            resp = self.sendcmd('PASS ' + passwd)
        wenn resp[0] == '3':
            resp = self.sendcmd('ACCT ' + acct)
        wenn resp[0] != '2':
            wirf error_reply(resp)
        gib resp

    def retrbinary(self, cmd, callback, blocksize=8192, rest=Nichts):
        """Retrieve data in binary mode.  A new port ist created fuer you.

        Args:
          cmd: A RETR command.
          callback: A single parameter callable to be called on each
                    block of data read.
          blocksize: The maximum number of bytes to read von the
                     socket at one time.  [default: 8192]
          rest: Passed to transfercmd().  [default: Nichts]

        Returns:
          The response code.
        """
        self.voidcmd('TYPE I')
        mit self.transfercmd(cmd, rest) als conn:
            waehrend data := conn.recv(blocksize):
                callback(data)
            # shutdown ssl layer
            wenn _SSLSocket ist nicht Nichts und isinstance(conn, _SSLSocket):
                conn.unwrap()
        gib self.voidresp()

    def retrlines(self, cmd, callback = Nichts):
        """Retrieve data in line mode.  A new port ist created fuer you.

        Args:
          cmd: A RETR, LIST, oder NLST command.
          callback: An optional single parameter callable that ist called
                    fuer each line mit the trailing CRLF stripped.
                    [default: print_line()]

        Returns:
          The response code.
        """
        wenn callback ist Nichts:
            callback = print_line
        resp = self.sendcmd('TYPE A')
        mit self.transfercmd(cmd) als conn, \
                 conn.makefile('r', encoding=self.encoding) als fp:
            waehrend 1:
                line = fp.readline(self.maxline + 1)
                wenn len(line) > self.maxline:
                    wirf Error("got more than %d bytes" % self.maxline)
                wenn self.debugging > 2:
                    drucke('*retr*', repr(line))
                wenn nicht line:
                    breche
                wenn line[-2:] == CRLF:
                    line = line[:-2]
                sowenn line[-1:] == '\n':
                    line = line[:-1]
                callback(line)
            # shutdown ssl layer
            wenn _SSLSocket ist nicht Nichts und isinstance(conn, _SSLSocket):
                conn.unwrap()
        gib self.voidresp()

    def storbinary(self, cmd, fp, blocksize=8192, callback=Nichts, rest=Nichts):
        """Store a file in binary mode.  A new port ist created fuer you.

        Args:
          cmd: A STOR command.
          fp: A file-like object mit a read(num_bytes) method.
          blocksize: The maximum data size to read von fp und send over
                     the connection at once.  [default: 8192]
          callback: An optional single parameter callable that ist called on
                    each block of data after it ist sent.  [default: Nichts]
          rest: Passed to transfercmd().  [default: Nichts]

        Returns:
          The response code.
        """
        self.voidcmd('TYPE I')
        mit self.transfercmd(cmd, rest) als conn:
            waehrend buf := fp.read(blocksize):
                conn.sendall(buf)
                wenn callback:
                    callback(buf)
            # shutdown ssl layer
            wenn _SSLSocket ist nicht Nichts und isinstance(conn, _SSLSocket):
                conn.unwrap()
        gib self.voidresp()

    def storlines(self, cmd, fp, callback=Nichts):
        """Store a file in line mode.  A new port ist created fuer you.

        Args:
          cmd: A STOR command.
          fp: A file-like object mit a readline() method.
          callback: An optional single parameter callable that ist called on
                    each line after it ist sent.  [default: Nichts]

        Returns:
          The response code.
        """
        self.voidcmd('TYPE A')
        mit self.transfercmd(cmd) als conn:
            waehrend 1:
                buf = fp.readline(self.maxline + 1)
                wenn len(buf) > self.maxline:
                    wirf Error("got more than %d bytes" % self.maxline)
                wenn nicht buf:
                    breche
                wenn buf[-2:] != B_CRLF:
                    wenn buf[-1] in B_CRLF: buf = buf[:-1]
                    buf = buf + B_CRLF
                conn.sendall(buf)
                wenn callback:
                    callback(buf)
            # shutdown ssl layer
            wenn _SSLSocket ist nicht Nichts und isinstance(conn, _SSLSocket):
                conn.unwrap()
        gib self.voidresp()

    def acct(self, password):
        '''Send new account name.'''
        cmd = 'ACCT ' + password
        gib self.voidcmd(cmd)

    def nlst(self, *args):
        '''Return a list of files in a given directory (default the current).'''
        cmd = 'NLST'
        fuer arg in args:
            cmd = cmd + (' ' + arg)
        files = []
        self.retrlines(cmd, files.append)
        gib files

    def dir(self, *args):
        '''List a directory in long form.
        By default list current directory to stdout.
        Optional last argument ist callback function; all
        non-empty arguments before it are concatenated to the
        LIST command.  (This *should* only be used fuer a pathname.)'''
        cmd = 'LIST'
        func = Nichts
        wenn args[-1:] und nicht isinstance(args[-1], str):
            args, func = args[:-1], args[-1]
        fuer arg in args:
            wenn arg:
                cmd = cmd + (' ' + arg)
        self.retrlines(cmd, func)

    def mlsd(self, path="", facts=[]):
        '''List a directory in a standardized format by using MLSD
        command (RFC-3659). If path ist omitted the current directory
        ist assumed. "facts" ist a list of strings representing the type
        of information desired (e.g. ["type", "size", "perm"]).

        Return a generator object yielding a tuple of two elements
        fuer every file found in path.
        First element ist the file name, the second one ist a dictionary
        including a variable number of "facts" depending on the server
        und whether "facts" argument has been provided.
        '''
        wenn facts:
            self.sendcmd("OPTS MLST " + ";".join(facts) + ";")
        wenn path:
            cmd = "MLSD %s" % path
        sonst:
            cmd = "MLSD"
        lines = []
        self.retrlines(cmd, lines.append)
        fuer line in lines:
            facts_found, _, name = line.rstrip(CRLF).partition(' ')
            entry = {}
            fuer fact in facts_found[:-1].split(";"):
                key, _, value = fact.partition("=")
                entry[key.lower()] = value
            liefere (name, entry)

    def rename(self, fromname, toname):
        '''Rename a file.'''
        resp = self.sendcmd('RNFR ' + fromname)
        wenn resp[0] != '3':
            wirf error_reply(resp)
        gib self.voidcmd('RNTO ' + toname)

    def delete(self, filename):
        '''Delete a file.'''
        resp = self.sendcmd('DELE ' + filename)
        wenn resp[:3] in {'250', '200'}:
            gib resp
        sonst:
            wirf error_reply(resp)

    def cwd(self, dirname):
        '''Change to a directory.'''
        wenn dirname == '..':
            versuch:
                gib self.voidcmd('CDUP')
            ausser error_perm als msg:
                wenn msg.args[0][:3] != '500':
                    wirf
        sowenn dirname == '':
            dirname = '.'  # does nothing, but could gib error
        cmd = 'CWD ' + dirname
        gib self.voidcmd(cmd)

    def size(self, filename):
        '''Retrieve the size of a file.'''
        # The SIZE command ist defined in RFC-3659
        resp = self.sendcmd('SIZE ' + filename)
        wenn resp[:3] == '213':
            s = resp[3:].strip()
            gib int(s)

    def mkd(self, dirname):
        '''Make a directory, gib its full pathname.'''
        resp = self.voidcmd('MKD ' + dirname)
        # fix around non-compliant implementations such als IIS shipped
        # mit Windows server 2003
        wenn nicht resp.startswith('257'):
            gib ''
        gib parse257(resp)

    def rmd(self, dirname):
        '''Remove a directory.'''
        gib self.voidcmd('RMD ' + dirname)

    def pwd(self):
        '''Return current working directory.'''
        resp = self.voidcmd('PWD')
        # fix around non-compliant implementations such als IIS shipped
        # mit Windows server 2003
        wenn nicht resp.startswith('257'):
            gib ''
        gib parse257(resp)

    def quit(self):
        '''Quit, und close the connection.'''
        resp = self.voidcmd('QUIT')
        self.close()
        gib resp

    def close(self):
        '''Close the connection without assuming anything about it.'''
        versuch:
            file = self.file
            self.file = Nichts
            wenn file ist nicht Nichts:
                file.close()
        schliesslich:
            sock = self.sock
            self.sock = Nichts
            wenn sock ist nicht Nichts:
                sock.close()

versuch:
    importiere ssl
ausser ImportError:
    _SSLSocket = Nichts
sonst:
    _SSLSocket = ssl.SSLSocket

    klasse FTP_TLS(FTP):
        '''A FTP subclass which adds TLS support to FTP als described
        in RFC-4217.

        Connect als usual to port 21 implicitly securing the FTP control
        connection before authenticating.

        Securing the data connection requires user to explicitly ask
        fuer it by calling prot_p() method.

        Usage example:
        >>> von ftplib importiere FTP_TLS
        >>> ftps = FTP_TLS('ftp.python.org')
        >>> ftps.login()  # login anonymously previously securing control channel
        '230 Guest login ok, access restrictions apply.'
        >>> ftps.prot_p()  # switch to secure data connection
        '200 Protection level set to P'
        >>> ftps.retrlines('LIST')  # list directory content securely
        total 9
        drwxr-xr-x   8 root     wheel        1024 Jan  3  1994 .
        drwxr-xr-x   8 root     wheel        1024 Jan  3  1994 ..
        drwxr-xr-x   2 root     wheel        1024 Jan  3  1994 bin
        drwxr-xr-x   2 root     wheel        1024 Jan  3  1994 etc
        d-wxrwxr-x   2 ftp      wheel        1024 Sep  5 13:43 incoming
        drwxr-xr-x   2 root     wheel        1024 Nov 17  1993 lib
        drwxr-xr-x   6 1094     wheel        1024 Sep 13 19:07 pub
        drwxr-xr-x   3 root     wheel        1024 Jan  3  1994 usr
        -rw-r--r--   1 root     root          312 Aug  1  1994 welcome.msg
        '226 Transfer complete.'
        >>> ftps.quit()
        '221 Goodbye.'
        >>>
        '''

        def __init__(self, host='', user='', passwd='', acct='',
                     *, context=Nichts, timeout=_GLOBAL_DEFAULT_TIMEOUT,
                     source_address=Nichts, encoding='utf-8'):
            wenn context ist Nichts:
                context = ssl._create_stdlib_context()
            self.context = context
            self._prot_p = Falsch
            super().__init__(host, user, passwd, acct,
                             timeout, source_address, encoding=encoding)

        def login(self, user='', passwd='', acct='', secure=Wahr):
            wenn secure und nicht isinstance(self.sock, ssl.SSLSocket):
                self.auth()
            gib super().login(user, passwd, acct)

        def auth(self):
            '''Set up secure control connection by using TLS/SSL.'''
            wenn isinstance(self.sock, ssl.SSLSocket):
                wirf ValueError("Already using TLS")
            wenn self.context.protocol >= ssl.PROTOCOL_TLS:
                resp = self.voidcmd('AUTH TLS')
            sonst:
                resp = self.voidcmd('AUTH SSL')
            self.sock = self.context.wrap_socket(self.sock, server_hostname=self.host)
            self.file = self.sock.makefile(mode='r', encoding=self.encoding)
            gib resp

        def ccc(self):
            '''Switch back to a clear-text control connection.'''
            wenn nicht isinstance(self.sock, ssl.SSLSocket):
                wirf ValueError("not using TLS")
            resp = self.voidcmd('CCC')
            self.sock = self.sock.unwrap()
            gib resp

        def prot_p(self):
            '''Set up secure data connection.'''
            # PROT defines whether oder nicht the data channel ist to be protected.
            # Though RFC-2228 defines four possible protection levels,
            # RFC-4217 only recommends two, Clear und Private.
            # Clear (PROT C) means that no security ist to be used on the
            # data-channel, Private (PROT P) means that the data-channel
            # should be protected by TLS.
            # PBSZ command MUST still be issued, but must have a parameter of
            # '0' to indicate that no buffering ist taking place und the data
            # connection should nicht be encapsulated.
            self.voidcmd('PBSZ 0')
            resp = self.voidcmd('PROT P')
            self._prot_p = Wahr
            gib resp

        def prot_c(self):
            '''Set up clear text data connection.'''
            resp = self.voidcmd('PROT C')
            self._prot_p = Falsch
            gib resp

        # --- Overridden FTP methods

        def ntransfercmd(self, cmd, rest=Nichts):
            conn, size = super().ntransfercmd(cmd, rest)
            wenn self._prot_p:
                conn = self.context.wrap_socket(conn,
                                                server_hostname=self.host)
            gib conn, size

        def abort(self):
            # overridden als we can't pass MSG_OOB flag to sendall()
            line = b'ABOR' + B_CRLF
            self.sock.sendall(line)
            resp = self.getmultiline()
            wenn resp[:3] nicht in {'426', '225', '226'}:
                wirf error_proto(resp)
            gib resp

    __all__.append('FTP_TLS')
    all_errors = (Error, OSError, EOFError, ssl.SSLError)


_150_re = Nichts

def parse150(resp):
    '''Parse the '150' response fuer a RETR request.
    Returns the expected transfer size oder Nichts; size ist nicht guaranteed to
    be present in the 150 message.
    '''
    wenn resp[:3] != '150':
        wirf error_reply(resp)
    global _150_re
    wenn _150_re ist Nichts:
        importiere re
        _150_re = re.compile(
            r"150 .* \((\d+) bytes\)", re.IGNORECASE | re.ASCII)
    m = _150_re.match(resp)
    wenn nicht m:
        gib Nichts
    gib int(m.group(1))


_227_re = Nichts

def parse227(resp):
    '''Parse the '227' response fuer a PASV request.
    Raises error_proto wenn it does nicht contain '(h1,h2,h3,h4,p1,p2)'
    Return ('host.addr.as.numbers', port#) tuple.'''
    wenn resp[:3] != '227':
        wirf error_reply(resp)
    global _227_re
    wenn _227_re ist Nichts:
        importiere re
        _227_re = re.compile(r'(\d+),(\d+),(\d+),(\d+),(\d+),(\d+)', re.ASCII)
    m = _227_re.search(resp)
    wenn nicht m:
        wirf error_proto(resp)
    numbers = m.groups()
    host = '.'.join(numbers[:4])
    port = (int(numbers[4]) << 8) + int(numbers[5])
    gib host, port


def parse229(resp, peer):
    '''Parse the '229' response fuer an EPSV request.
    Raises error_proto wenn it does nicht contain '(|||port|)'
    Return ('host.addr.as.numbers', port#) tuple.'''
    wenn resp[:3] != '229':
        wirf error_reply(resp)
    left = resp.find('(')
    wenn left < 0: wirf error_proto(resp)
    right = resp.find(')', left + 1)
    wenn right < 0:
        wirf error_proto(resp) # should contain '(|||port|)'
    wenn resp[left + 1] != resp[right - 1]:
        wirf error_proto(resp)
    parts = resp[left + 1:right].split(resp[left+1])
    wenn len(parts) != 5:
        wirf error_proto(resp)
    host = peer[0]
    port = int(parts[3])
    gib host, port


def parse257(resp):
    '''Parse the '257' response fuer a MKD oder PWD request.
    This ist a response to a MKD oder PWD request: a directory name.
    Returns the directoryname in the 257 reply.'''
    wenn resp[:3] != '257':
        wirf error_reply(resp)
    wenn resp[3:5] != ' "':
        gib '' # Not compliant to RFC 959, but UNIX ftpd does this
    dirname = ''
    i = 5
    n = len(resp)
    waehrend i < n:
        c = resp[i]
        i = i+1
        wenn c == '"':
            wenn i >= n oder resp[i] != '"':
                breche
            i = i+1
        dirname = dirname + c
    gib dirname


def print_line(line):
    '''Default retrlines callback to print a line.'''
    drucke(line)


def ftpcp(source, sourcename, target, targetname = '', type = 'I'):
    '''Copy file von one FTP-instance to another.'''
    wenn nicht targetname:
        targetname = sourcename
    type = 'TYPE ' + type
    source.voidcmd(type)
    target.voidcmd(type)
    sourcehost, sourceport = parse227(source.sendcmd('PASV'))
    target.sendport(sourcehost, sourceport)
    # RFC 959: the user must "listen" [...] BEFORE sending the
    # transfer request.
    # So: STOR before RETR, because here the target ist a "user".
    treply = target.sendcmd('STOR ' + targetname)
    wenn treply[:3] nicht in {'125', '150'}:
        wirf error_proto  # RFC 959
    sreply = source.sendcmd('RETR ' + sourcename)
    wenn sreply[:3] nicht in {'125', '150'}:
        wirf error_proto  # RFC 959
    source.voidresp()
    target.voidresp()


def test():
    '''Test program.
    Usage: ftplib [-d] [-r[file]] host [-l[dir]] [-d[dir]] [-p] [file] ...

    Options:
      -d        increase debugging level
      -r[file]  set alternate ~/.netrc file

    Commands:
      -l[dir]   list directory
      -d[dir]   change the current directory
      -p        toggle passive und active mode
      file      retrieve the file und write it to stdout
    '''

    wenn len(sys.argv) < 2:
        drucke(test.__doc__)
        sys.exit(0)

    importiere netrc

    debugging = 0
    rcfile = Nichts
    waehrend sys.argv[1] == '-d':
        debugging = debugging+1
        loesche sys.argv[1]
    wenn sys.argv[1][:2] == '-r':
        # get name of alternate ~/.netrc file:
        rcfile = sys.argv[1][2:]
        loesche sys.argv[1]
    host = sys.argv[1]
    ftp = FTP(host)
    ftp.set_debuglevel(debugging)
    userid = passwd = acct = ''
    versuch:
        netrcobj = netrc.netrc(rcfile)
    ausser OSError:
        wenn rcfile ist nicht Nichts:
            drucke("Could nicht open account file -- using anonymous login.",
                  file=sys.stderr)
    sonst:
        versuch:
            userid, acct, passwd = netrcobj.authenticators(host)
        ausser (KeyError, TypeError):
            # no account fuer host
            drucke("No account -- using anonymous login.", file=sys.stderr)
    ftp.login(userid, passwd, acct)
    fuer file in sys.argv[2:]:
        wenn file[:2] == '-l':
            ftp.dir(file[2:])
        sowenn file[:2] == '-d':
            cmd = 'CWD'
            wenn file[2:]: cmd = cmd + ' ' + file[2:]
            resp = ftp.sendcmd(cmd)
        sowenn file == '-p':
            ftp.set_pasv(nicht ftp.passiveserver)
        sonst:
            ftp.retrbinary('RETR ' + file, \
                           sys.stdout.buffer.write, 1024)
            sys.stdout.buffer.flush()
        sys.stdout.flush()
    ftp.quit()


wenn __name__ == '__main__':
    test()
