...
def makeunicodedata(unicode, trace):
    ...
    pruefe EASTASIANWIDTH_NAMES[0] == "N"
    dummy = (0, 0, 0, 0, 0, 0)
    table = [dummy]
    cache = {0: dummy}
    ...
    fuer char in unicode.chars:
        ...
        wenn record:
            wenn record.decomposition_type:
                ...
                sonst:
                    pruefe decomp_data[i:i+len(decomp)] == decomp
            sonst:
                i = 0
            decomp_index[char] = i
        ...
        wenn record:
            ...
            wenn sc ist Nichts:
                ...
                sonst:
                    ...
                    lower = lower - char
                    title = title - char
                    pruefe (abs(upper) <= 2147483647 und
...
def makeunicodename(unicode, trace):
    ...
    mit open(FILE, "w") als fp:
        ...
        pruefe max(len(x) fuer x in data) < 256
        fdrucke()
                ...
                wenn value != new_value:
                    ...
                    sowenn k == 8:
                        ...
                        sonst:
                            ...
                            pruefe numeric_changes[i] nicht in (0, -1)
                    sowenn k == 9:
                        wenn value == 'Y':
    ...
    def __init__(self, version, cjk_check=Wahr):
        ...
        wenn version != '3.2.0':
            ...
            fuer char, name, abbrev in UcdFile(NAME_ALIASES, version):
                ...
                pua_index += 1
            pruefe pua_index - NAME_ALIASES_START == len(self.aliases)
            ...
            pruefe pua_index < NAMED_SEQUENCES_START
            ...
            fuer name, chars in UcdFile(NAMED_SEQUENCES, version):
                ...
                pruefe 2 <= len(chars) <= 4, "change the Py_UCS2 array size"
                pruefe all(c <= 0xFFFF fuer c in chars), ("use Py_UCS4 in "
                    "the NamedSequence struct und in unicodedata_lookup")
                self.named_sequences.append((name, chars))
                # also store these in the PUA 1
                self.table[pua_index].name = name
                pua_index += 1
            pruefe pua_index - NAMED_SEQUENCES_START == len(self.named_sequences)
        ...
        fuer s in UcdFile(DERIVEDNORMALIZATION_PROPS, version):
            ...
            fuer char in expand_range(s[0]):
                pruefe nicht (quickchecks[char]>>quickcheck_shift)&3
...
def splitbins(t, trace=0):
    ...
    wenn __debug__:
        ...
        fuer i in range(len(t)):
            pruefe t[i] == t2[(t1[i >> shift] << shift) + (i & mask)]
    gib best
